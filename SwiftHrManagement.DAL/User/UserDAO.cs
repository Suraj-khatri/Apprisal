...
{
    public class UserDAO : BaseDAOInv
    {
        private StringBuilder _insertquery;
        private StringBuilder _updatequery;        
        public UserDAO()
        {
            this._insertquery = new StringBuilder("INSERT INTO ADMINS (UserName,UserPassword,Name,status,user_type) VALUES "
                + "('Usr', dbo.encryptDb('Pwd'),'Name_','status_','_userType')");

            this._updatequery = new StringBuilder("UPDATE ADMINS SET UserName='Usr', UserPassword=dbo.encryptDb('Pwd'),Name='Name_',"
                + " status= 'status_',user_type='_userType' WHERE ADMINID='ADMINID_' ");          
        }
        public override void Save(object obj)
        {
            ...
            this._insertquery.Replace("_userType", usr.UserType);
        ...
        public override void Update(object obj)
        {
            ...
            this._updatequery.Replace("_userType", usr.UserType);
            ExecuteQuery(this._updatequery.ToString());           
        }
        public void ChangePassword(string newPassword,long id)
        {
            string sSql = "update Admins set UserPassword=dbo.encryptDb(" + filterstring(newPassword) + ") where AdminID=" + id + "";
            ExecuteQuery(sSql);
        }
        public void InsertAssignRole(String roleId, long userId)
        {
            string sSql = "insert into user_role (user_id,role_id)Values('"+userId+"','"+roleId+"')";
            ExecuteQuery(sSql);
        }
        public void UpdateAssignRole(String roleId, long userId)
        {
            string sSql = "update user_role set role_id='"+roleId+"' where user_id='"+userId+"'";
            ExecuteQuery(sSql);
        }
        public LoginInformationCore Getsessioninformation(String UserName, String UserPassword)
        {
            string sSql = ("Exec ProcUserLogin " + filterstring(UserName) + "," + filterstring(UserPassword) + "").ToString();
        ...
        public SystemUser FindUserById(long Id)
        {
            ...
            + " 'In Active' WHEN status = 'Y' THEN 'Active' END AS status,user_type from Admins A INNER JOIN Employee E ON E.EMPLOYEE_ID=A.Name INNER JOIN"
        ...
        {
            string sSql = "select dbo.GetEmployeeFullNameOfId(E.Employee_Id)+' ('+B.BRANCH_NAME+', '+D.DEPARTMENT_NAME+')' as Name"
            + " from Employee E inner join Admins A on E.EMPLOYEE_ID=A.Name INNER JOIN Branches B ON B.BRANCH_ID=E.BRANCH_ID INNER JOIN"
            + " Departments D ON E.DEPARTMENT_ID=D.DEPARTMENT_ID  where A.UserName='" + userName + "' and status='Y'";             
            DataTable dt = SelectByQuery(sSql);
            SystemUser _user = null;
            if (dt != null)
        ...
        //new query to show only name
        public SystemUser FindFullName(string userName)
        {
            string sSql = "select dbo.GetEmployeeFullNameOfId(E.Employee_Id) as Name from Employee E"
            + " inner join Admins A on E.EMPLOYEE_ID=A.Name INNER JOIN Branches B ON B.BRANCH_ID=E.BRANCH_ID INNER JOIN"
            + " Departments D ON E.DEPARTMENT_ID=D.DEPARTMENT_ID  where A.UserName='" + userName + "' and status='Y'";
            DataTable dt = SelectByQuery(sSql);
            SystemUser _user = null;
            if (dt != null)
        ...
        public SystemUser FindUserRole(long admin_id)
        {
        ...
        public List<SystemUser> FindUserName(int Branch_Id, int position_hierarchy)
        {
            ...
            + " FROM Admins AS A JOIN dbo.Employee AS E ON E.EMPLOYEE_ID = A.Name JOIN "
            + " StaticDataDetail s ON E.POSITION_ID = s.ROWID "
            + " WHERE  s.TYPE_ID = 4 and (E.BRANCH_ID = " + Branch_Id + ")";
            DataTable dt = SelectByQuery(sSql);
            List<SystemUser> usr = new List<SystemUser>();
            if (dt != null)
            {
        ...
        public object MapObjectByID(DataRow dr)
        {
            ...
            usr.Name = dr["Name"].ToString();
            usr.Status = dr["status"].ToString();
            usr.UserType = dr["user_type"].ToString();            
            return usr;
        }        
    }
}
