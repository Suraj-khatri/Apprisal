    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]
                    { 
                        {"ACCT_NUM",    "Account Number",         "LT"},
                        {"ACCT_NAME",    "Account Name",         "LT"},
            ...
            String[,] matgrid = new String[,]
                        {
                        {"ACCT_NUM",            "Account Number",             "125",      "T"}, 
                        {"ACCT_NAME",            "Account Name",             "125",      "T"},
                        {"BRANCH_NAME",          "Branch Name",             "125",      "T"},
                        {"ACCT_CURRENCY",       "Account Currency",        "200",      "T"},
                        {"ACCT_BALANCE",       "Balance",        "200",      "T"}
                        };



            string sortby = "ACCT_ID";
            string table_name = "AC_MASTER";

            String sql = "";
            
            sql = " SELECT A.ACCT_ID,A.ACCT_BRANCH_ID,B.BRANCH_NAME,A.ACCT_NUM,A.ACCT_NAME,A.ACCT_CURRENCY,A.ACCT_BALANCE FROM AC_MASTER A"
                  + " INNER JOIN Branches B ON B.BRANCH_ID = A.ACCT_BRANCH_ID WHERE 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\"  href=\"/AccountMaintainance/Manage.aspx?Id=" + dr["ACCT_ID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/AccountMaintainance/Manage.aspx");
