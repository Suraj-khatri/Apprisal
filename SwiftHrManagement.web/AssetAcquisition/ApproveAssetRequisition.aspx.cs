...
{
    public partial class ApproveAssetRequisition : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public ApproveAssetRequisition()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
             if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 133) == false)
                {
        ...
        private void init()
        {
            ...
            {
                sql = "Select a.id,b.BRANCH_NAME as forwarded_branch,dbo.GetEmployeeFullNameOfId(approved_by) as frowarded_to,priority,status,narration,"
                + " convert(varchar,approved_date,107) as approved_date from ASSET_REQUISITION_MESSAGE a inner join Branches b on "
                + " a.forwarded_branch=b.BRANCH_ID and  status='Requested'";
            }
            else
            {
                sql = "Select a.id,b.BRANCH_NAME as forwarded_branch,dbo.GetEmployeeFullNameOfId(approved_by) as frowarded_to,priority,status,narration,"
                + " convert(varchar,approved_date,107) as approved_date from ASSET_REQUISITION_MESSAGE a inner join Branches b on "
                + " a.forwarded_branch=b.BRANCH_ID where approved_by=" + filterstring(ReadSession().Emp_Id.ToString()) + " and status='Requested'";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ApproveAssetRequisition.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            html.Append("<th class=\"text-center\" align=\"center\">Approve</th>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Approve\" href=\"/AssetAcquisition/ManageApprove.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                //html.Append("<td><a href=\"/AssetAcquisition/ManageApprove.aspx?Id=" + dr["id"] + "\">Approve</a></td>");                
                html.Append("</tr>");
            }
