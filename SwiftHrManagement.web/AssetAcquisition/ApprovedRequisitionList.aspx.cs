...
{
    public partial class ApprovedRequisitionList : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public ApprovedRequisitionList()
        {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"status",    "Status",     "200",      "T"},
                        {"approval_message",    "Approved Message",     "200",      "T"}                     
                        };

            string sortby = "id";
            string table_name = "ASSET_REQUISITION_MESSAGE";

            String sql = "";
            //if (ReadSession().UserType == "A")
            //{
                sql = "Select a.id,b.BRANCH_NAME as forwarded_branch,dbo.GetEmployeeFullNameOfId(approved_by) as approved_by,a.priority,a.status,"
                + " a.approval_message,convert(varchar,a.approved_date,107) as approved_date,dbo.GetEmployeeFullNameOfId(a.created_by) as requested_by from ASSET_REQUISITION_MESSAGE a inner join Branches b"
                + " on a.forwarded_branch=b.BRANCH_ID where a.status='Approved'";
            //}
            //else
            //{
            //    sql = "Select a.id,b.BRANCH_NAME as forwarded_branch,dbo.GetEmployeeFullNameOfId(approved_by) as approved_by,a.priority,a.status,"
            //    + " a.approval_message,convert(varchar,a.approved_date,107) as approved_date,dbo.GetEmployeeFullNameOfId(a.created_by) as requested_by from ASSET_REQUISITION_MESSAGE a inner join Branches b"
            //    + " on a.forwarded_branch=b.BRANCH_ID where a.status='Approved' and b.branch_id="+ReadSession().Branch_Id+"";
            //////}
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ApprovedRequisitionList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            html.Append("<th class=\"text-center\" align=\"center\">Order</th>");
            //html.Append("<th Class=\"HeaderStyle\" align=\"left\">Order</th>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Order\" href=\"/AssetAcquisition/AssetPurchaseOrderList.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-shopping-cart\"></i></a></td>");
                //html.Append("<td><a href=\"/AssetAcquisition/AssetPurchaseOrderList.aspx?Id=" + dr["id"] + "\">Order</a></td>");
                html.Append("</tr>");
            }
