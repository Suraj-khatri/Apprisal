...
{
    public partial class AssetBookingList : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public AssetBookingList()
        {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        ...
                        {"order_id",            "Order No",          "EN"}
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"asset_id",            "Asset Type",             "200",      "T"},
                        {"order_id",            "Order No",             "200",      "T"},
                        {"qty",       "Ordered Quantity",        "200",      "T"},
                        {"rate",    "Rate",     "200",      "M"},
                        {"amount",    "Amount",     "200",      "M"},
                        {"received_by",    "Received By",     "200",      "T"},
                        {"received_date",    "Received Date",     "200",      "D"},
                        {"receive_msg",    "Received Message",     "200",      "T"},
                        {"remaining_qty",    "Remaining Quantity",     "200",      "T"},
                        };



            string sortby = "id";
            string table_name = "ASSET_ORDER_RECEIVED";

            String sql = "";
            sql = "select a.id,a.id as 'recid',a.asset_id 'assetno',order_id,order_message_id,p.porduct_code+'-'+p.ASSET_CODE as asset_id,qty,convert(varchar,rate,107) as rate,"
            + " convert(varchar,amount,107) as amount,req_id,receive_msg,dbo.GetEmployeeFullNameOfId(received_by) as received_by,"
            + " CONVERT(varchar,received_date,107) as received_date, a.remaining_qty from "
            + " ASSET_ORDER_RECEIVED a inner join ASSET_PRODUCT p on p.id=a.asset_id where  remaining_qty > 0 and 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "AssetBookingList.aspx?sortd=" + sortd + "&sortby={sortby}";
                //&Id=" + GetOrderId() + "";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (int.Parse(dr["remaining_qty"].ToString()) > 0)
                {
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Book\" href=\"/AssetManagement/ManageAssetBooking.aspx?recid=" + dr["recid"] + "&assetno=" + dr["assetno"] + "\"><i class=\"fa fa-wpforms\"></i></a></td>");
                    //html.Append("<td><a href=\"/AssetManagement/ManageAssetBooking.aspx?recid=" + dr["recid"] + "&assetno=" + dr["assetno"] + "\">Book</a></td>");
                }
                else
                {
                    //html.Append("<th Class=\"HeaderStyle\" align=\"left\">N/A</th>");
                    html.Append("<th class=\"text-center\" align=\"center\">N/A</th>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, false, "/AssetAcquisition/ReceivePurchaseOrder.aspx");
