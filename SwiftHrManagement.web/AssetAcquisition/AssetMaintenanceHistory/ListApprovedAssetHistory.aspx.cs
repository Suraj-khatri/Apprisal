...
namespace SwiftHrManagement.web.AssetAcquisition.AssetMaintenanceHistory
{
    public partial class ListApprovedAssetHistory : BasePage
    {
        ClsDAOInv _clsdao = null;
        RoleMenuDAOInv _roleMenuDao = new RoleMenuDAOInv();
        public ListApprovedAssetHistory()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                if (_roleMenuDao.hasAccess(ReadSession().AdminId, 204) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"maint_date",        "Maintenence Date",   "200",      "D"},
                        {"book_value",        "Book Value",         "200",      "M"},
                        {"accumulate_dep",    "Acc. Dep.",          "200",      "M"},                        
                        {"maint_date",        "Maintained Date",    "200",      "D"},
                        {"expense_amt",       "Expense Amount",     "200",      "M"},
                        {"maint_by",          "Maintained By",      "200",      "LT"},
                        {"payment_ledger",          "Payment Ledger",      "200",      "LT"},
            ...
            string sortby = "id";
            string table_name = "ASSET_MAINTENANCE_HISTORY";
            String sql = "";
            if (ReadSession().UserType == "A")
            {
                sql = "SELECT  am.id,ai.asset_number + ' | ' + CONVERT(VARCHAR, ai.id) asset_number,dbo.GetEmployeeFullNameOfId(maint_by) as maint_by"
                        +" ,CONVERT(VARCHAR, am.maint_date,107) AS maint_date, am.accumulate_dep ,am.book_value , am.expense_amt,am.narration,payment_ledger FROM " 
                        +" ASSET_MAINTENANCE_HISTORY am INNER JOIN ASSET_INVENTORY ai ON am.asset_id = ai.id LEFT JOIN Employee e ON am.maint_by = e.EMPLOYEE_ID where 1=1";
            }
            else
            {
                sql = "SELECT  am.id,ai.asset_number + ' | ' + CONVERT(VARCHAR, ai.id) asset_number,dbo.GetEmployeeFullNameOfId(maint_by) as maint_by"
                +" ,CONVERT(VARCHAR, am.maint_date,107) AS maint_date, am.accumulate_dep ,am.book_value , am.expense_amt,am.narration,payment_ledger FROM " 
                +" ASSET_MAINTENANCE_HISTORY am INNER JOIN ASSET_INVENTORY ai ON am.asset_id = ai.id LEFT JOIN Employee e ON am.maint_by = e.EMPLOYEE_ID"
                +" where ai.branch_id="+ReadSession().Branch_Id+"";
            }   
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListApprovedAssetHistory.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "ManageAssetHistory.aspx");
