...
{
    public partial class ListAssetHistory : BasePage
    {
        ClsDAOInv _clsdao = null;
        RoleMenuDAOInv _roleMenuDao = new RoleMenuDAOInv();
        public ListAssetHistory()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                if (_roleMenuDao.hasAccess(ReadSession().AdminId, 117) == false)
                {
        ...
        private void init()
        {
            ...
            string sortby = "id";
            string table_name = "ASSET_MAINTENANCE_TEMP";

            String sql = "SELECT  am.id,ai.asset_number + ' | ' + CONVERT(VARCHAR, ai.id) asset_number,dbo.GetEmployeeFullNameOfId(maint_by) as maint_by,STATUS"
                ...
                +" ASSET_MAINTENANCE_TEMP am INNER JOIN ASSET_INVENTORY ai ON am.asset_id = ai.id LEFT JOIN Employee e ON am.maint_by = e.EMPLOYEE_ID "
                + " WHERE am.created_by=" + filterstring(ReadSession().Emp_Id.ToString()) + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListAssetHistory.aspx?sortd=" + sortd + "&sortby={sortby}";
