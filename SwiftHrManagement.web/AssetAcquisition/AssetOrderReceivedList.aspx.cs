...
{
    public partial class AssetOrderReceivedList : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public AssetOrderReceivedList()
        {
        ...
        private void init()
        {
            ...
            + " CONVERT(varchar,received_date,107) as received_date,isnull(a.remaining_qty,0) as remaining_qty from "
            + " ASSET_ORDER_RECEIVED a inner join ASSET_PRODUCT p on p.id=a.asset_id where order_message_id='" + GetOrderId() + "'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "AssetOrderReceivedList.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetOrderId() + "";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                {
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Book\" href=\"/AssetManagement/ManageAssetBooking.aspx?recid=" + dr["recid"] + "&assetno=" + dr["assetno"] + "\"><i class=\"fa fa-calendar-check-o\"></i></a>");
                }
                else
                {
                    html.Append("<td align=\"left\">N/A</td>");
                }
                html.Append("</tr>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/AssetAcquisition/ReceivePurchaseOrder.aspx?Id=" + GetOrderId() + "");
