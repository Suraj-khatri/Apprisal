...
{
    public partial class AssetPurchaseOrderList : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public AssetPurchaseOrderList()
        {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"vendor",    "Vendor Name",     "200",      "L"},
                        {"narration",    "Desciption",     "200",      "T"},
                        {"created_by",    "Order By",     "200",      "T"}
                        };
            ...
            String sql = "";
            sql = "select a.id,order_no,convert(varchar,order_date,107) as order_date,c.CustomerName as vendor,narration,dbo.GetEmployeeFullNameOfId(created_by) as created_by from "
                    + " ASSET_PURCHASE_ORDER_MESSAGE a inner join  Customer c on a.vendor_id=c.ID where requistion_message_id=" + GetReqMessId() + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "AssetPurchaseOrderList.aspx?sortd=" + sortd + "&sortby={sortby}&Id="+GetReqMessId()+"";
            ...
            html.Append("<th Class=\"HeaderStyle\" align=\"left\"  width=\"115px\">Actions</th>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/AssetAcquisition/ViewAssetPurchaseOrder.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a>");
                html.Append("<a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Print Order\" href=\"/AssetAcquisition/PrintPurchaseOrder.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-print\"></i></a>");
                html.Append("<a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Receive\" href=\"/AssetAcquisition/AssetOrderReceivedList.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-download\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/AssetAcquisition/AssetPurchaseOrder.aspx?Id="+GetReqMessId()+"");
