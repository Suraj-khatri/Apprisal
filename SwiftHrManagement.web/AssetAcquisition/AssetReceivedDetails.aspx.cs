...
{
    public partial class AssetReceivedDetails : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public AssetReceivedDetails()
        {
       ...
       private long GetOrderMsgId()
       {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"req_id",                   "Requisition ID",            "200",      "N"},
                        {"asset_type",                          "Asset Type",                   "200",      "T"},
                        {"qty",                    "Ordered Qty",           "200",      "N"},                        
                        {"rate",                   "Rate(Unit Price)",            "200",      "M"},
                        {"amount",                   "Amount (Rs.)",            "200",      "M"},
                        {"received_qty",                   "Received Qty",            "200",      "N"}
            ...
            String sql = "SELECT o.id,req_id,a.porduct_code as asset_type,qty,dbo.ShowDecimal(rate) as rate,dbo.ShowDecimal(amount) as amount,received_qty "
                    +" FROM ASSET_PURCHASE_ORDER o with(nolock) inner join ASSET_PRODUCT a with(nolock) on a.id=o.asset_id WHERE order_message_id="+GetOrderMsgId()+"";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "AssetReceivedDetails.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetOrderMsgId() + "";
