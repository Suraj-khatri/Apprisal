...
{
    public partial class AssetRequisitionHistoryDetailsList : BasePage
    {
       RoleMenuDAOInv _RoleMenuDAOInv = null;
       public AssetRequisitionHistoryDetailsList()
        {
       ...
       private long GetRequisitionID()
       {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"requisition_id",                   "Requisition ID",            "200",      "EN"},
                        {"order_no",                          "Order No",                   "200",      "EN"},
                        {"order_date",                    "Order Date",           "200",      "D"},                        
                        {"VENDOR",                   "Vendor Name",            "200",      "T"},
                        {"ordered_by",                   "Ordered By",            "200",      "T"}
                        };

            string sortby = "id";
            string table_name = "ASSET_PURCHASE_ORDER_MESSAGE";

            //String sql = " select r.id,r.asset_id,p.porduct_code,r.qty as request_qty,r.price,r.approved_qty,"
            //             + " ar.qty as received_qty from ASSET_REQUISITION r inner join ASSET_PRODUCT p on p.id = r.asset_id"
            //             + " left join ASSET_ORDER_RECEIVED ar on ar.order_message_id = r.requistion_message_id"
            //             + " where r.requistion_message_id =" + GetRequisitionID() + " and 1=1";
            //String sql = " select r.id,r.asset_id,p.porduct_code,r.qty as request_qty,r.price,r.approved_qty,"
            //             + " SUM(ar.qty) as received_qty from ASSET_REQUISITION r inner join ASSET_PRODUCT p"
            //             + " on p.id = r.asset_id left join ASSET_ORDER_RECEIVED ar"
            //             + " on ar.order_message_id = r.requistion_message_id where r.requistion_message_id =1";
            String sql = "SELECT PO.id,PO.requistion_message_id as requisition_id,order_no,CONVERT(VARCHAR,order_date,107) AS order_date,"
                     +" C.CustomerName AS VENDOR,dbo.GetEmployeeFullNameOfId(created_by) as ordered_by FROM ASSET_PURCHASE_ORDER_MESSAGE PO"
                     + " WITH(NOLOCK) INNER JOIN Customer C WITH(NOLOCK) ON C.ID=PO.vendor_id WHERE requistion_message_id=" + GetRequisitionID() + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "AssetRequisitionHistoryDetailsList.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetRequisitionID() + "";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View Details\" href=\"/AssetAcquisition/AssetReceivedDetails.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                //html.Append("<td><a href=\"/AssetAcquisition/AssetReceivedDetails.aspx?Id=" + dr["id"] + "\">View Details</a></td>"); 
                html.Append("</tr>");
            }
