...
{
    public partial class AssetRequisitionHistoryList : BasePage
    {
         RoleMenuDAOInv _RoleMenuDAOInv = null;
         public AssetRequisitionHistoryList()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
             if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 134) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        ...
                        {"m.approved_date",      "Approved Date",         "ED"},
                        {"BRANCH_NAME",        "Requesting Branch",     "LT"},
                        {"DEPARTMENT_NAME",    "Requesting Department", "LT"}
                    };
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"Created_Emp",              "Requested By",            "200",      "T"},
                        {"BRANCH_NAME",              "Requesting Branch",       "200",      "T"},
                        {"DEPARTMENT_NAME",          "Requesting Department",   "200",      "T"},
                        {"created_date",             "Requested Date",         "200",      "T"},
                        {"Approved_Emp",             "Approved By",             "200",      "T"},
                        {"approved_date",            "Approved Date",           "200",      "T"},
            ...
            String sql = " select m.id,m.created_by,e.FIRST_NAME + ' ' + e.MIDDLE_NAME + ' '+ e.LAST_NAME as Created_Emp,"
                         + "Convert(varchar,m.created_date,107) as created_date,m.branch_id, b.BRANCH_NAME, m.dept_id,"
                         + "d.DEPARTMENT_NAME,m.approved_by,ee.FIRST_NAME + ' ' + ee.MIDDLE_NAME + ' '+ ee.LAST_NAME"
                         + " as Approved_Emp, convert(varchar,m.approved_date,107) as approved_date,m.priority"
                         + " from ASSET_REQUISITION_MESSAGE m inner join Branches b"
                         + " on b.BRANCH_ID = m.branch_id inner join Departments d on"
                         + " d.DEPARTMENT_ID = m.dept_id inner join Employee e"
                         + " on e.EMPLOYEE_ID = m.created_by inner join Employee ee on"
                         + " ee.EMPLOYEE_ID = m.approved_by where m.status='Received'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "AssetRequisitionHistoryList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
               ...
               //html.Append("<td><a href=\"/AssetAcquisition/AssetRequisitionHistoryDetailsList.aspx?Id=" + dr["id"] + "\">Details</a></td>");                
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Details\" href=\"/AssetAcquisition/AssetRequisitionHistoryDetailsList.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-table\"></i></a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/AssetAcquisition/AssetRequisitionHistoryDetailsList.aspx");
