...
{
    public partial class AssetRequisitionList : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public AssetRequisitionList()
        {
        ...
        private void init()
        {
            ...
            if (ReadSession().UserId == "admin")
            {
                ...
                + " a.forwarded_branch=b.BRANCH_ID where status='Requested'";
            }
            else
            {
                sql = "Select a.id,b.BRANCH_NAME as forwarded_branch,dbo.GetEmployeeFullNameOfId(approved_by) as frowarded_to,priority,status,narration,"
                + " convert(varchar,approved_date,107) as approved_date from ASSET_REQUISITION_MESSAGE a inner join Branches b on "
                + " a.forwarded_branch=b.BRANCH_ID where a.created_by=" + filterstring(ReadSession().Emp_Id.ToString()) + " and status='Requested'";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "AssetRequisitionList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/AssetAcquisition/ModifyAssetRequisition.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
               // html.Append("<td><a href=\"/AssetAcquisition/ModifyAssetRequisition.aspx?Id=" + dr["id"] + "\">View</a></td>");                
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/AssetAcquisition/AssetRequisition.aspx");
