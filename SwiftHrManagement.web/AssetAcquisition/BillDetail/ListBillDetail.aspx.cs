...
{
    public partial class ListBillDetail : BasePage
    {
...
#region SwiftGrid
        ...
        private void init()
        {
            ...
            String sql = "select ab.id, ab.bill_no, ab.bill_date,dbo.ShowDecimal(ab.bill_amt) as bill_amt, c.CustomerName, b.BRANCH_NAME  from ASSET_BILL ab "
            + " inner join Customer c on ab.vendor_id = c.ID inner join Branches b on ab.branch_id = b.BRANCH_ID where 1 = 1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListBillDetail.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Modify\"  href=\"ManageBill.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-pencil\"></i></a>");
                html.Append("<a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Upload Bill\" href=\"UploadBill.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-upload\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "ManageBill.aspx");
