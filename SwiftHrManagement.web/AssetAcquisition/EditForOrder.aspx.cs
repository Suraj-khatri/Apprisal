...
{
    public partial class MakeOrder : BasePage
    {
        ...
        {
            return (Request.QueryString["ReqMessId"] != null ? long.Parse(Request.QueryString["ReqMessId"].ToString()) : 0);
        }
        private void populateValue()
        {
            DataTable dt = new DataTable();
            dt = _clsdao.getDataset("select qty,cast(rate as varchar) as rate,cast(amount as varchar) as amount from asset_temp_order where id=" + GetID() + "").Tables[0];
        ...
        {
            DataTable dt = new DataTable();
            dt = _clsdao.getDataset("select t.id,a.porduct_code+'-'+a.ASSET_CODE  AS asset_type,r.approved_qty-ISNULL(r.order_qty,0)as Qty"
            +" from asset_temp_order t inner join ASSET_PRODUCT a on t.asset_id=cast(a.id as varchar) inner join ASSET_REQUISITION"
            +" r on r.id=t.req_id WHERE t.id='"+GetID()+"'").Tables[0];

            foreach (DataRow dr in dt.Rows)
            {
                lblAssetType.Text = dr["asset_type"].ToString();
                lblQty.Text = dr["Qty"].ToString();
                hdnReqId.Value = dr["id"].ToString();
            }
        }
        //#region SwiftGrid
        ...
        //    string sortby = "id";
        //    string table_name = "ASSET_PURCHASE_ORDER";

        //    String sql = "";         
        //        sql = "select o.id,o.order_message_id,a.porduct_code+'-'+a.ASSET_CODE as asset_id,o.qty,dbo.ShowDecimal(o.rate) as rate,"
        //        +" dbo.ShowDecimal(o.amount) as amount,o.req_id from ASSET_PURCHASE_ORDER o inner join ASSET_PRODUCT a on "
        //        +" o.asset_id=cast(a.id as varchar) where o.req_id="+GetReqId()+"";
        ...
        //    string thisPage = "MakeOrder.aspx?sortd=" + sortd + "&sortby={sortby}&Id="+GetReqId()+"";
        ...
        protected void BtnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (Double.Parse(lblQty.Text) < Double.Parse(qty.Text) || Double.Parse(qty.Text) == 0)
                {
                    LblMsg.Text = "Invalid qty, Quantity out of range!";
                    LblMsg.ForeColor = System.Drawing.Color.Red;
                    return;
                }
                else
                {
                    _clsdao.runSQL("update asset_temp_order set qty=" + filterstring(qty.Text) + ",rate=" + filterstring(rate.Text) + ",amount=" + filterstring(amount.Text) + " where session_id=" + filterstring(ReadSession().Sessionid) + " and id=" + filterstring(hdnReqId.Value) + "");
                    Response.Redirect("AssetPurchaseOrder.aspx?Id=" + GetReqMessId() + "");
                }
            }
            catch
            {
        ...
        protected void rate_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                LblMsg.Text = "";
                totAmt = (Double.Parse(rate.Text) * Double.Parse(qty.Text));
