...
{
    public partial class ListInsuranceDetail : BasePage
    {
        ClsDAOInv _claDao = null;
        public ListInsuranceDetail()
        {
            _claDao = new ClsDAOInv();
        }
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"insurer",           "Insurer",            "200",  "LT"},
                        {"insured_date",      "Insured Date",       "200",  "D"},
                        {"insured_amt",       "insured Amount",     "200",  "M"},
                        {"expiry_date",       "Expiry Date",        "200",  "D"}
                        };

            string sortby = "id";
            string table_name = "ASSET_INSURENCE";

            String sql = "select a.id ,s.DETAIL_TITLE as insurer,dbo.ShowDecimal(a.insured_amt) as insured_amt, convert(varchar,a.insured_date, 107) as insured_date, "
            + " convert(varchar, a.[expiry_date], 107) as expiry_date, a.narration from ASSET_INSURENCE a inner join StaticDataDetail s on a.insurer = s.ROWID";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListInsuranceDetail.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Modify\" href=\"ManageInsurance.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-pencil\"></i></a>");
                html.Append("<a class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Upload Ploicy\" href=\"UploadPremiumpaid.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-upload\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "ManageInsurance.aspx");
