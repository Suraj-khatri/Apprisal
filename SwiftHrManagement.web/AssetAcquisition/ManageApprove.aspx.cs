...
{
    public partial class ManageApprove : BasePage
    {
        ClsDAOInv _clsdao = null;
        public ManageApprove()
        {
        ...
        private void getcompinfo()
        {
            ...
            + "from ASSET_REQUISITION_MESSAGE ir inner join Branches b on ir.branch_id = b.BRANCH_ID inner join Departments d on ir.dept_id = d.DEPARTMENT_ID"
            + " where ir.id=" + GetId() + "");
        }
        #region SwiftGrid
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"a.porduct_code",    "Asset Type",         "LT"}

                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{                        
                        {"asset_type",           "Asset Type",              "200",      "LT"},
                        {"Qty",   "Requested Qty",             "200",      "EN"},
                        {"approved_qty",   "Approved Qty",             "200",      "EN"},
                        {"Amount",                "Amount",                 "200",      "M"}
                      
                        };

            string sortby = "id";
            string table_name = "ASSET_REQUISITION";

            String sql = "select t.id,t.approved_qty,a.porduct_code+'-'+a.ASSET_CODE  AS asset_type,t.qty as Qty,dbo.ShowDecimal(t.price) as Amount from "
            +" ASSET_REQUISITION t inner join ASSET_PRODUCT a on t.asset_id=cast(a.id as varchar) WHERE requistion_message_id='"+GetId()+"'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ManageApprove.aspx?sortd=" + sortd + "&sortby={sortby}&Id="+GetId()+"";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/AssetAcquisition/ModifyAssetInfo.aspx?Id=" + dr["id"] + "\">Edit</a></td>");
                html.Append("</tr>");
            }
        ...
        protected void BtnSave_Click(object sender, EventArgs e)
        {
            try
            {
                _clsdao.runSQL("UPDATE ASSET_REQUISITION_MESSAGE SET status='Approved',approval_message=" + filterstring(message.Text) + ",approved_date=" + filterstring(System.DateTime.Now.ToString()) + " WHERE id='" + GetId() + "'");
                Response.Redirect("ApproveAssetRequisition.aspx");
            }
            catch
            {
        ...
        {
            Response.Redirect("ManageApprove.aspx?Id=" + GetId() + "");
        }
