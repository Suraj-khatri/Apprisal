...
{
    public partial class ViewAssetPurchaseOrder : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public ViewAssetPurchaseOrder()
        {
        ...
        private void init()
        {
            ...
            String sql = "";
            sql = "select a.id,order_message_id,p.porduct_code+'-'+p.ASSET_CODE  AS asset_id,qty,dbo.ShowDecimal(rate) as rate,dbo.ShowDecimal(amount) as amount,req_id from ASSET_PURCHASE_ORDER a"
                    +" inner join ASSET_PRODUCT p on p.id=a.asset_id where order_message_id="+GetOrderMessId()+"";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ViewAssetPurchaseOrder.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetOrderMessId() + "";
