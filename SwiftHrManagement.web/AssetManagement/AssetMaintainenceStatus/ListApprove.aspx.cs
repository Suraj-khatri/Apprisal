...
namespace SwiftHrManagement.web.AssetManagement.AssetMaintainenceStatus
{
    public partial class ListApprove :  BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_roleMenuDao.hasAccess(ReadSession().AdminId, 215) == false)
                {
        ...
        private void init()
        {
            ...
            string sortby = "RowID";
            string table_name = "ASSET_MAINTAINANCE";
            ...
            //+ " OR AM.RequestingUser =" + ReadSession().Emp_Id;


            if (ReadSession().Branch_Id != 1)
            {
                sql += "  AND (CASE WHEN (AM.ProcessStatus = 'FWDTBNCH' OR AM.ProcessStatus = 'ACK' OR am.ProcessStatus='FWDTVO') THEN AM.RequestingUser  ELSE AM.ForwardedToUser END) ='" + ReadSession().Emp_Id + "' AND AM.RequestingUser = CASE WHEN (AM.ProcessStatus = 'FWDTBNCH' OR AM.ProcessStatus = 'ACK') THEN '" + ReadSession().Emp_Id + "' ELSE  AM.RequestingUser END ";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (dr["pstatus"].ToString() == "ACK")
                {
                    html.Append("<td><a href=\"#\" onclick=\"'buttonClick()'\">View</a></td>");
                    
                }
                else
                {
                    html.Append("<td><a href=\"/AssetManagement/AssetMaintainenceStatus/Manage.aspx?RowId=" + dr["rowid"] + "&statusFor=" + dr["pstatus"] + "\" onclick=\"'buttonClick()'\">View</a></td>");
