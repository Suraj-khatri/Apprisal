...
{
    public partial class depreciation_booking_history : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public depreciation_booking_history()
        {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"fiscal_year",    "Fiscal Year",         "LT"},  
                        {"m.Name",    "Run Month",         "LT"},
                        {"b.BRANCH_NAME",    "Booked Date",         "LT"},
                        {"status",    "Status",         "LT"}
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"fiscal_year",            "Fiscal Year",             "200",      "T"},
                        {"Name",       "Run Month",        "200",      "T"},
                        {"BRANCH_NAME",    "Branch Name",     "200",      "T"},
                        {"requested_by",    "Requested By",     "200",      "T"},
                        {"requested_date",    "Requested Date",     "200",      "D"},
                        {"forwarded_to",    "Forwarded To",     "200",      "T"},
                        {"status",    "Status",     "200",      "T"},
                        {"approved_by",    "Approved By",     "200",      "T"},
                        {"approved_date",    "Approved Date",     "200",      "D"}
                        };



            string sortby = "id";
            string table_name = "DEPRECIATION_BOOKING_REQUEST";

            String sql = "";
            if (ReadSession().UserType == "A")
            {
                sql = "SELECT dh.id,fiscal_year,m.Name,isnull(b.BRANCH_NAME,'All') BRANCH_NAME,dbo.GetEmployeeFullNameOfId(dh.created_by) as requested_by,"
		                +" convert(varchar,dh.created_date,107) as requested_date,dbo.GetEmployeeFullNameOfId(forwarded_to) as forwarded_to,"
		                +" convert(varchar,forwarded_date,107) as forwarded_date,status,dbo.GetEmployeeFullNameOfId(approved_by) as approved_by,"
		                +" convert(varchar,approved_date,107) as approved_date"
		                +" FROM DEPRECIATION_BOOKING_REQUEST dh inner join MonthList m on m.Month_Number=dh.run_month "
		                +" left join Branches b on b.BRANCH_ID=dh.branch_id where 1=1";
            }
            else
            {
                sql = "SELECT dh.id,fiscal_year,m.Name,isnull(b.BRANCH_NAME,'All') BRANCH_NAME,dbo.GetEmployeeFullNameOfId(dh.created_by) as requested_by,"
                        ...
                        + " convert(varchar,approved_date,107) as approved_date"
                        + " FROM DEPRECIATION_BOOKING_REQUEST dh inner join MonthList m on m.Month_Number=dh.run_month "
                        + " left join Branches b on b.BRANCH_ID=dh.branch_id where b.branch_id=" + ReadSession().Branch_Id + "";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "depreciation_booking_history.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/AssetManagement/Depreciation/depreciationUpdate.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "depreciationUpdate.aspx");
