...
{
    public partial class depreciation_setup_list : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public depreciation_setup_list()
        {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        ...
                        {"a.asset_number",    "Asset Number",         "LT"},
            ...
            String[,] matgrid = new String[,]{
                        {"porduct_code",            "Asset Type",             "100",      "T"},
                        {"asset_number",       "Asset No",        "150",      "T"},
                        {"fiscal_year",    "Fiscal Year",     "75",      "D"},
                        ...
                        {"m3",    "Ashadh",     "70",      "M"},
                        {"dep_pct",    "Dep(%)",     "200",      "D"},
                        {"acc_dep",    "Acc Dep",     "200",      "M"},
                        {"base_price",    "Book Value",     "200",      "M"}
                        };



            string sortby = "id";
            string table_name = "depreciation_setup";
            ClsDAOInv _clsdao = new ClsDAOInv();

            string current_FY = _clsdao.GetSingleresult("SELECT FISCAL_YEAR_NEPALI FROM FiscalYear WHERE FLAG=1");

            String sql = "";
            sql = "select d.id,p.porduct_code,a.asset_number,fiscal_year,dbo.showdecimal(m1) as m1,dbo.showdecimal(m2) as m2,dbo.showdecimal(m3) as m3,"
            +" dbo.showdecimal(m4) as m4,dbo.showdecimal(m5) as m5,dbo.showdecimal(m6) as m6,dbo.showdecimal(m7) as m7,dbo.showdecimal(m8) as m8,"
            +" dbo.showdecimal(m9) as m9,dbo.showdecimal(m10) as m10,dbo.showdecimal(m11) as m11,dbo.showdecimal(m12) as m12,dep_pct,"
            + " dbo.showdecimal(acc_dep) as acc_dep,dbo.showdecimal(base_price) as base_price"
            +" from depreciation_setup  d with(nolock) inner join ASSET_INVENTORY a with(nolock) on a.id=d.asset_id inner join ASSET_PRODUCT p"
            + " with(nolock) on p.id=a.product_id where fiscal_year='" + current_FY + "'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "depreciation_setup_list.aspx?sortd=" + sortd + "&sortby={sortby}";
           ...
           // html.Append("<th Class=\"HeaderStyle\" align=\"left\">View</th>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/AssetManagement/Depreciation/ManageDepreciationSetup.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                //html.Append("<td><a href=\"/AssetManagement/Depreciation/ManageDepreciationSetup.aspx?Id=" + dr["id"] + "\">View</a></td>");
                html.Append("</tr>");
            }

            html.Append("<tr> <td align=\"center\" colspan=\"19\"> <strong> Page " + _page.ToString() + " Of " + total_page + "</strong></td></tr>");
            html.Append("</table>");
            html.Append("</div>");
