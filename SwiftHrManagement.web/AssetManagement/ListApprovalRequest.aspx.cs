...
{
    public partial class ListApprovalRequest : BasePage
    {
       RoleMenuDAOInv _RoleMenuDAOInv = null;
       public ListApprovalRequest()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 193) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"modType",    "Request Nature",         "LT"},  
                        {"description",    "Request(Table) Desc",         "LT"}
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"dataid",            "Table ID",             "200",      "T"},
                        {"identifierField",    "ID Field",     "200",      "T"},
                        {"modType",    "Request Nature",     "200",      "T"},
                        {"tableName",       "Table Name",        "200",      "T"},
                        {"description",    "Table Desc",     "200",      "T"},                        
                        {"req_by",    "Request By",     "200",      "T"},
                        {"rq_date",    "Request Date.",     "200",      "D"}
            ...
            string sortby = "id";
            string table_name = "changesApprovalQueue";

            String sql = "";
            if (ReadSession().UserName == "admin")
            {
                sql = "select rowId as id,dataid,tableName,description,identifierField,modType,dbo.GetEmployeeFullNameOfId(createdBy) as req_by,"
                        +" convert(varchar,createdDate,107) as rq_date from changesApprovalQueue where 1=1";
            }
            else
            {
                sql = "select rowId as id,dataid,tableName,description,identifierField,modType,dbo.GetEmployeeFullNameOfId(createdBy) as req_by,"
                        + " convert(varchar,createdDate,107) as rq_date from changesApprovalQueue where forwarded_to=" + ReadSession().Emp_Id + "";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListApprovalRequest.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/AssetManagement/ManageApproval.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
