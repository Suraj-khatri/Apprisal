...
{
    public partial class ListApprovedAssetCapilized : BasePage
    {
        ClsDAOInv _clsdao = null;
        RoleMenuDAOInv _roleMenuDao = new RoleMenuDAOInv();
        public ListApprovedAssetCapilized()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                if (_roleMenuDao.hasAccess(ReadSession().AdminId, 202) == false)
                {
        ...
        private void init()
        {
            ...
            string sortby = "Id";
            string table_name = "ASSET_CAPITALIZATION_HISTORY";
            String sql = "";
            if (ReadSession().UserType == "A")
            {
                sql = "SELECT c.Id,c.Asset_id,i.asset_number,CONVERT(VARCHAR,c.Capitalization_date,107) AS Capitalization_date"
                ...
                + " FROM ASSET_CAPITALIZATION_HISTORY c INNER JOIN asset_inventory i ON i.id = c.asset_id  WHERE 1=1";
            }
            else
            {
                sql = "SELECT c.Id,c.Asset_id,i.asset_number,CONVERT(VARCHAR,c.Capitalization_date,107) AS Capitalization_date"
                + " ,dbo.ShowDecimal(c.Capitalization_amt) AS  Capitalization_amt,dbo.ShowDecimal(i.purchase_value) AS Value_After_Capitalized,dbo.ShowDecimal(acc_depriciation) as acc_depriciation,c.narration"
                + " FROM ASSET_CAPITALIZATION_HISTORY c INNER JOIN asset_inventory i ON i.id = c.asset_id  where i.branch_id="+ReadSession().Branch_Id+"";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListApprovedAssetCapilized.aspx?sortd=" + sortd + "&sortby={sortby}";
