...
{
    public partial class ListAssetBooking : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public ListAssetBooking()
        {
        ...
        private void init()
        {
            ...
            if (ReadSession().UserType == "A")
            {
                ...
                + " on p.id=a.product_id inner join Branches b on b.BRANCH_ID=a.branch_id where 1=1";
            }
            else
            {
                sql = "select a.id,b.BRANCH_NAME as branch_id,p.porduct_code as product_id,convert(varchar,booked_date,107) as booked_date,asset_number,"
                + " dbo.ShowDecimal(purchase_value) as purchase_value,dbo.ShowDecimal(acc_depriciation) as acc_depriciation,asset_status,"
                + " convert(varchar,depr_start_date,107) as depr_start_date,a.narration from ASSET_INVENTORY a inner join ASSET_PRODUCT p"
                + " on p.id=a.product_id inner join Branches b on b.BRANCH_ID=a.branch_id where a.branch_id=" + ReadSession().Branch_Id + "";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListAssetBooking.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\"  href=\"/AssetManagement/ManageAssetBooking.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/AssetManagement/ManageAssetBooking.aspx");
