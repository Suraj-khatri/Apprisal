...
{
    public partial class ListAssetCapitalization : BasePage
    {
        ClsDAOInv _clsdao = null;
        RoleMenuDAOInv _roleMenuDao = new RoleMenuDAOInv();
        public ListAssetCapitalization()
        {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"Capitalization_date",        "Capitalized Date",   "200",      "D"},
                        {"Capitalization_amt",        "Capitalized Amt",         "200",      "M"},
                        {"Value_After_Capitalized",          "Cost After Capitalized",      "200",      "M"} ,
            ...
            string sortby = "Id";
            string table_name = "ASSET_CAPITALIZATION_TEMP";


            String sql = "SELECT c.Id,c.Asset_id,i.asset_number,CONVERT(VARCHAR,c.Capitalization_date,107) AS Capitalization_date,status"
                + " ,dbo.ShowDecimal(c.Capitalization_amt) AS  Capitalization_amt,dbo.ShowDecimal(i.purchase_value) AS Value_After_Capitalized,dbo.ShowDecimal(acc_depriciation) as acc_depriciation,c.narration"
                + " FROM ASSET_CAPITALIZATION_TEMP c INNER JOIN asset_inventory i ON i.id = c.asset_id  WHERE c.created_by="+ReadSession().Emp_Id+"";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListAssetCapitalization.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Modify\" href=\"/AssetManagement/ManageAssetCapitalization.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-pencil\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/AssetManagement/ManageAssetCapitalization.aspx");
