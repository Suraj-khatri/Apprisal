...
{
    public partial class ListAssetDelete : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 279) == false)
                {
        ...
        private void init()
        {
            ...
            string sortby = "id";
            string table_name = "ASSET_INVENTORY";
            ...
            if (ReadSession().UserType == "A")
            {
                ...
                + " convert(varchar,depr_start_date,107) as depr_start_date,a.narration, a.IsDeleteStatus as Status from ASSET_INVENTORY a inner join ASSET_PRODUCT p"
                + " on p.id=a.product_id inner join Branches b on b.BRANCH_ID=a.branch_id where IsDeleteStatus ='Pending' and approved_by=" + ReadSession().Emp_Id.ToString();
            }
            else
            {
                sql = "select a.id,b.BRANCH_NAME as branch_id,p.porduct_code as product_id,convert(varchar,booked_date,107) as booked_date,asset_number,"
                + " dbo.ShowDecimal(purchase_value) as purchase_value,dbo.ShowDecimal(acc_depriciation) as acc_depriciation,asset_status,"
                + " convert(varchar,depr_start_date,107) as depr_start_date,a.narration, a.IsDeleteStatus as Status from ASSET_INVENTORY a inner join ASSET_PRODUCT p"
                + " on p.id=a.product_id inner join Branches b on b.BRANCH_ID=a.branch_id where a.branch_id=" + ReadSession().Branch_Id + " and where IsDeleteStatus ='Pending' and approved_by=" + ReadSession().Emp_Id.ToString();
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListAssetDelete.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/AssetManagement/DeleteAssetBooking.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/AssetManagement/DeleteAssetBooking.aspx");
            ...
            for (int i = 0; i <= arr_cnt; i++)
            {
                ...
                if (ctlType == "ED")
                {
                    html.Append("<input type=\"text\" name=\"" + ctlName + "\" value=\"" + defValue + "\" class=\"form-control\">" +
                        "<script language=\"JavaScript\"> new tcal ({ 'formname': 'form1','controlname': '" + ctlName + "'});</script> </td>");
                }
                else
