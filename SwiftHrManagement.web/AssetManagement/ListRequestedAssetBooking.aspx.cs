...
{
    public partial class ListRequestedAssetBooking : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        string finalSql = "";
        public ListRequestedAssetBooking()
        {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        ...
                        {"p.porduct_code",    "Asset Type",         "LT"},
                        {"booked_date",    "Booked Date",         "ED"},
                        {"purchase_value",    "Purchase Value",         "EN"},
                        {"b.BRANCH_NAME",    "Assigned Branch",         "LT"},
                        {"asset_status",    "Asset Status",         "LT"},
                        {"a.narration",    "Asset Narration",         "LT"}
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        //{"asset_number",            "Asset Number",             "200",      "T"},
                        {"product_id",       "Asset Type",        "200",      "T"},
                        {"booked_date",    "Booked Date",     "200",      "D"},
                        {"depr_start_date",    "Dep. Start Date",     "200",      "D"},
                        {"purchase_value",    "Purchase Value",     "200",      "M"},
                        {"acc_depriciation",    "Acc. Dep.",     "200",      "M"},
                        {"asset_status",    "Status",     "200",      "T"},
                        {"branch_id",    "Assigned Branch",     "200",      "T"},
                        {"status",    "Req. Status",     "200",      "T"}
                        };



            string sortby = "id";
            string table_name = "ASSET_INVENTORY_TEMP";

            String sql = "";
            
                sql = "select a.id,b.BRANCH_NAME as branch_id,p.porduct_code as product_id,convert(varchar,booked_date,107) as booked_date,asset_number,"
                + " dbo.ShowDecimal(purchase_value) as purchase_value,dbo.ShowDecimal(acc_depriciation) as acc_depriciation,asset_status,"
                + " convert(varchar,depr_start_date,107) as depr_start_date,a.narration,status from ASSET_INVENTORY_TEMP a inner join ASSET_PRODUCT p"
                + " on p.id=a.product_id inner join Branches b on b.BRANCH_ID=a.branch_id where a.created_by=" + ReadSession().Emp_Id + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListRequestedAssetBooking.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/AssetManagement/AssetBookingRequest.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/AssetManagement/AssetBookingRequest.aspx");
        ...
        private string make_filter_form(string table_name, string[,] matfilter)
        {
            ...
            string defValue = "";
            string ctlType = "";
            StringBuilder html = new StringBuilder("<table width=\"850\" cellspacing=\"2\" cellpadding=\"2\" border=\"0\">");
        ...
        private void ExportReport()
        {
            ...
            {  
                if(i==5)
                     result.Append("<th Class=\"HeaderStyle\" align=\"left\">Asset Type</a></th>");              
                 if (i == 4)
                    result.Append("<th Class=\"HeaderStyle\" align=\"left\">Booked Date</a></th>");
                 if (i == 9)
                    result.Append("<th Class=\"HeaderStyle\" align=\"left\">Dep.Start Date</a></th>");               
                 if (i == 6)
                    result.Append("<th Class=\"HeaderStyle\" align=\"left\">Purchase Value</a></th>");
                 if (i == 7)
                    result.Append("<th Class=\"HeaderStyle\" align=\"left\">Acc.Dep</a></th>");
                 if (i == 8)
                    result.Append("<th Class=\"HeaderStyle\" align=\"left\">Status</a></th>");
                 if (i == 2)
                    result.Append("<th Class=\"HeaderStyle\" align=\"left\">Assigned Branch</a></th>");             
                 if (i == table.Columns.Count - 1)
                    result.Append("<th Class=\"HeaderStyle\" align=\"left\">Req.Status</a></th>");
            }
            result.Append("</tr>");
