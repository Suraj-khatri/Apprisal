...
{
    public partial class ListRevAssetBooking : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public ListRevAssetBooking()
        {
        ...
        private void init()
        {
            ...
            sql = "SELECT a.id,b.BRANCH_NAME AS branch_id,p.porduct_code AS product_id,CONVERT(VARCHAR,booked_date,107) AS booked_date,asset_number,"
                    +" dbo.ShowDecimal(purchase_value) AS purchase_value,asset_status FROM ASSET_INVENTORY a INNER JOIN ASSET_PRODUCT p"
                    +" ON p.id=a.product_id INNER JOIN Branches b ON b.BRANCH_ID=a.branch_id WHERE asset_type='R'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListRevAssetBooking.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/AssetManagement/ManageRevAssetBooking.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/AssetManagement/ManageRevAssetBooking.aspx");
