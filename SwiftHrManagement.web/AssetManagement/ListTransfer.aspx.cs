...
{
    public partial class ListTransfer : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"forwarded_date",    "Forwarded Date",     "200",      "D"},
                        {"status",    "Status",     "200",      "T"},
                        };
            ...
            string sortby = "id";
            string table_name = "ASSET_TRANSFER_TEMP";

            String sql = "";
           
                sql = "SELECT t.id,i.asset_number,a.porduct_code AS asset_type,dbo.GetBranchName(t.from_branch) from_branch,"
                        +" dbo.GetBranchName(t.to_branch) to_branch,dbo.GetEmployeeFullNameOfId(t.forwarded_to) AS forwarded_to,"
                        +" convert(varchar,t.forwarded_date,107) as forwarded_date,status FROM ASSET_TRANSFER_TEMP t WITH(NOLOCK)" 
                        +" inner join ASSET_INVENTORY i with(nolock) on i.id=t.asset_id inner join ASSET_PRODUCT a WITH(NOLOCK) "
                        +" ON a.id=i.product_id where t.created_by="+ReadSession().Emp_Id+"";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/AssetManagement/ManageTransfer.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/AssetManagement/ManageTransfer.aspx");
