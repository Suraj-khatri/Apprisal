...
{
    public partial class ListModAsset : BasePage
    ...
    public ListModAsset()
        ...
        private void init()
        {
            ...
            if (ReadSession().UserType == "A")
            {
                ...
                + " convert(varchar,depr_start_date,107) as depr_start_date,a.narration from ASSET_INVENTORY a inner join ASSET_PRODUCT p"
                + " on p.id=a.product_id inner join Branches b on b.BRANCH_ID=a.branch_id where (IsDeleteStatus is null or IsDeleteStatus='Rejected') and  a.branch_id=" + ReadSession().Branch_Id + "";
            }
            else
            {
                sql = "select a.id,b.BRANCH_NAME as branch_id,p.porduct_code as product_id,convert(varchar,booked_date,107) as booked_date,asset_number,"
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListModAsset.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/AssetManagement/ModifyAssetLocation/ManageModAsset.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/AssetManagement/ModifyAssetLocation/ManageModAsset.aspx");
