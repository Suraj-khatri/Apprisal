...
{
    public partial class ViewAssetTransferRequest : BasePage
    {
        RoleMenuDAOInv _roleMenuDao = null;
        public ViewAssetTransferRequest()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_roleMenuDao.hasAccess(ReadSession().AdminId, 209) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"i.asset_number",    "Asset Number",         "LT"}
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"from_branch",       "From Branch",        "200",      "T"},
                        {"to_branch",    "To Branch",     "200",      "T"},
            ...
            string sortby = "id";
            string table_name = "ASSET_TRANSFER_HISTORY";

            String sql = "";
            if (ReadSession().UserType == "A")
            {
                sql = "SELECT t.id,i.asset_number,t.book_value,dbo.GetBranchName(from_branch) from_branch ,dbo.GetBranchName(to_branch) to_branch,"
                        +" t.narration,dbo.GetEmployeeFullNameOfId(t.created_by) AS requested_by,"
                        +" CONVERT(varchar,t.created_date,107) as requested_date,dbo.GetEmployeeFullNameOfId(t.approved_by) AS approved_by,"
                        +" convert(varchar,t.approved_date,107) AS approved_date FROM ASSET_TRANSFER_HISTORY t WITH(NOLOCK) inner join "
                        +" ASSET_INVENTORY i with(nolock) on i.id=t.asset_id where 1=1";
            }
            else
            {
                sql = "SELECT t.id,i.asset_number,t.book_value,dbo.GetBranchName(from_branch) from_branch ,dbo.GetBranchName(to_branch) to_branch,"
                        ...
                        + " CONVERT(varchar,t.created_date,107) as requested_date,dbo.GetEmployeeFullNameOfId(t.approved_by) AS approved_by,"
                        + " convert(varchar,t.approved_date,107) AS approved_date FROM ASSET_TRANSFER_HISTORY t WITH(NOLOCK) inner join "
                        + " ASSET_INVENTORY i with(nolock) on i.id=t.asset_id where i.branch_id=" + ReadSession().Branch_Id + "";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListTransfer.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //html.Append("<td><a href=\"/AssetManagement/ApproveTransfer.aspx?Id=" + dr["id"] + "\">View</a></td>");
                html.Append("</tr>");
            }
