...
namespace SwiftAssetManagement.AssetParameters.Item
{
    ...
    public partial class EditBranchProduct : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = new RoleMenuDAOInv();
        ClsDAOInv _clsdao = null;

        public EditBranchProduct()
        {
        ...
        {
            if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 127) == false)
            {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"item_name",    "Product Name",         "LT"},
                        {"BRANCH_NAME",    "Branch Name",         "LT"},
                        {"ASSET_AC",    "Asset A/C",         "LT"},                        
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        
                        {"product_desc",       "Product Name",      "200",      "LT"},
                        {"porduct_code",       "Product Code",      "200",      "LT"},
                        {"BRANCH_NAME",        "Branch Name",       "200",      "LT"},
                        {"ASSET_AC",           "Asset A/C",         "200",      "LT"},
                        {"DEPRECIATION_EXP_AC",       "Depreciation A/C",     "200",      "LT"},
                        {"ACCUMULATED_DEP_AC",            "Accu. Dep. A/C",         "200",      "LT"}
                        };
            ...
            if (Request.QueryString["product"] != null)
                hdnproduct.Value = Request.QueryString["product"].ToString();


            //String sql = "select I.ID, P.ITEM_ID, I.SALES_AC,I.INVENTORY_AC, I.COMM_AC, B.BRANCH_NAME, IT.item_name, P.porduct_code, p.product_desc from ASSET_BRANCH I with(nolock)"
            //             + "INNER JOIN Branches B with(nolock) ON I.BRANCH_ID = B.BRANCH_ID INNER JOIN ASSET_PRODUCT P ON I.PRODUCT_ID = P.id "
            //             + "INNER JOIN ASSET_ITEM IT with(nolock) ON P.item_id = IT.id WHERE I.PRODUCT_ID = '" + hdnproductId.Value + "'";
            ...
            if (GetProdutId() > 0)
            {
                 ...
                 sql = " SELECT I.ID, P.ITEM_ID, I.ASSET_AC,I.DEPRECIATION_EXP_AC, I.ACCUMULATED_DEP_AC,I.WRITE_OFF_EXP_AC,"
                             + "I.SALES_PROFIT_LOSS_AC,I.MAINTAINANCE_EXP_AC,B.BRANCH_NAME, IT.item_name, P.porduct_code, p.product_desc from ASSET_BRANCH I with(nolock)"
                             + "INNER JOIN Branches B with(nolock) ON I.BRANCH_ID = B.BRANCH_ID INNER JOIN ASSET_PRODUCT P ON I.PRODUCT_ID = P.id "
                             + "INNER JOIN ASSET_ITEM IT with(nolock) ON P.item_id = IT.id WHERE I.PRODUCT_ID = '" + GetProdutId() + "' and 1=1";
            }
            else
            {
                 sql = " SELECT I.ID, P.ITEM_ID, I.ASSET_AC,I.DEPRECIATION_EXP_AC, I.ACCUMULATED_DEP_AC,I.WRITE_OFF_EXP_AC,"
                             + "I.SALES_PROFIT_LOSS_AC,I.MAINTAINANCE_EXP_AC,B.BRANCH_NAME, IT.item_name, P.porduct_code, p.product_desc from ASSET_BRANCH I with(nolock)"
                             + "INNER JOIN Branches B with(nolock) ON I.BRANCH_ID = B.BRANCH_ID INNER JOIN ASSET_PRODUCT P ON I.PRODUCT_ID = P.id "
                             + "INNER JOIN ASSET_ITEM IT with(nolock) ON P.item_id = IT.id WHERE 1=1";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "EditBranchProduct.aspx?sortd=" + sortd + "&sortby={sortby}&productId=" + GetProdutId() + "";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/AssetParameters/Item/BranchProdSetup.aspx?id=" + dr["id"] + "&product=" + dr["product_desc"].ToString() + "&productId=" + GetProdutId() + "\"><i class=\"fa fa-eye\"></i></a></td>");
                //html.Append("<td><a href=\"/Project/Task/Manage.aspx?ID=" + dr["id"] + "/a></td>");
                html.Append("</tr>");
            }

            html.Append(" <tr> <td colspan=\"6\" align=\"center\"> <strong> Page " + _page.ToString() + " Of " + total_page + "</strong></td></tr>");
            html.Append("</table>");
            html.Append("</div>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/AssetParameters/Item/BranchProdSetup.aspx?productId=" + GetProdutId() + "&product=" + hdnproduct.Value);
