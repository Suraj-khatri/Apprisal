...
namespace SwiftAssetManagement.AssetParameters.Item
{
    public partial class EditList : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 128) == false)
                {
    ...
    #region SwiftGrid
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"item_name",    "Item Name",         "LT"}
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        
                        {"item_name",           "Item Name",         "200",      "LT"},
                        {"item_desc",           "Item Descreiption", "200",      "LT"},                        
                        };

            string sortby = "item_name";
            string table_name = "IN_ITEM";

            String sql = "select id ,item_name, item_desc from ASSET_ITEM where ISNULL(is_product,0) <> 1 and 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "EditList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/AssetParameters/Item/Manage.aspx?id=" + dr["id"] + "\">View</a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/AssetParameters/Item/Manage.aspx");
