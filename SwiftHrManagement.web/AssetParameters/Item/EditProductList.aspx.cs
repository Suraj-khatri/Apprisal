...
#region SwiftGrid
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"item_name",    "Asset Type Name",         "LT"},
                        {"porduct_code", "Asset Type Code",         "LT"},
                        {"product_desc", "Asset Type Description",  "LT"}
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        
                        {"ID",        "Asset Type ID",      "200",      "LT"},
                        {"porduct_code",        "Asset Type Name",      "200",      "LT"},
                        {"product_desc",        "Asset Type Desc",     "200",      "LT"},
                        {"ASSET_CODE",        "Asset Type Code",     "200",      "LT"},
                        };

            string sortby = "ID";
            string table_name = "IN_PRODUCT";

            String sql = "SELECT P.ID,P.PORDUCT_CODE,P.ITEM_ID,I.ITEM_NAME,P.PRODUCT_DESC,P.ASSET_CODE FROM  ASSET_PRODUCT P"
                        + " WITH(NOLOCK), ASSET_ITEM I WITH(NOLOCK) WHERE I.ID = P.ITEM_ID";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">View Branch</th>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/AssetParameters/Item/ProductSetup.aspx?productId=" + dr["id"] + "\">View</a></td>");
                html.Append("<td><a href=\"/AssetParameters/Item/EditBranchProduct.aspx?productId=" + dr["id"] + "&product=" + dr["product_desc"] + "\">View Branch</a></td>");
                //html.Append("<td><a href=\"/Inventory/Item/BranchProdSetup.aspx?ID=" + dr["id"] + "&product=" + dr["item_name"] + "\"/a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/AssetParameters/AssetGrouping.aspx?");
