    ...
    public partial class ApproveList : BasePage
    {
       ...
       public ApproveList()
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 239) == false)
                {
        ...
        private void init()
        {
            ...
            sql = @"SELECT A.id,emp_id,E.EMP_CODE,E.FIRST_NAME + ' ' + E.MIDDLE_NAME + ' ' + E.LAST_NAME AS EMP_NAME,
                        ...
                        left join attendance_reason c on c.att_id=a.id where att_status='Requested' and approved_by="+ReadSession().Emp_Id+"";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
           ...
           // sql1 += " order by " + sortby + " " + sortd;
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //html.Append("<td><a href=\"/AttendenceWeb/ApproveRequest.aspx?Id=" + dr["id"] + "\">View</a></td>");
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/AttendenceWeb/ApproveRequest.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a>");
                html.Append("</tr>");
            }
