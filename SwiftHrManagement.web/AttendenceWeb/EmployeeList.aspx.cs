...
namespace SwiftHrManagement.web.AttendenceWeb
{
    public partial class EmployeeList : BasePage
    {
        ...
        {
            return (Request.QueryString["group_id"] != null ? (Request.QueryString["group_id"].ToString()) : "");
        }
        #region SwiftGrid
        ...
        private void init()
        {
            ...
            string sortby = "id";
            string table_name = "attendance_group_details";

            String sql = "";

            sql = @"select a.id,sdd.DETAIL_TITLE group_name,c.FIRST_NAME+' '+c.MIDDLE_NAME+' '+LAST_NAME emp_name,a.status,
                    dbo.GetEmployeeFullNameOfId(a.created_by) created_by,CONVERT(varchar,a.created_date,107) as created_date
	                from attendance_group_details a inner join attendance_group b on a.att_group_id=b.id
	                inner join StaticDataDetail sdd on sdd.ROWID=b.name inner join Employee c on c.EMPLOYEE_ID=a.emp_id
	                where a.status='Active' and a.att_group_id="+GetGroupId()+"";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "EmployeeList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/AttendenceWeb/AddEmployee.aspx?group_id=" + GetGroupId() + "&Id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/AttendenceWeb/AddEmployee.aspx?group_id=" + GetGroupId() + "");
