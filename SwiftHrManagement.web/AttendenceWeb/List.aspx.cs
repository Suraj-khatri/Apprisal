...
namespace SwiftHrManagement.web.AttendenceWeb
{
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            sql = @"SELECT A.id,E.EMP_CODE,E.FIRST_NAME + ' ' + E.MIDDLE_NAME + ' ' + E.LAST_NAME AS EMP_NAME,A.login_time,cast((A.login_time)as date) LoginDate,cast((A.logout_time)as Date) LogoutDate,A.logout_time,
                        c.login_reason,c.logout_reason,A.login_ip,a.logout_ip
                        FROM atttendance A inner join Employee E on A.emp_id=E.EMPLOYEE_ID 
                        left join attendance_reason c on c.att_id=a.id
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/AttendenceWeb/BackDateLogin.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/AttendenceWeb/BackDateLogin.aspx");
