...
{
    public partial class ListOfficeTime : BasePage
    {
        RoleMenuDAOInv _roleMenuDao = null;
        public ListOfficeTime()
        {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                            ...
                            {"ID",            "ID",             "200",      "M"},
                            {"flag",            "Office Timing",             "200",      "T"},
                            {"from_date",            "From Date",             "200",      "D"},
                            {"to_date",            "To Date",             "200",      "D"},
                            {"from_time",            "From Time",             "200",      "D"},
                            {"to_time",            "To Time",             "200",      "D"},
                            {"toleranceFromTime",            "Tol. From Time",             "200",      "D"},
                            {"toleranceToTime",            "Tol. To Time",             "200",      "D"},
            ...
            string sortby = "ID";
            string table_name = "OfficeTimeSetup";
            ...
            sql = @"select * from 
(select id as ID,case when flag='s' then 'Summer Office Time' when flag='w' then 'Winter Office Time' else 'Friday Office Time' end as flag,
               convert(varchar,from_date,107) from_date, convert(varchar,to_date,107) to_date,from_time,to_time,dbo.GetEmployeeFullNameOfId(created_by) created_by,
                convert(varchar,created_date,107) created_date,toleranceFromTime,toleranceToTime from OfficeTimeSetup where 1=1
            )a where 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListOfficeTime.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/AttendenceWeb/ManageOfficeTime.aspx?Id=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/AttendenceWeb/ManageOfficeTime.aspx");
