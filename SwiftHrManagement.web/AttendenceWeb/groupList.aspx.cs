    ...
    public partial class groupList : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"sdd.DETAIL_TITLE",    "Group Name",         "LT"},  
            ...
            string sortby = "id";
            string table_name = "attendance_group";

            String sql = "";

            sql = @"select a.id,sdd.DETAIL_TITLE group_name,a.description,a.status,
                    dbo.GetEmployeeFullNameOfId(a.created_by) created_by,CONVERT(varchar,a.created_date,107) as created_date
                    from attendance_group a inner join StaticDataDetail sdd on sdd.ROWID=a.name
                    where 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
           ...
           // sql1 += " order by " + sortby + " " + sortd;
            ...
            string thisPage = "groupList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/AttendenceWeb/groupManage.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a>");
                html.Append("<a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\"  href=\"/AttendenceWeb/EmployeeList.aspx?group_id=" + dr["id"] + "\"><i class=\"fa fa-plus\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/AttendenceWeb/groupManage.aspx");
