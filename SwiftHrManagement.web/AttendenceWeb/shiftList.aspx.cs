    ...
    public partial class shiftList : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"sdd.DETAIL_TITLE",    "Shift Name",         "LT"},  
                        {"a.status",    "Status",         "LT"}, 
                        {"b.name",    "Weekly Schedule Name",         "LT"}                   
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"shift_name",            "Shift Name",             "200",      "T"},
                        {"shift_desc",            "Description",             "200",      "T"},
                        {"shift_status",            "Status",             "200",      "T"},
                        {"weekly_schedule",       "Weekly Schedule Name",        "200",      "T"}
                        };



            string sortby = "id";
            string table_name = "attendance_shift";

            String sql = "";

            sql = @"select a.id,sdd.DETAIL_TITLE shift_name,a.description shift_desc,a.status shift_status,sdd1.DETAIL_TITLE [weekly_schedule] 
                    from attendance_shift a inner join att_weekly_schedule b on b.id=a.att_ws_id 
                    inner join StaticDataDetail sdd on sdd.ROWID=a.name inner join StaticDataDetail sdd1 on sdd1.ROWID=b.name
                    where 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
           ...
           // sql1 += " order by " + sortby + " " + sortd;
            ...
            string thisPage = "shiftList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\"  href=\"/AttendenceWeb/shiftManage.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a>");
                html.Append("<a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/AttendenceWeb/addGroup.aspx?shift_id=" + dr["id"] + "\"><i class=\"fa fa-plus\"></i></a></td>");                
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/AttendenceWeb/shiftManage.aspx");
