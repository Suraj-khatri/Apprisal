...
{
    public partial class weeklyScheduleList : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public weeklyScheduleList()
        {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"sdd.DETAIL_TITLE",    "Weekly Schedule Name",         "LT"},  
                        {"a.description",    "Description",         "LT"}, 
                        {"a.status",    "Status",         "LT"}                 
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"schedule_name",            "Weekly Schedule Name",             "200",      "T"},
                        {"description",            "Description",             "200",      "T"},
                        {"status",            "Status",             "200",      "T"},
                        {"created_by",       "Created By",        "200",      "T"},
                        {"created_date",       "Created Date",        "200",      "D"}
                        };



            string sortby = "id";
            string table_name = "att_weekly_schedule";

            String sql = "";

            sql = @"select a.id,sdd.DETAIL_TITLE schedule_name,a.description,a.status,
                        dbo.GetEmployeeFullNameOfId(a.created_by) created_by,
                        CONVERT(varchar,a.created_date,107) created_date
                        from att_weekly_schedule a inner join StaticDataDetail sdd on sdd.ROWID=a.name
                        where 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
           ...
           // sql1 += " order by " + sortby + " " + sortd;
            ...
            string thisPage = "weeklyScheduleList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/AttendenceWeb/weeklyScheduleManage.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a>");
                html.Append("<a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Add\"href=\"/AttendenceWeb/AddSchedule.aspx?ws_id=" + dr["id"] + "\"><i class=\"fa fa-plus\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/AttendenceWeb/weeklyScheduleManage.aspx");
