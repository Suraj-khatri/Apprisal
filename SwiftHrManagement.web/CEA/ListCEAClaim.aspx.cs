...
{
    public partial class ListCEAClaim : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 251) == false || _RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 258) == false)
                {
        ...
        private void init()
        {
            ...
            else
            {
                sql = @"select cr.ID,rtrim(dbo.GetEmployeeFullNameOfId(Emp_ID)) Emp_ID,convert(varchar,Bill_date,107) Bill_date,Bill_Amount,From_FY,ml.Name From_month,To_FY, ml1.Name To_month ,cea_status
                        From CEA_Reimbursment cr
	                    Inner join MonthList ml on cr.from_month=ml.Month_Number
	                    Inner join MonthList ml1 on cr.to_month=ml1.Month_Number WHERE 1=1"; 
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/CEA/ManageCEACliam.aspx?ID=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            if (GetFlag() == "i")
            {
                navigation_bar= get_paging_block(total_record, _page, page_size, true, "/CEA/ManageCEACliam.aspx?flag=i");
            }
            else
            {
               navigation_bar= get_paging_block(total_record, _page, page_size, true, "/CEA/ManageCEACliam.aspx");
            }
