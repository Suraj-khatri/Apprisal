...
{
    public partial class ListCEAClaimIndividual : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public ListCEAClaimIndividual()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 34) == false)
                {
        ...
        private void init(long EmpId)
        {
            ...
            String[,] matgrid = new String[,]{
                            ...
                            {"Bill_date",           "Bill No",          "200",      "T"},
                            {"Bill_Amount",         "Amount",           "200",      "M"},
                            {"From_FY",             "From FY",          "200",      "T"},
                            {"From_month",          "From FY",          "200",      "T"},
                            {"To_FY",               "To FY"  ,          "200",      "T"},
            ...
            if (GetFlag() == "i")
            {
                sql = @"select cr.ID,rtrim(dbo.GetEmployeeFullNameOfId(Emp_ID)) Emp_ID,convert(varchar,Bill_date,107) Bill_date,Bill_Amount,From_FY,ml.Name From_month,To_FY, ml1.Name To_month, cea_status
                        From CEA_Reimbursment cr
	                    Inner join MonthList ml on cr.from_month=ml.Month_Number
	                    Inner join MonthList ml1 on cr.to_month=ml1.Month_Number WHERE emp_id=" + filterstring(ReadSession().Emp_Id.ToString()) + "";
            }
            else
            {
                sql = @"select cr.ID,rtrim(dbo.GetEmployeeFullNameOfId(Emp_ID)) Emp_ID,convert(varchar,Bill_date,107) Bill_date,Bill_Amount,From_FY,ml.Name From_month,To_FY, ml1.Name To_month ,cea_status
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListCEAClaim.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            //html.Append("<th Class=\"HeaderStyle\" align=\"left\">View</td>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //html.Append("<td><a href=\"/CEA/ManageCEACliamIndividual.aspx?ID=" + dr["ID"] + "\">View</a></td>");
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/CEA/ManageCEACliamIndividual.aspx?ID=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
            ...
            if (GetFlag() == "i")
            {
                navigation_bar = get_paging_block(total_record, _page, page_size, true, "/CEA/ManageCEACliamIndividual.aspx?flag=i");
            }
            else
            {
                navigation_bar = get_paging_block(total_record, _page, page_size, true, "/CEA/ManageCEACliamIndividual.aspx");
            }
