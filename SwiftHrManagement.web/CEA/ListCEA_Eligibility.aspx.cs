...
{
    public partial class ListCEA_Eligibility : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public ListCEA_Eligibility()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 250) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                            ...
                            {"Eligible_amt",        "Eligible Amt",     "200",      "M"},
                            {"Claimed_amt",          "Claimed Amt",     "200",      "M"}
                        };

            string sortby = "ID";
            string table_name = "CEA_Eligibility";

            String sql = "";

            sql = @"Select ID,rtrim(dbo.GetEmployeeFullNameOfId(Emp_ID)) Emp_ID,Eligible_amt,Claimed_amt from CEA_Eligibility WHERE 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, false, "/EmployeeMovement/EmployeeDiscontinuous/Manage.aspx");
