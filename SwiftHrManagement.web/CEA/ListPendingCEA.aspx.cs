...
{
    public partial class ListPendingCEA : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        CEADao _ceaDao = null;
        public ListPendingCEA()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 256) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"DBO.GetEmployeeFullNameOfId(Emp_ID)",    "Employee Name",         "LT"}                
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                            {"Emp_ID",              "Employee Name",    "200",      "T"},
                            {"Child_Name",          "Child Name",       "200",      "T"},
                            {"Bill_date",           "Bill Date",        "200",      "T"},                            
                            {"From_FY",             "From FY",          "200",      "T"},
                            {"From_month",          "From Month",       "200",      "T"},
                            {"To_FY",               "To FY"  ,          "200",      "T"},
                            {"To_month",            "To Month",         "200",      "T"},                           
                            {"eligibleValue",       "Eligible Amount",  "200",      "M"},
                            {"Bill_Amount",         "Amount",           "200",      "M"},
                            {"cea_status",          "Status",           "200",      "T"}
                        };

            string sortby = "ID";
            string table_name = "CEA_Assignment";

            String sql = @"select *,ceavalue * total_month as eligibleValue ,
                           reimbursed_Amount = case when x.bill_Amount> (ceavalue * total_month) then (ceavalue * total_month) else x.bill_Amount end from (
                                select cr.ID,rtrim(dbo.GetEmployeeFullNameOfId(Emp_ID)) Emp_ID ,convert(varchar,Bill_date,107) Bill_date
                                ,RTRIM(fm.FIRST_NAME+ ' ' + isnull(fm.MIDDLE_NAME,'') + ' ' + fm.LAST_NAME)[Child_Name]
                                ,Bill_Amount,From_FY,ml.Name From_month,To_FY, ml1.Name To_month, cr.File_type ,sd.ceavalue,cr.cea_status
                                ,To_month - From_month+1 as total_month
                                 From CEA_Reimbursment cr
                                Inner join MonthList ml on cr.from_month=ml.Month_Number
                                Inner join MonthList ml1 on cr.to_month=ml1.Month_Number 
                                inner join Employee e on e.EMPLOYEE_ID = cr.Emp_id 
                                inner join familymembers fm on fm.id = cr.Child_Name
                                inner join StaticDataDetail sd on sd.ROWID = e.POSITION_ID 
                                WHERE cea_status <> 'Rejected' and cea_status <> 'Posted' and 1=1) x";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListPendingCEA.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            html.Append("<th class=\"text-center\" align=\"center\">Reimbursed Amount</th>");
            html.Append("<th class=\"text-center\" style=\"width:160px;\" align=\"center\">Action</th>");

            //html.Append("<th Class=\"HeaderStyle\" align=\"left\">Reimbursed Amount</td>");
            //html.Append("<th Class=\"HeaderStyle\" align=\"left\">View Bill</td>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //html.Append("<td><a href=\"/CEA/ApproveCEA.aspx?ID=" + dr["ID"] + "\">View Bill</a></td>");
                html.Append("<td align=\"right\" nowrap=\"nowrap\"><input type=\"textarea\" class=\"form-control\"  cols=\"20\" rows=\"2\" value=\"" + dr["reimbursed_Amount"] + "\" name=\"txtarea_" + dr["ID"] + "\" id=\"txtarea_" + dr["ID"] + "\"> </td>");
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View Bill\" href=\"/doc/CEABills/" + dr["id"].ToString() + "." + dr["file_type"].ToString() + "\"><i class=\"fa fa-eye\"></i></a> <a  class=\"tool btn btn-info btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Approve\" href=\"#\" onClick=ApproveReject(" + dr["ID"] + ",'A');><i class=\"fa fa-pencil\"></i></a>");
                html.Append("<a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Reject\" href=\"#\" onClick='ApproveReject(" + dr["ID"] + ",'R')'><i class=\"fa fa-eye\"></i></a></td>");

                //html.Append("<td><a href=\"/CEA/ApproveCEA.aspx?ID=" + dr["ID"] + "&flag=R\">Reject</a></td>");
            ...
            string navigation_bar = "";

            navigation_bar = get_paging_block(total_record, _page, page_size, false, "/CEA/ApproveCEA.aspx.aspx");
