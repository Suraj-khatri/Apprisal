    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        ...
                        {"DEPARTMENT_NAME",    "Department Name",         "LT"}                
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"BRANCH_ID",            "Branch Name",             "200",      "T"},
                        {"DEPARTMENT_NAME",            "Department Name",             "200",      "T"},
                        {"DEPARTMENT_SHORT_NAME",       "Department Short Name",        "200",      "T"}
                        };
            ...
            sql = "SELECT DEPARTMENT_ID,b.BRANCH_NAME as BRANCH_ID,DEPARTMENT_SHORT_NAME,DEPARTMENT_NAME FROM DEPARTMENTS d inner join "
                    +" Branches b on b.BRANCH_ID=d.branch_id where 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/DepartmentWeb/Manage.aspx?Id=" + Uri.EscapeUriString(StringEncryption.Encrypt(dr["DEPARTMENT_ID"].ToString())) + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/DepartmentWeb/Manage.aspx");
