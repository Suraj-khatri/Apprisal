...
{
    public partial class EmpInfoIndiv : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            session.CanViewProfile = false;

            if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 12) == false)
                {
        ...
        private void init()
        {
            ...
            {
                sql = "SELECT  E.EMPLOYEE_ID,E.EMP_CODE,dbo.GetEmployeeFullNameOfId(E.Employee_Id) AS EmpName,B.BRANCH_NAME,"
                + "D.DEPARTMENT_NAME ,S.DETAIL_TITLE, s.DETAIL_TITLE as Post, E.PERSONAL_MOBILE as phone FROM Employee E INNER JOIN Branches B ON B.BRANCH_ID=E.BRANCH_ID"
            ...
            else
            {
                sql = "SELECT  E.EMPLOYEE_ID,E.EMP_CODE,dbo.GetEmployeeFullNameOfId(E.Employee_Id) AS EmpName,B.BRANCH_NAME,"
                + "D.DEPARTMENT_NAME ,S.DETAIL_TITLE, s.DETAIL_TITLE as Post, E.PERSONAL_MOBILE as phone FROM Employee E INNER JOIN Branches B ON B.BRANCH_ID=E.BRANCH_ID"
                + " INNER JOIN Departments D ON D.DEPARTMENT_ID=E.DEPARTMENT_ID INNER JOIN StaticDataDetail S ON S.ROWID=E.POSITION_ID"
                + "  WHERE e.EMP_CODE =  " + filterstring(ReadSession().UserId) + " and 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"All Info\" href=\"/Company/EmployeeWeb/EmployeeAllInfo.aspx?flag=IPR&Id=" + Uri.EscapeDataString(Crypto(dr["EMPLOYEE_ID"].ToString())) + "\"><i class=\"fa fa-eye\"></i></a></td>");
            }
