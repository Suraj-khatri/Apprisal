...
namespace SwiftHrManagement.web.Company.EmployeeWeb.ExtendHistory
{
    public partial class List : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 227) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"dbo.GetEmployeeFullNameOfId(EMPLOYEE_ID)",   "Employee Name",        "LT"},
                        {"dbo.GetDetailTitle(emp_type)",    "Employee Type",        "LT"},
                        {"dbo.GetBranchName(branch_id)",    "Branch Name",        "LT"},
            ...
            string sortby = "id";
            string table_name = "Employee_Contract";

            String sql = "";
            sql = @"SELECT RowID id,e.EMP_CODE,dbo.GetEmployeeFullNameOfId(ec.EMPLOYEE_ID) EMP_NAME,dbo.GetBranchName(ec.branch_id)BRANCH_NAME,
                        dbo.GetDeptName(dept_id) DEPT_NAME,dbo.GetDetailTitle(ec.position_id) POST_NAME,dbo.GetDetailTitle(ec.emp_type) EMP_TYPE,
                        CONVERT(VARCHAR,Cont_DateFrm,101) AS FROM_DATE,CONVERT(VARCHAR,Cont_DateTo,101) TO_DATE,
                        ec.Created_By CREATED_BY,CONVERT(VARCHAR,ec.Created_Date,101) CREATED_DATE
                        FROM Employee_Contract ec
						inner join Employee e on e.EMPLOYEE_ID = ec.EMPLOYEE_ID WHERE 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"Manage.aspx?id=" + StringEncryption.Encrypt( dr["id"].ToString()) + "\"><i class=\"fa fa-eye\"></i></a></td>");
            }
