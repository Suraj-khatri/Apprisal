...
{
    public partial class ListGrade : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"GRADE_TYPE",   "Grade Type",        "LT"},
                        {"GradeFlag",    "Grade Flag",        "LT"},
            ...
            String[,] matgrid = new String[,]{
                            ...
                            {"FY_ID", "Fiscal Year", "200", "T"},
                            {"appRating", "Appraisal Rating", "200", "T"},
                            {"Grade_Year", "Grade", "200", "T"},
                            {"NewBasic", "New Basic", "200", "M"},
                            {"OldBasic", "Old Basic", "200", "M"},
                            {"incrementSal", "Increment", "200", "M"},
                            {"Effective_Date", "Effective Date", "200", "D"},
                            {"Apply_Date", "Apply On", "200", "D"},
                            {"CreatedBy", "Created By", "200", "D"},
...
//		                ,case gradeFlag when 'i' then 'Increment' else 'Decrement' end gradeFlag
//		                ,CONVERT(varchar,effectiveDate,107) effectiveDate 
//	                from Grade g 
//	                inner join StaticDataDetail s on cast(g.grade as varchar)=s.DETAIL_TITLE
//	                where empId = '" +GetEmpId() + "'";

           sql=@"select a.id,b.EMP_CODE,dbo.GetEmployeeFullNameOfId(b.EMPLOYEE_ID) empName,
                FY_ID,dbo.GetDetailTitle(App_Rating) appRating,Grade_Year,dbo.ShowDecimal(amount) NewBasic,dbo.ShowDecimal(amount-old_basic) incrementSal,convert(varchar,Effective_Date,101) Effective_Date,
                convert(varchar,Apply_Date,101) Apply_Date,dbo.GetEmployeeFullNameOfId(a.Created_By) CreatedBy
                ,convert(varchar,a.Created_Date,101) CreatedDate,dbo.ShowDecimal(old_basic) OldBasic
                from Grade a inner join Employee b on a.Emp_Id=b.EMPLOYEE_ID where a.Emp_Id  = '" + GetEmpId() + "'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //html.Append("<td><a href=\"/Company/EmployeeWeb/Grade/ManageGrade.aspx?empId="+GetEmpId()+"&gradeId=" + dr["id"] + "\">View</a></td>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/Grade/ManageGrade.aspx?empId="+GetEmpId()+"");
