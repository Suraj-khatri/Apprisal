...
{
    public partial class ListGradeSetup : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public ListGradeSetup()
        {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"SDD.DETAIL_TITLE",   "POSITION NAME",        "LT"},
                        {"A.RATE",    "RATE",        "LT"}                       
            ...
            string sortby = "ID";
            string table_name = "GRADE_SETUP";

            String sql = "";
            sql = @"SELECT A.ID,SDD.DETAIL_TITLE POSITION,A.RATE,dbo.GetEmployeeFullNameOfId(A.CREATED_BY) CREATED_BY,
                    CONVERT(VARCHAR,A.CREATED_DATE,107) CREATED_DATE
                    FROM GRADE_SETUP A INNER JOIN StaticDataDetail SDD ON SDD.ROWID=A.POSITION_ID  WHERE 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListGradeSetup.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"ManageGradeSetup.aspx?id=" + dr["id"] + "\">View</a></td>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "ManageGradeSetup.aspx");
