...
{
    public partial class JDList : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 1118) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        {"Employee",        "Employee Name",         "100",       "T"},
                        {"Supervisor",      "Supervisor Name",       "100",       "T"},
                        {"Position",        "Position",              "100",       "T"},
                        {"startDate",        "Start Date",              "100",       "T"},
                        {"endDate",        "EndDate",              "100",       "T"},
                        {"Status",          "Status",                "500",       "T"}
                        };

            string sortby = "rowid";
            string table_name = "jobDescription";

            String sql = "";

            if (_db.CheckHrAdmin(ReadSession().Emp_Id))
            {
                sql = @"SELECT  j.rowId ,
                            ...
                            [Status] = CASE WHEN flag = 'Y' THEN 'ACCEPTED'
                                            WHEN flag = 'N' THEN 'PENDING'
                                            WHEN flag = 'A' THEN 'APPROVED'
                                            WHEN flag = 'D' THEN 'DISAGREED'

                                       END
                    FROM    jobDescription j
                            INNER JOIN dbo.Employee E ON j.empId = E.EMPLOYEE_ID";
            }
            else
            {
                //sql = @"SELECT  j.rowId ,
                ...
                //                       END
                //    FROM    jobDescription j
                //            INNER JOIN dbo.Employee E ON j.supervisorId = E.EMPLOYEE_ID WHERE j.flag IN ('Y','A','N','D') AND j.empId = " + ReadSession().Emp_Id + "";
                sql = sql + @"SELECT  j.rowId ,
                            E.FIRST_NAME + ' ' + ISNULL(E.MIDDLE_NAME, '') + ' ' + E.LAST_NAME AS Employee ,
                            dbo.GetBranchName(branchId) Branch ,
                            dbo.GetPosOfId(positionId) Position ,
                            dbo.GetEmployeeFullNameOfId(supervisorId) Supervisor ,
                            ...
                            [Status] = CASE WHEN flag = 'Y' THEN 'ACCEPTED'
                                       ...
                                       END
                    FROM    jobDescription j
                            INNER JOIN dbo.Employee E ON j.empId = E.EMPLOYEE_ID WHERE j.flag IN ('Y','A','N','D') AND j.supervisorId= " + ReadSession().Emp_Id + "";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            String sql1 = "select * from (select ROW_NUMBER() OVER (ORDER BY " + sortby + " ) as Id,* from(" + sql + " ) as aa) as tmp where 1 = 1 ";
            sql1 += " and  tmp.Id between " + ((_page - 1) * page_size + 1).ToString() + " and " + (_page * page_size).ToString();

            sql1 += " order by " + sortby + " " + sortd;

            string sqlCount = "select count(*) as totalRecord from (" + sql + ") as tmp;";

            string thisPage = "JDList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            html.Append("<td class='center'>Action </td>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Info\" href=\"/Company/EmployeeWeb/JobDescription/JobDescription.aspx?Id=" + Uri.EscapeDataString(Crypto(dr["rowId"].ToString())) + "&flag=" + Uri.EscapeDataString(Crypto(dr["Status"].ToString())) + "\"><i class=\"fa fa-info\"></i></a></td>");
            }
        ...
        private string get_paging_block(int _total_record, int _page, int _page_size, Boolean _show_add_button, string _add_page)
        {
            ...
            html.Append("<a class=\"tool btn btn-default\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Add\" href='/Company/EmployeeWeb/JobDescription/JobDescription.aspx' title=\"Add a new record\"><i class=\"fa fa-plus\"></i></a>");
            html.Append("</div>");
            html.Append("</div></div>");
        ...
        {
            id= Uri.UnescapeDataString(Crypto(id, false));

            string res = _db.Delete(id);
