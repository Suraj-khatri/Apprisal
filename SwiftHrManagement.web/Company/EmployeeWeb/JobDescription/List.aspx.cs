    ...
    public partial class List : BasePage
    {
            ...
            else
            {
                sql = @"SELECT  j.rowId ,
                            ...
                            [Status] = CASE WHEN flag = 'Y' THEN 'ACCEPTED'
                                       ...
                                       END
                    FROM    jobDescription j
                            INNER JOIN dbo.Employee E ON j.empId = E.EMPLOYEE_ID WHERE j.flag IN ('N','A','Y','D') AND j.empId = " + ReadSession().Emp_Id + "";
                //sql = sql + @"UNION ALL  SELECT  j.rowId ,
                //            E.FIRST_NAME + ' ' + ISNULL(E.MIDDLE_NAME, '') + ' ' + E.LAST_NAME AS Employee ,
                //            dbo.GetBranchName(branchId) Branch ,
                //            dbo.GetPosOfId(positionId) Position ,
                //            dbo.GetEmployeeFullNameOfId(supervisorId) Supervisor ,
                ...
                //                       END
                //    FROM    jobDescription j
                //            INNER JOIN dbo.Employee E ON j.empId = E.EMPLOYEE_ID WHERE j.flag IN ('Y','A','D','N') AND j.supervisorId= " + ReadSession().Emp_Id + "";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/EmployeeWeb/JobDescription/JobDescription.aspx?Id=" + Uri.EscapeDataString(Crypto(dr["rowId"].ToString())) + "&flag=" + Uri.EscapeDataString(Crypto(dr["Status"].ToString())) + "\"><i class=\"fa fa-info\"></i></a></td>");
            }
