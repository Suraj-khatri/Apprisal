    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{                         
                        {"EMP_CODE",        "Employee Code",        "100",      "T"},
                        {"EmpName",        "Full Name",        "200",      "T"},
                        {"BRANCH_NAME",        "Branch Name",        "200",      "T"},
                        {"DEPARTMENT_NAME",        "Department Name",        "200",      "T"},
                        {"Position",        "Designation",        "200",      "T"},
                        {"PERSONAL_MOBILE",        "Mobile Number",        "200",      "T"}
            ...
            sql = "SELECT E.EMPLOYEE_ID,E.EMP_CODE,dbo.GetEmployeeFullNameOfId(E.Employee_Id) AS EmpName,B.BRANCH_NAME,D.DEPARTMENT_NAME,"
            + "  dbo.GetDetailTitle(POSITION_ID) as Position,E.PERSONAL_MOBILE,sdd.DETAIL_DESC FROM Employee E left JOIN Branches B ON B.BRANCH_ID=E.BRANCH_ID left JOIN"
            + " Departments D ON D.DEPARTMENT_ID=E.DEPARTMENT_ID left join StaticDataDetail sdd on E.emp_status=sdd.ROWID WHERE 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"All Info\" href=\"/Company/EmployeeWeb/EmployeeAllInfo.aspx?Id=" + Uri.EscapeDataString(Crypto(dr["EMPLOYEE_ID"].ToString())) + "\"><i class=\"fa fa-info\"></i></a></td>");
            }
