...
{
    public partial class ListAdhocPayment : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public ListAdhocPayment()
        {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"s.DETAIL_TITLE",    "Payment Head",         "LT"}                 
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"ADD_DEDUCT",            "Type",             "200",      "T"},
                        {"HEAD_ID",            "Head",             "200",      "T"},
                        {"APPLIED_DATE",       "Applied Date",        "200",      "T"},
                        {"AMOUNT",    "Amount",     "200",      "M"},
                        {"TAX_DEDUCTED",    "Tax Deducted Amt",     "200",      "M"},
                        {"IS_PAID",    "Is Applied",     "200",      "T"}
                        };



            string sortby = "ID";
            string table_name = "AdhocPayment";

            String sql = "";
// sql = "SELECT ID,CASE WHEN ADD_DEDUCT='A' THEN 'Add' WHEN ADD_DEDUCT='D' THEN 'Deduct' end 'ADD_DEDUCT',"
//+ " S.DETAIL_TITLE AS HEAD_ID,convert(varchar,APPLIED_DATE,107) as APPLIED_DATE,dbo.showdecimal(AMOUNT) as AMOUNT,dbo.showdecimal(TAX_DEDUCTED) as TAX_DEDUCTED,CASE WHEN IS_PAID='True' then 'Yes' when IS_PAID='False'"
//+ " then 'No' end 'IS_PAID', case when IS_TAXED='True' then 'Yes' when IS_TAXED='False' then 'No' end 'IS_TAXED'"
//+ " from AdhocPayment A inner join StaticDataDetail S ON A.HEAD_ID=S.ROWID where EMPLOYEE_ID=" + GetEmpId() + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListAdhocPayment.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetEmpId() + "";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/EmployeeWeb/ManageAdhocPayment.aspx?Id=" + dr["ID"] + "&EmpId=" + GetEmpId() + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/ManageAdhocPayment.aspx?EmpId=" + GetEmpId() + "");
