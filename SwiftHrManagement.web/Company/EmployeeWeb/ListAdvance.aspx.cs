...
{
    public partial class ListAdvance : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"s.DETAIL_TITLE",    "Advance Type",         "LT"}
                     
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{ 
                        {"DETAIL_TITLE",        "Advance Type",        "200",      "T"},        
                        {"AMOUNT",        "Amount",        "200",      "M"},  
                        {"DATE_TAKEN",            "Taken Date",             "200",      "D"},
                        {"DEDUCTION_AMT",    "Deduction Amt",     "200",      "M"},                      
                        {"IS_FULLYPAID",    "Fully Paid?",     "200",      "T"},  
                        {"DEDUCTION_START_DATE",      "Deduction Start Date",     "200",      "D"},
                        {"REMAINING_BAL",      "Remaining Bal.",     "200",      "M"}
                        };

            string sortby = "ID";
            string table_name = "Advance";

            String sql = "";
            sql = "SELECT  A.ID,dbo.ShowDecimal(A.AMOUNT) as AMOUNT,dbo.showdecimal(isnull(A.REMAINING_BAL,0)) 'REMAINING_BAL', "
                    + " case when A.IS_FULLYPAID='True' then 'Yes' when A.IS_FULLYPAID='False' then 'No' end as IS_FULLYPAID,"
                    +" CONVERT(VARCHAR,A.DEDUCTION_START_DATE,107) AS DEDUCTION_START_DATE,dbo.showdecimal(A.DEDUCTION_AMT)"
                    +" as DEDUCTION_AMT,s.DETAIL_TITLE,CONVERT(VARCHAR,A.DATE_TAKEN,107) AS DATE_TAKEN FROM  Advance AS A inner join "
                    +" (select * from StaticDataDetail where TYPE_ID=35) s on s.ROWID = A.ADVANCE_TYPE "
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListAdvance.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetEmpId() + "";
            ...
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Advance Deduction</th>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Deduction\" href=\"/Company/EmployeeWeb/ListAdvanceCollection.aspx?Id=" + dr["ID"] + "&EmpId=" + GetEmpId() + "\">Deduction</a></td>");
                html.Append("<td><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/EmployeeWeb/ManageAdvance.aspx?Id=" + dr["ID"] + "&EmpId=" + GetEmpId() + "\"><i class=\"fa fa-eye\"></i></a></td>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/ManageAdvance.aspx?EmpId=" + GetEmpId() + "");
