...
{
    public partial class ListAdvanceCollection : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public ListAdvanceCollection()
        {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{     
                        {"ADVANCE_TYPE",        "Advance Type",        "200",      "T"},  
                        {"PAID_AMOUNT",        "Paid Amount",        "200",      "M"},  
                        {"PAID_DATE",            "Paid Date",             "200",      "D"}
                        };

            string sortby = "ID";
            string table_name = "ADVANCE_COLLECTION";

            String sql = "";
            sql = "SELECT AC.ID,S.DETAIL_TITLE AS ADVANCE_TYPE,dbo.ShowDecimal(AC.PAID_AMOUNT) as PAID_AMOUNT,"
            +" convert(varchar,AC.PAID_DATE,107) as PAID_DATE FROM AdvanceCollection AC INNER JOIN Advance A ON A.ID=AC.ADVANCE_ID INNER JOIN"
            +" StaticDataDetail S ON S.ROWID=A.ADVANCE_TYPE where A.ID="+GetAdvanceId()+"";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListAdvanceCollection.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetAdvanceId() + "&EmpId=" + GetEmpId() + "";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/EmployeeWeb/ManageAdvanceCollection.aspx?Id=" + dr["ID"] + "&AdvanceId=" + GetAdvanceId() + "&EmpId=" + GetEmpId() + "\"><i class=\"fa fa-eye\"></i></a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/ManageAdvanceCollection.aspx?EmpId=" + GetEmpId() + "&AdvanceId=" + GetAdvanceId()+ "");
