...
{
    public partial class ListBankAccounts : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"s.DETAIL_TITLE",    "Account Provider",         "LT"}
                     
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{ 
                        {"ACCOUNT_PROVIDER",        "Account Provider",        "200",      "T"},        
                        {"ACCOUNT_NUMBER",        "Account Number",        "200",      "T"},  
                        {"ACCOUNT_DETAILS",            "Account Details",             "200",      "T"},
                        {"IS_DEFAULT",    "Is Default?",     "200",      "T"}
                        };

            string sortby = "ID";
            string table_name = "BankAccounts";

            String sql = "";
            sql = "SELECT  B.ID,B.ACCOUNT_NUMBER, s.DETAIL_TITLE as 'ACCOUNT_PROVIDER',B.ACCOUNT_DETAILS,"
                    +" case when B.IS_DEFAULT='True' then 'Yes' when B.IS_DEFAULT='False' then 'No' end as IS_DEFAULT"
                    +" FROM  BankAccounts AS B inner join StaticDataDetail s on s.ROWID=B.ACCOUNT_PROVIDER "
                    + " where B.EMPLOYEE_ID=" + GetEmpId() + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListBankAccounts.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetEmpId() + "";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/EmployeeWeb/ManageBankAccounts.aspx?Id=" + dr["ID"] + "&EmpId=" + GetEmpId() + "\"><i class=\"fa fa-eye\"></i></a></td>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/ManageBankAccounts.aspx?EmpId=" + GetEmpId() + "");
