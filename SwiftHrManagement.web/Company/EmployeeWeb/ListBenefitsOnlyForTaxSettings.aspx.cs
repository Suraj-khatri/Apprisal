...
{
    public partial class ListBenefitsOnlyForTaxSettings : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public ListBenefitsOnlyForTaxSettings()
        {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"S.DETAIL_TITLE",    "Interest Benefit Type",         "LT"},
                        {"IB.FISCAL_YEAR",  "Fiscal Year",           "LT"} 
                     
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"INTEREST_BENIFIT_TYPE",            "Interest Benefit Type",             "200",      "LT"},
                        {"FISCAL_YEAR",        "Fiscal Year",        "200",      "LT"},                        
                        {"ACTUAL_INT_PAID",    "Actual Int. Amt",     "200",      "M"},                      
                        {"MARKET_INT_RATE",    "Market Interest Rate",     "200",      "LT"},  
                        {"TAXABLE_INT_AMT",      "Taxable Int. Amt",     "200",      "M"},
                        {"NARRATION",    "Narration",     "200",      "LT"}                       
                        };

            string sortby = "ID";
            string table_name = "InterestBenefitsOnlyForTaxSetting";

            String sql = "";
            sql = "SELECT IB.ID,S.DETAIL_TITLE 'INTEREST_BENIFIT_TYPE', IB.FISCAL_YEAR,dbo.ShowDecimal(IB.ACTUAL_INT_PAID) as ACTUAL_INT_PAID, "
                    +" IB.MARKET_INT_RATE,dbo.ShowDecimal(IB.TAXABLE_INT_AMT) as TAXABLE_INT_AMT,IB.NARRATION FROM "
                    +" InterestBenefitsOnlyForTaxSetting IB inner join (SELECT * FROM StaticDataDetail WHERE TYPE_ID=42)  s ON s.ROWID = IB.INTEREST_BENIFIT_TYPE "
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListBenefitsOnlyForTaxSettings.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetEmpId() + "";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/EmployeeWeb/ManageBenefitsOnlyForTaxSettings.aspx?ID=" + dr["ID"] + "&EmpId=" + GetEmpId() + "\"><i class=\"fa fa-eye\"></i></a></td>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/ManageBenefitsOnlyForTaxSettings.aspx?EmpId=" + GetEmpId() + "");
