...
{
    public partial class ListContribution : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public ListContribution()
        {
        ...
        {
            return (Request.QueryString["ContributionId"] != null ? long.Parse(Request.QueryString["ContributionId"].ToString()) : 0);
        }
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"A.DETAIL_TITLE",    "Contribution To",         "LT"}                 
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"CONTRIBUTION_CODE",            "Contri. Code",             "200",      "T"},
                        {"CONTRIBUTED_TO",            "Contri. To",             "200",      "T"},
                        {"VALUE_EMPLOYEE",       "Employee Contri. Amt or %",        "200",      "T"},
                        {"VALUE_EMPLOYER",    "Employer Contri. Amt or %",     "200",      "T"},
                        {"CONTRIBUTION_BASIS_EMPLOYEE",    "Employee C. Basic",     "200",      "T"},
                        {"CONTRIBUTION_BASIS_EMPLOYER",    "Employer C. Basic",     "200",      "T"}
                        };
            ...
            sql = "SELECT  C.ID,A.DETAIL_TITLE AS CONTRIBUTED_TO,C.VALUE_EMPLOYEE,CASE WHEN C.CONTRIBUTION_BASIS_EMPLOYEE='36' THEN 'BASIC SALARY' "
+ " WHEN C.CONTRIBUTION_BASIS_EMPLOYEE='36,37' THEN 'BASIC & GRADE' WHEN C.CONTRIBUTION_BASIS_EMPLOYEE='36,37,38' THEN  'GROSS SALARY' END AS CONTRIBUTION_BASIS_EMPLOYEE,C.VALUE_EMPLOYER,"
...
+ " THEN 'BASIC & GRADE' WHEN C.CONTRIBUTION_BASIS_EMPLOYER='36,37,38' THEN  'GROSS SALARY' END AS CONTRIBUTION_BASIS_EMPLOYER FROM Contribution AS C WITH (NOLOCK) INNER JOIN StaticDataDetail A WITH (NOLOCK) ON C.CONTRIBUTED_TO=A.ROWID "
+ " WHERE C.EMPLOYEE_ID=" + GetEmpId() + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListContribution.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetEmpId() + "";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/EmployeeWeb/ManageContribution.aspx?Id=" + dr["ID"] + "&EmpId=" + GetEmpId() + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            

            html.Append(" <tr> <td colspan=\"7\" align=\"center\"> <strong> Page " + _page.ToString() + " Of " + total_page + "</strong></td></tr>");
            html.Append("</table>");
            html.Append("</div>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/ManageContribution.aspx?EmpId="+GetEmpId()+"");
