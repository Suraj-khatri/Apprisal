...
{
    public partial class ListDonations : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"D.DETAILED_DESCRIPTION",    "Description",         "LT"}
                     
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{  
                        {"DONATION_DATE",    "Donation Date",     "200",      "D"},      
                        {"DONATION_AMOUNT",        "Amount",        "200",      "M"},
                        {"DETAILED_DESCRIPTION",    "Description",     "200",      "T"},               
                        {"GOVT_APPROVED_RATE",      "Govt. Approved Rate",     "200",      "T"}                  
                        };

            string sortby = "ID";
            string table_name = "Donations";

            String sql = "";
            sql = "SELECT  D.ID, D.EMPLOYEE_ID, dbo.showdecimal(D.DONATION_AMOUNT) as DONATION_AMOUNT,CONVERT(VARCHAR,D.DONATION_DATE,107)"
                +" AS DONATION_DATE, D.DETAILED_DESCRIPTION,GOVT_APPROVED_RATE FROM  Donations AS D INNER JOIN Employee AS E ON "
                +" D.EMPLOYEE_ID = E.EMPLOYEE_ID WHERE D.EMPLOYEE_ID="+GetEmpId()+"";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListDonations.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetEmpId() + "";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/EmployeeWeb/ManageDonations.aspx?Id=" + dr["ID"] + "&EmpId=" + GetEmpId() + "\"><i class=\"fa fa-eye\"></i></a></td>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/ManageDonations.aspx?EmpId=" + GetEmpId() + "");
