...
{
    public partial class ListEmployeeReferience : BasePage
    {
         clsDAO _clsdao = null;
         RoleMenuDAOInv _roleMenuDao = null;
         public ListEmployeeReferience()
        {
            _clsdao = new clsDAO();
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"Full_Name",    "Employ Reference Name ",         "LT"}                 
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                       
                        {"Full_Name",       "Employ Reference Name",        "200",      "T"},
                        {"Address",    "Address",     "200",      "T"},
                        {"mobile",    " mobile",     "200",      "EN"},                      
                        };



            string sortby = "Id";
            string table_name = "Employee_Refrences";

            String sql = "";
            //sql = " SELECT TRAINING_ID,EMPLOYEE_ID,S.DETAIL_TITLE AS FECULTY,DURATION,"
            //     + " NAMEOF_INSTITUTION, ADDRESS,TRN_DESC FROM Employee_Training E INNER JOIN StaticDataDetail S ON S.ROWID=E.FECULTY"
            //     + " WHERE EMPLOYEE_ID=" + GetEmpId() + "";
            sql = " select Id,EMPLOYEE_ID, Full_Name, [Address], home_phone, office_phone, mobile, fax, email from Employee_Refrences where EMPLOYEE_ID=" + GetEmpId() + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
           ...
           // string thisPage = "ListEmployeeReferience.aspx?sortd=" + sortd + "&sortby={sortby}";

            string thisPage = "ListEmployeeReferience.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetEmpId() + "";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/EmployeeWeb/ManageEmployeeReference.aspx?Id=" + dr["Id"] + "&EmpId=" + GetEmpId() + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/ManageEmployeeReference.aspx?EmpId=" + GetEmpId() + "");
           // string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/ManageEmployeeReference.aspx");
