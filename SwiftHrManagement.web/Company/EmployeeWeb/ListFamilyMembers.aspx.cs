...
{
    public partial class ListFamilyMembers : BasePage
    {
        RoleMenuDAOInv _roleMenuDao = null;
        EmployeeDAO empDao = null;
        Employee empCore = null;
        public ListFamilyMembers()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                //this.empCore = this.empDao.FindFullNameById(GetEmpId());
                //this.LblEmpName.Text = this.empCore.EmpName;
            }
            init();
        }
        #region SwiftGrid
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"S.DETAIL_TITLE",    "Relation",        "LT"}                 
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"RELATION",            "Relation",             "200",      "T"},
                        {"NAME",            "Full Name",             "200",      "T"},
                        {"DATE_OF_BIRTH",    "Date of Birth",     "200",      "D"},
                        {"MARITAL_STATUS",       "Marital Status",        "200",      "T"},
                        {"GENDER",    "Gender",     "200",      "T"},
                       {"NATIONALITY",    "Nationality",     "200",      "T"},
                        {"MOBILE_NUMBER",    "Mobile No",     "200",      "T"}
                        };

            string sortby = "ID";
            string table_name = "FamilyMembers";
            String sql = "";

            sql = "SELECT  FM.ID,FM.EMPLOYEE_ID,S.DETAIL_TITLE AS RELATION,FM.FIRST_NAME+' '+ISNULL(FM.MIDDLE_NAME,'')+' '+FM.LAST_NAME AS NAME,"
             +" S3.DETAIL_TITLE AS MARITAL_STATUS,S1.DETAIL_TITLE AS GENDER,CONVERT(VARCHAR,FM.DATE_OF_BIRTH,107) AS DATE_OF_BIRTH,"
             +" FM.MOBILE_NUMBER, FM.NATIONALITY FROM  FamilyMembers AS FM LEFT JOIN StaticDataDetail S ON S.ROWID=FM.RELATION LEFT JOIN"
             +" StaticDataDetail S1 ON S1.ROWID=FM.GENDER LEFT JOIN StaticDataDetail S3 ON S3.ROWID=FM.MARITAL_STATUS"
             +" WHERE FM.EMPLOYEE_ID="+GetEmpId()+"";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListFamilyMembers.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetEmpId() + "";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/EmployeeWeb/ManageFamilyMembers.aspx?Id=" + dr["ID"] + "&EmpId=" + GetEmpId() + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }

            html.Append(" <tr> <td colspan=\"8\" align=\"center\"> <strong> Page " + _page.ToString() + " Of " + total_page + "</strong></td></tr>");
            html.Append("</table>");
            html.Append("</div>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/ManageFamilyMembers.aspx?EmpId=" + GetEmpId() + "");
