...
{
    public partial class ListFutureContribution : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        {"SN",          "SN",                   "200",      "T"},
                        {"AMOUNT",      "Contribution Amount",  "200",      "M"},
                        {"Date",        "Contribution Date",    "200",      "D"},
                        {"Remarks",     "Remarks",              "200",      "D"},
                        {"STATUS",      "Status",               "200",      "T"}
                        };
            ...
            string sortby = "ID";
            string table_name = "FutureContribution";

            String sql = "";

            sql = "select ROW_NUMBER() over (order by ID) [SN],ID,CONVERT(varchar,Contribution_Date,107)[Date],dbo.ShowDecimal(AMOUNT) AMOUNT,Remarks,[STATUS] from FutureContributionForTax WHERE Employee_Id=" + GetEmpId() + " and Inactive='Y'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/EmployeeWeb/ManageFutureContribution.aspx?Id=" + dr["ID"] + "&EmpId=" + GetEmpId() + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/ManageFutureContribution.aspx?EmpId=" + GetEmpId() + "");
