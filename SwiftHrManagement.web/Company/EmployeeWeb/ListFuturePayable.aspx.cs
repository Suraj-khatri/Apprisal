...
{
    public partial class ListFuturePayable : BasePage
    {
       RoleMenuDAOInv _roleMenuDao = null;
       clsDAO _clsDao = new clsDAO();
       public ListFuturePayable()
        {
        ...
        {
            string fy_id = "";
            fy_id = _clsDao.GetSingleresult("select FISCAL_YEAR_NEPALI from fiscalYear where flag=1");
            return fy_id;
        }
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_roleMenuDao.hasAccess(ReadSession().AdminId, 11) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"s.DETAIL_TITLE",    "Benefit Name",         "LT"}                 
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"BENEFIT_NAME",            "Benefit Name",             "200",      "T"},
                        {"AMOUNT",            "Benefit Amount",             "200",      "M"},
                        {"FY_ID",            "Fiscal Year",             "200",      "M"},
                        {"STATUS",            "Status",             "200",      "M"}
                        };
            ...
            string sortby = "ID";
            string table_name = "Payable";

            String sql = "";

            sql = "SELECT p.ID, s.DETAIL_TITLE 'BENEFIT_NAME',p.FY_ID,p.STATUS, dbo.showdecimal(p.AMOUNT) as AMOUNT from FuturePayable p inner join  "
                    + " StaticDataDetail s on s.ROWID=P.BENEFIT_ID WHERE P.EMPLOYEE_ID=" + GetEmpId() + " and fy_id='" +Getfiscalyear() + "'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListFuturePayable.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetEmpId() + "";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/EmployeeWeb/ManageFuturePayable.aspx?Id=" + dr["ID"] + "&EmpId=" + GetEmpId() + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/ManageFuturePayable.aspx?EmpId=" + GetEmpId() + "");
