...
{
    public partial class ListIdentification : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"S.DETAIL_TITLE",    "Card Type",         "LT"}                 
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"CARD_TYPE",            "Card Type",             "200",      "T"},
                        {"CARD_NO",            "Card No",             "200",      "T"},
                        {"ISSUE_PLACE",       "Issue Place",        "200",      "T"},
                        {"ISSUE_DATE",    "Issue Date",     "200",      "D"},
                        {"EXPIRY_DATE",    "Expiry Date",     "200",      "D"}
                        };

            string sortby = "ID";
            string table_name = "EMPLOYEE_IDENTIFICATION";
            String sql = "";
            sql = "SELECT  I.ID, I.EMPLOYEE_ID, S.DETAIL_TITLE AS CARD_TYPE,I.CARD_NO,I.ISSUE_PLACE,convert(varchar,I.ISSUE_DATE,107) as "
                    +" ISSUE_DATE,isnull(convert(varchar,I.EXPIRY_DATE,107),'') as  EXPIRY_DATE FROM  EMPLOYEE_IDENTIFICATION AS I INNER JOIN StaticDataDetail S "
                    +" ON I.CARD_TYPE=S.ROWID WHERE I.EMPLOYEE_ID="+GetEmpId()+"";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListIdentification.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetEmpId() + "";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/EmployeeWeb/ManageIdentification.aspx?Id=" + dr["ID"] + "&EmpId=" + GetEmpId() + "\"><i class=\"fa fa-eye\"></i></a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/ManageIdentification.aspx?EmpId=" + GetEmpId() + "");
