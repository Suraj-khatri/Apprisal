...
{
    public partial class ListInsurance : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{                       
                        {"INSURER",             "Insurer",          "200",      "T"},
                        {"Insurance_For",       "Insurance For",    "200",      "T"},  
                        {"INSURED_AMOUNT",      "Insured Amount",   "200",      "M"},
                        {"INSURANCE_POLICY",    "Insurance Policy", "200",      "T"},                      
            ...
            sql = @"SELECT
		                I.ID
		                ,dbo.GetDetailTitle(Insurance_For) Insurance_For
		                ,dbo.showdecimal(I.INSURED_AMOUNT) as INSURED_AMOUNT
		                ,CONVERT(VARCHAR,I.INSURED_DATE,107) AS INSURED_DATE
		                ,CONVERT(VARCHAR,I.[EXPIRY_DATE],107) AS [EXPIRY_DATE]
		                , s.DETAIL_TITLE as 'INSURER',INSURANCE_POLICY
                     FROM  Insurance AS I inner join 
                     (select * from StaticDataDetail where TYPE_ID=33) s on s.ROWID=I.INSURER
                     where EMPLOYEE_ID = " + GetEmpId() + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListInsurance.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetEmpId() + "";
            ...
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Deduction</th>");
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Premium</th>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Deduction\" href=\"/Company/EmployeeWeb/ListPremiumDeduction.aspx?EmpId=" + GetEmpId() + "&deduction_Id=" + dr["ID"] + "\">Deduction</a></td>");
                html.Append("<td><a class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Premimum\" href=\"/Company/EmployeeWeb/ListInsurancePremium.aspx?EmpId=" + GetEmpId() + "&Insurance_Id=" + dr["ID"] + "\">Premimum</a></td>");
                html.Append("<td><a class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/EmployeeWeb/ManageInsurance.aspx?Insurance_Id=" + dr["ID"] + "&EmpId=" + GetEmpId() + "\"><i class=\"fa fa-eye\"></i></a></td>");
            } 
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/ManageInsurance.aspx?EmpId=" + GetEmpId() + "");
