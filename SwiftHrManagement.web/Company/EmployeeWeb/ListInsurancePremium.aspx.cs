...
{
    public partial class ListInsurancePremium : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public ListInsurancePremium()
        {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"IP.PREMIUM_RECEIPT_NUMBER",    "Premium Receipt. No.",         "EN"}
            ...
            String[,] matgrid = new String[,]{                       
                        {"INSURANCE_POLICY",        "Insurance Policy",        "200",      "T"},  
                         {"PAYMENT_AMOUNT",            "Payment Amount",             "200",      "M"},
                        {"PREMIUM_RECEIPT_NUMBER",    "Premium Receipt. No.",     "200",      "T"},                      
                        {"PAYMENT_DATE",    "Payment Date",     "200",      "D"}             
                        };

            string sortby = "ID";
            string table_name = "InsurancePremium";

            String sql = "";
            sql = "SELECT IP.ID,I.INSURANCE_POLICY,IP.INSURANCE_ID, convert(varchar,IP.PAYMENT_DATE,107) as PAYMENT_DATE,"
            +" IP.PREMIUM_RECEIPT_NUMBER,dbo.showdecimal(IP.PAYMENT_AMOUNT) as PAYMENT_AMOUNT FROM  dbo.Insurance AS I INNER JOIN "
            +" dbo.InsurancePremium  IP ON I.ID = IP.INSURANCE_ID where IP.INSURANCE_ID ="+GetInsuranceId()+"";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListInsurancePremium.aspx?sortd=" + sortd + "&sortby={sortby}&EmpId=" + GetEmpId() + "&Insurance_Id="+GetInsuranceId()+"";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/EmployeeWeb/ManageInsurancePremium.aspx?Id=" + dr["ID"] + "&EmpId=" + GetEmpId() + "&Insurance_Id=" + GetInsuranceId() + "\"><i class=\"fa fa-eye\"></i></a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/ManageInsurancePremium.aspx?Insurance_Id=" + GetInsuranceId() + "&EmpId=" + GetEmpId() + "");
