...
{
    public partial class ListJD : BasePage
    {
       ...
       public ListJD()
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        ...
                        {"e.DEPARTMENT_NAME",    "Dept Name",         "LT"},
                        {"a.from_date",    "From Date (MM-DD-YYYY)",         "D"}
            ...
            String[,] matgrid = new String[,]{ 
                        ...
                        {"DEPARTMENT_NAME",        "Dept Name",        "200",      "T"},
                        {"post_name",        "Position",        "200",      "T"},        
                        {"sup_name",        "SuperVisor Name",        "200",      "T"},
                        {"DEPARTMENT_NAME",        "Dept Name",        "200",      "T"},
                        {"from_date",        "From Date",        "200",      "D"},        
                        {"to_date",        "To Date",        "200",      "D"},
                        {"file_desc",        "File Desc",        "200",      "T"},
                        {"file_type",        "File Type",        "200",      "T"}                      
                        };

            string sortby = "id";
            string table_name = "JD_Employee";

            String sql = "";
                sql = @"select a.id,a.employee_id,b.FIRST_NAME+' '+b.MIDDLE_NAME+' '+b.LAST_NAME emp_name,c.BRANCH_NAME,e.DEPARTMENT_NAME,
                        sdd.DETAIL_TITLE post_name,d.FIRST_NAME+' '+d.MIDDLE_NAME+' '+d.LAST_NAME sup_name,
                        CONVERT(varchar,a.from_date,107) from_date,convert(varchar,a.to_date,107) to_date,file_desc,file_type
                        from JD_Employee a inner join Employee b on a.employee_id=b.EMPLOYEE_ID
                        inner join Branches c on a.branch_id=c.BRANCH_ID 
                        inner join Departments e on e.DEPARTMENT_ID=a.dept_id
                        inner join StaticDataDetail sdd on sdd.ROWID=a.position_id
                        inner join Employee d on d.EMPLOYEE_ID=a.supervisor_id	where a.employee_id=" + GetEmpId() + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListJD.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetEmpId() + "";
            ...
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Browse Docs</th>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/EmployeeWeb/ManageJD.aspx?Id=" + dr["employee_id"] + "&P_Id=" + dr["id"].ToString() + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Browser\"  href='/doc/JobDescription/" + dr["id"].ToString() + "." + dr["file_type"].ToString() + "'> <i class=\"fa fa-firefox\"></i> </a></td>");
                html.Append("</tr>");

            }

            html.Append(" <tr> <td colspan=\"12\" align=\"center\"> <strong> Page " + _page.ToString() + " Of " + total_page + "</strong></td></tr>");
            html.Append("</table>");
            html.Append("</div>");
            string filter_form = make_filter_form(table_name, matfilter);
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/ManageJD.aspx?Id=" + GetEmpId() + "");
