...
{
    public partial class ListLoan : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"S.DETAIL_TITLE",    "Loan Type",         "LT"},
                        {"L.LEDGER_CODE",    "Ledger Code",         "LT"}
                     
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{                       
                        {"LOAN_TYPE",        "Loan Type",        "200",      "T"},  
                        {"LOAN_AMOUNT",            "Loan Amount",             "200",      "M"},
                        {"LEDGER_CODE",    "Ledger Code",     "200",      "T"}, 
                        {"INTEREST_RATE",    "Interest Rate",     "200",      "T"},
                        {"DATE_TAKEN",    "Sanctioned Date",     "200",      "D"},  
                        {"INSTALLMENT_START_DATE",    "Installment Start Date",     "200",      "D"},  
                        {"REMAINING_INSTALLMENT",      "Remaining Installment",     "200",      "T"}                  
                        };

            string sortby = "ID";
            string table_name = "Loan";

            String sql = "";
            sql = "SELECT  L.ID,CONVERT(VARCHAR,L.DATE_TAKEN,107) AS DATE_TAKEN,CONVERT(VARCHAR,L.INSTALLMENT_START_DATE,107)AS"
            +" INSTALLMENT_START_DATE,dbo.showdecimal(L.LOAN_AMOUNT) as LOAN_AMOUNT,L.REMAINING_INSTALLMENT, S.DETAIL_TITLE AS LOAN_TYPE,"
            +" L.NO_OF_INSTALLMENTS,L.INTEREST_RATE, REPAYMENT_FREQUENCY,dbo.showdecimal(L.INSTALLMENT_AMT) as INSTALLMENT_AMT,"
            +" L.LEDGER_CODE,L.NARATION,L.NEXT_RUN_MONTH FROM  Loan AS L inner join (select * from StaticDataDetail"
            +" where TYPE_ID=46) s on s.ROWID= L.LOAN_TYPE WHERE EMPLOYEE_ID=" + GetEmpId() + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListLoan.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetEmpId() + "";
            ...
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Collection</th>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Collect\" href=\"/Company/EmployeeWeb/ListLoanCollection.aspx?Id=" + dr["ID"] + "&EmpId=" + GetEmpId() + "\">Collect</a></td>");
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/EmployeeWeb/ManageLoan.aspx?Id=" + dr["ID"] + "&EmpId=" + GetEmpId() + "\"><i class=\"fa fa-filter\" id=\"img_Search\"></i></a></td>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/ManageLoan.aspx?EmpId=" + GetEmpId() + "");
