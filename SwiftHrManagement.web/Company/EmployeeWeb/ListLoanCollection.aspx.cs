...
{
    public partial class ListLoanCollection : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public ListLoanCollection()
        {
        ...
        private void init()
        {
            ...
            string sortby = "id";
            string table_name = "loan_collection";

            String sql = "";
            sql = "SELECT lc.id,lc.emp_id,lc.loan_id,dbo.ShowDecimal(paid_amount) as paid_amount,convert(varchar,paid_date,107) as paid_date,narration,lc.created_by,lc.created_date,s.DETAIL_TITLE as loan_type "
                    + " FROM loan_collection lc inner join Loan l on l.ID=lc.loan_id inner join StaticDataDetail s on s.ROWID=l.LOAN_TYPE "
                    + " where lc.loan_id=" + filterstring(GetLoanId().ToString()) + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListLoanCollection.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetLoanId() + "&EmpId=" + GetEmpId() + "";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/Company/EmployeeWeb/ManageLoanCollection.aspx?Id=" + dr["ID"] + "&LoanId=" + GetLoanId() + "&EmpId=" + GetEmpId() + "\">View</a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/ManageLoanCollection.aspx?EmpId=" + GetEmpId() + "&LoanId=" + GetLoanId() + "");
