...
{
    public partial class ListPayable : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"DETAIL_TITLE",    "Benefit Name",         "LT"}                 
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"DETAIL_TITLE",        "Benefit Name",             "200",      "T"},
                        {"AMOUNT",              "Benefit Amount",           "200",      "M"},
                        {"EFFECTIVE_FROM",      "Effective From",           "200",      "D"},
                        {"APPLY_FROM",          "Apply From",               "200",      "D"}
                        };
            ...
            sql = @"select ID,SET_ID,d.DETAIL_TITLE,dbo.ShowDecimal(AMOUNT) AMOUNT,convert(varchar,EFFECTIVE_FROM,107) EFFECTIVE_FROM,
                    convert(varchar,APPLY_FROM,107) APPLY_FROM from Payable p
                    inner join StaticDataDetail d on d.ROWID=p.BENEFIT_ID where p.EMPLOYEE_ID=" + GetEmpId() + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListPayable.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetEmpId() + "";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/EmployeeWeb/ManagePayable.aspx?Id=" + dr["ID"] + "&EmpId=" + GetEmpId() + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/ManagePayable.aspx?EmpId=" + GetEmpId() + "");
