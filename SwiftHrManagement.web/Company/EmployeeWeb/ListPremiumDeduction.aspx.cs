...
{
    public partial class ListPremiumDeduction : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public ListPremiumDeduction()
        {
        ...
        {
            return (Request.QueryString["deduction_Id"] != null ? long.Parse(Request.QueryString["deduction_Id"].ToString()) : 0);
        }
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{                       
                         {"SNO",                "SNO",                  "200",      "T"}, 
                         {"Deduction_Amt",      "Deduction Amount",     "200",      "M"}, 
                         {"Deduction_Date",     "Deduction Date",       "200",      "D"}, 
                        };
            ...
            String sql = "";
            sql = "select ROW_NUMBER() over (order by Deduction_Date) SNO,ID,Deduction_Amt,Deduction_Date from PremiumDeduction where Insurance_ID="+ GetInsuranceID() +"";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //html.Append("<td><a href=\"/Company/EmployeeWeb/ListPremiumDeduction.aspx?EmpId=" + GetEmpId() + "&deduction_Id=" + dr["ID"] + "\">Deduction</a></td>");
                //html.Append("<td><a href=\"/Company/EmployeeWeb/ListInsurancePremium.aspx?EmpId=" + GetEmpId() + "&Insurance_Id=" + dr["ID"] + "\">Premimum</a></td>");
                //html.Append("<td><a href=\"/Company/EmployeeWeb/ManageInsurance.aspx?Insurance_Id=" + dr["ID"] + "&EmpId=" + GetEmpId() + "\">View</a></td>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, false, "/Company/EmployeeWeb/ManageInsurance.aspx?EmpId=" + GetEmpId() + "");
