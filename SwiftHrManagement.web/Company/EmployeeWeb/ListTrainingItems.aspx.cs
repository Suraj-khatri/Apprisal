...
{
    public partial class ListTrainingItems : BasePage
    {
        clsDAO _clsdao = null;
        RoleMenuDAOInv _roleMenuDao = null;
        public ListTrainingItems()
        {
            _clsdao = new clsDAO();
        ...
        //   // LblEmpName.Text = _clsdao.GetSingleresult("SELECT EMP_CODE+' | '+FIRST_NAME + ' ' + MIDDLE_NAME + ' ' + LAST_NAME AS EmpName FROM Employee where EMPLOYEE_ID='" + GetEmpId() + "'");      
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"S.DETAIL_TITLE",    "Training Type",         "LT"}                 
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                       
                        {"FECULTY",       "Training Name",        "200",      "T"},
                        {"NAMEOF_INSTITUTION",    "Institution",     "200",      "T"},
                        {"DURATION",    "Duration(days)",     "200",      "T"},                      
                        };



            string sortby = "TRAINING_ID";
            string table_name = "Employee_Training";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListTrainingItems.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetEmpId() + "";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/EmployeeWeb/TrainingDetails.aspx?Id=" + dr["TRAINING_ID"] + "&EmpId=" + GetEmpId() + "\"><i class=\"fa fa-eye\"></i></a></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/TrainingDetails.aspx?EmpId=" + GetEmpId() + "");
