    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"PATIENT_NAME",       "Patient Name",        "200",      "LT"},
                        {"SYMPTOMS",    "Symptoms",     "200",      "LT"},  
                        {"EXTENSION_NUM",    "Extension Num",     "200",      "LT"}, 
                        {"IS_CONSULTED",    "Is Consulted?",     "200",      "D"}, 
            ...
            string sortby = "ID";
            string table_name = "DOCTOR_APPOINTMENT";
            String sql = "";

            if (DoesHrUser(ReadSession().Emp_Id.ToString()))
            {

                sql = @"SELECT ID,dbo.GetEmployeeFullNameOfId(EMP_ID) EMP_NAME,PATIENT_NAME,SYMPTOMS,EXTENSION_NUM,
                    IS_CONSULTED,dbo.GetEmployeeFullNameOfId(CREATED_BY) CREATED_BY,convert(varchar,CREATED_DATE,107) CREATED_DATE
                    FROM DOCTOR_APPOINTMENT WHERE IS_CONSULTED='No'";
            }
            else
            {
                sql = @"SELECT ID,dbo.GetEmployeeFullNameOfId(EMP_ID) EMP_NAME,PATIENT_NAME,SYMPTOMS,EXTENSION_NUM,
                    IS_CONSULTED,dbo.GetEmployeeFullNameOfId(CREATED_BY) CREATED_BY,convert(varchar,CREATED_DATE,107) CREATED_DATE
                    FROM DOCTOR_APPOINTMENT WHERE EMP_ID="+ReadSession().Emp_Id.ToString()+"";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/EmployeeWeb/MedicalAppointment/Manage.aspx?ID=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");

                //html.Append("<td><a href=\"/Company/EmployeeWeb/MedicalAppointment/Manage.aspx?ID=" + dr["ID"] + "\">View</a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/MedicalAppointment/Manage.aspx");
