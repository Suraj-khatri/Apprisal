...
{
    public partial class ListPersionalPromotion : BasePage
    {
        RoleMenuDAOInv _roleMenuDao = null;
        public ListPersionalPromotion()
        {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"Service_Event",    "Event",           "LT"},
                        {"EffectiveDate",   "Effective Date",  "LT"} 
                     
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"Service_Event",        "Event",             "200",      "LT"},
                        {"EffectiveDate",       "Effective Date",    "200",      "LT"},                     
                        };
            ...
            sql = "SELECT flag,[Service_Event],Convert(varchar,[Effective_Date],107) EffectiveDate,Effective_Date  FROM "
                    + " ("
                    +" SELECT 'APP' as flag, "
                    +" 'Appointment' AS [Service_Event]"
                    +" ,JOINED_DATE AS [Effective_Date]"
                    +" FROM Employee "
                    + " WHERE EMPLOYEE_ID=" + filterstring(GetEmpId().ToString()) + ""

                    +" UNION ALL"

                    +" SELECT 'P',"
                    +" 'Promotion' AS Service_Event"
                    +" ,PROMOTION_DATE Effective_Date"
                    +" FROM Promotion "
                    + " WHERE EMP_ID=" + filterstring(GetEmpId().ToString()) + ""

                    +" UNION ALL"

                    +" SELECT 'T',"
                    +" CASE WHEN TRANSFER_TYPE='i' THEN 'Internal Transfer'"
                    +" when TRANSFER_TYPE='e' THEN 'External Transfer' End AS Service_Event"
                    +" ,EFFECTIVE_DATE Effective_Date"
                    +" FROM ExternalTransferPlan "
                    + " WHERE STAFF_ID=" + filterstring(GetEmpId().ToString()) + ""

                    +" UNION ALL"

                    +" SELECT 'TT',"
                    +" 'Temporary Transfer' AS Service_Event "
                    +" ,EFFECTIVE_DATE Effective_Date"
                    ...
                    +" SELECT 'C',"
                    +" 'Confirmation' As Service_Event "
                    +" ,PERMANENT_DATE Effective_Date"
                    +" FROM Employee "
                    + " WHERE EMPLOYEE_ID=" + filterstring(GetEmpId().ToString()) + ""
                    +" AND PERMANENT_DATE <> '1900-01-01'"
                    +" )A ";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListServiceHistory.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/Company/EmployeeWeb/Promotion/ListEventDetails.aspx?flag=" + dr["flag"] + "&date=" + dr["Effective_Date"] + "&empid="+GetEmpId()+"\">View</a></td>");
