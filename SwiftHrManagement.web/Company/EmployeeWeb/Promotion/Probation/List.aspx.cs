    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        ...
                        {"effectiveDate",  "Employee Position",           "LT"} 
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"effectiveDate",                "Effective Date",     "200",      "D"}, 
                        {"newEmpType",       "New Employee Type",             "200",      "LT"},
                        {"promotionFlag",       "Promotion Flag",             "200",      "LT"},
                        {"transferFlag",         "Transfer Flag",             "200",      "LT"}
                        };



            string sortby = "effectiveDate";
            string table_name = "probationSalary";

            String sql = "";
             sql = @"select proSalId,e.FIRST_NAME +' '+e.MIDDLE_NAME +' '+ e.LAST_NAME empName,CONVERT(varchar,effectiveDate,107)effectiveDate,d.DETAIL_DESC newEmpType,
                        promotionFlag = case promotionFlag when 'WP' then 'With Promotion' else 'Without Promotion' end
                        ,transferFlag = case transferFlag when 'WT' then 'With Transfer' else 'Without Transfer' end
                        from probationSalary p inner join
                        Employee e on p.empId = e.EMPLOYEE_ID
                        INNER JOIN StaticDataDetail d
                        ON e.POSITION_ID = d.ROWID";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/Company/EmployeeWeb/Promotion/Probation/Manage.aspx?ID=" + dr["proSalId"] + "\">View</a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/Promotion/Probation/Manage.aspx");
