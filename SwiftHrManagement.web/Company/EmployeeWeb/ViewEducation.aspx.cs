...
{
    public partial class ViewEducation : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        {"LEVEL",            "Level",             "200",      "T"},
                        {"DEGREE",            "Degree",             "200",      "T"},
                        {"FECULTY",       "Faculty",        "200",      "T"},
                        {"PASSED_YEAR",    "Passed Year",     "200",      "T"},
                        {"NAMEOF_INSTITUTION",    "Institution",     "200",      "T"},
                        {"DIVISION",    "Division",     "200",      "T"}
                        };



            string sortby = "EDUCATION_ID";
            string table_name = "Employee_Education";

            String sql = "";

            //sql = "SELECT EDUCATION_ID,EMPLOYEE_ID,LEVELS,DEGREE,S.DETAIL_TITLE AS FECULTY,DIVISION,PERCENTAGE,PASSED_YEAR,NAMEOF_INSTITUTION,"
            //+" COUNTRY_NAME FROM Employee_Education E INNER JOIN StaticDataDetail S ON S.ROWID=E.FECULTY WHERE EMPLOYEE_ID=" + GetEmpId() + "";

            sql = " SELECT EDUCATION_ID,EMPLOYEE_ID,LEV.DETAIL_TITLE AS LEVEL,DEGREE,S.DETAIL_TITLE AS FECULTY,DIV.DETAIL_TITLE AS DIVISION,PERCENTAGE,"
                  + " PASSED_YEAR,NAMEOF_INSTITUTION, COUNTRY_NAME FROM Employee_Education E INNER JOIN StaticDataDetail S ON S.ROWID=E.FECULTY"
                  + " INNER JOIN StaticDataDetail DIV ON DIV.ROWID = E.DIVISION"
                  + " INNER JOIN StaticDataDetail LEV ON LEV.ROWID = E.LEVELS  WHERE EMPLOYEE_ID=" + GetEmpId() + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ViewEducation.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetEmpId() + "";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/EmployeeWeb/EducationDetails.aspx?Id=" + dr["EDUCATION_ID"] + "&EmpId="+GetEmpId()+"\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/EmployeeWeb/EducationDetails.aspx?EmpId=" + GetEmpId() + "");
