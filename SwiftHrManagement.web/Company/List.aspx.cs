    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"COMP_ADDRESS",            "Address",             "200",      "T"},
                        {"COMP_PHONE_NO",       "Phone No",        "200",      "T"},
                        {"COMP_EMAIL",    "Email",     "200",      "T"},
                        {"COMP_URL",    "URL",     "200",      "T"},
                        {"COMP_CONTACT_PERSON",    "Contact Person",     "200",      "T"}
                        };



            string sortby = "COMP_ID";
            string table_name = "COMPANY";

            String sql = "";

            sql = "SELECT COMP_ID, COMP_NAME, COMP_SHORT_NAME, COMP_ADDRESS, COMP_ADDRESS2, COMP_PHONE_NO, COMP_FAX_NO, COMP_CONTACT_PERSON,"
                +" COMP_MAP_CODE, CASE WHEN COMP_STATUS ='True' THEN 'Active' WHEN COMP_STATUS='False' then 'In Active'  END 'COMP_STATUS'," 
                +" COMP_URL,COMP_EMAIL FROM COMPANY  where 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Company/Manage.aspx?Id=" + Uri.EscapeUriString(StringEncryption.Encrypt(dr["COMP_ID"].ToString())) + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Company/Manage.aspx");
        ...
        {
            StringBuilder html = new StringBuilder("<div class=\"col-md-12 \">");
