...
{
    public partial class ListVerify : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"dbo.GetEmployeeFullNameOfId(empId)",    "Requested By",         "LT"}            
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"deptName",                 "Department",               "200",      "LT"},
                        {"position",                 "Position",                 "200",      "LT"},
                        {"reqBy",                    "Requested By",             "200",      "LT"},
                        {"enterDate",                "Requested Date",           "200",      "LT"}, 
                        {"status",                   "Satuse",                   "200",      "LT"}, 
                        };



            string sortby = "enterDate";
            string table_name = "activityMaster";

            String sql = "";

            sql = "SELECT am.activityId,dbo.GetDeptName(department)deptName"
                + " ,d.DETAIL_TITLE position,dbo.GetEmployeeFullNameOfId(empId) reqBy,enterDate,case status when 'r' then 'Requested' when 'v' then 'Verify' end status,status [statusElement]"
                +" FROM activityMaster am"
                +" INNER JOIN StaticDataDetail d ON d.ROWID = am.position"
                + " WHERE am.recommendedTo ="+ReadSession().Emp_Id;
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/DailyActivityReport/ActivityDetailReport.aspx?status=" + dr["statusElement"] + "&activityId=" + dr["activityId"] + "\">Approve</a></td>");
