...
namespace SwiftHrManagement.web.DutyRooster
{
    public partial class List : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                //if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 20) == false)
                //{
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"dbo.GetEmployeeFullNameOfId(EMP_ID)",    "Emp Name ",         "LT"},  
                        {"START_DATE",    "Date (MM-DD-YYYY)",         "ED"}                   
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"EMP_NAME",            "Emp Name",             "200",      "T"},
                        {"FROM_DATE",            "Login Date",             "200",      "D"},
                         {"IN_TIME_OFFICE",       "Login Time",        "200",      "T"},
                        {"TO_DATE",            "Logout Date",             "200",      "D"},
                        {"OUT_TIME_OFFICE",    "Logout Time",     "200",      "T"},
                        {"IN_TIME_LUNCH",    "Lunch Out Time",     "200",      "T"},
                        {"OUT_TIME_LUNCH",    "Lunch In Time",     "200",      "T"},
                        {"DAY_NAME",    "Day",     "200",      "T"},
                        {"IS_HOLIDAY",    "Off Day",     "200",      "T"}
                        };
            ...
            sql = @"select ID,dbo.GetEmployeeFullNameOfId(EMP_ID) EMP_NAME,EMP_ID,CONVERT(VARCHAR,START_DATE,107) FROM_DATE,
CONVERT(VARCHAR,END_DATE,107) TO_DATE,IN_TIME_OFFICE,OUT_TIME_OFFICE,IN_TIME_LUNCH,OUT_TIME_LUNCH,
DAY_NAME,CASE WHEN ISNULL(IS_HOLIDAY,'N')='Y' THEN 'YES' ELSE 'NO' END IS_HOLIDAY FROM DUTY_ROOSTER WHERE 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/DutyRooster/ManageInv.aspx?Id=" + dr["id"] + "\">Update</a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/DutyRooster/Manage.aspx");
