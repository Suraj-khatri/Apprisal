    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        {"requestedBy",         "Employee",         "200",      "T"},
                        {"description",         "Description",      "500",      "T"}, 
                        {"Status",              "Status",      "500",      "T"}
                        };
            ...
            sql = @"SELECT 
                        onsiteID,RTRIM(dbo.GetEmployeeFullNameOfId(empID)) requestedBy,[description],
                        RTRIM(dbo.GetEmployeeFullNameOfId(Approved_by)) Approved_by,
					    CONVERT(varchar,Approved_Date,107) Approved_Date,[Status]
                    FROM onsiteduty WHERE Recorded='N' 
                    AND Status='Pending' AND Approved_by=" + filterstring(ReadSession().Emp_Id.ToString());
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //html.Append("<td><a href=\"/EmployeeMovement/ApproveOnsiteDuty/Manage.aspx?ID=" + dr["onsiteID"] + "\">View</a></td>");
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/EmployeeMovement/ApproveOnsiteDuty/Manage.aspx?ID=" + dr["onsiteID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, false, "/EmployeeMovement/ApproveOnsiteDuty/Manage.aspx?flag=" + GetFlag());
