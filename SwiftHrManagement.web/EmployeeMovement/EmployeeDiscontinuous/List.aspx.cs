    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"DBO.GetEmployeeFullNameOfId(STAFF_ID)",    "Employee  Name",         "LT"}                
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                            {"EMPLOYEE_NAME",            "Employee Name",             "200",      "T"},
                            {"BRANCH_NAME",             "Branch",          "200",      "T"},
                            {"DEPT_NAME",          "Dept. Name",       "200",      "T"},
                            {"MODE",          "Mode",       "200",      "T"},
                            {"EFFECTIVE_DATE",          "Effective Date",       "200",      "T"},
                            {"DESC",          "Description",       "200",      "T"}
                        };



            string sortby = "ID";
            string table_name = "EmployeeDiscontinuousPlan";

            String sql = "";

            sql = @"SELECT ID,DBO.GetEmployeeFullNameOfId(STAFF_ID) EMPLOYEE_NAME,DBO.GetBranchName(BRANCH_ID) BRANCH_NAME,
                    DBO.GetDeptName(DEPT_ID) DEPT_NAME,DBO.GetDetailTitle(DISCONTINUOUS_REASON) MODE,
                    CONVERT(VARCHAR,EFFECTIVE_DATE,107) EFFECTIVE_DATE,DISCRIPTION [DESC],IS_CLEARANCE,IS_EXIT_INTERVIEW 
                    FROM EmployeeDiscontinuousPlan   WHERE 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            html.Append("<th class=\"text-center\" align=\"center\">Print CS</th>");
            html.Append("<th class=\"text-center\" align=\"center\">Print EI</th>");

                //html.Append("<th Class=\"HeaderStyle\" align=\"left\">View</th>");
                //html.Append("<th Class=\"HeaderStyle\" align=\"left\">Print CS</th>");
                //html.Append("<th Class=\"HeaderStyle\" align=\"left\">Print EI</th>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //html.Append("<td><a href=\"/EmployeeMovement/EmployeeDiscontinuous/Manage.aspx?ID=" + dr["ID"] + "\">View</a></td>");
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/EmployeeMovement/EmployeeDiscontinuous/Manage.aspx?ID=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                if (dr["IS_CLEARANCE"].ToString() == "Y")
                {
                    //html.Append("<td><a href=\"/EmployeeMovement/Clearance/PrintCS.aspx?ID=" + dr["ID"] + "\">Print Clearance Sheet</a></td>");
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Print Clearance Sheet\" href=\"/EmployeeMovement/Clearance/PrintCS.aspx?ID=" + dr["ID"] + "\"><i class=\"fa fa-print\"></i></a></td>");
                ...
                if (dr["IS_EXIT_INTERVIEW"].ToString() == "Y")
                {
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Print Exit Interview\" href=\"/EmployeeMovement/ExitInterview/PrintExitInterview.aspx?ID=" + dr["ID"] + "\"><i class=\"fa fa-print\"></i></a></td>");
                    //html.Append("<td><a href=\"/EmployeeMovement/ExitInterview/PrintExitInterview.aspx?ID=" + dr["ID"] + "\">Print Exit Interview</a></td>");
                }
                else
                {
                    html.Append("<td>N/A</td>");
            ...
            html.Append("<tr> <td align=\"center\" colspan=\"9\"> <strong> Page " + _page.ToString() + " Of " + total_page + "</strong></td></tr>");
            html.Append("</table>");
            html.Append("</div>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/EmployeeMovement/EmployeeDiscontinuous/Manage.aspx");
