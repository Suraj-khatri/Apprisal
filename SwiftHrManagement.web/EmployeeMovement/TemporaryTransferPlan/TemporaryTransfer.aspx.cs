...
{
    public partial class TemporaryTransfer : BasePage
    {
        RoleMenuDAOInv _roleMenuDao = null;
        public TemporaryTransfer()
        {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        ...
                        {"B1.BRANCH_NAME",    "From Branch",         "LT"},
                        {"D1.DEPARTMENT_NAME",    "From Dept",         "LT"},
                        {"D.DEPARTMENT_NAME",    "To Dept",         "LT"},
                        {"ETP.Service_Flag",    "Service Event",         "LT"} 
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        
                        {"EMPLOYEE_ID",             "Employee Name",      "200",    "LT"},  
                        {"FROM_BRANCH",             "From Branch",        "200",    "LT"},
                        {"FROM_DEPARTMENT",         "From Dept",          "200",    "D"},
                        {"WHICH_DEPARTMENT",        "To Dept",            "200",    "LT"},
                        {"EFFECTIVE_DATE",          "Effective Date",     "200",    "D"},
                        {"TRANSFER_DESCRIPTION",    "Description",        "200",    "LT"},
                        {"Service_Flag",            "Service Event",      "200",    "LT"} 
                        };

            string sortby = "EMPLOYEE_ID";
            string table_name = "temporary_transfer";

            String sql = "";
            sql = @"SELECT ID,FROM_DEPARTMENT,FROM_BRANCH,EMPLOYEE_ID,EFFECTIVE_DATE,END_DATE,WHICH_DEPARTMENT,TRANSFER_DESCRIPTION, CREATED_BY
                    , CREATED_DATE, MODIFIED_BY, MODIFIED_DATE,service_flag 
                    FROM  
                    ( 
                    SELECT ETP.ID, D1.DEPARTMENT_NAME AS FROM_DEPARTMENT, B1.BRANCH_NAME AS FROM_BRANCH, 
                    E.FIRST_NAME + ' ' + E.MIDDLE_NAME + ' ' + E.LAST_NAME AS EMPLOYEE_ID,
                    convert(varchar,ETP.EFFECTIVE_DATE,107) as EFFECTIVE_DATE, convert(varchar,ETP.END_DATE,107) as END_DATE,  
                    D.DEPARTMENT_NAME AS WHICH_DEPARTMENT,ETP.TRANSFER_DESCRIPTION,  ETP.CREATED_BY, ETP.CREATED_DATE, ETP.MODIFIED_BY,
                    ETP.MODIFIED_DATE
                    ,service_flag = CASE 
                    WHEN ETP.service_flag='T' 
                    THEN 'Temporary Transfer' 
                    WHEN ETP.service_flag='AR'
                    THEN 'Additional Responsibility' 
                    WHEN ETP.service_flag='A'
                    THEN 'Assignment' 
                    Else 'Temporary Assignment' END  
			
                    FROM dbo.temporary_transfer AS ETP 
                    INNER JOIN  dbo.Employee AS E ON E.EMPLOYEE_ID = ETP.EMPLOYEE_ID   
                    INNER JOIN dbo.Departments AS D ON D.DEPARTMENT_ID = ETP.WHICH_DEPARTMENT  
                    INNER JOIN dbo.Branches AS B1 ON B1.BRANCH_ID = ETP.FROM_BRANCH   
                    INNER JOIN dbo.Departments AS D1 ON D1.DEPARTMENT_ID = ETP.FROM_DEPARTMENT 
                    )
                    X where 1=1";

            //sql = "SELECT ID,FROM_DEPARTMENT,FROM_BRANCH,EMPLOYEE_ID,EFFECTIVE_DATE,END_DATE,WHICH_DEPARTMENT,TRANSFER_DESCRIPTION,"
            //    +" CREATED_BY, CREATED_DATE, MODIFIED_BY, MODIFIED_DATE,service_flag"
            //    +" FROM "
            //    +" ("
            //    +" SELECT ETP.ID, D1.DEPARTMENT_NAME AS FROM_DEPARTMENT, B1.BRANCH_NAME AS FROM_BRANCH, "
            //    +" E.FIRST_NAME + ' ' + E.MIDDLE_NAME + ' ' + E.LAST_NAME AS EMPLOYEE_ID,"
            //    +" convert(varchar,ETP.EFFECTIVE_DATE,107) as EFFECTIVE_DATE, convert(varchar,ETP.END_DATE,107) as END_DATE, "
            //    +"  D.DEPARTMENT_NAME AS WHICH_DEPARTMENT,ETP.TRANSFER_DESCRIPTION,"
            //    +"  ETP.CREATED_BY, ETP.CREATED_DATE, ETP.MODIFIED_BY, ETP.MODIFIED_DATE,"
            //    +" case when ETP.service_flag='T' Then 'Temporary Transfer' when ETP.service_flag='A' Then 'Additional Responsibility' Else '' END service_flag"
            //    +" FROM dbo.temporary_transfer AS ETP"
            //    +"  INNER JOIN  dbo.Employee AS E ON E.EMPLOYEE_ID = ETP.EMPLOYEE_ID "
            //    +"  INNER JOIN dbo.Departments AS D ON D.DEPARTMENT_ID = ETP.WHICH_DEPARTMENT "
            //    +"  INNER JOIN dbo.Branches AS B1 ON B1.BRANCH_ID = ETP.FROM_BRANCH "
            ...
            //    +"  SELECT ETP.ID, D1.DEPARTMENT_NAME AS FROM_DEPARTMENT, B1.BRANCH_NAME AS FROM_BRANCH, "
            //    +"  E.FIRST_NAME + ' ' + E.MIDDLE_NAME + ' ' + E.LAST_NAME AS EMPLOYEE_ID,"
            //    +" convert(varchar,ETP.EFFECTIVE_DATE,107) as EFFECTIVE_DATE, '' as END_DATE, "
            //    +" D.DEPARTMENT_NAME AS WHICH_DEPARTMENT,ETP.TRANSFER_DESCRIPTION,"
            //    +" ETP.CREATED_BY, ETP.CREATED_DATE, ETP.MODIFIED_BY, ETP.MODIFIED_DATE,"
            //    +" case when ETP.TRANSFER_TYPE='i' Then 'Internal Transfer' when ETP.TRANSFER_TYPE='e' Then 'External Transfer' Else '' END service_flag"
            //    +" FROM ExternalTransferPlan AS ETP"
            //    +" INNER JOIN  dbo.Employee AS E ON E.EMPLOYEE_ID = ETP.STAFF_ID "
            //    +"  INNER JOIN dbo.Departments AS D ON D.DEPARTMENT_ID = ETP.WHICH_DEPARTMENT "
            //    +" INNER JOIN dbo.Branches AS B1 ON B1.BRANCH_ID = ETP.FROM_BRANCH "
            //    +" INNER JOIN dbo.Departments AS D1 ON D1.DEPARTMENT_ID = ETP.FROM_DEPARTMENT"
            //    +" )X"
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "TemporaryTransfer.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //html.Append("<td><a href=\"/EmployeeMovement/TemporaryTransferPlan/ManageTemporaryTransfer.aspx?ID=" + dr["ID"] + "&TransType=" + dr["Service_Flag"] + "\">View</a></td>");
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/EmployeeMovement/TemporaryTransferPlan/ManageTemporaryTransfer.aspx?ID=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/EmployeeMovement/TemporaryTransferPlan/ManageTemporaryTransfer.aspx");
