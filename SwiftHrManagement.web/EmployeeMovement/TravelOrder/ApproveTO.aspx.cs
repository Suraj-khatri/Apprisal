...
{
    public partial class ApproveTO : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 174) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"travelorderReqDate",    "Request Date",         "D"}            
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"travelOrderId",       "SN",               "200",      "LT"},
                        {"reqBy",               "Requested By",     "200",      "LT"}, 
                        {"travelorderReqDate",  "Travel From Date", "200",      "D"},
                        {"recommendedBy",       "Recommended By",      "200",      "LT"},
                        {"status",              "status",           "200",      "LT"},
                        {"recommendedRemarks",  "Remarks",          "200",      "LT"},
                        {"approveByHr",         "Verified By",         "200",      "LT"},
                        {"hrRemarks",           "Remarks",          "200",      "LT"}
                        };



            string sortby = "travelOrderId";
            ...
            +",dbo.GetEmployeeFullNameOfId(recommendedBy) recommendedBy, recommendedRemarks,status,dbo.GetEmployeeFullNameOfId(approveByHr) approveByHr"
            +" ,hrRemarks from travelOrder"
            + " where approveByHrDate is not null and finalApproveByDate IS NULL and finalApproveBy=" + filterstring(ReadSession().Emp_Id.ToString());
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/EmployeeMovement/TravelOrder/RecommendTO.aspx?flag=a&TravelOID=" + dr["travelOrderId"] + "\">Approve</a></td>");
