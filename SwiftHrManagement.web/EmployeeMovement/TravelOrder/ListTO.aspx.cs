...
{
    public partial class ListTO : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 172) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"branch",              "Branch",           "200",      "LT"},
                        {"dept",                "Department",       "200",      "LT"}, 
            ...
            string sortby = "travelorderReqDate";
            string table_name = "ExternalTransferPlan";

            String sql = "";

            sql = "SELECT travelOrderId,dbo.GetBranchOfId(branch)branch,dbo.GetDeptName(dept)dept,dbo.GetEmployeeFullNameOfId(reqBy)reqBy"
                  + ",convert(varchar,travelorderReqDate,107)travelorderReqDate FROM travelOrder WHERE recommendedDate IS NULL and recommendedBy=" + filterstring(ReadSession().Emp_Id.ToString());
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/EmployeeMovement/TravelOrder/RecommendTO.aspx?TravelOID=" + dr["travelOrderId"] + "\">Approve</a></td>");
