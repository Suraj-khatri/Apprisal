...
{
    public partial class ApproveSettlement : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 16) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"ReqDate",    "Request Date",         "D"}            
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"travelOrdSetId",       "SN",              "200",      "LT"},
                        {"travelOrderId",       "Order Id",         "200",      "LT"},
                        {"reqBy",               "Requested By",     "200",      "LT"}, 
                        {"ReqDate",             "Requested Date",     "200",      "D"},
                        //{"settlementfromDate",  "Settlement From",  "200",      "D"},
                        //{"settlementtoDate",    "Settlement From",  "200",      "D"},
                        //{"requestRemarks",      "Request Remarks",  "200",      "LT"},
                        {"recommendedBy",       "Recommended By",   "200",      "LT"},
                        {"recommendedDate",     "Recommended Date",   "200",      "D"},
                        //{"recommendedRemarks",  "Recommend Remarks","200",      "LT"}, 
                        {"approveToVerify",     "Verified By",   "200",      "LT"},
                        {"approveToVerifyDate", "Verified Date",   "200",      "D"},
                        //{"verifyRemarks",       "Recommend Remarks","200",      "LT"}
                        };

            string sortby = "ReqDate";
            string table_name = "ExternalTransferPlan";

            String sql = "";

            sql = "SELECT travelOrdSetId,travelOrderId,dbo.GetEmployeeFullNameOfId(reqBy)reqBy,ReqDate,settlementfromDate,settlementtoDate,requestRemarks,dbo.GetEmployeeFullNameOfId(recommendedBy) recommendedBy,recommendedDate,recommendedRemarks,dbo.GetEmployeeFullNameOfId(approveToVerify) approveToVerify,approveToVerifyDate,verifyRemarks FROM travelOrderSettlement WHERE approveToVerifyDate IS NOT NULL AND finalApproveByDate IS NULL and finalApproveBy=" + filterstring(ReadSession().Emp_Id.ToString()) ;
            ...
            rptApprove.InnerHtml = "<center>" + load_report(table_name, sql, matgrid, matfilter, sortd, sortby, _page, page_size) + "</center>";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "RecommendSettlement.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/EmployeeMovement/TravelOrder/Settlement/ManageRecoSettlement.aspx?flag=a&TravelOID=" + dr["travelOrderId"] + "&TravelOSID=" + dr["travelOrdSetId"] + "\">Approve</a></td>");
