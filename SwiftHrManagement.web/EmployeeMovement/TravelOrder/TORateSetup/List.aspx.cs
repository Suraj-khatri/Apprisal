    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        {"travelRateId",    "Rate ID",   "200",      "T"},
                        {"position",        "Position",         "200",      "T"},
                        {"place",           "Place",            "200",      "T"},
                        {"allowanceType",   "Allowance Type",   "200",      "T"},
                        {"rate",            "Rate",             "200",      "T"}
                        };



            string sortby = "travelRateId";
            string table_name = "TravelOrderRate";

            String sql = "";

            sql = "select travelRateId, sdd.DETAIL_DESC[position],sdd1.DETAIL_TITLE [place],sdd2.DETAIL_TITLE [allowanceType] ,dbo.ShowDecimal(rate) [rate]  from travelOrderRate tor "
                   + " inner join StaticDataDetail sdd on sdd.ROWID = tor.position"
                   + " inner join StaticDataDetail sdd1 on sdd1.ROWID = tor.place"
                   + " inner join StaticDataDetail sdd2 on sdd2.ROWID = tor.allowanceType"
                   + " where 1 = 1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/EmployeeMovement/TravelOrder/TORateSetup/Manage.aspx?ID=" + dr["travelRateId"].ToString() + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/EmployeeMovement/TravelOrder/TORateSetup/Manage.aspx");
