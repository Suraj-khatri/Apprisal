...
{
    public partial class onSiteDutyDetail : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public onSiteDutyDetail()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 48) == false && _RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 37) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        {"siteLocation",    "Site Location",    "100",      "T"},
                        {"staff_name",      "Employee",         "100",      "T"},
                        {"FromDate",        "Branch",           "100",      "T"},
                        {"ToDate",          "Department",       "100",      "T"},
                        {"purpose",         "Department",       "100",      "T"},
                        {"approvedBy",      "Approved By",      "200",      "T"},
                        {"Approved_Date",   "Approved Date",    "200",      "D"},
                        {"Status",          "Status",           "500",      "T"}                        
                        };

            string sortby = "onsiteID";
            string table_name = "onsiteDuty";
            ...
            if (GetFlag()=="i")
            {
                sql = @"SELECT 
	                              onsiteID
								, siteLocation
								, RTRIM(dbo.GetEmployeeFullNameOfId(empId)) staff_name
	                            , CONVERT(varchar,sitedatefrom,107) FromDate
	                            , CONVERT(varchar,sitedateto,107) ToDate
	                            , purpose
	                            , RTRIM(dbo.GetEmployeeFullNameOfId(Approved_by)) approvedBy
                                , CONVERT(varchar,Approved_Date,107) Approved_Date
                                , [Status] 
                            FROM onsiteDuty 
                            WHERE EmpId =" + ReadSession().Emp_Id.ToString();
            }
            else
            {
                sql = @"SELECT
                                ...
                                , [Status] 
                            FROM onsiteDuty osd
                            INNER JOIN Employee e ON e.EMPLOYEE_ID = osd.empID
                            WHERE 1=1";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //html.Append("<td><a href=\"/EmployeeMovement/onSiteDuty/onSiteDutyManage.aspx?flag="+GetFlag()+"&ID=" + dr["onsiteID"] + "\">View</a></td>");
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/EmployeeMovement/onSiteDuty/onSiteDutyManage.aspx?flag=" + GetFlag() + "&ID=" + dr["onsiteID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/EmployeeMovement/onSiteDuty/onSiteDutyManage.aspx?flag="+GetFlag());
