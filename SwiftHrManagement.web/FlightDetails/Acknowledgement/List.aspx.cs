...
namespace SwiftHrManagement.web.FlightDetails.Acknowledgement
{
    public partial class List : BasePage
    {
        ...
        {
            if (_roleMenuDao.hasAccess(ReadSession().AdminId, 284) == false)
            {
        ...
        private void init(long EmpId)
        {
            ...
            sql = @"SELECT
		            ...
		            WHERE fd.emp_id = " + ReadSession().Emp_Id + "and (fd.ApproveStatus = 'Approved' or fd.ApproveStatus = 'Rejected')";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/FlightDetails/Acknowledgement/Manage.aspx?flagID=1&id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
