...
namespace SwiftHrManagement.web.FlightDetails.Acknowledgement
{
    public partial class Manage : BasePage
    {
        ...
        public void PopulateFlightDetails()
        {
            ...
            txtAckInfo.Text = dr["ackMsg"].ToString();

            if (dr["ackStatus"].ToString() == "Pending")
            {
                txtAckInfo.ReadOnly = false;
            }

            else if (dr["ackStatus"].ToString() == "Ack")
            {
                txtAckInfo.ReadOnly = true;
        ...
        {
            Response.Redirect("../Acknowledgement/List.aspx");
        }
        ...
        {
            string sql;
            sql = "EXEC procManageFlightDetails @flag='ack', @user=" + filterstring(ReadSession().Emp_Id.ToString()) + ", @id=" + filterstring(GetID().ToString()) + ", @ackMsg=" + filterstring(txtAckInfo.Text);
