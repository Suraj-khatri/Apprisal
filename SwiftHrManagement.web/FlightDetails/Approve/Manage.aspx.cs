...
namespace SwiftHrManagement.web.FlightDetails.Approve
{
    public partial class Manage : BasePage
    {
        ...
        public void PopulateFlightDetails()
        {
            ...
            txtApprovalMsg.Text = dr["approvalMsg"].ToString();


            if (dr["ApproveStatus"].ToString() == "Pending")
            {
                txtApprovalMsg.ReadOnly = false;
            }

            else if (dr["ApproveStatus"].ToString() == "Approved")
            {
                txtApprovalMsg.ReadOnly = true;
            ...
            else if (dr["ApproveStatus"].ToString() == "Rejected")
            {
                txtApprovalMsg.ReadOnly = true;
                this.BtnReject.Visible = false;
                this.BtnSave.Visible = false;
        ...
        {
            CLsDAo.GetSingleresult(("UPDATE FlightDetails set ApproveStatus='Rejected',ApprovedBy = " + filterstring(ReadSession().Emp_Id.ToString()) + ", approvalMsg=" + filterstring(txtApprovalMsg.Text) + " WHERE id=" + filterstring(GetID().ToString())));
        }
        ...
        {
            string sql;
            sql = "EXEC procManageFlightDetails @flag='ae', @user=" + filterstring(ReadSession().Emp_Id.ToString()) + ", @id=" + filterstring(GetID().ToString()) + ", @approvalMsg=" + filterstring(txtApprovalMsg.Text);
