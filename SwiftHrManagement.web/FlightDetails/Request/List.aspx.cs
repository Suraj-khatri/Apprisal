...
namespace SwiftHrManagement.web.FlightDetails.Request
{
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            if (ReadSession().AdminId == 1)
            {
                sql = @"SELECT * FROM
                        ...
                        INNER JOIN Fiscal_Month F on cast(flightDate as Date)>=f.engDateBaisakh
                        WHERE 1=1 and f.DefaultYr='1'
		            ...
		            SELECT 
                            ...
                            ,'N/A' from tada 
                        INNER JOIN Fiscal_Month F on cast(flightDate as Date)>=f.engDateBaisakh
                        WHERE (flightDate IS NOT NULL OR returnFlightDate IS NOT NULL) and f.DefaultYr='1')x ";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Verify</th>");
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Ack</th>");
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Payment</th>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"Manage.aspx?id=" + dr["id"] + "\">View</a></td>");
                string status = dr["ApproveStatus"].ToString();
                ...
                if (status == "Approved")
                {
                    html.Append("<td><a href=\"/FlightDetails/Approve/Manage.aspx?flagID=1&id=" + dr["id"] + "\">View</a></td>");
                }
                if (status == "Rejected")
                {
                    html.Append("<td><a href=\"/FlightDetails/Approve/Manage.aspx?flagID=1&id=" + dr["id"] + "\">View</a></td>");
                ...
                string vStatus = dr["verificationStatus"].ToString();
                ...
                {
                    html.Append("<td><a href=\"/FlightDetails/FlightDetailsVerify/Manage.aspx?flagID=1&id=" + dr["id"] + "\">View</a></td>");
                }
                ...
                string ackStatus = dr["ackStatus"].ToString();

                if (ackStatus == "Pending")
                {
                    html.Append("<td>N/A</td>");
                }
                else if (ackStatus == "Ack")
                {
                    html.Append("<td><a href=\"/FlightDetails/Acknowledgement/Manage.aspx?flagID=1&id=" + dr["id"] + "\">View</a></td>");
                }
                string pStatus = dr["paidStatus"].ToString();
                ...
                {
                html.Append("<td><a href=\"/FlightDetails/PaymentDetails/Manage.aspx?flagID=1&id=" + dr["id"] + "\">View</a></td>");
                }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/FlightDetails/Request/Manage.aspx");
        ...
        private string get_filter_sql(String table_name, String[,] matfilter)
        {
            ...
            for (int i = 0; i <= arr_cnt; i++)
            {
                ...
                if (defValue.Trim() != "")
                {
                    ...
                    {
                        case "LT":
                            query = query + " where " + matfilter.GetValue(i, 0).ToString() + " LIKE '%" + defValue + "%'";
                            break;
                        case "ET":
