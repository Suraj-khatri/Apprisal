...
{
    public partial class GatepassHistory : BasePage
    {
        ClsDAOInv _clsdao = null;
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public GatepassHistory()
        {
        ...
        {
            if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 145) == false)
            {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"Id",               "Gatepass No",           "EN"},
                        {"delivered_to",     "Delivered To",          "LT"},

                        //     {"asset_number",    "Asset No",         "LT"},  
                        //{"p.porduct_code",    "Asset Name",         "LT"},
                        //{"b.BRANCH_NAME",    "Branch Name",         "LT"},
                        //{"d.DEPARTMENT_NAME",    "Dept Name",         "LT"}
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        
                        {"Id",               "Gatepass No",        "200",      "T"},
                        {"Get_pass_date",    "Gatepass Date",      "200",      "D"},
                        {"Returnable",       "Returnable",         "200",      "T"},
                        {"Out_Message",      "Out Message",        "200",      "T"},
                        {"delivered_to",     "Delivered To",       "200",      "T"}
                        };

            string sortby = "id";
            string table_name = "Asset_Getpass_Out_Message";

            String sql = "select id,convert(varchar,Get_Pass_Date, 107) as Get_Pass_Date,case when Returnable = 'Y' then 'Yes' "
            + " when Returnable = 'N' then 'No' end 'Returnable',Out_Message, delivered_to from Asset_Getpass_Out_Message with (nolock) where 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "GatepassHistory.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">In</th>");
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Out</th>");
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Print</th>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"In\" href=\"GatepasshistoryIn.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Out\" href=\"GatepassHistoryOut.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Print\" href=\"/Report/Gatepassreport.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
