...
{
    public partial class GatepassHistoryOut : BasePage
    {
        ClsDAOInv _clsdao = null;
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public GatepassHistoryOut()
        {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        //{"Out_Message",   "Out Message",        "200",      "LT"},
                        {"delivered_to",  "Delivered to",   "200",      "LT"},
                        {"porduct_code",  "Product Code",      "200",      "LT"},
                        {"GatepassDate",  "Gatepass Date",     "200",      "D"}
                        };

            string sortby = "GatepassDate";
            string table_name = "Asset_Getpass_In_Message";
            string id = "";
            if (Request.QueryString["id"] != null)
                id = Request.QueryString["id"];
            LblGpno.Text = id;
            String sql = "select CONVERT(varchar, am.Get_Pass_Date, 107) 'GatepassDate', ap.porduct_code ,am.Out_Message, am.delivered_to "
            + " from Asset_Getpass_Out_Message am  inner join Asset_Get_Pass agp on am.id = agp.Get_pass_message_id inner join "
            + " ASSET_PRODUCT ap on agp.asset_id = ap.id where agp.Get_pass_message_id = " + filterstring(id) + "";
