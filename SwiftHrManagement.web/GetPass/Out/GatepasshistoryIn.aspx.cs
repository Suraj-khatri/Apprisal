...
{
    public partial class GatepasshistoryIn : BasePage
    {
        ClsDAOInv _clsdao = null;
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public GatepasshistoryIn()
        {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"AcknowledgedBy",   "Acknowledged By",      "200",      "LT"}
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        
                        //{"In_Message",    "In Message",        "200",      "LT"},
                        {"AcknowledgedBy",   "Acknowledged By",   "200",      "LT"},
                        {"porduct_code",  "Product Code",      "200",      "LT"},
                        {"Return_Date",   "Returned Date",     "200",      "D"}
                        };

            string sortby = "Return_Date";
            string table_name = "Asset_Getpass_In_Message";

            string id = "";
            if (Request.QueryString["id"] != null)
                id = Request.QueryString["id"];
            lblGpNo.Text = id;
            String sql = "select CONVERT(varchar, am.Return_Date, 107) 'Return_Date', ap.porduct_code ,am.In_Message, e.FIRST_NAME + ' ' + e.MIDDLE_NAME + ' ' + e.LAST_NAME "
            + " 'AcknowledgedBy' from Asset_Getpass_In_Message am inner join Employee e on am.Acknowledged_By = e.EMPLOYEE_ID "
            + " inner join Asset_Get_Pass agp on am.Get_Pass_Message_Id = agp.Get_pass_In_messageid inner join ASSET_PRODUCT ap on agp.asset_id = ap.id"
            + " where agp.Get_pass_In_messageid = " + filterstring(id) + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //html.Append("<td><a href=\"GatepasshistoryIn.aspx?id=" + dr["id"] + "\">In</a></td>");
                //html.Append("<td><a href=\"GatepassHistoryOut.aspx?id=" + dr["id"] + "\">Out</a></td>");
                html.Append("</tr>");
            }
