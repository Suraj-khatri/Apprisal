...
{
    public partial class ListReturnGatepass : BasePage
    {
        ClsDAOInv _clsdao = null;
        string ApproveGatepass = "";
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public ListReturnGatepass()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request.Form["ckID"] != null)
                ApproveGatepass = Request.Form["ckID"].ToString();
            TxtReturnDate.Text = System.DateTime.Now.ToString("MM/dd/yyyy");
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"BRANCH_NAME",      "Branch",             "200",      "LT"},
                        {"DEPARTMENT_NAME",  "Department",         "200",      "LT"},
                        {"accumulated_dep",  "Accumulated Dep.",   "200",      "M"},
                        {"book_value",       "Book Value",         "200",      "M"}
                        };

            string sortby = "id";
            string table_name = "Asset_Get_Pass";

            String sql = "select gp.id,  ai.asset_number, b.BRANCH_NAME, d.DEPARTMENT_NAME ,gp.accumulated_dep, gp.book_value "
            + " from Asset_Get_Pass gp inner join Branches b on gp.branch_id = b.BRANCH_ID inner join Departments d on gp.dept_id = "
            + " d.DEPARTMENT_ID inner join ASSET_INVENTORY ai on gp.asset_id = ai.id where gp.Get_pass_message_id = "+ GetId() +" and gp.in_out = 'o'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListReturnGatepass.aspx?sortd=" + sortd + "&sortby={sortby}&Id="+GetId()+"";
            ...
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Return</th>");
            
            //html.Append("<th Class=\"HeaderStyle\" align=\"left\">Upload Bill</th>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td align=\"left\"><input type=\"Checkbox\" id=\"ckID\" name=\"ckID\" value=\"" + dr["id"].ToString() + "\"></td>");
                //html.Append("<td><a href=\"ListReturnGatepass.aspx?id=" + dr["id"] + "\">View</a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, false, "ManageGetpassDetail.aspx");
        ...
        #endregion
        private void managegatepassin()
        {
            if (String.IsNullOrEmpty(ApproveGatepass))
            {
                LblMsg.Text = "Please select assets to return !";
                LblMsg.ForeColor = System.Drawing.Color.Red;
                return;
            }
            _clsdao.runSQL("exec INV_FIXEDASST_MGMT..Proc_AssetGatepassIn 'i'," + ReadSession().Emp_Id + ",'" + ApproveGatepass + "'," + GetId() + "," + filterstring(TxtReturnDate.Text) + ""
            + " ,"+ filterstring(TxtAppMessage.Text) +"");
        }
        protected void BtnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                TxtReturnDate.Text="";
                init();
                LblMsg.Text = "Returned successfully";
            }
            catch
            {
        ...
        protected void TxtAppMessage_TextChanged(object sender, EventArgs e)
        {
