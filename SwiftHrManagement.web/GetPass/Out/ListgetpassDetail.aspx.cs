...
{
    public partial class ListgetpassDetail : BasePage
    {
        ClsDAOInv _clsdao = null;
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public ListgetpassDetail()
        {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"Id",               "Get Pass Number",           "200",      "EN"},
                        {"delivered_to",     "Delivered To",              "200",      "LT"}
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"Get_Pass_Date",     "Get Pass Date",      "200",      "D"},
            ...
            + " when Returnable = 'N' then 'No' end 'Returnable',Out_Message, delivered_to from Asset_Getpass_Out_Message a with (nolock), "
            + " (select distinct Get_pass_message_id   from Asset_Get_Pass with (nolock) "
            + " where in_out='o') b where a.id=b.Get_pass_message_id and Returnable='Y'";

            //String sql = "select id,convert(varchar,Get_Pass_Date, 107) as Get_Pass_Date,case when Returnable = 'Y' then 'Yes' "
            //            + " when Returnable = 'N' then 'No' end 'Returnable',Out_Message, delivered_to from Asset_Getpass_Out_Message";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListgetpassDetail.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                string isreturnable = "";
                isreturnable = dr["Returnable"].ToString();
                if (isreturnable == "Yes")
                {
                    html.Append("<td><a href=\"ListReturnGatepass.aspx?id=" + dr["id"] + "\">Return</a></td>");
                }
                else
                {
                    html.Append("<td>N/A</a></td>");
                }
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\"href=\"/Report/Gatepassreport.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-print\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "ManageGetpassDetail.aspx");
