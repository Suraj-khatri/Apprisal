...
namespace SwiftHrManagement.web.AllocationSetup
{
    public partial class List : BasePage
    {
        ...
        {
            if (_roleMenuDao.hasAccess(ReadSession().AdminId, 137) == false)
            {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"ALLOCATION",      "Allocated Capacity",   "200",      "T"}
                        };



            string sortby = "ID";
            string table_name = "Darbandi_Allocation";

            String sql = "";

            sql = @"SELECT 
	                    ID,
	                    dbo.GetBranchName(Branch_ID) AS Branch,
	                    dbo.GetDeptName(Dept_ID) AS DEPT,
	                    SDD.DETAIL_DESC AS POSITION,
	                    Allocation
                    FROM Darbandi_Allocation DA
                    Inner Join StaticDataDetail SDD on DA.Position=SDD.ROWID
                    where 1=1";
