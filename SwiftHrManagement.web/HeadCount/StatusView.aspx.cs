...
namespace SwiftHrManagement.web.Company.Internship
{
    public partial class StatusView : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_roleMenuDao.hasAccess(ReadSession().AdminId, 136) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{ 
                        ...
                        {"Employee_ALLOCATION",        "Employee Allocation",        "200",      "T"},
                        {"CURRENT_TOTAL_Employee",        "Current Total Employee",        "200",      "T"},
                        {"STATUS",        "Status",        "200",      "T"}
                        };

            string sortby = "BRANCH_SHORT_NAME";
            string table_name = "Branches";

            String sql = "";
            sql = @"SELECT B.BRANCH_ID,B.BRANCH_SHORT_NAME,B.BRANCH_NAME,ISNULL(D.ALLOCATION,0) AS Employee_ALLOCATION,
                    ISNULL(E.Total_Employee,0) AS CURRENT_TOTAL_Employee,CASE WHEN CAST(ISNULL(D.ALLOCATION,0) AS INT)<ISNULL(E.Total_Employee,0) THEN 'CAPACITY EXCEEDED'
                    WHEN ISNULL(D.ALLOCATION,0)=ISNULL(E.Total_Employee,0) THEN 'NO VACANCY' WHEN ISNULL(D.ALLOCATION,0)>ISNULL(E.Total_Employee,0) THEN 'VACANCY' END AS [STATUS]
                    FROM Branches B
                    LEFT JOIN (SELECT BRANCH_ID,ALLOCATION FROM Darbandi_Allocation) D ON B.BRANCH_ID=D.Branch_ID
                    LEFT JOIN (SELECT BRANCH_ID,COUNT(*) Total_Employee FROM Employee GROUP BY BRANCH_ID) E on B.BRANCH_ID=E.BRANCH_ID";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                for (int i = 0; i <= arr_cnt; i++)
                {
                    if (i==2)
                    {
                        ...
                        {
                            case "M":
                                html.Append("<td align=\"right\"><a href=\"/Company/HeadCountList/HeadCountList.aspx?Id=" + dr["BRANCH_ID"] + "\">" + dr[matgrid.GetValue(i, 0).ToString()].ToString() + "</a></td>");
                                break;
                            case "D":
                                html.Append("<td align=\"center\"><a href=\"/Company/HeadCountList/HeadCountList.aspx?Id=" + dr["BRANCH_ID"] + "\">" + dr[matgrid.GetValue(i, 0).ToString()].ToString() + "</a></td>");
                                break;
                            default:
                                html.Append("<td align=\"left\"><a href=\"/Company/HeadCountList/HeadCountList.aspx?Id=" + dr["BRANCH_ID"] + "\">" + dr[matgrid.GetValue(i, 0).ToString()].ToString() + "</a></td>");
                                break;
                        }
                    }
                    else
