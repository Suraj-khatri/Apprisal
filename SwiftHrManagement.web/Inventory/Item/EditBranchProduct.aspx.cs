...
#region SwiftGrid
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        {"prod_code",       "Prod Code",      "200",      "LT"},
                        {"porduct_code",       "Product",      "200",      "LT"},
                        {"BRANCH_NAME",        "Branch",       "200",      "LT"},
                        {"INVENTORY_AC",           "Stock A/C",         "200",      "LT"},
                        {"PURCHASE_AC",            "Exp. A/C",         "200",      "LT"},
                        {"REORDER_LEVEL",            "Reorder Level",         "200",      "LT"},
                        {"REORDER_QTY",            "Reorder Qty",         "200",      "LT"},
                        {"MAX_HOLDING_QTY",            "Max Qty",         "200",      "LT"},
                        {"STOCK",            "Current Stock",         "200",      "LT"},
            ...
            String sql = @"select a.ID, b.id prod_code,b.porduct_code,c.BRANCH_NAME,a.INVENTORY_AC,a.PURCHASE_AC,a.REORDER_LEVEL,
a.REORDER_QTY,a.MAX_HOLDING_QTY,ISNULL(A.stock_in_hand,0) STOCK,
case when a.IS_ACTIVE='True' then 'Active' else 'Inactive' end as [STATUS]
from IN_BRANCH a inner join IN_PRODUCT b on a.PRODUCT_ID=b.ID inner join Branches c on c.BRANCH_ID=a.BRANCH_ID WHERE b.id=" + hdnproductid.Value + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "EditBranchProduct.aspx?product=" + hdnproduct.Value + "&productId="+hdnproductid.Value+"&sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Inventory/Item/BranchProdSetup.aspx?id=" + dr["id"] + "&product=" + dr["porduct_code"].ToString() + "\"><i class=\"fa fa-eye\"></i></a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Inventory/Item/BranchProdSetup.aspx?productId=" + hdnproductid.Value + "&product=" + hdnproduct.Value);
