...
namespace SwiftAssetManagement.Inventory.Item
{
    public partial class EditList : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 106) == false)
                {
    ...
    #region SwiftGrid
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"item_name",           "Group Name",         "200",      "LT"},
                        {"item_desc",           "Group Descreiption", "200",      "LT"}                        
                        };

            string sortby = "item_name";
            string table_name = "IN_ITEM";

            String sql = "select id ,item_name, item_desc from IN_ITEM  where isnull(is_product,0) <> 1 and 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //html.Append("<td><a href=\"/Inventory/Item/Manage.aspx?id=" + dr["id"] + "\">View</a></td>");
                html.Append("<td><a href='#' class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\"  onclick=\"ShowGroupView('" + dr["id"] + "')\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
