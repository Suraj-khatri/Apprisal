...
{
    public partial class EditProductList : BasePage
    {
...
#region SwiftGrid
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                {"p.id", "Product Code",         "EN"},
                        {"porduct_code", "Product Name",         "LT"},
                        {"price", "Price",  "ET"},
                        {"package_unit", "Package Unit",  "LT"}
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        
                        {"id",        "Product Code",      "200",      "T"},
                        {"porduct_code",        "Product Name",      "200",      "T"},
                        {"package_unit",        "Package Unit",      "200",      "T"},
                        {"price",         "Unit Price",       "200",      "M"},
                        {"batch_condition",     "Is Batched",   "200",      "T"},
                        {"serial_no",           "Is Serialed",       "200",      "T"},
                        {"is_active",           "Is Active",         "200",      "T"},
                        };

            string sortby = "porduct_code";
            string table_name = "IN_PRODUCT";

            String sql = "select p.id,a.id group_id ,p.porduct_code,p.package_unit,dbo.ShowDecimal(p.purchase_base_price) as price,"
                        +" case when p.batch_condition = 'True' then 'Yes' when p.batch_condition = 'False' then 'No' end 'batch_condition',"
            +" case when p.serial_no = 'True' then 'Yes' when p.serial_no = 'False' then 'No' end 'serial_no', case when p.is_active = 'True' "
            + " then 'Yes' when p.is_active = 'False' then 'No' end 'is_active' from IN_PRODUCT p, IN_ITEM i , IN_ITEM a "
            + " where i.id = p.item_id and a.id=i.parent_id";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "EditProductList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href='#' class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" onclick=\"ShowProductView('" + dr["id"] + "','" + dr["group_id"] + "')\"><i class=\"fa fa-eye\"></i></a> ");
                html.Append("<a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View Branch\" href=\"/Inventory/Item/EditBranchProduct.aspx?productId=" + dr["id"] + "&product=" + dr["porduct_code"] + "\"><i class=\"fa fa-building-o\"></i></a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Inventory/Item/List.aspx?");
