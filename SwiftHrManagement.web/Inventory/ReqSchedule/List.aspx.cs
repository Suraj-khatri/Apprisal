...
namespace SwiftHrManagement.web.Inventory.ReqSchedule
{
    public partial class List : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_roleMenuDao.hasAccess(ReadSession().AdminId, 241) == false)
                {
        ...
        private void init()
        {
            ...
            string sortby = "ID";
            string table_name = "IN_REQ_SCHEDULE";

            String sql = "";

            sql = @"SELECT ID,dbo.GetBranchName(BRANCH_ID) BRANCH_NAME,DAY_NUM,
dbo.GetEmployeeFullNameOfId(CREATED_BY) CREATED_BY,CONVERT(VARCHAR,CREATED_DATE,107) CREATED_DATE
FROM IN_REQ_SCHEDULE WHERE 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Inventory/ReqSchedule/Manage.aspx?Id=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Inventory/ReqSchedule/Manage.aspx");
