    ...
    #region SwiftGrid
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"quantity",            "Requested Quantity",             "200",      "EN"},
                        {"unit",                "Package Unit",                 "200",      "LT"},
                        {"Approved_Quantity",   "Approved Quantity",    "200",      "EN"}
                        };
            ...
            if (Request.QueryString["id"] != null)
                hdnapid.Value = (Request.QueryString["id"]);


            String sql = "select r.id, p.porduct_code ,r.quantity, r.unit, r.Approved_Quantity  from IN_Requisition r  "
            + " inner join IN_PRODUCT p on r.item = p.id  where r.Requistion_message_id = " + hdnapid.Value + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Modify\" href=\"/Inventory/Requisition/Approve/Manage.aspx?id=" + dr["id"] + "&msgid=" + hdnapid.Value + "\"><i class=\"fa fa-pencil-square-o\"></i></a></td>");       
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, false, "Manage.aspx");
        ...
        {
            DataTable dt = _clsdao.getTable(@"select Requ_Message,is_schedule,recommed_message,UNSCHEDULE_REASON from IN_Requisition_Message where id=" + hdnapid.Value + "");
            foreach (DataRow dr in dt.Rows)
            {
                txtReqMsg.Text = dr["Requ_Message"].ToString();
                txtRecmsg.Text = dr["recommed_message"].ToString();
        ...
        {
            _clsdao.runSQL("exec proc_Approve_requisition @flag='m',@id=" + filterstring(hdnapid.Value) + ","
        ...
        protected void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                manageapproval();
                Response.Redirect("/Inventory/Requisition/List.aspx");
            }
            catch
            {
        ...
        private void ManageRejection()
        {
            _clsdao.runSQL("exec proc_Approve_requisition 'rej'," + hdnapid.Value + ",'" + ReadSession().Emp_Id + "','" + TxtAppMessage.Text + "'");
        }
    }
}
