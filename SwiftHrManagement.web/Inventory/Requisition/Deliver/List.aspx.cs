    ...
    public partial class List : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (GetStringFlag() == "U")
                {
                    _clsdao.runSQL("Exec ManageInsertIntoTemp @id=" +filterstring(GetReqId().ToString()) + ",@session_id="+filterstring(ReadSession().Sessionid)+"");
                }
                ...
                string checkFlag = _clsdao.GetSingleresult("EXEC [ProcChkReqForStock] 'a'," + filterstring(hdnapid.Value) + "");
                if (checkFlag == "Y")
                {
                    DdlCheckReqType.SelectedValue = checkFlag;
                    DdlCheckReqType.Enabled = false;
                }
                else
                {
                    DdlCheckReqType.SelectedValue = checkFlag;
        ...
        private void OnPopulateReq()
        {
            DataTable dt = _clsdao.getTable(@"select dbo.GetBranchName(branch_id) branch,Requ_Message,recommed_message,
                                        Approver_message,is_schedule,UNSCHEDULE_REASON from IN_Requisition_Message where id=" + GetReqId() + "");
            foreach (DataRow dr in dt.Rows)
            {
                hdnapid.Value = GetReqId().ToString();
                txtDispatchDate.Text = System.DateTime.Now.ToString("MM/dd/yyyy");
                txtReqMsg.Text = dr["Requ_Message"].ToString();
                txtRecommedMsg.Text = dr["recommed_message"].ToString();
                txtAppMsg.Text = dr["Approver_message"].ToString();
                ...
                else
                {
                    ...
                    txtUnschReason.Text = dr["UNSCHEDULE_REASON"].ToString();
                }
            }
        }
    ...
    #region SwiftGrid
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"Approved_Quantity",   "Approved QTY",             "200",      "EN"},
                        {"unit",                "Unit",                 "200",      "LT"},
                        {"Dispatched_qty",  "Dispatched QTY",    "200",      "EN"},
                        {"remain",  "Remain To Dispatch",    "200",      "EN"},
                        {"stock_in_hand",  "Stock In Hand",    "200",      "EN"}
            ...
            String sql = "SELECT r.id, i.porduct_code , r.Approved_Quantity, r.unit,r.Approved_Quantity-r.Delivered_Quantity remain,"
                        + " r.Delivered_Quantity Dispatched_qty,b.stock_in_hand FROM IN_Requisition_detail r "
                        +" INNER JOIN IN_PRODUCT i ON i.id = r.item INNER JOIN IN_BRANCH b ON b.PRODUCT_ID=r.item "
                        + " WHERE r.Requistion_message_id =" + hdnapid.Value + " AND b.BRANCH_ID=(SELECT forwarded_to FROM IN_Requisition_Message WHERE id="+hdnapid.Value+") "
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "/Inventory/Requisition/Deliver/List.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            for (int i = 0; i <= arr_cnt; i++)
            {
                ...
                html.Append("<th Class=\"HeaderStyle\" align=\"left\">" +  matgrid.GetValue(i, 1).ToString() + "</a></th>");
            }
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Actions</th>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Modify\" href=\"/Inventory/Requisition/Deliver/Manage.aspx?id=" + dr["id"] + "&appid=" + hdnapid.Value + "\"><i class=\"fa fa-pencil-square-o\"></i></a>");
                html.Append("<span onclick=\"addOther('" + dr["id"].ToString() + "')\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Add\" href=\"#\"><i class=\"fa fa-plus\" aria-hidden=\"true\"></i></a></span></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, false, "/Inventory/Requisition/Deliver/Manage.aspx?"+ hdnapid.Value +"");
        ...
        private void manageDispatchProduct()
        {
            string IsDirectExp="";
           
            IsDirectExp = DdlCheckReqType.Text;
            LblMsg.Text = IsDirectExp;
            string msg = _clsdao.GetSingleresult("exec proc_Approve_requisition @flag='d',@id=" + filterstring(hdnapid.Value) + ","
                        + " @approver_id=" + filterstring(ReadSession().Emp_Id.ToString()) + ",@approver_msg=" + filterstring(TxtAppMessage.Text) + ","
                        + " @stkFlag=" + filterstring(IsDirectExp) + ",@session_id=" + filterstring(ReadSession().Sessionid) + ","
                        + " @dispatch_date="+filterstring(txtDispatchDate.Text)+"");
            if (msg != "")
            {
                LblMsg.Text = msg;
                LblMsg.ForeColor = System.Drawing.Color.Red;
        ...
        //{
        //   _clsdao.runSQL("update IN_Requisition_Message set status='Full Dispatched' where id=" + GetReqId() + "");
        //   Response.Redirect("ViewDelivery.aspx");
        //}
    }
}
