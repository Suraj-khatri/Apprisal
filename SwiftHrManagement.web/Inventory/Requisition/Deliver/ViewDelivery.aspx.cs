...
{
    public partial class ViewDelivery : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"rm.id",    "Requisition ID",         "EN"},
                        {"rm.Requ_date",    "Requested Date",         "ED"},
                        {"Approved_Date",    "Approved Date",         "ED"},
                        {"b.BRANCH_NAME",    "Requesting Branch",         "LT"},
                        {"d.DEPARTMENT_NAME",    "Requesting Dept",         "LT"}
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        
                        {"id",    "Requisition Id",       "200",      "T"},
                        {"Requ_date",                "Requested Date",      "200",      "D"},
                        {"Requ_by",                  "Requested By",        "200",      "T"},
                        {"recommend_by",              "Recommended By",         "200",      "T"}, 
                        {"Approver_id",              "Approved By",         "200",      "T"},                        
                        {"BRANCH_NAME",              "Requesting Branch",   "200",      "T"},
                        {"DEPARTMENT_NAME",          "Requesting Department",         "200",      "T"},
                        {"priority",              "Priority",         "200",      "T"},
                        {"is_schedule",              "Is Schedule",         "200",      "T"},
                        {"status",              "Status",         "200",      "T"}
                        };

            string sortby = "Requ_date";
            string table_name = "IN_Requisition_Message";
            String sql = "";
            if (ReadSession().UserName == "admin")
            {
                sql = "SELECT rm.id, dbo.GetEmployeeFullNameOfId(rm.Requ_by) AS Requ_by, CONVERT(VARCHAR,rm.Requ_date ,107) AS Requ_date,"
                        + " b.BRANCH_NAME,d.DEPARTMENT_NAME,dbo.GetEmployeeFullNameOfId(rm.recommed_by) as recommend_by,dbo.GetEmployeeFullNameOfId(rm.Approver_id) AS Approver_id,"
                        + " CASE WHEN priority='N' THEN 'Normal' WHEN	priority='H' THEN 'High' ELSE 'Low' END AS priority,status,is_schedule "
                        + " FROM IN_Requisition_Message rm INNER JOIN Branches b ON rm.branch_id = b.BRANCH_ID  "
                        + " INNER JOIN Departments d ON rm.dept_id = d.DEPARTMENT_ID WHERE "
                        + " status<>'Full Dispatched' AND rm.Approved_Date IS NOT NULL and status<>'Rejected' and Delivered_Date is null";
            }
            else
            {
                sql = "SELECT rm.id, dbo.GetEmployeeFullNameOfId(rm.Requ_by) AS Requ_by, CONVERT(VARCHAR,rm.Requ_date ,107) AS Requ_date,"
                        ...
                        + " FROM IN_Requisition_Message rm INNER JOIN Branches b ON rm.branch_id = b.BRANCH_ID  "
                        + " INNER JOIN Departments d ON rm.dept_id = d.DEPARTMENT_ID WHERE "
                        + " status<>'Full Dispatched' AND rm.Approved_Date IS NOT NULL and status<>'Rejected'  and Delivered_Date is null AND  Forwarded_To = '" + ReadSession().Branch_Id + "'";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ViewDelivery.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"List.aspx?id=" + dr["id"] + "&FLAG=U\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, false, "List.aspx?");
        ...
        private string make_filter_form(string table_name, string[,] matfilter)
        {
            ...
            for (int i = 0; i <= arr_cnt; i++)
            {
                ...
                if (ctlType == "ED")
                {
                    html.Append("<input type=\"text\" name=\"" + ctlName + "\" value=\"" + defValue + "\" class=\"form-control datepicker\"></td>");
