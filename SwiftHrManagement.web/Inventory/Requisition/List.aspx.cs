       ...
       public List()
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"e.First_name",    "Requested First Name",             "ET"},
                        {"Requ_date",    "Requested Date",             "ED"},
                        {"BRANCH_NAME",     "Requesting Branch", "T"}
            ...
            if (ReadSession().UserName.ToString() == "admin")
            {
                sql = "SELECT m.id,CONVERT(VARCHAR,Requ_date ,107) AS Requ_date,dbo.GetEmployeeFullNameOfId(e.employee_id) AS Requ_by,"
                                + " dbo.GetEmployeeFullNameOfId(recommed_by) AS recommed_by,CONVERT(VARCHAR,recommed_date ,107) AS recommed_date,"
                                + " b.BRANCH_NAME ,d.DEPARTMENT_NAME,B1.BRANCH_NAME AS FORWARDED_BRANCH,"
                                + " case when priority='N' then 'Normal' when	priority='H' then 'High' else 'Low' end as priority,is_schedule FROM IN_Requisition_Message m INNER JOIN "
                                + " Departments d ON m.dept_id = d.DEPARTMENT_ID INNER JOIN Branches b ON m.branch_id = b.BRANCH_ID INNER JOIN "
                                + " Employee e ON e.EMPLOYEE_ID = m.Requ_by INNER JOIN Branches B1 ON B1.BRANCH_ID=m.Forwarded_To"
                                + " WHERE m.Approved_Date IS NULL  AND m.recommed_date IS NOT NULL and status<>'Rejected' ";
            }
            else
            {
                sql = "SELECT m.id,CONVERT(VARCHAR,Requ_date ,107) AS Requ_date,dbo.GetEmployeeFullNameOfId(e.employee_id) AS Requ_by,"
                                ...
                                + " Departments d ON m.dept_id = d.DEPARTMENT_ID INNER JOIN Branches b ON m.branch_id = b.BRANCH_ID INNER JOIN "
                                + " Employee e ON e.EMPLOYEE_ID = m.Requ_by INNER JOIN Branches B1 ON B1.BRANCH_ID=m.Forwarded_To"
                                + " WHERE m.Approved_Date IS NULL  AND m.recommed_date IS NOT NULL and status<>'Rejected' and Forwarded_To='" + ReadSession().Branch_Id + "'";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            String sql1 = "select * from (select ROW_NUMBER() OVER (ORDER BY "+sortby+" asc) as rowId,* from(" + sql + " ) as aa) as tmp where 1 = 1 ";
            sql1 += " and  tmp.rowId between " + ((_page - 1) * page_size + 1).ToString() + " and " + (_page * page_size).ToString();

            sql1 += " order by " + sortby + " desc";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Inventory/Requisition/Approve/List.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");       
                html.Append("</tr>");
            }
        ...
        private string make_filter_form(string table_name, string[,] matfilter)
        {
            ...
            for (int i = 0; i <= arr_cnt; i++)
            {
                ...
                if (ctlType=="ED")
                {
                    html.Append("<input type=\"text\" id=\"datepicker\" name=\"" + ctlName + "\" value=\"" + defValue + "\" class=\"form-control\"></td>");
