...
namespace SwiftAssetManagement.Inventory.Requisition
{
    public partial class Manage : BasePage
    {
        ...
        private void ManageTempRequisition()
        {
            ...
            string msg = _clsdao.GetSingleresult("exec [proc_Requisition] @flag='i',@item=" + filterstring(itemID.ToString()) + ","
                ...
                + " @branch="+filterstring(ReadSession().Branch_Id.ToString())+"");
        ...
        private void ManageRequestion()
        {
            ...
            //    emp_id = item[1];
                ...
                string msg = _clsdao.GetSingleresult("exec [proc_Requisition] 'j',@session_id=" + filterstring(ReadSession().Sessionid) + ","
                + " @approver=" + filterstring(DdlEmpName.Text) + ",@message=" + filterstring(TxtMessage.Text) + ","
                + " @forwarded_to=" + filterstring(DdlBranchRqe.Text) + ",@priority=" + filterstring(Ddlpriority.Text) + ","
                + " @branch=" + filterstring(ReadSession().Branch_Id.ToString()) + ",@department=" + filterstring(ReadSession().Department.ToString()) + ","
                + " @user=" + filterstring(ReadSession().Emp_Id.ToString()) + ",@unSchReason="+filterstring(txtUnschReqMsg.Text)+"");
                ...
                {
                    Response.Redirect("/Inventory/Requisition/RequisitionList.aspx");
                }
            //}
            //else
            //{
            //    LblMsg.Text = "Please Choose Recommend Employee By Autocomplete!";
            //    LblMsg.ForeColor = System.Drawing.Color.Red;
            //    return;
            //}
        ...
        private void PopulateDdl()
        {
            if (ReadSession().UserType == "A")
                _clsdao.CreateDynamicDDl(DdlBranchRqe, "EXEC [procGetBranchList] @flag='A'", "BRANCH_ID", "BRANCH_NAME", "", "SELECT");

            else if (ReadSession().UserType == "B")
                _clsdao.CreateDynamicDDl(DdlBranchRqe, "EXEC [procGetBranchList] @flag='B',"
                    + " @branch_id=" + filterstring(ReadSession().Branch_Id.ToString()) + "", "BRANCH_ID", "BRANCH_NAME", "", "SELECT");

            else if (ReadSession().UserType == "N")
                _clsdao.CreateDynamicDDl(DdlBranchRqe, "EXEC [procGetBranchList] @flag='N',"
                    + " @branch_id=" + filterstring(ReadSession().Branch_Id.ToString()) + "", "BRANCH_ID", "BRANCH_NAME", "", "SELECT");
