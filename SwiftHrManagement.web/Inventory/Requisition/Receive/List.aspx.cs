    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"package_unit",                "Unit",              "200",   "T"},
                        {"dispatched_qty",  "Dispatched QTY", "200",      "EN"},                        
                        {"received_qty",   "Received QTY", "200",     "EN"},
                        {"remain",   "Remain To Received", "200",     "EN"},
                        };
            ...
            String sql = "select d.id,p.porduct_code,d.dispatched_qty,p.package_unit,d.received_qty,remain from IN_DISPATCH d inner join IN_PRODUCT p on "
                            +" p.id=d.product_id where d.dispatched_qty<>0 and d.dispatch_msg_id="+hdnapid.Value+"";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //html.Append("<td><a href=\"/Inventory/Requisition/Receive/Manage.aspx?id=" + dr["id"] + "&msg_id=" + hdnapid.Value + "&req_id="+GetReqId()+"\">Modify</a></td>");
        ...
        {
            long req_id = GetReqId();

            _clsdao.runSQL("exec proc_Approve_requisition @flag='r',@id=" + filterstring(GetId().ToString()) + ","
                            + " @approver_id=" + filterstring(ReadSession().Emp_Id.ToString()) + ","
                            + " @approver_msg=" + filterstring(txtReceivedMsg.Text) + ",@req_id="+filterstring(GetReqId().ToString())+"");
        }
        private long GetId()
        {
        ...
        protected void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                manageapproval();
                Response.Redirect("/Inventory/Requisition/Receive/ViewReceived.aspx");
            }
            catch
            {
        ...
        private void  GetApprovedMessage()
        {
         
             DataTable dt=_clsdao.getTable("exec [proc_Requisition] @flag='apm',@id=" + filterstring(GetReqId().ToString()));
             foreach (DataRow dr in dt.Rows)
             {
                 txtReceivedMsg.Text = dr["Approver_message"].ToString();
             }
