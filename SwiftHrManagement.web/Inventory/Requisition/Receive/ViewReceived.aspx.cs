...
{
    public partial class ViewReceived : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"rm.id",    "Requisition ID",         "LT"}, 
                        {"rm.Requ_date",    "Requested Date (MM/DD/YYYY)",         "ED"}                     
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        
                        {"id",                       "Dispatch ID",       "200",      "T"},
                        {"req_id",                       "Requisition ID",       "200",      "T"},
                        {"req_by",                  "Requested By",        "200",      "T"},
                        {"req_branch",              "Requested Branch",         "200",      "T"},
                        {"req_dept",          "Requested Dept.",        "200",      "T"},
                        {"dis_by",           "Dispatched By",         "200",      "T"},
                        {"dis_date",           "Dispatched Date",         "200",      "D"},
                        {"dis_branch",           "Dispatched Branch",         "200",      "T"}
                        };



            string sortby = "id";
            string table_name = "IN_DISPATCH_MESSAGE";
            string sql = "";
            if (ReadSession().UserName == "admin")
            {
                sql = "SELECT dm.id AS id,rm.id AS req_id, dbo.GetEmployeeFullNameOfId(rm.Requ_by) AS req_by, CONVERT(VARCHAR,rm.Requ_date ,107)"
                        + " AS req_date, b.BRANCH_NAME req_branch, d.DEPARTMENT_NAME req_dept,dbo.GetEmployeeFullNameOfId(e.employee_id) AS dis_by,"
                        + " CONVERT(VARCHAR,dm.dispatched_date,107) AS dis_date,b1.BRANCH_NAME AS dis_branch FROM IN_DISPATCH_MESSAGE dm "
                        + " INNER JOIN IN_Requisition_Message rm ON rm.id=dm.req_id"
                        + " INNER JOIN Branches b ON rm.branch_id = b.BRANCH_ID "
                        + " INNER JOIN Departments d ON rm.dept_id = d.DEPARTMENT_ID"
                        + " INNER JOIN Employee e ON e.EMPLOYEE_ID=dm.dispatched_by "
                        + " INNER JOIN Branches b1 ON b1.BRANCH_ID=rm.Forwarded_To"
                        + " WHERE 1=1 and dm.status<>'Full Acknowledged'";
            }
            else
            {
                ...
                sql = "SELECT dm.id AS id,rm.id AS req_id, dbo.GetEmployeeFullNameOfId(rm.Requ_by) AS req_by, CONVERT(VARCHAR,rm.Requ_date ,107)"
                        ...
                        +" INNER JOIN Employee e ON e.EMPLOYEE_ID=dm.dispatched_by "
                        +" INNER JOIN Branches b1 ON b1.BRANCH_ID=rm.Forwarded_To"
                        + " WHERE rm.Requ_by=" + ReadSession().Emp_Id + " and dm.status<>'Full Acknowledged'";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ViewReceived.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Action</th>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"List.aspx?id=" + dr["id"] + "&req_id=" + dr["req_id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
        ...
        private string make_filter_form(string table_name, string[,] matfilter)
        {
            ...
            for (int i = 0; i <= arr_cnt; i++)
            {
                ...
                if (ctlType == "ED")
                {
                    html.Append("<td width=\"200\"><input type=\"text\"  id=\"datepicker\"  name=\"" + ctlName + "\" value=\"" + defValue + "\"></td>"); 
                }
                else
