...
namespace SwiftHrManagement.web.Inventory.Requisition.Recommed
{
    public partial class List : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 124) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"DEPARTMENT_NAME",          "Requested Department",  "200",      "T"},
                        {"forwardbranch",          "Forwarded Branch",  "200",      "T"},
            ...
            String sql = "select m.id, CONVERT(VARCHAR,Requ_date ,107) as Requ_date,dbo.GetEmployeeFullNameOfId(e.employee_id) as Requ_by, b.BRANCH_NAME ,d.DEPARTMENT_NAME,"
            + " case when priority='N' then 'Normal' when	priority='H' then 'High' else 'Low' end as priority,is_schedule,dbo.GetBranchName(forwarded_to) forwardbranch from "
            + " IN_Requisition_Message m inner join Departments d on m.dept_id = d.DEPARTMENT_ID inner join Branches b on "
            + " m.branch_id = b.BRANCH_ID inner join Employee e on e.EMPLOYEE_ID = m.Requ_by where m.recommed_date is null"
            +" and status<>'Rejected' and recommed_by = '" + ReadSession().Emp_Id + "'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Recommed</th>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Inventory/Requisition/Recommed/Manage.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
