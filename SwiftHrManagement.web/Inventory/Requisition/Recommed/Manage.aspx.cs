...
namespace SwiftHrManagement.web.Inventory.Requisition.Recommed
{
    ...
    #region SwiftGrid
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "Manage.aspx?sortd=" + sortd + "&sortby={sortby}";
        ...
        {
            _clsdao.runSQL("exec proc_Approve_requisition @flag='rec',@id=" + filterstring(hdnapid.Value) + ","
                        + " @approver_id=" + filterstring(ReadSession().Emp_Id.ToString()) + ",@approver_msg=" + filterstring(txtRecommedMsg.Text) + "");
        }
        private void OnPopulateReq()
        {
            DataTable dt = _clsdao.getTable(@"select Requ_Message,is_schedule,UNSCHEDULE_REASON from IN_Requisition_Message where id=" + hdnapid.Value + "");
            foreach (DataRow dr in dt.Rows)
            {
        ...
        protected void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                manageapproval();
                Response.Redirect("/Inventory/Requisition/Recommed/List.aspx");
            }
            catch
            {
        ...
        private void MangeRejection()
        {
            _clsdao.runSQL("exec proc_Approve_requisition 'rej'," + hdnapid.Value + ",'" + ReadSession().Emp_Id + "','" + txtRecommedMsg.Text + "'");
        }
    }
}
