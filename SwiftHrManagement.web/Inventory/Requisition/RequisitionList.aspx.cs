...
namespace SwiftAssetManagement.Inventory
{
    public partial class RequisitionList : BasePage
    {
        ...
        private void init()
        {
            ...
            if (ReadSession().UserName.ToString() == "admin")
            {
                ...
                + " b.BRANCH_NAME ,d.DEPARTMENT_NAME,status,IS_SCHEDULE from IN_Requisition_Message m inner join Departments d on m.dept_id = d.DEPARTMENT_ID inner join"
                + " Branches b on m.branch_id = b.BRANCH_ID inner join Employee e on e.EMPLOYEE_ID = m.Requ_by where 1=1";
            }
            else
            {
                sql = "select m.id, CONVERT(VARCHAR,Requ_date ,107) as Requ_date, dbo.GetEmployeeFullNameOfId(E.Employee_Id) as Requ_by, b.BRANCH_NAME,"
                + " d.DEPARTMENT_NAME,status,IS_SCHEDULE from IN_Requisition_Message m inner join Departments d on m.dept_id = d.DEPARTMENT_ID inner join Branches b on "
                     + " m.branch_id = b.BRANCH_ID inner join Employee e on e.EMPLOYEE_ID = m.Requ_by where Requ_by ='" + ReadSession().Emp_Id + "'";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "RequisitionList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Modify</th>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                for (int i = 0; i <= arr_cnt; i++)
                {
                 ...
                 html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Modify\" href=\"EditRequisition.aspx?Id=" + dr["ID"] + "\"><i class=\"fa fa-pencil-square-o\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "Manage.aspx");
