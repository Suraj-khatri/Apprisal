...
namespace SwiftHrManagement.web.Inventory.budget
{
    public partial class List : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_roleMenuDao.hasAccess(ReadSession().AdminId, 240) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        {"FY",            "Fiscal Year",             "200",      "T"},
                        {"BRANCH_NAME",            "Branch Name",             "200",      "T"},
                        {"PRODUCT_ID",       "Prod Code",        "200",      "T"},
                        {"PRODUCT_NAME",       "Product Name",        "200",      "T"},
                        {"BUDGET_QTY",    "Budget QTY",     "200",      "T"},
                        {"RATE",    "Rate (Rs.)",     "200",      "T"},
                        {"REMARKS",    "Remarks",     "200",      "T"},
                        {"IS_ACTIVE",    "Status",     "200",      "T"}
                        };



            string sortby = "ID";
            string table_name = "IN_BUDGET";

            String sql = "";

            sql = @"SELECT A.ID,FY,dbo.GetBranchName(BRANCH_ID) BRANCH_NAME,A.PRODUCT_ID,
B.porduct_code PRODUCT_NAME,BUDGET_QTY,dbo.ShowDecimal(RATE) RATE,REMARKS,A.IS_ACTIVE 
FROM IN_BUDGET A INNER JOIN IN_PRODUCT B ON A.PRODUCT_ID=B.id WHERE 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Inventory/budget/Manage.aspx?Id=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Inventory/budget/Manage.aspx");
        ...
        private string get_paging_block(int _total_record, int _page, int _page_size, Boolean _show_add_button, string _add_page)
        {
            ...
            html.Append("<span class=\"btn btn-default\"> <a href=\"/Inventory/budget/UploadBudget.aspx\" title=\"Upload Budget\" > <i class=\"fa fa-upload\" aria-hidden=\"true\"></i></a></span>");
