...
{
    public partial class deptWiseExp : BasePage
    {
        ...
        private void ShowBranchWiseRpt()
        {
            ...
            var ds = _clsdao.getDataset("exec procStockDepartWiseRpt @flag='c',@branch_id=" + filterstring(branchId) + ","
            ...
            for (int i = 4; i < 7; i++)
            {
                if ( i == 6)
                    ...
                    str.Append("<td align = 'center'><b>" + ShowDecimal(total[i]) + "</b></td>");
                else
        ...
        private string PrintRegionBody(ref DataTable dt, string regionName, string deptId, ref Double[] total)
        {
            ...
            html.Append("<tr>");
            html.Append("<td colspan='5'><b><a href='#' onclick=\"OpenInNewWindow('userWiseRpt.aspx?branchId=" + branchId + "&fromDate=" + fromDate + "&toDate=" + toDate + "&deptId=" + deptId + "&deptName=" + regionName + "&branchName="+branchName+"')\">" + regionName + "</a></b></td>");
            html.Append("</tr>");
            int bag_sno = 0;
