    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        {"BranchName",       "Branch Name",             "200",      "T"},
                        {"TITLE",           "Event Name",             "200",      "T"},
                        {"DATE",            "Event Date",             "200",      "D"},
                        {"VENUE",           "Venue",                 "200",      "T"},
            ...
            string sortby = "ID";
            string table_name = "OfficialCalendar";

            String sql = "";


            //sql = "SELECT ID, TITLE, CONVERT(VARCHAR,DATE,107) AS DATE, DESCRIPTION,VENUE FROM OfficialCalendar where "
            //    +" TYPE ='E'";

            sql = "SELECT ID,case when b.BRANCH_NAME IS null then 'All Branch' else b.BRANCH_NAME end as BranchName,"
                  + " TITLE,CONVERT(VARCHAR,DATE,107) AS DATE, NATURE, DESCRIPTION,VENUE FROM OfficialCalendar oc"
                  + " left join Branches b on oc.BRANCHID=b.branch_id where "
                  + " TYPE ='E'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\"  href=\"/LeaveManagementModule/Calender/Events/Manage.aspx?Id=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/LeaveManagementModule/Calender/Events/Manage.aspx");
