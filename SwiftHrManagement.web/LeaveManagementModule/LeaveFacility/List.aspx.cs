    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{                        
                        {"b.DETAIL_TITLE",    "Employee Type",         "LT"},  
                        {"c.NAME_OF_LEAVE",    "Leave Type",         "LT"}
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"DETAIL_TITLE",            "Employee Type",             "200",      "T"},
                        {"NAME_OF_LEAVE",            "Leave Type",             "200",      "T"},
                        {"created_by",            "Added By",         "200",      "T"},
                        {"created_date",       "Added Date",        "200",      "D"}
                        };

            string sortby = "id";
            string table_name = "globalLeaveSetup";

            String sql = "";

            sql = @"select a.id,a.emp_typeId,b.DETAIL_TITLE,c.NAME_OF_LEAVE,dbo.GetEmployeeFullNameOfId(a.created_by) created_by,
CONVERT(varchar,a.created_date,107) created_date
from globalLeaveSetup a inner join StaticDataDetail b on a.emp_typeId=b.ROWID inner join LeaveTypes c on c.ID=a.leave_typeId where a.flag='y'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"GlobalLeaveSetup.aspx?empType=" + dr["emp_typeId"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "GlobalLeaveSetup.aspx");
