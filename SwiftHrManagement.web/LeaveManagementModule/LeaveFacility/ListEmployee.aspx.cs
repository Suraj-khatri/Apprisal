...
{
    public partial class ListEmployee : BasePage
    {
        ...
        private void init()
        {
            ...
            string branchId = Request.QueryString["BranchId"] != "" ? Request.QueryString["BranchId"] : "";
            string positionId = Request.QueryString["postId"] != "" ? Request.QueryString["postId"] : "";
            string empId = Request.QueryString["EmpId"] != "" ? Request.QueryString["EmpId"] : "";
            //column type : LT = like text , ET = Equal to text, EN= equal to numbert
            String[,] matfilter = new String[,]{
                        {"EmpName",         "Employee Name",    "LT"},
                        {"NAME_OF_LEAVE",   "Leave Type",       "LT"}
                    };
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"EMP_CODE",            "Employee Code",        "200",      "D"},
                        {"EmpName",             "Employee Name",        "200",      "LT"},
                        {"Position",            "Position",             "200",      "LT"},
                        {"NAME_OF_LEAVE",       "Leave Type",           "200",      "LT"},
                        {"NO_OF_DAYS_ACTUAL",   "Assigned Days",        "200",      "D"}
                        };

            string sortby = "EMPLOYEE_ID";
            string table_name = "Employee List";
            string sql = "";
            if (empId != "")
            {
                sql = "SELECT x.EMPLOYEE_ID, x.BRANCH_NAME,x.EMP_CODE,x.EmpName,x.Position,lt.NAME_OF_LEAVE,la.NO_OF_DAYS_ACTUAL FROM(" +
                       "SELECT EMP_CODE,EMPLOYEE_ID,FIRST_NAME +' '+MIDDLE_NAME +' '+ LAST_NAME [EmpName]" +
                       ",sd.DETAIL_TITLE Position,b.BRANCH_NAME FROM Employee e INNER JOIN StaticDataDetail sd ON e.POSITION_ID = sd.ROWID " +
                       "INNER JOIN Branches b ON b.BRANCH_ID = e.BRANCH_ID WHERE e.BRANCH_ID = " + filterstring(branchId)
                       + " AND POSITION_ID = " + filterstring(positionId) + " AND EMPLOYEE_ID=" + filterstring(empId) + ")x " +
                       "LEFT JOIN leaveAssignment la ON la.EMPLOYEE_ID = x.EMPLOYEE_ID LEFT JOIN LeaveTypes lt ON lt.ID = la.LEAVE_TYPE_ID WHERE 1 = 1";
            ...
            else
            {
                sql = "SELECT x.EMPLOYEE_ID, x.BRANCH_NAME,x.EMP_CODE,x.EmpName,x.Position,lt.NAME_OF_LEAVE,la.NO_OF_DAYS_ACTUAL FROM(" +
                        ...
                        ",sd.DETAIL_TITLE Position,b.BRANCH_NAME FROM Employee e INNER JOIN StaticDataDetail sd ON e.POSITION_ID = sd.ROWID " +
                        "INNER JOIN Branches b ON b.BRANCH_ID = e.BRANCH_ID WHERE e.BRANCH_ID = " + filterstring(branchId)
                        + " AND POSITION_ID = " + filterstring(positionId) + ")x " +
                        "LEFT JOIN leaveAssignment la ON la.EMPLOYEE_ID = x.EMPLOYEE_ID LEFT JOIN LeaveTypes lt ON lt.ID = la.LEAVE_TYPE_ID WHERE 1 = 1";
            ...
            rptEmployee.InnerHtml = "<center>" + load_report(table_name, sql, matgrid, matfilter, sortd, sortby, _page, page_size) + "</center>";
           ...
           // html.Append("<th Class=\"HeaderStyle\" align=\"left\">View</th>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                for (int i = 0; i <= arr_cnt; i++)
                {
                    ...
                    {
                        case "M":
                            html.Append("<td align=\"right\">" + ShowDecimal(dr[matgrid.GetValue(i, 0).ToString()].ToString()) + "</td>");
                            break;
                        case "D":
                ...
                //html.Append("<td><a href=\"/LeaveManagementModule/LeaveAssignment/Manage.aspx?ID=" + dr["ID"] + "&EmpID=" + GetEmployeeId().ToString() + "\">View</a></td>");
                html.Append("</tr>");
            ...
            //    string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/LeaveManagementModule/LeaveAssign/Manage.aspx?EmpId=" + GetEmployeeId() + "&LoadPage_ID=" + 11 + "");
        ...
        private string make_filter_form(string table_name, string[,] matfilter)
        {
            ...
            html.Append("<table width=\"7ssss00\" cellspacing=\"2\" cellpadding=\"2\" border=\"0\">");
            html.Append("<tr>");
