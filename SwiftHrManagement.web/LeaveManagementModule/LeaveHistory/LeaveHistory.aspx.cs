...
namespace SwiftHrManagement.web.LeaveManagement.LeaveHistory
{
    public partial class LeaveHistory : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{                        
                        ...
                        {"A.REQUESTED_BY",    "Requested By",         "LT"},
                        {"A.RECOMMEND_PERSON",    "Recommend Person",         "LT"},
                        {"A.APPROVE_PERSON",    "Approve Person",         "LT"}

                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"NAME_OF_LEAVE",           "Leave Name",               "100",             "T"},
                        {"REQUESTED_BY",            "Requested By",             "200",             "T"},
                        {"REQUESTED_FROM",               "Request From",                "100",             "D"},
                        {"REQUESTED_TO",                 "Request To",                  "100",             "D"},                        
                        {"Requested_days",              "Request Days",               "200",             "T"}, 
                        {"RECOMMEND_PERSON",            "Recommend Person",             "50",              "T"},
                        {"APPROVED_FROM",            "Approve From",             "50",              "D"},
                        {"APPROVED_TO",            "Approve To",             "50",              "D"},
                        {"Approved_days",            "Approve Days",             "50",              "T"},
                        {"APPROVE_PERSON",            "Approve Person",             "50",              "T"},
                        {"LEAVE_STATUS",            "Leave Status",             "50",              "T"}
                        };
            ...
            sql = "SELECT A.ID,A.NAME_OF_LEAVE,A.REQUESTED_BY,A.REQUESTED_FROM,A.REQUESTED_TO,A.Requested_days,A.RECOMMEND_PERSON,"
+" A.APPROVED_FROM,A.APPROVED_TO,A.Approved_days,A.APPROVE_PERSON,A.LEAVE_STATUS FROM ("
+" SELECT "
    +"  LR.ID "
	+" ,LT.NAME_OF_LEAVE "
	+" ,DBO.GetEmployeeFullNameOfId(REQUESTED_BY) AS REQUESTED_BY"
	+" ,CONVERT(VARCHAR,FROM_DATE,107) AS REQUESTED_FROM"
	+" ,CONVERT(VARCHAR,TO_DATE,107) AS REQUESTED_TO"
	+" ,CASE WHEN REQUESTED_HRS_STATUS='1' THEN '1st Half' WHEN REQUESTED_HRS_STATUS='2'"
	+" THEN '2nd Half' ELSE CAST(ISNULL(REQUESTED_DAYS,0) AS VARCHAR) END AS Requested_days"
	+" ,dbo.GetEmployeeFullNameOfId(REQUESTED_WITH) AS RECOMMEND_PERSON"
	+" ,CASE WHEN LEAVE_STATUS='Approved' THEN CONVERT(VARCHAR,APPROVED_FROM,107)  ELSE '' END AS APPROVED_FROM"
	+" ,CASE WHEN LEAVE_STATUS='Approved' THEN CONVERT(VARCHAR,APPROVED_TO,107)  ELSE '' END AS APPROVED_TO"
	+" ,CASE WHEN LEAVE_STATUS='Approved' THEN CASE WHEN REQUESTED_HRS_STATUS='1' THEN '1st Half' WHEN REQUESTED_HRS_STATUS='2'"
	+" THEN '2nd Half' ELSE CAST(ISNULL(APPROVED_DAYS,0) AS VARCHAR) END ELSE '' END AS Approved_days"
	+" ,dbo.GetEmployeeFullNameOfId(APPROVED_BY) AS APPROVE_PERSON"
	+" ,LEAVE_STATUS"
+" FROM LeaveRequest LR INNER JOIN LeaveTypes LT ON LT.ID=LR.LEAVE_TYPE_ID"
+" WHERE LEAVE_STATUS IN ('Approved','Recommended','Pending' )AND "
+" APPROVED_BY="+filterstring(ReadSession().Emp_Id.ToString())+" OR REQUESTED_WITH="+filterstring(ReadSession().Emp_Id.ToString())+""
+" )A ";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "LeaveHistory.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                for (int i = 0; i <= arr_cnt; i++)
                {
                    ...
                    {
                        case "M":
                            html.Append("<td align=\"right\" width=\""+ matgrid.GetValue(i, 2).ToString() +"\">" + dr[matgrid.GetValue(i, 0).ToString()].ToString() + "</td>");
                            break;
                        case "D":
                            html.Append("<td align=\"center\" width=\"" + matgrid.GetValue(i, 2).ToString() + "\">" + dr[matgrid.GetValue(i, 0).ToString()].ToString() + "</td>");
                            break;
                        default:
                            html.Append("<td align=\"left\" width=\"" + matgrid.GetValue(i, 2).ToString()+ "\">" + dr[matgrid.GetValue(i, 0).ToString()].ToString() + "</td>");
                            break;
                    }
                }
        ...
        {
            StringBuilder html = new StringBuilder("<table width=\"900\" cellspacing=\"2\" cellpadding=\"2\" border=\"0\">");
