...
namespace SwiftHrManagement.web.LeaveManagementModule.LeaveRecord
{
    public partial class List : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                //if (_roleMenuDao.hasAccess(ReadSession().AdminId, 113) == false)
                //{
                //    Response.Redirect("/Error.aspx");
                //}
            }
        ...
        private void init()
        {
            ...
            //column type : LT = like text , ET = Equal to text, EN= equal to number
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"FRM_DATE",            "From Date",        "200",      "D"},
                        {"TO_DATE",             "To Date",          "200",      "D"},
                        {"LEAVE_STATUS",        "Leave Status",     "200",      "T"},
                        {"LEAVE_PURPOSE",       "Leave Purpose",    "200",      "T"}
                        };
            ...
            sql = @"SELECT
                    ...
                    ,convert(varchar,isnull(REQUEST_DATE
                    ,lr.CREATED_DATE),107) request_date
                    ,convert(varchar,LR.FROM_DATE,107) as FRM_DATE
                    ,dbo.GetEmployeeFullNameOfId(LR.APPROVED_BY) APPROVED_BY
                    ,convert(varchar,LR.TO_DATE,107) as TO_DATE
                    ,rtrim(dbo.GetEmployeeFullNameOfId(REQUESTED_BY)) AS REQUESTED_BY
                    ,rtrim(dbo.GetEmployeeFullNameOfId(REQUESTED_WITH)) AS REQUESTED_WITH
                    ,LR.LEAVE_STATUS
                    ,LR.REQUESTED_DAYS
                    ,LR.LEAVE_PURPOSE
                    ,LR.REMAINING_DAYS
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\"  href=\"Manage.aspx?Id=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
