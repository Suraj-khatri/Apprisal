    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{                        
                        ...
                        {"E.FIRST_NAME",    "Requested By",         "LT"},
                        {"E.EMP_CODE",    "Employee Code",         "LT"},
                        {"LEAVE_STATUS",    "Leave Status",         "LT"}
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"LEAVE_TYPE_ID",           "Leave Name",               "100",             "T"},
                        {"REQUESTED_BY",            "Requested By",             "200",             "T"},
                        {"request_date",            "Requested Date",           "200",             "T"},
                        {"FRM_DATE",               "From Date",                 "100",             "D"},
                        {"TO_DATE",                 "To Date",                  "100",             "D"},
                        {"REQUESTED_WITH",          "Recommend By",             "200",             "T"},
                        {"APPROVE_BY",              "Approve By",               "200",             "T"}, 
                        {"LEAVE_STATUS",            "Leave Status",             "50",              "T"},
                        };

            string sortby = "FROM_DATE";
            ...
            sql = @"SELECT 
		                    ...
		                    ,CONVERT(varchar,isnull(REQUEST_DATE,lr.CREATED_DATE),107) request_date
		                    ,CONVERT(varchar,LR.FROM_DATE,107) as FRM_DATE
		                    ,CONVERT(varchar,LR.TO_DATE,107) as TO_DATE
		                    ,E.FIRST_NAME+' '+E.MIDDLE_NAME+' '+E.LAST_NAME  AS REQUESTED_BY
		                    ,E1.FIRST_NAME+' '+E1.MIDDLE_NAME+' '+E1.LAST_NAME AS REQUESTED_WITH
		                    ,LR.LEAVE_STATUS
		                    ,LR.REQUESTED_DAYS
		                    ,E2.FIRST_NAME+' '+E2.MIDDLE_NAME+' '+E2.LAST_NAME AS APPROVE_BY
		                    ,LR.LEAVE_PURPOSE
		                    ,LR.REMAINING_DAYS 
                    FROM LeaveRequest LR  
                    INNER JOIN leaveAssignment la ON la.ID=LR.leaveAssignId  
                    INNER JOIN Employee E ON E.EMPLOYEE_ID=LR.REQUESTED_BY  
                    INNER JOIN Employee E1 ON E1.EMPLOYEE_ID=LR.REQUESTED_WITH  
                    INNER JOIN LeaveTypes LT ON LT.ID=LR.LEAVE_TYPE_ID  
                    INNER JOIN Employee E2 ON E2.EMPLOYEE_ID=LR.APPROVED_BY  
                    WHERE 1=1 AND la.IS_DISABLED=1   ";

                if (ReadSession().BranchLevelAccess == "one")
                sql = sql + " and E.BRANCH_ID='" + ReadSession().Branch_Id + "'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\"  href=\"/LeaveManagementModule/LeaveRequest/Manage.aspx?Id=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/LeaveManagementModule/LeaveRequest/Manage.aspx");
