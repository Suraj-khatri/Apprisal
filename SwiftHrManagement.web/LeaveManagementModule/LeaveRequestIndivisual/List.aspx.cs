    ...
    public partial class List : BasePage
    {
        ...
        private void init(long EmpId)
        {
            ...
            String[,] matfilter = new String[,]{                        
                        ...
                        {"LR.LEAVE_STATUS",    "Leave Status",         "LT"}
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"LEAVE_TYPE_ID",            "Leave Name",          "50",       "T"},
                        {"REQUESTED_WITH",          "Requested With",       "50",      "T"},
                        {"FROM_DATE",               "From Date",            "50",      "D"},
                         {"TO_DATE",                "To Date",              "50",      "D"},
                         {"LEAVE_STATUS",           "Leave Status",         "100",      "T"},
                         {"LEAVE_PURPOSE",          "Leave Purpose",        "300",      "T"}
                        };
            ...
            + " LR.LEAVE_PURPOSE,LR.REMAINING_DAYS from LeaveRequest LR inner join leaveAssignment la on la.ID=LR.leaveAssignId INNER JOIN Employee E ON E.EMPLOYEE_ID=LR.REQUESTED_BY INNER JOIN "
            + " Employee E1 ON E1.EMPLOYEE_ID=LR.REQUESTED_WITH INNER JOIN LeaveTypes LT ON LT.ID=LR.LEAVE_TYPE_ID where la.IS_DISABLED=1 and REQUESTED_BY='" + EmpId + "'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\"  href=\"/LeaveManagementModule/LeaveRequestIndivisual/Manage.aspx?Id=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/LeaveManagementModule/LeaveRequestIndivisual/Manage.aspx");
