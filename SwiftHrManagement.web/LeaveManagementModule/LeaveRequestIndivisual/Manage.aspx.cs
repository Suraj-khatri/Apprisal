...
namespace SwiftHrManagement.web.LeaveManagementModule.LeaveRequestIndivisual
{
    public partial class Manage : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            lblEmpName.Text = _clsdao.GetSingleresult("EXEC proc_GetLeaveData @jobflag = 'SE',@searchEmpBy = " + filterstring(ReadSession().Emp_Id.ToString()));
        ...
        {
            var ds = _leaveRequestDao.SelectLeaveTypesCondition(ReadSession().Emp_Id.ToString(), DdlLeaveName.Text);
            if (ds == null)
            {
                ResetField();
                return;
        ...
        private void GetDateDiffrence()
        {
            ...
            {
                var dr = _leaveRequestDao.SelectRequestedDays(ReadSession().Emp_Id.ToString(), DdlLeaveName.Text, TxtFromDate.Text, TxtToDate.Text);
               
                if (dr == null)
                    return;

                //if (dr["errorcode"].ToString() == "1")
                //{
                //    LblMsg.Text = dr["msg"].ToString();
                //}
                //else if (dr["errorcode"].ToString() == "0")
                //{
                ...
                if (long.Parse(hdnReqDays.Value) >= 15 && DdlLeaveName.Text == "1" && isLfa == "True" )
                {
                    if (lfaTaken == "True")
                    {
        ...
        {
            _clsdao.CreateDynamicDDl(DdlLeaveName, "EXEC proc_GetLeaveData @jobflag = 'CE', @emp_ID=" + filterstring(ReadSession().Emp_Id.ToString()), "LEAVE_ID", "LEAVE_NAME", "", "SELECT");
        ...
        {
            var dt = _leaveRequestDao.Save(GetLeaveRequestId().ToString(), ReadSession().Emp_Id.ToString(), DdlLeaveName.Text,
