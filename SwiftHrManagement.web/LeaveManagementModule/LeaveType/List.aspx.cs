    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"NO_OF_DAYS_DEFAULT",            "Default Days",             "200",      "D"},
                        {"MAX_ACCUMULATION",            "Max. Accumulation",             "200",      "D"},
                        {"IS_ACTIVE",    "Is Active",     "200",      "T"},
                        {"IS_LFA",    "Is LFA",     "200",      "T"},
                        {"LEAVE_DETAILS",    "Description",     "200",      "T"},
                        };



            string sortby = "ID";
            string table_name = "LeaveTypes";

            String sql = "";

            sql = @"SELECT ID, NAME_OF_LEAVE,NO_OF_DAYS_DEFAULT,LEAVE_DETAILS,MAX_ACCUMULATION,
case when  IS_LFA='True' then 'Yes' when  IS_LFA='False' then 'No' end 'IS_LFA',
case when  IS_ACTIVE='True' then 'Yes' when  IS_ACTIVE='False' then 'No' end 'IS_ACTIVE' FROM LeaveTypes with (nolock) where 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\"  href=\"/LeaveManagementModule/LeaveType/Manage.aspx?Id=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/LeaveManagementModule/LeaveType/Manage.aspx");
