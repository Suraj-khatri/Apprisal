    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"WORKING_DATE",       "Working Date",        "200",      "D"},
            ...
            string sortby = "ID";
            string table_name = "LeaveAssignSubstitute";
            String sql = "";
            ...
            if (DoesHrUser(ReadSession().Emp_Id.ToString()))
            {
                ...
                sql = @"SELECT 
                                ID
                                ,e.EMP_CODE
                                ,dbo.GetEmployeeFullNameOfId(EMP_ID) EMP_NAME
                                ,CONVERT(VARCHAR,WORKING_DATE,107) WORKING_DATE
                                ,DBO.GetEmployeeFullNameOfId(la.CREATED_BY) CREATED_BY
                                ,CONVERT(VARCHAR,la.CREATED_DATE,107) CREATED_DATE 
                            FROM LeaveAssignSubstitute la 
                            INNER JOIN Employee e ON e.EMPLOYEE_ID = la.EMP_ID
							 INNER JOIN Fiscal_Month F ON cast((WORKING_DATE)as Date)>=f.engDateBaisakh
            ...
            else
            {
                sql = @"SELECT ID
                            ...
                            INNER JOIN Employee e on e.EMPLOYEE_ID = la.EMP_ID
							 INNER JOIN Fiscal_Month F ON cast((WORKING_DATE)as Date)>=f.engDateBaisakh
                            WHERE 1=1 AND f.DefaultYr='1'
                            AND la.CREATED_BY=" + ReadSession().Emp_Id.ToString() + "";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
           ...
           // sql1 += " order by " + sortby + " " + sortd;
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/LeaveManagementModule/Substitute/Manage.aspx?ID=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/LeaveManagementModule/Substitute/Manage.aspx");
