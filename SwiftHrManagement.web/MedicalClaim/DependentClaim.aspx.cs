...
{
    public partial class DependentClaim : BasePage
    {
        clsDAO CLsDAo = null;
        ...
        {
            CLsDAo.setDDL(ref ddlDependedName, "Exec [procMedicalClaim] @flag='gd',@empId=" + filterstring(ReadSession().Emp_Id.ToString()) + "", "ID", "familyName", "", "Select");
            CLsDAo.setDDL(ref ddlParticulars, "Exec ProcStaticDataView 'sd','87'", "ROWID", "DETAIL_TITLE", "", "Select");
        }
        ...
        private void OnPopulateData()
        {
            ...
            if (ddlClaimType.Text == "Hospitalization")
            {
                divHospitalize.Visible = true;
        ...
        private void OnSave()
        {
            ...
            string msg = CLsDAo.GetSingleresult("exec [procMedicalClaim] @flag=" + filterstring(flag) + ",@id=" + filterstring(GetID().ToString()) + ""
                        ...
                        + "  @claimType=" + filterstring(ddlClaimType.Text) + ",@hosDiagnosis=" + filterstring(hosDiagnosis.Text) + ","
                        + "  @accAttendingDoctor=" + filterstring(txtAccAttendingDoctor.Text) + ",@hosAdmissionDate=" + filterstring(hosAdmissionDate.Text) + ","
                        + "  @sickDateTime=" + filterstring(txtSickDate.Text) + ",@sickAttendingDoctor=" + filterstring(txtSickAttendingDoctor.Text) + ","
                        + "  @sickDiagnosis=" + filterstring(txtSickDiagnosis.Text) + ","                    
                        + "  @USER=" + filterstring(ReadSession().Emp_Id.ToString()) + ",@sessionId=" + filterstring(ReadSession().Sessionid) + ","
                        + "  @claimFor=" + filterstring("Dependent") + "");

            if (msg.Contains("Success"))
            {
                Response.Redirect("DependentClaimList.aspx");
            }
            else
            {
                LblMsg.Text = msg;
        ...
        protected void btnDeleteRecord_Click(object sender, EventArgs e)
        {
            try
            {
                CLsDAo.runSQL("Exec [procMedicalClaim] @flag='d',@id='" + "" + "'");
        ...
        protected void btnDeleteRptType_Click(object sender, EventArgs e)
        {
            try
            {
                CLsDAo.runSQL("Exec procMedicalClaim @flag='rptDelete',@id='" + "" + "'");
        ...
        protected void btnDeleteDocType_Click(object sender, EventArgs e)
        {
            try
            {
                CLsDAo.runSQL("Exec [procMedicalClaim] @flag='docDelete',@id='" + "" + "'");
        ...
        protected void ddlDependedName_SelectedIndexChanged1(object sender, EventArgs e)
        {
            if (ddlDependedName.Text == ReadSession().Emp_Id.ToString())
            {
                lblDependendAge.Text = CLsDAo.GetSingleresult("select DATEDIFF(YEAR,BIRTH_DATE,getdate()) from employee where employee_id=" + filterstring(ddlDependedName.Text) + "");
        ...
        protected void ddlClaimType_SelectedIndexChanged1(object sender, EventArgs e)
        {
            if (ddlClaimType.Text == "Hospitalization")
            {
                divHospitalize.Visible = true;
