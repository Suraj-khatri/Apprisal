    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{                        
                        {"dbo.GetEmployeeFullNameOfId(empId) ",     "Employee Name",    "LT"},  
                        {"dbo.GetDeptName(deptId) ",                "Department",       "LT"},  
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"deptName",        "Department",       "200",      "T"},
                        {"postName",        "Position",         "200",      "T"},
                        {"age",             "Age",              "200",      "T"},
                        {"dependendName",   "Dependent Name",   "200",      "T"},
                        {"relationName",    "Relation Name",    "200",      "T"},
                        {"dependendAge",    "Dependend Age",    "200",      "T"},
                        {"claimType",       "Claim Type",       "200",      "T"},
                        {"claimStatus",     "Claim Status",     "200",      "T"},
                         {"claimFor",       "Claim For",        "200",      "T"}
                        };

            string sortby = "id";
            string table_name = "medicalClaim";
            ...
            sql = @"SELECT 
                             id
                            ,e.EMP_CODE
                            ,dbo.GetEmployeeFullNameOfId(empId) empName
                            ,dbo.GetDeptName(deptId) deptName
                            ,dbo.GetDetailTitle(positionId) postName
                            ,age
                            ,dependendName
                            ,case when relationId is null Then 'Self' else relationId end relationName
                            ,dependendAge
                            ,claimType
                            ,claimStatus
                            ,claimFor 
                        FROM medicalClaim mc
					    INNER JOIN Employee e ON e.EMPLOYEE_ID = mc.empId 
                        WHERE empId=" + ReadSession().Emp_Id + " and claimFor='Self'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
           ...
           // html.Append("<th Class=\"HeaderStyle\" align=\"left\"></th>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (dr["claimStatus"].ToString() != "Requested")
                {
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/MedicalClaim/ManageClaim.aspx?Id=" + dr["ID"] + "&flag=p\"><i class=\"fa fa-eye\"></i></a></td>");

                    //html.Append("<td><a href=\"/MedicalClaim/ManageClaim.aspx?Id=" + dr["ID"] + "&flag=p\">View</a></td>");
                }
                else
                {
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/MedicalClaim/Manage.aspx?Id=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                    //html.Append("<td><a href=\"/MedicalClaim/Manage.aspx?Id=" + dr["ID"] + "\">View</a></td>");
                }
                
               // html.Append("<td><a href=\"/MedicalClaim/Upload.aspx?Id=" + dr["id"] + "\">Upload File</a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/MedicalClaim/Manage.aspx");
