...
{
    public partial class HrOJTList : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"dbo.GetEmployeeFullNameOfId(JF.FEEDBACK_BY)",          "SuperVisro/Trainee",            "LT"},
            ...
            String[,] matgrid = new String[,]{ 
                {"DETAIL_DESC",                     "Job Type",                      "100",      "T"},        
                {"SUPERVISOR_TRAINEE",              "SuperVisor/Trainee",            "100",      "T"},
                //{"OJT_TITLE",                       "OJT Title",                     "100",      "T"},
                {"JOB_DESC",                        "Job Desc",                      "300",      "J"},
                {"WEIGHT",                          "Weight",                        "100",      "T"},
                {"FEEDBACK_CAT",                    "FeedBack Category",             "100",      "T"},
                       
                        };

            string sortby = "JFD_ID";
            string table_name = "JobFeedBackDetails";

            String sql = "";

            sql = "SELECT JFD_ID,SD.DETAIL_DESC,ISNULL(JF.JOB_DESC,'') [JOB_DESC],dbo.GetEmployeeFullNameOfId(JF.FEEDBACK_BY) [SUPERVISOR_TRAINEE]"
                +" ,CASE [WEIGHT] WHEN '4' THEN 'EXCELLENT' "
                +"  WHEN '3' THEN 'GOOD' WHEN '2' THEN 'SATISFACTORY' WHEN '1' THEN 'BAD' ELSE '' END AS [WEIGHT]"
                +" ,CASE ISNULL(JF.JOB_FEEDBACK_CAT,'') WHEN '' THEN '' ELSE SD1.DETAIL_TITLE END [FEEDBACK_CAT]"
                +" FROM JobFeedBackDetails JF "
                +"  INNER JOIN JOBDESCRIPTION JD "
                + "   ON JD.JOB_ID=JF.JOB_ID"
                +" INNER JOIN StaticDataDetail SD " 
                +"  ON SD.ROWID=JD.JOB_STATIC_TYPE"
                +" LEFT JOIN StaticDataDetail SD1 "
                +"   ON SD1.ROWID=JF.JOB_FEEDBACK_CAT"
                + " WHERE  JF.OJT_ID="+filterstring(GetOJTId().ToString());
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "HrOJTList.aspx?OJT_ID="+GetOJTId()+"&sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                for (int i = 0; i <= arr_cnt; i++)
                {
                    ...
                    switch (matgrid.GetValue(i, 3).ToString())
                    {
                        ...
                        case "J":
                            //html.Append("<td align=\"left\"> <input  textmode=\"multiline\" rows=\"3\" cols=\"5\" value= " + dr[matgrid.GetValue(i, 0).ToString()].ToString() + "\"> </td>");
                            html.Append("<td align=\"right\" width='300'><div class ='scroll-div' > " + dr[matgrid.GetValue(i, 0).ToString()].ToString() + "</div>");
                            html.Append("</td>");
                            break;
                        default:
                ...
                //html.Append("<td><a href=\"/OnTheJobTraining/JobFeedBackDetails.aspx?OJT_ID=" + dr["OJT_ID"] + "&JOB_GROUP_ID=" + dr["JOB_GROUP_ID"] + "&TraineeId=" + dr["empId"] + "\">FeedBack</a></td>");
