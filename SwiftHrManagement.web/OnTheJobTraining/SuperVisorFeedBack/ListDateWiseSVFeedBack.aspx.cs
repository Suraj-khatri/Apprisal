...
{
    public partial class ListDateWiseSVFeedBack : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"dbo.GetEmployeeFullNameOfId(JF.FEEDBACK_BY)",      "FeedBack By",               "LT"},
            ...
            String[,] matgrid = new String[,]{ 
                {"TRAINING_TYPE",        "Training Type",                   "200",      "T"},        
                {"FEEDBACK_BY",           "FeedBack By",                 "200",      "T"},
            ...
            string sortby = "JFBM_ID";
            string table_name = "JobFeedBackMaster";

            String sql = "";

            sql = " SELECT JF.JFBM_ID,JF.OJT_ID,JF.JOB_GROUP_ID, dbo.GetEmployeeFullNameOfId(JF.FEEDBACK_BY) [FEEDBACK_BY],JF.FROM_DATE,JF.TO_DATE,CASE  OJ.TRN_TYPE WHEN 'o' THEN 'OJT'"
                  + " WHEN 'b' THEN 'BUDDY' END [TRAINING_TYPE] "
                  + " FROM JobFeedBackMaster JF"
                  +" INNER JOIN OJTDETAILS OJ ON JF.OJT_ID = OJ.OJT_ID"
                  + " WHERE JF.FEEDBACK_BY = "+filterstring(ReadSession().Emp_Id.ToString());
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/OnTheJobTraining/JobFeedBackDetails.aspx?OJT_ID=" + dr["OJT_ID"] + "&JOB_GROUP_ID=" + dr["JOB_GROUP_ID"] + "&JFBM_ID=" + dr["JFBM_ID"] + "\">View</a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/OnTheJobTraining/SuperVisorFeedBack/ManageMultipleFD.aspx?JOB_GROUP_ID="+GetJobGroupId()+"&OJT_ID="+GetOJTID());
