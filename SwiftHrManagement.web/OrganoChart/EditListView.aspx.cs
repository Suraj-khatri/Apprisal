...
{
    public partial class EditListView : BasePage
    {
        clsDAO _clsdao = null;
        public EditListView()
        {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"unit_name",           "Unit Name",         "200",      "LT"},
                        {"unit_desc",           "Unit Descreiption", "200",      "LT"},
                        {"IsLeaf",              "Is Leaf Node",      "200",      "LT"}
                        };



            string sortby = "unit_name";
            string table_name = "OrgChart";

            String sql = "select id, unit_name, unit_desc, case when isleaf ='True' then 'Yes' when isleaf = 'False' then 'No' end 'IsLeaf' from OrgChart where 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/OrganoChart/Manage.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/OrganoChart/Manage.aspx?");
