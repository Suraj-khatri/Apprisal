    ...
    public partial class List : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                if (Getflag() == 0)
                {
                    _clsDao.runSQL("exec [proc_OverTimeDetails] @flag='d',@OtRequest_id=" + filterstring(GetId().ToString()));

                }

            }
            init(ReadSession().Emp_Id);
        }

        private long Getflag()
        {
            return (Request.QueryString["deleteflag"] != null ? long.Parse(Request.QueryString["deleteflag"].ToString()) : 0);
        }
        private long GetId()
        {
        ...
        private void init(long EmpId)
        {
            ...
            String[,] matfilter = new String[,]{
                            {"requested_date", "Reqested Date", "ED"}
                            };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"requested_by",        "Requested By",     "200",      "T"},
                        {"requested_with",      "Approved By",      "200",      "T"},
                        {"requested_date",      "Date",             "200",      "T"},
                        {"req_type",            "Req. Type",        "200",      "T"},
                        {"hardship_type",       "Hardship Type",    "200",      "T"},
                        {"request_in_time",     "Time In",          "200",      "D"},
                        {"request_out_time",    "Time Out",        "200",      "D"},
                        {"status",              "Status",           "200",      "T"},
                        {"hr_approve_date",     "HR Approve Date",  "100",      "T"},
                        {"supervisor_approved_remark",  "Sup. Remarks",     "200",      "T"},
                        {"hr_paid_remark",              "HR Remarks",       "200",      "T"}
                        };
            ...
            sql = @"SELECT OtRequest_id
                            ,dbo.GetEmployeeFullNameOfId(requested_by) [requested_by]
                            ,dbo.GetEmployeeFullNameOfId(requested_with) as requested_with
                            ,Convert(varchar,requested_date,101) [requested_date]
                            ...
                            ,request_in_time
                            ,request_out_time
                            ,case status when 'p' then 'pending' when 's' then 'SuperVisor Approved' when 'a' then 'Hr Approved' end as status
                            ,Convert(varchar,hr_approve_date,101) [hr_approve_date]
                            ,is_holiday
                            ,o.created_by
                            ,supervisor_approved_remark
                            ,hr_paid_remark
                        FROM OverTimeDetails as o 
                        inner join FiscalYear F on cast((o.requested_date)as Date)>=f.EN_YEAR_START_DATE 
                        WHERE REQUESTED_BY=" + EmpId + " and f.FLAG='1'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Delete</th>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (dr["status"].ToString() == "SuperVisor Approved" || dr["status"].ToString() == "Hr Approved")
                {
                    html.Append("<td><a>N/A</a> </td>");
                }
                else
                {
                    html.Append("<td><a onclick=\"return confirm('Are You Sure To Delete?');\" href=\"/OverTime/List.aspx?Id=" + dr["OtRequest_id"] + "&deleteflag=0\">delete</a></td>");
                }
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/OverTime/ManageRequestingOverTime.aspx?Status=r");
        ...
        private string make_filter_form(string table_name, string[,] matfilter)
        {
            ...
            for (int i = 0; i <= arr_cnt; i++)
            {
                ...
                if (Request.Cookies[table_name + "sch" + ctlName] != null)
                    defValue = Request.Cookies[table_name + "sch" + ctlName].Value.ToString();

                if (Request.Form[ctlName] != null)
                    defValue = Request.Form[ctlName].ToString();

                Response.Cookies[table_name + "sch" + ctlName].Value = defValue;
