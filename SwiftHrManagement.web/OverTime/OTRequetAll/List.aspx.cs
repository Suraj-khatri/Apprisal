...
namespace SwiftHrManagement.web.OverTime.OTRequetAll
{
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            sql = @"SELECT OtRequest_id,e.EMP_CODE,dbo.GetEmployeeFullNameOfId(requested_by) as requested_by
                    ,dbo.GetEmployeeFullNameOfId(requested_with) as requested_with 
                    ,Convert(varchar,requested_date,101) as requested_date,request_in_time,request_out_time,
                    case  status when 'p' then 'pending' when 's' then 'SuperVisor Approved' when 'a' then 'Hr Approved' end as status
                    ,Convert(varchar,hr_approve_date,101) as hr_approve_date,is_holiday,ot.created_by
                    ,case when requesting_type='650' then 'OT' when requesting_type='1453' then 'Hardship' end req_type 
                    FROM OverTimeDetails ot 
                    inner join employee e on e.EMPLOYEE_ID = ot.requested_by                     
                    WHERE 1=1";
...
//                    FROM OverTimeDetails ot 
//                    inner join employee e on e.EMPLOYEE_ID = ot.requested_by 
//                    inner join FiscalYear F on cast((ot.requested_date)as Date)>=f.EN_YEAR_START_DATE 
//                    WHERE f.FLAG='1' and 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                {
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Delete\" onclick=\"return confirm('Are You Sure To Delete?');\" href=\"/OverTime/OTRequetAll/List.aspx?Id=" + dr["OtRequest_id"] + "&deleteflag=0\"><i class=\"fa fa-times\"></i></a></td>");
                }
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/OverTime/OTRequetAll/Manage.aspx");
