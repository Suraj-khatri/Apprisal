    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        {"no_of_request",            "No. of Request",             "200",      "T"},
                        {"requested_by",            "Requested By",             "200",      "T"},
                         {"requesting_type",            "Requested Type",             "200",      "T"} 
                        //{"request_in_time",            "Login Date & Time",             "200",      "D"},
                        //{"request_out_time",          "Logout Date & Time",        "200",      "D"},
                        //{"status",                  "Status",     "200",      "T"}
                        };



            string sortby = "requested_id ";
            ...
            sql = "select SUM(no_of_request) as [no_of_request],ot.requested_by,ot.requested_id,ot.requesting_type,ot.reqType_id from"
               + "  ( "
               + " select COUNT(requested_by) [no_of_request],"
               + " dbo.GetEmployeeFullNameOfId(requested_by) [requested_by],requested_by [requested_id],"
			   + "case when requesting_type='650' then 'OT' when requesting_type='1453' then 'Hardship' end requesting_type,"
			   + " requesting_type [reqType_id]"
               + "  from OverTimeDetails  "
               + "  where  approved_date IS NOT NULL and paid_date IS NULL"
               + " group by OtRequest_id,requested_by,requested_by,requesting_type,requesting_type"
               + " ) as ot WHERE 1=1";
            ...
            sql = sql +  "group by requested_by, requested_id,requesting_type,reqType_id";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/OverTime/OtApproveHr/ListDetails.aspx?Req_Id=" + dr["requested_id"] + "&RType=" + dr["reqType_id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
