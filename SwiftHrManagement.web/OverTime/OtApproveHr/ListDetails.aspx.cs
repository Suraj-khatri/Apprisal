using System;
using System.Data;
using System.Text;
using SwiftHrManagement.DAL.Role;

namespace SwiftHrManagement.web.OverTime.OtApproveHr
{
    public partial class ListDetails : BasePage
    {
          RoleMenuDAOInv _roleMenuDao = null;
          clsDAO _clsDao = null;
          public ListDetails()
        {
            this._roleMenuDao = new RoleMenuDAOInv();
            this._clsDao = new clsDAO();
        }
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_roleMenuDao.hasAccess(ReadSession().AdminId, 103) == false)
                {
                    Response.Redirect("/Error.aspx");
                }
            }
            init();
        }
         private long GetRtype()
        {
            return (Request.QueryString["RType"] != null ? long.Parse(Request.QueryString["RType"].ToString()) : 0);
        }

        private long GetReqId()
        {
            return (Request.QueryString["Req_Id"] != null ? long.Parse(Request.QueryString["Req_Id"].ToString()) : 0);
        }

        string MakeDDL(string defValue, string keyValue, ref DataTable dt)
        {
            var html = new StringBuilder("");
            html.Append("<select id = \"rate_type_" + keyValue + "\" name = \"rate_type_" + keyValue + "\">");
            foreach (DataRow dr in dt.Rows)
            {
                html.Append("<option value = \"" + dr["ot_rate_id"].ToString() + "\"" + autoSelect(dr["ot_rate_id"].ToString(), defValue) + ">" + dr["description"].ToString() + "</option>");                          
            }
            html.Append("</select>");
            return html.ToString();
        }

        #region SwiftGrid
        private string autoSelect(string str1, string str2)
        {
            if (str1 == str2)
                return "selected=\"selected\"";
            else
                return "";

        }
        protected void btnHidden_Click(object sender, EventArgs e)
        {
            init();

        }
        private void init()
        {
            /************************* editable vairable starts***************************************/


            //column type : LT = like text , ET = Equal to text, EN= equal to numbert,ED=equal to date
            String[,] matfilter = new String[,]{  
                      
                        {"requested_date",       "Date",                    "ED"},
                        {"approved_date",       "Approved Date",             "ED"},                   
                    };

            if (GetRtype().ToString() == "650")
            {
                //column type : T = text , M = Money, D = date , I = image

                String[,] matgrid = new String[,]{
                        {"Request_Name",        "Requested By",         "200",          "T"},
                        {"req_type",                    "Req. Type",        "200",      "T"},
                        {"Ot_Request_date",             "Date",             "200",      "T"},
                        {"Att_In",              "Att Login",            "200",          "D"},
                        {"Att_Out",             "Att Logout",           "200",          "D"},
                        {"from",                "OT From",              "200",          "T"},
                        {"to",                  "OT To",                "200",          "T"}, 
                        {"approved_by",                 "Approved By",      "200",      "T"},
                        {"requested_time",              "Req HRs",          "200",      "O"},
                        {"SuperVisor_Approved_Period",  "Sup. App. HRs",    "200",      "O"}
                      
                        };



                string sortby = "OtRequest_id ";
                string table_name = "OverTimeDetails";

                String sql = "";



                sql = @"SELECT OtRequest_id
			            ,dbo.GetEmployeeFullNameOfId(requested_by) [Request_Name]
			            ,convert(varchar,requested_date,101) [Ot_Request_date]
			            ,convert(varchar,approved_date,101) [approved_date]
			            ,rate_type_status,dbo.GetEmployeeFullNameOfId(ot.approved_by) [approved_by]
			            ,cast(datediff(minute, request_in_time,request_out_time )/60 as varchar(5)) + ':' + 
                                RIGHT('0' + cast(datediff(minute, request_in_time, request_out_time )%60 as varchar(2)), 2) [requested_time]
			            ,REPLACE(supervisor_approv_time,':00.00','') as [SuperVisor_Approved_Period]
			            ,request_in_time [from]
			            ,request_out_time [to]
			            ,CAST(ISNULL(ot_amount, 0) AS DECIMAL(38, 2))  ot_amount
			            ,case when requesting_type='650' then 'OT' when requesting_type='1453' then 'Hardship' end req_type
                        ,status
                        ,hr_paid_remark
			            ,convert(varchar,att.login_time,108) [Att_In]
			            ,convert(varchar,logout_time,108) [Att_Out]
		            FROM OverTimeDetails ot
					INNER JOIN atttendance att on ot.requested_by = att.emp_id
					WHERE requested_by=" + GetReqId() + " AND approved_date IS NOT NULL AND paid_date IS NULL AND requesting_type=" + GetRtype() +
                        "AND convert(varchar,att.att_date,101) = CONVERT(varchar,ot.requested_date,101) AND ot.status!='x'";

                /************************* editable vairable ends***************************************/
                string _page_size = "10";
                string sortd = "desc";
                int _page = 1;

                if (Request.Form["hdd_curr_page"] != null)
                    _page = Convert.ToInt32(Request.Form["hdd_curr_page"]);

                if (Request.Cookies["page_size"] != null)
                    _page_size = Request.Cookies["page_size"].Value;

                if (Request.Form["ddl_per_page"] != null)
                    _page_size = Request.Form["ddl_per_page"];

                Response.Cookies["page_size"].Value = _page_size;
                int page_size = Convert.ToUInt16(_page_size);


                if (Request.QueryString["sortd"] != null)
                    sortd = Request.QueryString["sortd"];

                if (Request.QueryString["sortby"] != null)
                    sortby = Request.QueryString["sortby"];

                sql = sql + get_filter_sql(table_name, matfilter);

                rpt.InnerHtml = "<center>" + load_report(table_name, sql, matgrid, matfilter, sortd, sortby, _page, page_size) + "</center>";
            }
            else
            {
                String[,] matgrid = new String[,]{
                        {"Request_Name",        "Requested By",         "200",      "T"},                       
                        {"Ot_Request_date",     "Date",                 "200",      "T"},
                        {"login_time",          "Att Login",            "200",      "T"},
                        {"logout_time",         "Att Logout",           "200",      "T"},
                        {"day",                 "Day",                  "200",      "T"},
                        {"Day_Type",            "Day Type",             "200",      "T"},
                        {"req_type",            "Req. Type",            "200",      "T"},
                        {"hardship_type",       "Hardship Type",        "200",      "T"}, 
                        {"approved_by",         "Approved By",          "200",      "T"}                      
                        };



                string sortby = "OtRequest_id ";
                string table_name = "OverTimeDetails";

                String sql = "";

                sql= @"SELECT OtRequest_id
			            ,dbo.GetEmployeeFullNameOfId(requested_by) [Request_Name]
			            ,convert(varchar,requested_date,101) [Ot_Request_date]
			            ,att.login_time [login_time]
			            ,att.logout_time [logout_time]
			            ,DATENAME(DW,ot.requested_date) [day]
			            ,[Day_Type]=case when exists(select * from OfficialCalendar where convert(VARCHAR,DATE,101)= ot.requested_date and TYPE='H') 
									or  (select datename(DW,ot.requested_date))='Saturday'
									then 'Holiday' else 'Working Day' end 
			            ,convert(varchar,approved_date,101) [approved_date]
                        ,CAST(ISNULL(ot_amount, 0) AS DECIMAL(38, 2))  ot_amount
			            ,case when requesting_type='650' then 'OT' when requesting_type='1453' then 'Hardship' end req_type
                        ,status
                        ,hr_paid_remark,dbo.GetEmployeeFullNameOfId(ot.approved_by) [approved_by]
                        ,case when requesting_type='650' then 'OT' when requesting_type='1453' then ISNULL(dbo.GetDetailTitle(head_id),'Hardship') end as hardship_type
		            FROM OverTimeDetails ot
					INNER JOIN atttendance att on ot.requested_by = att.emp_id
					INNER JOIN employee e on ot.requested_by=e.EMPLOYEE_ID 
					WHERE requested_by=" + GetReqId() + " AND approved_date IS NOT NULL AND paid_date IS NULL AND requesting_type=" + GetRtype() +
                       "AND convert(varchar,att.att_date,101) = CONVERT(varchar,ot.requested_date,101) AND ot.status!='x'";


                /************************* editable vairable ends***************************************/
                string _page_size = "10";
                string sortd = "desc";
                int _page = 1;

                if (Request.Form["hdd_curr_page"] != null)
                    _page = Convert.ToInt32(Request.Form["hdd_curr_page"]);

                if (Request.Cookies["page_size"] != null)
                    _page_size = Request.Cookies["page_size"].Value;

                if (Request.Form["ddl_per_page"] != null)
                    _page_size = Request.Form["ddl_per_page"];
                Response.Cookies["page_size"].Value = _page_size;


                int page_size = Convert.ToUInt16(_page_size);


                if (Request.QueryString["sortd"] != null)
                    sortd = Request.QueryString["sortd"];

                if (Request.QueryString["sortby"] != null)
                    sortby = Request.QueryString["sortby"];

                sql = sql + get_filter_sql(table_name, matfilter);

                rpt.InnerHtml = "<center>" + load_report(table_name, sql, matgrid, matfilter, sortd, sortby, _page, page_size) + "</center>";
 
            }

        }
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            clsDAO db = new clsDAO();

            String sql1 = "select * from (select ROW_NUMBER() OVER (ORDER BY " + sortby + " " + sortd + " ) as rowId,* from(" + sql + " ) as aa) as tmp where 1 = 1 ";
            sql1 += " and  tmp.rowId between " + ((_page - 1) * page_size + 1).ToString() + " and " + (_page * page_size).ToString();

            sortd = (sortd == "asc" ? "desc" : "asc");

            string sqlCount = "select count(*) as totalRecord from (" + sql + ") as tmp;";


            string thisPage = "List.aspx?sortd=" + sortd + "&sortby={sortby}";


            DataSet ds = db.getDataset(sqlCount + sql1);

            DataTable dtTotal = ds.Tables[0];
            int total_record = Convert.ToInt16(dtTotal.Rows[0]["totalRecord"].ToString());
            int remainder = total_record % page_size;
            int total_page = (total_record - remainder) / page_size;

            if (remainder > 0)
                total_page++;

            DataTable dt = ds.Tables[1];

            //get_dt(matgrid);
            int arr_cnt = matgrid.GetUpperBound(0);

            var dtRate = db.getDataset("SELECT ot_rate_id, description FROM OtRateSetUp").Tables[0];

            StringBuilder html = new StringBuilder("<div class=\"table-responsive\"> <table class=\"table table-bordered table-striped table-condensed\">");

            html.Append("<tr>");

            for (int i = 0; i <= arr_cnt; i++)
            {
                string sortLink = "<a href=\"" + thisPage.Replace("{sortby}", matgrid.GetValue(i, 0).ToString()) + "\" class=\"GridHeader\" >";

                html.Append("<th nowrap Class=\"HeaderStyle\" align=\"left\">" + sortLink + matgrid.GetValue(i, 1).ToString() + "</a></th>");
            }
            if (GetRtype().ToString() == "650")
            {
                html.Append("<th Class=\"HeaderStyle\" align=\"left\">HR app. Hrs</th>");
                html.Append("<th Class=\"HeaderStyle\" align=\"left\">Rate Decription</th>");
                html.Append("<th Class=\"HeaderStyle\" align=\"left\">Amount</th>");
                html.Append("<th Class=\"HeaderStyle\" align=\"left\">Approve</th>");
                html.Append("<th Class=\"HeaderStyle\" align=\"left\">Reject</th>");
            }
            else
            {
                html.Append("<th Class=\"HeaderStyle\" align=\"left\">Amount</th>");
                html.Append("<th Class=\"HeaderStyle\" align=\"left\">Remarks</th>");
                html.Append("<th Class=\"HeaderStyle\" align=\"left\">Approve</th>");
                html.Append("<th Class=\"HeaderStyle\" align=\"left\">Reject</th>");
            }
            html.Append("</tr>");

            int cnt = 0;
            double totalOTAmount = 0;
            foreach (DataRow dr in dt.Rows)
            {
                cnt++;

                if (cnt % 2 == 0)
                {
                    html.Append("<tr class=\"GridOddRow\" onMouseOver=\"this.className='GridOddRowOver'\" onMouseOut=\"this.className='GridOddRow'\" >");
                }
                else
                {
                    html.Append("<tr class=\"GridEvenRow\" onMouseOver=\"this.className='GridEvenRowOver'\" onMouseOut=\"this.className='GridEvenRow'\">");
                }

                for (int i = 0; i <= arr_cnt; i++)
                {

                    switch (matgrid.GetValue(i, 3).ToString())
                    {
                        case "M":
                            html.Append("<td align=\"right\">" + dr[matgrid.GetValue(i, 0).ToString()] + "</td>");
                            break;
                        case "D":
                            html.Append("<td align=\"center\">" + dr[matgrid.GetValue(i, 0).ToString()] + "</td>");
                            break;
                        case "O":
                            html.Append("<td align=\"left\" nowrap=\"nowrap\"><input class=\"form-control\"  type=\"text\" readonly=\"readonly\" maxlength=\"5\" size=\"5\" name=\"ReqTime_" + dr["OtRequest_id"] + "\" id=\"ReqTime_" + dr["OtRequest_id"] + "\" value=" + dr[matgrid.GetValue(i, 0).ToString()].ToString() + ">"
                             + " </span> </td>");
                            break;
                        default:
                            html.Append("<td align=\"left\">" + dr[matgrid.GetValue(i, 0).ToString()] + "</td>");
                            break;
                    }
                }
                if (GetRtype().ToString() == "1453") //Hardship
                {
                    html.Append("<td align=\"right\" nowrap><span id = \"spn_" + dr["OtRequest_id"] + "\"><input class=\"form-control\" readonly = \"readonly\" type=\"text\" maxlength=\"5\" size=\"7\" name=\"amount\" id=\"amount_" + dr["OtRequest_id"] + "\" value = \"" + dr["ot_amount"] + "\"></span>");
                    html.Append("</td>");
                    html.Append("<td align=\"left\" nowrap=\"nowrap\"><input class=\"form-control\" type=\"textarea\" cols=\"20\" rows=\"2\" value=\"" + dr["hr_paid_remark"] + "\" name=\"txtarea_" + dr["OtRequest_id"] + "\" id=\"txtarea_" + dr["OtRequest_id"] + "\"> </td>");
                    html.Append("<td align=\"right\" id='TDA_" + dr["OtRequest_id"] + "' nowrap><span style='color:blue;cursor:pointer' onclick='ApprovedHardship(" + dr["OtRequest_id"] + "," + GetReqId() + ")'>Approve</span>");
                    html.Append("<td align=\"right\" id='TDR_" + dr["OtRequest_id"] + "' nowrap><span style='color:blue;cursor:pointer' onclick='RejectHardship(" + dr["OtRequest_id"] + "," + GetReqId() + ")'>Reject</span>");
                    html.Append("</tr>");
                    totalOTAmount = totalOTAmount + Convert.ToDouble(dr["ot_amount"].ToString());
                }
                else
                {
                    html.Append("<td align=\"left\" nowrap=\"nowrap\"><input class=\"form-control\" type=\"text\" maxlength=\"5\" size=\"3\" name=\"Reqhrs_" + dr["OtRequest_id"] + "\" id=\"Reqhrs_" + dr["OtRequest_id"] + "\"></td>");
                    html.Append("<td align=\"right\" nowrap>" + MakeDDL(dr["rate_type_status"].ToString(), dr["OtRequest_id"].ToString(), ref dtRate));
                    html.Append("</td>");
                    html.Append("<td align=\"right\" nowrap><span id = \"spn_" + dr["OtRequest_id"] + "\"><input readonly = \"readonly\" type=\"text\" maxlength=\"5\" size=\"7\" name=\"amount\" id=\"amount_" + dr["OtRequest_id"] + "\" value = \"" + dr["ot_amount"] + "\"></span>");
                    html.Append("</td>");
                    html.Append("<td align=\"left\" nowrap=\"nowrap\"><input class=\"form-control\" type=\"textarea\" cols=\"20\" rows=\"2\" value=\"" + dr["hr_paid_remark"] + "\" name=\"txtarea_" + dr["OtRequest_id"] + "\" id=\"txtarea_" + dr["OtRequest_id"] + "\"> </td>");
                    html.Append("<td align=\"right\" nowrap><span style='color:blue;cursor:pointer' onclick='UpdateApprovedPeriod(" + dr["OtRequest_id"] + "," + GetReqId() + ")'>Approve</span>");
                    html.Append("<td align=\"right\" nowrap><span style='color:blue;cursor:pointer' onclick='RejectHardship(" + dr["OtRequest_id"] + "," + GetReqId() + ")'>Reject</span>");                    
                    html.Append("</tr>");
                    totalOTAmount = totalOTAmount + Convert.ToDouble(dr["ot_amount"].ToString());
                }
            }

            html.Append("<tr>");
            html.Append("<td colspan ='9' align=\"right\"><b>Total</b></td>");
            html.Append("<td  align=\"left\"><input class=\"form-control\" size=\"7\" readonly = \"readonly\" type=\"text\" name=\"totalAmount\" id=\"totalAmount\" value = \"" + totalOTAmount.ToString() + "\"></td>");
            html.Append("</tr>");
            html.Append("<tr> <td align=\"center\" colspan=\"14\"> <strong> Page " + _page.ToString() + " Of " + total_page + "</strong></td></tr>");
            html.Append("</table>");
            html.Append("</div>");


            string filter_form = make_filter_form(table_name, matfilter);
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/OverTime/ManageRequestingOverTime.aspx");


            return filter_form + navigation_bar + html.ToString();
        }
        private string make_filter_form(string table_name, string[,] matfilter)
        {
            StringBuilder html = new StringBuilder("<div class=\"col-md-12 filter-bg\">");

            html.Append("<div class=\"pull-right\" ><a class=\"btn btn-primary  btn-md\" align=\"center\" href=\"javascript:newTableToggle('td_Search', 'img_Search');\"><i class=\"fa fa-filter\" id=\"img_Search\"></i> </a></div>");
            html.Append("<div class=\"clear-fix\"></div>");
            html.Append("<div id=\"td_Search\" style=\"display:none; width:100%; float:left;\" align=\"left\">");

            int arr_cnt = matfilter.GetUpperBound(0);
            string ctlName = "";
            string defValue = "";
            for (int i = 0; i <= arr_cnt; i++)
            {
                html.Append("<div class=\" col-md-3\">");
                html.Append("<div class=\" form-group\">");
                html.Append("<label align=\"left\">" + matfilter.GetValue(i, 1).ToString() + "</label>");

                ctlName = "sch_" + matfilter.GetValue(i, 0).ToString();

                if (Request.Cookies[table_name + "_sch_" + ctlName] != null)
                    defValue = Request.Cookies[table_name + "_sch_" + ctlName].Value.ToString();

                if (Request.Form[ctlName] != null)
                    defValue = Request.Form[ctlName].ToString();

                Response.Cookies[table_name + "_sch_" + ctlName].Value = defValue;

                html.Append("<input type=\"text\" name=\"" + ctlName + "\" value=\"" + defValue + "\" class=\"form-control\">");
                html.Append("</div>");
                html.Append("</div>");
            }
            html.Append("<div class=\" col-md-3\">");
            html.Append("<div><input type=\"button\" value=\"Filter\" class=\"btn-filter btn btn-success\" onclick=\"submit_form();\"></div>");
            html.Append("</div>");
            html.Append("</div>");
            html.Append("</div>");

            return html.ToString();
        }
        private string get_filter_sql(String table_name, String[,] matfilter)
        {
            int arr_cnt = matfilter.GetUpperBound(0);
            string query = "";
            string ctlName = "";
            string defValue = "";

            for (int i = 0; i <= arr_cnt; i++)
            {
                ctlName = "sch_" + matfilter.GetValue(i, 0).ToString();


                if (Request.Cookies[table_name + "_sch_" + ctlName] != null)
                    defValue = Request.Cookies[table_name + "_sch_" + ctlName].Value.ToString();

                if (Request.Form[ctlName] != null)
                    defValue = Request.Form[ctlName].ToString();

                if (defValue.Trim() != "")
                {
                    switch (matfilter.GetValue(i, 2).ToString().ToUpper())
                    {
                        case "LT":
                            query = query + " and " + matfilter.GetValue(i, 0).ToString() + " LIKE '%" + defValue + "%'";
                            break;

                        case "ET":
                            query = query + " and " + matfilter.GetValue(i, 0).ToString() + " = '" + defValue + "'";
                            break;

                        case "EN":
                            query = query + " and " + matfilter.GetValue(i, 0).ToString() + " = " + defValue;
                            break;

                        case "ED":
                            query = query + " and CONVERT(varchar, " + matfilter.GetValue(i, 0).ToString() + ",101) = CONVERT(varchar,CAST( " + filterstring(defValue) + "AS DATETIME),101)";
                            break;

                    }
                }
            }
            return query;
        }
        private string get_paging_block(int _total_record, int _page, int _page_size, Boolean _show_add_button, string _add_page)
        {

            StringBuilder html = new StringBuilder("<input type = \"hidden\" name=\"hdd_curr_page\" id = \"hdd_curr_page\" value=\"" + _page.ToString() + "\">");
            html.Append("<div class=\"col-md-12 result-bg\">");
            html.Append("<div class=\"row\">");
            html.Append("<div class=\"col-md-9 text-left\">");
            html.Append("<div class=\"form-inline\">");
            html.Append("<div class=\"form-group\">Result");
            html.Append("</div>");
            html.Append("<div class=\"form-group\"><b>" + _total_record.ToString() + "</b>");
            html.Append("</div>");
            html.Append("<div class=\"form-group\">");
            html.Append("<select class=\"form-control\"  aria-describedby=\"sizing-addon3\" name=\"ddl_per_page\" onChange=\"submit_form();\">");
            html.Append("<option value=\"10\"" + autoSelect("10", _page_size.ToString()) + ">10</option>");
            html.Append("<option value=\"20\"" + autoSelect("20", _page_size.ToString()) + ">20</option>");
            html.Append("<option value=\"30\"" + autoSelect("30", _page_size.ToString()) + ">30</option>");
            html.Append("<option value=\"40\"" + autoSelect("40", _page_size.ToString()) + ">40</option>");
            html.Append("<option value=\"50\"" + autoSelect("50", _page_size.ToString()) + ">50</option>");
            html.Append("</select>");
            html.Append("</div>");
            html.Append("<div class=\"form-group\"> Per Page");
            html.Append("</div>");
            html.Append("</div>");
            html.Append("</div>");
            html.Append("<div class=\"col-md-3 text-right\">");

            if (_page > 1)
                html.Append("<a class=\"tool btn btn-default\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Previous\"  href=\"JavaScript:nav('" + (_page - 1).ToString() + "');\" title='Previous page'><i class=\"fa fa-arrow-left\"></i></a>");
            else
                html.Append("<span class=\"btn btn-default\"> <i class=\"fa fa-arrow-left\" aria-hidden=\"true\"></i></span>");

            if (_page * _page_size < _total_record)
                html.Append("<a class=\"tool btn btn-default\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Next\" href=\"JavaScript:nav('" + (_page + 1).ToString() + "');\" title='Next page'><i class=\"fa fa-arrow-right\"></i></a>");
            else
                html.Append("<span class=\"btn btn-default\"><i class=\"fa fa-arrow-right\" aria-hidden=\"true\"></i></span>");

            html.Append("</div>");
            html.Append("</div></div>");

            return html.ToString();
        }
        #endregion

        protected void BtnApprove_Click(object sender, EventArgs e)
        {
            FinalAprovHR();
            Response.Redirect(Request.RawUrl);
        }

        private void FinalAprovHR()
        {
            _clsDao.runSQL("Exec [proc_OverTimeDetails] @flag='fa',@EMPLOYEEID=" + filterstring(GetReqId().ToString()) + ""
            + ",@paid_by=" + filterstring(ReadSession().Emp_Id.ToString()) + ",@sessionId=" + filterstring(ReadSession().Sessionid) + "");

        }
    }
}
