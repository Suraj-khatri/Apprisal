...
{
    public partial class ListOperation : BasePage
    {
        ...
        {
            string ss = Request.QueryString["PositionId"];
            if (ss != "")
        ...
        private long GetHeadId()
        {
           return ReadNumericDataFromQueryString("headId");
        }
        private long GetRangeFrom()
        {
            return ReadNumericDataFromQueryString("rangeFrom");
        }
        private long GetRangeTo()
        {
            return ReadNumericDataFromQueryString("rangeTo");
        }
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{ 
                    ...
                    {"Position",                   "Position",          "200",      "T"},
                    {"EMP_CODE",                   "Emp Code",          "200",      "T"},
                    {"EmpName",                    "Employee",          "200",      "T"},
                    {"BENEFIT_ID",                 "Head",              "200",      "T"},
                    {"AMOUNT",                     "Value",             "200",      "T"},
                        };
            ...
            String sql = "";
            sql = "SELECT x.BRANCH_NAME,x.EMP_CODE,x.EmpName,x.Position,sd.DETAIL_TITLE BENEFIT_ID,p.AMOUNT,p.CREATED_DATE FROM "
                    +" (SELECT EMP_CODE,EMPLOYEE_ID,FIRST_NAME +' '+MIDDLE_NAME +' '+ LAST_NAME EmpName,sd.DETAIL_TITLE Position,b.BRANCH_NAME "
                    +" FROM Employee e INNER JOIN StaticDataDetail sd ON e.POSITION_ID = sd.ROWID INNER JOIN Branches b "
                    +"	ON b.BRANCH_ID = e.BRANCH_ID WHERE e.BRANCH_ID = "+GetBranchId()+" AND POSITION_ID = "+GetPositionId()+""
                    +"  )x"
                    +" LEFT JOIN Payable p ON p.EMPLOYEE_ID = x.EMPLOYEE_ID Left join StaticDataDetail sd on sd.ROWID = p.BENEFIT_ID"
                    + " where 1 = 1";
            ...
            if (GetEmpId() != 0)
            {
                ...
                sql = sql + "and p.EMPLOYEE_ID=" + GetEmpId();
            }
            if(GetHeadId() != 0)
            {
                sql = sql + "and p.BENEFIT_ID ="+GetHeadId()+" and p.AMOUNT between "+GetRangeFrom()+" and "+GetRangeTo()+"";
            }
