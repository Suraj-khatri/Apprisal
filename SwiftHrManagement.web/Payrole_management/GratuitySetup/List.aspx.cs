    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        {"Id",              "Id",           "200",       "N"},
                        {"Start_From",      "Start Year",   "200",       "T"},
                        {"Calculate_On",    "Calculate On", "200",      "T"},
                        {"Start_Year",      "Start Year",   "200",      "N"}, 
                        {"End_Year",        "End Year",     "200",      "N"},
                    };

            string sortby = "Id";
            string table_name = "GratuityMaster_Setup";

            String sql = "";

            sql = "SELECT Id,Case Start_From when 'jd' then 'Joined Date' when 'pd' then 'Permanent Date' when 'ad' then 'Appointment Date' When 'gd' Then 'Gratuity Effective Date' END Start_From, "
                + " case Calculate_On when '36' then 'Basic Salary' When '36,37' then 'Basic Salary and Grade' When '36,37,38' then 'Gross salary' end  Calculate_On "
                + " ,Start_Year,End_Year FROM GratuityMaster_Setup WHERE 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            //html.Append("<th Class=\"HeaderStyle\" align=\"left\">Gratuity Setup</th>");
            html.Append("<th class=\"text-center\" align=\"center\">Gratuity Setup</th>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Payrole_management/GratuitySetup/Manage.aspx?Id=" + dr["Id"] + "\"><i class=\"fa fa-eye\"></i></a>");
                // html.Append("<td><a href=\"/Payrole_management/GratuitySetup/Manage.aspx?Id=" + dr["Id"] + "\">View</a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, total_record > 0 ? false : true, "/Payrole_management/GratuitySetup/Manage.aspx");

            //return filter_form + navigation_bar + html.ToString();
            return navigation_bar + html.ToString();
        }
