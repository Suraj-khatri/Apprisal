...
{
    public partial class ListManualAddDed : BasePage
    {
        RoleMenuDAOInv _roleMenuDao = null;
        public ListManualAddDed()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_roleMenuDao.hasAccess(ReadSession().AdminId, 5) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        {"Name",             "Name",             "200",      "T"},
                        {"Code",            "Code",             "200",      "T"},
                        {"IsEnabled",       "IsEnabled",        "200",      "T"}
                        };



            string sortby = "Id";
            string table_name = "ManualBenifitDeduction_Head";

            String sql = "";

            sql = "SELECT	ManualEntryID 'Id',Name, AddDeduct 'Code',case when enable = 'True' then 'Yes' when [enable] = 'False' then 'No' end 'IsEnabled'"
		                    +" from ManualBenifitDeduction_Head where [enable]='1' ";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListManualAddDed.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/Payrole_management/ManageManualAddDed.aspx?Id=" + dr["Id"] + "\">View</a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Payrole_management/ManageManualAddDed.aspx");
        ...
        //{
        //    Response.Redirect("ManageManualAddDed.aspx");
        //}
    }
}
