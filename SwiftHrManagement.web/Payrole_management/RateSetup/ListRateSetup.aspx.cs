...
{
    public partial class ListRateSetup : BasePage
    {
        public ListRateSetup()
        {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        {"SNO",             "SNO",              "200",      "LT"},
                        {"Category_for",    "Category For",     "200",      ""},
                        {"category",        "Category",         "200",      ""},
                        {"Amount",          "Amount",           "200",      "M"},
                        };
            string sortby = "id";
            string table_name = "payratesetup";
            ...
            String sql = "select ROW_NUMBER() over (Order by ID) SNO,ID,dbo.GetDetailTitle(Category_for) Category_for,dbo.GetDetailTitle(category) category,dbo.showdecimal(Amount) Amount" +
                         " from payratesetup where flag='Y'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"ManageRateSetup.aspx?ID=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                //html.Append("<td><a href=\"ManageRateSetup.aspx?ID=" + dr["ID"] + "\">View</a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "ManageRateSetup.aspx");
