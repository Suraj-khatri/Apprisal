    ...
    public partial class MemberList : System.Web.UI.Page
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{   
                        {"dbo.GetEmployeeFullNameOfId(rcm.empId)",    "Emp Name ",         "LT"},                    
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"name",            "Member Name",             "200",      "T"},                                                                      
                        {"typ",             "Review type",             "200",      "T"},                        
                        {"FirstRater",    "First Rater",             "200",      "T"},                        
                        {"frmMem",          "Member From",             "200",      "T"},                        
                        {"ToMem",           "Member To",                "200",      "T"}                        
                        };



            string sortby = "ID";
            string table_name = "reviewCommitteeMembers";

            String sql = "";

            sql = @"select rcm.rowId as ID
                , ISNULL(ee.FIRST_NAME,' ')+ISNULL(' '+ee.MIDDLE_NAME,' ')+ isnull(ee.LAST_NAME,' ') as name  
                ,rcm.empId         
                ,arc.typeDescription as typ
                ,case when isFirstRater='Y' then 'Yes' else 'No' end   as FirstRater          
                ,convert(varchar(10),memberFrom,101) as frmMem
                ,convert(varchar(10),memberTo,101) as ToMem
                from reviewCommitteeMembers rcm                        
				        inner join employee ee on ee.EMPLOYEE_ID = rcm.empId
				        inner join appriasalReviewCommittee arc on arc.rowid = rcm.memberTypeId where memberTypeId=" + GetId() + " and isnull(isDelete,'N')<>'Y'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"  style=\"width:160px;\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\"href=\"/PerformanceAppraisal/AppprialCommitteeReview/AppraisalCommitteeReviewMembers.aspx?Id=" + GetId() + "&editId=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a>");
                html.Append("<a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View Schedule\" href=\"#\" onclick=\"deleteMembers('" + dr["ID"] + "')\"><i class=\"fa fa-clock-o\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "AppraisalCommitteeReviewMembers.aspx");
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            var result = _cls.GetSingleresult("EXEC proc_AppriasalReviewDetails @flag='deleteMem',@rowId='"+id+"'");
            if (result == "0")
            {
                GetStatic.AlertMessage(this, "Succesfully Deleted !!!");
                Response.Redirect("MemberList.aspx?Id="+GetId()+"");
                return;
            }
            else
            {
                GetStatic.AlertMessage(this, "Operation Terminates");
                Response.Redirect("MemberList.aspx?Id=" + GetId() + "");
                return;
            }
        }
