    ...
    {        
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        {"EMPLOYEENAME",            "Employee Name",             "200",      "T"},
                        {"TITLE",       "Appraisal Title",        "200",      "T"},
                        {"FEEDBACK_DETAILS",    "Feedback Details",     "200",      "T"},
                        {"CREATEDDATE",    "Feedback Date",     "200",      "D"},
                         {"CREATEDBY",    "Feedback By",     "200",      "T"}
                        };



            string sortby = "ID";
            string table_name = "APPRAISALFEEDBACK";

            String sql = "";

            sql = "SELECT F.ID, F.APPRAISAL_ID,A.TITLE,ISNULL(F.FEEDBACK_DETAILS,'') AS FEEDBACK_DETAILS,"
            +" ISNULL(CONVERT(VARCHAR,F.CREATEDDATE,107),'') AS CREATEDDATE ,ISNULL(F.CREATEDBY,'') AS CREATEDBY, "
            +" E.FIRST_NAME + ' ' + E.MIDDLE_NAME + ' ' + E.LAST_NAME AS EMPLOYEENAME FROM APPRAISALFEEDBACK F   LEFT JOIN APPRAISAL A ON "
            +" F.APPRAISAL_ID = A.ID  INNER JOIN EMPLOYEE E ON E.EMPLOYEE_ID = A.EMPLOYEE_ID WHERE F.APPRAISAL_ID = "+GetAppraisalId()+"";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "FeedbackList.aspx?sortd=" + sortd + "&sortby={sortby}&Id="+GetAppraisalId()+"&EmpId="+GetEmpId()+"";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/PerformanceAppraisal/AppraisalFeedback/FeedbackManage.aspx?Id=" + dr["ID"] + "&EmpId="+GetEmpId()+"\">View</a></td>");                
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/PerformanceAppraisal/AppraisalFeedback/FeedbackManage.aspx?AppId=" + GetAppraisalId() + "&EmpId="+GetEmpId()+"");
