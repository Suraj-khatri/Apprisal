...
{
    public partial class AppraisalProgress : System.Web.UI.Page
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        ...
                        {"AP.TITLE",    "Appraisal Title",         "LT"}                   
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"EmpName",             "Employee Name",        "300",      "T"},
                        {"FROM_DATE",           "Appraisal From",       "300",      "D"},
                        {"TO_DATE",             "Appraisal To",         "300",      "D"}, 
                        {"APPRAISEE_DATE",      "Appraisee",            "300",      "D"},
                        {"APPRAISER_DATE",      "Appraiser",            "300",      "D"},
                        {"REVIEWER_DATE",       "Reviewer",             "300",      "D"},
                        {"HRC_DATE",            "Reviewer Committee",         "300",      "D"},
                        {"CEO_DATE",            "HR",                  "300",      "D"},
                        {"Self_Deadline",       "Self DeadLine",        "300",      "D"},
                        {"Supervisor_Deadline",  "Supervisor DeadLine", "300",      "D"},
                        {"Reviewer_Deadline",    "Reviewer DeadLine",   "300",      "D"}
                        };



            string sortby = "ID";
            string table_name = "APPRAISALPROGRESS";

            String sql = "";

            sql = @"SELECT ID,dbo.GetEmployeeFullNameOfId(EMPLOYEE_ID)[EmpName],CONVERT(VARCHAR,FROM_DATE,107)FROM_DATE,CONVERT(VARCHAR,TO_DATE,107)TO_DATE
                        ,CONVERT(VARCHAR,APPRAISEE_DATE,107)APPRAISEE_DATE,CONVERT(VARCHAR,APPRAISER_DATE,107)APPRAISER_DATE,CONVERT(VARCHAR,REVIEWER_DATE,107)REVIEWER_DATE
                        ,CONVERT(VARCHAR,HRC_DATE,107)HRC_DATE,CONVERT(VARCHAR,CEO_DATE,107)CEO_DATE
                        , CONVERT(VARCHAR,asa.sf,107) [Self_Deadline]
                        ,CONVERT(VARCHAR,asa.s,107)   [Supervisor_Deadline]
                        ,CONVERT(VARCHAR,asa.r,107)   [Reviewer_Deadline]
                        FROM Appraisal a 
                        inner join
                        ( 
		                        select * from(
		                        select appraisal_id,days,SUPERVISOR_TYPE from appraisalSupervisorAssignment
                                
		                        )a
		                        pivot (
		                          max ([days]) for SUPERVISOR_TYPE IN([sf],[s],[r])
		                        )
		                        p
                        )asa 
                         
                         on a.ID = asa.appraisal_id
                        ";
