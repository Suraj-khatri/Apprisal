...
using SwiftHrManagement.web.DAL.CommonServices;
...
namespace SwiftHrManagement.web.PerformanceAppraisal.Details
{
    public partial class List : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_roleMenuDao.hasAccess(ReadSession().AdminId, 27) == false && _roleMenuDao.hasAccess(ReadSession().AdminId, 104) == false)
                {
        ...
        {
            Page.MasterPageFile = (GetEmpId() == 0 ? "~/SwiftHRManager.Master" : "~/ProjectMaster.Master");
        }
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"name",    "Employee First Name",         "LT"}               
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"NAME",                     "Employee Name",             "200",     "T"},
                        {"FROM_DATE",                "From Date",                   "",      "D"}, 
                        {"TO_DATE",                  "To Date",                     "",      "D"},
                        {"appraisee_task_date",      "Appraisee Task",              "",      "T"},
                        {"appraisee_date",           "Appraisee Rating",                   "",      "D"},
                        {"appraiser",                "Appraiser",                   "",      "D"},
                        {"Appraiser_Date",           "Appraiser Rating",                   "",      "D"},
                        {"appraiseer_task_date",     "Appraiser Comment",              "",      "D"},
                        {"reviewer",            "Reviewer",                    "",      "D"},
                        {"Reviewer_date",            "Review Date",                    "",      "D"},
                        {"appraisee_acpt_date",      "Acceptance by Appraisee",     "",      "D"},
                        {"HRC_Date",                 "Reviewer Commt.",             "",      "D"},                        
                        {"CEO_date",                 "HR",                          "",      "D"},
            ...
            string sortby = "ID";
            string table_name = "APPRAISAL";

            String sql = "";
            if (_commonServices.DoesHrUser(ReadSession().Emp_Id.ToString()))
            {
                sql = "select * from (SELECT AP.ID,AP.POSITION_ID,CONVERT(VARCHAR,AP.FROM_DATE,107) AS FROM_DATE,CONVERT(VARCHAR,AP.TO_DATE,107) AS TO_DATE,"
                                  + " E.FIRST_NAME + ' ' + E.MIDDLE_NAME + ' ' + E.LAST_NAME AS NAME, E.EMPLOYEE_ID, "
                                  + " CONVERT(VARCHAR,aTask.commentDate,107) appraisee_task_date,"
                                  + "dbo.GetEmployeeFullNameOfId (sup.SUPERVISOR) appraiser,"
                                  + "dbo.GetEmployeeFullNameOfId(rev.SUPERVISOR) reviewer,"
                                  + " CONVERT(VARCHAR,apAcpt.commentDate,107) appraisee_acpt_date,"
                                  + " CONVERT(VARCHAR,sTask.commentDate,107) appraiseer_task_date,"
                                  + " CONVERT(VARCHAR,AP.appraisee_date,107) appraisee_date,"
                                  + " CONVERT(VARCHAR,AP.Appraiser_Date,107) Appraiser_Date,"
                                  + " CONVERT(VARCHAR,AP.Reviewer_date,107) Reviewer_date, CONVERT(VARCHAR,AP.HRC_Date,107) HRC_Date,"
                                  + " CONVERT(VARCHAR,AP.CEO_date,107) CEO_date FROM APPRAISAL AS AP"                                  
                                  ...
                                  + ")m where appraisee_acpt_date is null and 1=1 ";
            }
            else
            {
                sql = "SELECT AP.ID,AP.POSITION_ID,CONVERT(VARCHAR,AP.FROM_DATE,107) AS FROM_DATE,CONVERT(VARCHAR,AP.TO_DATE,107) AS TO_DATE,"
                                  + " E.FIRST_NAME + ' ' + E.MIDDLE_NAME + ' ' + E.LAST_NAME AS NAME,E.EMPLOYEE_ID, "
                                  + " CONVERT(VARCHAR,aTask.commentDate,107) appraisee_task_date,"
                                  + " CONVERT(VARCHAR,apAcpt.commentDate,107) appraisee_acpt_date,"
                                  + " CONVERT(VARCHAR,sTask.commentDate,107) appraiseer_task_date,"
                                  ...
                                  + " CONVERT(VARCHAR,AP.Appraiser_Date,107) Appraiser_Date,"
                                  + " CONVERT(VARCHAR,AP.Reviewer_date,107) Reviewer_date, CONVERT(VARCHAR,AP.HRC_Date,107) HRC_Date,"
                                  + " CONVERT(VARCHAR,AP.CEO_date,107) CEO_date"
                                  + " FROM APPRAISAL AS AP inner join Employee AS E on AP.EMPLOYEE_ID=E.EMPLOYEE_ID"
                                  + " LEFT JOIN (select appraisal_id,supervisor from appraisalSupervisorAssignment where SUPERVISOR_TYPE='s') sup on sup.appraisal_id=ap.ID"
                                  + " LEFT JOIN (select appraisal_id,supervisor from appraisalSupervisorAssignment where SUPERVISOR_TYPE='r') rev on rev.appraisal_id=ap.ID" 
                                  + " LEFT JOIN(SELECT appraisalId,commentDate FROM appraisalComments WHERE raterTypeFlag = 'a' and flag='t')aTask on aTask.appraisalId = ap.ID "
                                  + " LEFT JOIN(SELECT appraisalId,commentDate FROM appraisalComments WHERE raterTypeFlag = 'a' and flag='f')apAcpt on apAcpt.appraisalId = ap.ID "
                                  + " LEFT JOIN(SELECT appraisalId,commentDate FROM appraisalComments WHERE raterTypeFlag = 's' and flag='t')sTask on sTask.appraisalId = ap.ID "
                                  + " WHERE AP.EMPLOYEE_ID=" + ReadSession().Emp_Id.ToString();
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            //html.Append("<th Class=\"HeaderStyle\" align=\"left\">Appraisal</th>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (_commonServices.DoesHrUser(ReadSession().Emp_Id.ToString()))
                {
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/PerformanceAppraisal/Details/Manage.aspx?masterpage=" + Getmasterpage() + "&Id=" + dr["ID"] + "&EmpId=" + dr["EMPLOYEE_ID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                }

                //html.Append("<td><a href=\"/PerformanceAppraisal/AppraisalAllInfo.aspx?appraisalId=" + dr["ID"] + "&positionId=" + dr["POSITION_ID"] + "&EmpId=" + dr["EMPLOYEE_ID"] + "\">Appraisal</a></td>");
                //html.Append("<td><a href=\"/PerformanceAppraisal/Manage.aspx?masterpage=" + Getmasterpage() + "&Id=" + dr["ID"] + "&positionId=" + dr["POSITION_ID"] + "&EmpId=" + GetEmpId() + "\">Appraisal</a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/PerformanceAppraisal/Details/Manage.aspx?masterpage="+ Getmasterpage());
