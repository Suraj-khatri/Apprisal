...
{
    public partial class SelftInitiationList : BasePage
    {
        RoleMenuDAOInv _roleMenuDao = null;
        CommonServices _commonServices = null;

        public SelftInitiationList()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_roleMenuDao.hasAccess(ReadSession().AdminId, 27) == false && _roleMenuDao.hasAccess(ReadSession().AdminId, 288) == false)
                {
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (_commonServices.DoesHrUser(ReadSession().Emp_Id.ToString()))
                {
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/PerformanceAppraisal/Details/SelfInitiation.aspx?masterpage=" + Getmasterpage() + "&Id=" + dr["ID"] + "&EmpId=" + dr["EMPLOYEE_ID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/PerformanceAppraisal/Details/SelfInitiation.aspx?masterpage=" + Getmasterpage());
