...
{
    public partial class DisciplinaryList : BasePage
    {
        RoleMenuDAOInv _roleMenuDao = null;
        public DisciplinaryList()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if(!IsPostBack)
            {
                if (_roleMenuDao.hasAccess(ReadSession().AdminId, 10071) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"empId",           "Employee Name",        "LT"},
                        {"branchId",        "Branch Name",          "LT"},
                        {"commentType",     "Comment Type",         "LT"},
            ...
            String[,] matgrid = new String[,]{
                        {"branchId",        "Branch",               "200",      "T"},
                        {"deptId",          "Department",           "200",      "T"},
                        {"empName",         "Employee Name",        "200",      "T"},
                        {"commentType",     "Comment Type",         "200",      "T"},
                        {"categoryName",    "Category",             "200",      "T"},
            ...
            string sortby = "empId";
            string table_name = "DisciplinaryRecord";

            String sql = "";

            sql = "select commentId, category, empId, dbo.GetEmployeeFullNameOfId(empId) empName, dbo.GetBranchOfId(branch_Id) [branchId], dbo.GetDeptName(department_Id) [deptId]"
                   + ",commentType,sdd.DETAIL_TITLE [categoryName],CONVERT(varchar,startDate,101)startDate,CONVERT(varchar,endDate,101)endDate from disciplinaryComments dc "
                   + "inner join StaticDataDetail sdd on sdd.ROWID=dc.category where 1 = 1";
            ...
            rptComment.InnerHtml = "<center>" + load_report(table_name, sql, matgrid, matfilter, sortd, sortby, _page, page_size) + "</center>";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "DisciplinaryList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"DisciplinaryComments.aspx?flag=v&empId=" + dr["empId"] + "&cType=" + dr["commentType"] + "&category=" + dr["category"] + "&commentId=" + dr["commentId"] + "\">View</a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "DisciplinaryComments.aspx");
