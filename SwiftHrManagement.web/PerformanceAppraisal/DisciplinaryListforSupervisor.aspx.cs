...
{
    public partial class DisciplinaryListforSupervisor : BasePage
    {
        ...
        private void init()
        {
            ...
            sql = "select commentId, category, empId, dbo.GetEmployeeFullNameOfId(empId) empName, dbo.GetBranchOfId(branch_Id) [branchId], dbo.GetDeptName(department_Id) [deptId]"
                   ...
                   + "inner join StaticDataDetail sdd on sdd.ROWID=dc.category where branch_Id =" + filterstring(ReadSession().Department.ToString());
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "DisciplinaryComments.aspx?flag=s");
