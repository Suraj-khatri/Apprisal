...
namespace SwiftHrManagement.web.PerformanceAppraisal.TemplateRecord
{
    public partial class TemplateList : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if(!IsPostBack)
            {
                if (_roleMenuDao.hasAccess(ReadSession().AdminId, 17) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"templateName",    "Template Name",         "LT"}
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"templateId",          "Template ID",      "200",      "T"},
                        {"templateName",        "Template Name",    "200",      "T"}
                        };



            string sortby = "templateId";
            string table_name = "TemplateRecord";

            String sql = "";

            sql = "select templateId, templateName from templateRecord WHERE 1=1";
            ...
            rptTemplate.InnerHtml = "<center>" + load_report(table_name, sql, matgrid, matfilter, sortd, sortby, _page, page_size) + "</center>";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "StaticDataTypeView.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            html.Append("<th clas   s=\"text-center\" align=\"center\">Position</th>");
            html.Append("<th clas   s=\"text-center\" align=\"center\">Matrix</th>");

            //html.Append("<th Class=\"HeaderStyle\" align=\"left\">History</th>");
            html.Append("<th clas   s=\"text-center\" align=\"center\">Other Section</th>");
            html.Append("<th clas   s=\"text-center\" align=\"center\">View</th>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Other Section\" href=\"/PerformanceAppraisal/AppraisalPosition/ManagePosition.aspx?templateId=" + dr["templateId"] + "\"><i class=\"fa fa-plus\" aria-hidden=\"true\"></i></a></td>");
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Matrix Setup\" href=\"/PerformanceAppraisal/AppraisalMatrix/ManageSetUp.aspx?ID=" + dr["templateId"] + "\"><i class=\"fa fa-plus\"></i></a></td>");
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Assign Position\" href=\"/PerformanceAppraisal/AppraisalMatrix/ManageOther.aspx?templateId=" + dr["templateId"] + "\"><i class=\"fa fa-plus\"></i></a></td>");
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"TemplateManage.aspx?flag=v&templateId=" + dr["templateId"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                
                //html.Append("<td><a href=\"/StaticView/StaticDataDetailView.aspx?ID=" + dr["templateId"] + "\">Add</a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "TemplateManage.aspx");
