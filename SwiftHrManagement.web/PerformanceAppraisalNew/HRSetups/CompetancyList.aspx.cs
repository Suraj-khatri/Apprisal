...
{
    public partial class CompetancyList : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public CompetancyList()
        {           
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 1114) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        ...
                        {"E.EMP_CODE",    "Employee Code",         "LT"},
                        {"sdd.DETAIL_DESC",    "Employee Status",         "LT"}
            ...
            String[,] matgrid = new String[,]{                         
                        {"SN",        "SNo",        "100",      "T"},
                        {"LevelName",        "Level Name",        "200",      "T"}
                        };

            string sortby = "SN";
            string table_name = "Competency";

            String sql = "";
            //sql = "SELECT E.EMPLOYEE_ID,dbo.GetEmployeeFullNameOfId(E.Employee_Id) AS EmpName,B.BRANCH_NAME,D.DEPARTMENT_NAME,"
...
//            sql = @"SELECT  SN = ROW_NUMBER() OVER ( ORDER BY rowid ) ,
//                            C.CompetencyMatrixName,
//                            C.rowId
//                    FROM    Competency C
//                            INNER JOIN ( SELECT MAX(ROWID) AS ID ,
//                                                CompetencyMatrixName
//                                         FROM   Competency
//                                         GROUP BY CompetencyMatrixName
//                                       ) ID ON ID.ID = C.ROWID";

            sql = @"SELECT  SN = ROW_NUMBER() OVER ( ORDER BY A.rowid ),
                            A.LevelName,
                            A.rowId,AppraisalLevelId
                    FROM    AppraisalLevel A
	                LEFT JOIN dbo.weightageDefination W ON A.RowId =W.AppraisalLevelId
					 INNER JOIN ( SELECT MAX(ROWID) AS ID ,
                                                LevelName
                                         FROM   AppraisalLevel
                                         GROUP BY LevelName
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            String sql1 = "select * from (select ROW_NUMBER() OVER (ORDER BY " + sortby + " ) as rowId1,* from(" + sql + " ) as aa) as tmp where 1 = 1 ";
            sql1 += " and  tmp.rowId1 between " + ((_page - 1) * page_size + 1).ToString() + " and " + (_page * page_size).ToString();
            ...
            StringBuilder html = new StringBuilder("<br/> <table class=\"table table-bordered \">");
            ...
            for (int i = 0; i <= arr_cnt; i++)
            {
                ...
                html.Append("<th Class=\"HeaderStyle\" align=\"left\">" + matgrid.GetValue(i, 1).ToString() + "</th>");
            }
            html.Append("<th class=\"text-center\" align=\"center\">Weightage Defination</th>");
            html.Append("<th class=\"text-center\" align=\"center\">Review Committee</th>");
            //html.Append("<th class=\"text-center\" align=\"center\">Appraisal Matrix KRA</th>");
            html.Append("<th class=\"text-center\" align=\"center\">Critical Jobs</th>");
            html.Append("<th class=\"text-center\" align=\"center\">Competency Matrix</th>");
            html.Append("<th class=\"text-center\" align=\"center\">Edit</th>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Edit\" href=\"/PerformanceAppraisalNew/HRSetups/WeightageDefination.aspx?rowId=" + dr["rowId"] + "&wRowId=" + dr["AppraisalLevelId"] + "\">Edit</a></td>");

                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Assign\" href=\"/PerformanceAppraisalNew/HRSetups/AssignReviewCommittee.aspx?rowId=" + dr["rowId"] + "\">Assign</a></td>");

                //html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Add\" href=\"/PerformanceAppraisalNew/HRSetups/LevelAppraisalMatrixKRA.aspx?LrowId=" + dr["rowId"] + "\">Add</a></td>");
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Add\" href=\"/PerformanceAppraisalNew/HRSetups/LevelCriticalJobs.aspx?LrowId=" + dr["rowId"] + "\">Add</a></td>");

                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Add\" href=\"/PerformanceAppraisalNew/HRSetups/CompetencyMatrixSetup.aspx?LrowId=" + dr["rowId"] + "\">Add</a></td>");

                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Edit\" href=\"/PerformanceAppraisalNew/HRSetups/CompetancyManage.aspx?rowId=" + dr["rowId"] + "\"><i class=\"fa fa-pencil\"></i></a></td>");
            }
            ...
            string filter_form = "";
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/PerformanceAppraisalNew/HRSetups/CompetancyManage.aspx");
        ...
        private string get_paging_block(int _total_record, int _page, int _page_size, Boolean _show_add_button, string _add_page)
        {
            ...
            StringBuilder html = new StringBuilder("");
            //"<input type = \"hidden\" name=\"hdd_curr_page\" id = \"hdd_curr_page\" value=\"" + _page.ToString() + "\">");
