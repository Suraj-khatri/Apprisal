...
{
    public partial class ReviewInitiationList : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public ReviewInitiationList()
        {
        ...
        private void init()
        {
            ...
            sql = @"Select * from (SELECT  DISTINCT AI.employeeId AS EMPLOYEE_ID ,
                    ...
                    OR ((SELECT DISTINCT 'A'
                         FROM   dbo.user_role r (Nolock)
                                INNER JOIN dbo.StaticDataDetail s (Nolock) ON r.role_id = s.ROWID
                                INNER JOIN dbo.Admins a (Nolock) ON a.AdminID = r.user_id
                         WHERE  s.TYPE_ID = '25'
                    ...
                    AND rl.Active <> 'N' 
                    AND AI.Status =1)as X Where 1=1 ";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ReviewInitiationList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"ReviewInitiation.aspx?appId=" + Uri.EscapeDataString(Crypto(dr["appraisalId"].ToString())) + "&empId=" + Uri.EscapeDataString(Crypto(dr["EMPLOYEE_ID"].ToString())) + "\"><i class=\"fa fa-eye\"></i></a>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/PerformanceAppraisalNew/PerformanceAgreement/ReviewInitiation.aspx");
