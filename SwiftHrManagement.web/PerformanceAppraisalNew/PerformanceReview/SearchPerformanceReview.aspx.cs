...
{
    public partial class SearchPerformanceReview : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public SearchPerformanceReview()
        {
        ...
        private void init()
        {
            ...
            //, From date, To date, Status, View field.
            //column type : LT = like text , ET = Equal to text, EN= equal to numbert,ED=equal to date
            String[,] matfilter = new String[,]{
                        {"EMPNAME",    "Appraisee Name",    "LT"},
                        {"AppraisalStartDate",    "From Date",  "ED"},
                        {"AppraisalEndDate",      "To date",        "ED2"},
                        {"STATUS",      "Status",        "LT"}
                        //{"E.EMP_CODE",      "Status",        "LT"}
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"EMPNAME",        "Appraisee name",    "200",      "T"},
                        {"AppraisalStartDate",      "From Date",       "200",      "D"},
                        {"AppraisalEndDate",     "To date",      "200",      "D"},
                        {"Status",    "Status",     "200",      "T"}
            ...
            sql = sql = @"Select * from (SELECT  DISTINCT AI.employeeId AS EMPLOYEE_ID ,
                    ...
                    where 
                    (AI.employeeId=ISNULL(" + ReadSession().Emp_Id + @",AI.employeeId)
                    OR AI.supervisorId=ISNULL(" + ReadSession().Emp_Id + @",AI.supervisorId)
                    OR AI.reviewerId=ISNULL(" + ReadSession().Emp_Id + @",AI.reviewerId)
                    OR RL.EmployeeName=ISNULL(" + ReadSession().Emp_Id + @",RL.EmployeeName)
                    OR ((  SELECT DEPARTMENT_SHORT_NAME FROM dbo.Departments (NOLOCK)
                            WHERE DEPARTMENT_ID=" + ReadSession().Department + @"
                         ) = 'Human Resources')
                    OR ((SELECT DISTINCT 'A'
                     ...
                     OR ((
                        SELECT DETAIL_TITLE FROM dbo.Employee E 
                        INNER JOIN dbo.StaticDataDetail S 
                        ON S.ROWID = E.POSITION_ID
                        WHERE S.TYPE_ID = 4 AND E.EMPLOYEE_ID=" + ReadSession().Emp_Id + @"
                        ) = 'CEO')
                    ) 
                    AND rl.Active <> 'N' 
                    AND AI.status in( 11,6,8,13,9,12,7))as X Where 1=1 ";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "SearchPerformanceReview.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">View/Print</th>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //html.Append("<td><a href=\"/AttendenceWeb/BackDateLogin.aspx?Id=" + dr["id"] + "\">View</a></td>");
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"Main.aspx?appId=" + Uri.EscapeDataString(Crypto(dr["appraisalId"].ToString())) + "&empId=" + Uri.EscapeDataString(Crypto(dr["EMPLOYEE_ID"].ToString())) + "\"><i class=\"fa fa-eye\"></i></a><button class='btn btn-xs btn-info' onclick=\"return pop('"+Uri.EscapeDataString(Crypto(dr["EMPLOYEE_ID"].ToString()))+"'"+",'"+Uri.EscapeDataString(Crypto(dr["appraisalId"].ToString()))+"')\" ><i class='fa fa-print'></i></button>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, false, "/AttendenceWeb/BackDateLogin.aspx");
        ...
        private string make_filter_form(string table_name, string[,] matfilter)
        {
            ...
            for (int i = 0; i <= arr_cnt; i++)
            {
                ...
                if (ctlType == "ED")
                {
                    html.Append("<input type=\"text\" class=\"form-control\" id=\"datepicker\"  name=\"" + ctlName + "\" value=\"" + defValue + "\">");
                }
                else if (ctlType == "ED2")
                {
                    html.Append("<input type=\"text\" class=\"form-control\" id=\"datepicker2\"  name=\"" + ctlName + "\" value=\"" + defValue + "\">");
                }
                else
