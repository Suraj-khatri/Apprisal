    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"P.PROJECT_TITLE",    "Project Title",         "LT"},
                        {"S.DETAIL_TITLE",    "Priority",         "LT"},
                        {"P.START_DATE",    "Start From (MM-DD-YYYY)",         "ED1"},
                        {"P.END_DATE",    "Start To (MM-DD-YYYY)",         "ED2"}                  
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"PROJECT_TITLE",            "Title",             "200",      "T"},
                        {"START_DATE",            "Start Date",             "200",      "D"},
                        {"END_DATE",       "End Date",        "200",      "D"},
                        {"PRIORITY",    "Priority",     "200",      "T"},
                        {"COMPLETED",    "Is Completed",     "200",      "T"},
                        {"PROJECT_MANAGER",    "Manager",     "200",      "T"},
                        {"OWNER",    "Owner",     "200",      "T"}
                        };



            string sortby = "PROJECT_ID";
            string table_name = "Projects";

            String sql = "";
            if (ReadSession().UserId == "admin")
            {
                sql = "SELECT P.PROJECT_TITLE, P.PROJECT_ID, convert(varchar, P.START_DATE ,107) as START_DATE, convert(varchar,P.END_DATE,107)"
                + " as END_DATE, P.CATEGORY, S.DETAIL_TITLE AS PRIORITY,case when P.COMPLETED='false' then 'No' else 'Yes' end as COMPLETED,dbo.EmployeeFullName(PROJECT_MANAGER) AS PROJECT_MANAGER,"
                + " dbo.EmployeeFullName(OWNER) AS OWNER FROM   Projects AS P with(nolock) INNER JOIN StaticDataDetail S ON S.ROWID=P.PRIORITY "
                + " where 1=1";
            }
            else
            {
                sql = "SELECT P.PROJECT_TITLE, P.PROJECT_ID,convert(varchar, P.START_DATE ,107) as START_DATE, convert(varchar,P.END_DATE,107)"
                        + " as END_DATE, P.CATEGORY, P.PRIORITY,case when P.COMPLETED='false' then 'No' else 'Yes' end as COMPLETED,dbo.EmployeeFullName(PROJECT_MANAGER) AS PROJECT_MANAGER," 
                        +" dbo.EmployeeFullName(OWNER) AS [OWNER] FROM   Projects  AS P with(nolock)"
                        +" where 1=1 and P.PROJECT_ID in (select PROJECT_ID from Assign_Project with (nolock) where EMP_ID='"+ReadSession().UserId+"')";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Assign Project</th>");
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Task List</th>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/Project/Assignproject.aspx?Project_Id=" + dr["PROJECT_ID"] + "\">Assign</a></td>");
                html.Append("<td><a href=\"/Project/Task/List.aspx?Project_Id=" + dr["PROJECT_ID"] + "\">Tasks</a></td>");
                html.Append("<td><a href=\"/Project/Manage.aspx?Project_Id=" + dr["PROJECT_ID"] + "\">View</a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Project/Manage.aspx");
                ...
                if (defValue.Trim() != "")
                {
                    switch (matfilter.GetValue(i, 2).ToString().ToUpper())
                    {
                        ...
                        case "EN":
                            ...
                            break;
                        case "ED1":
                            query = query + " and START_DATE between  "+ filterstring(defValue) +"";
                            break;
                        case "ED2":
                            query = query + " and " + filterstring(defValue) + "";
                            break;
                    }
                }
            }
