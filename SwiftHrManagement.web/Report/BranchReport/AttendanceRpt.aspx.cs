...
{
    public partial class AttendanceRpt : BasePage
    {
        ...
        #region
        //made by bibhut
        public string loadAttendanceRptExcel(string from,string to,string emp_id)
        {
           return "EXEC [ProcBranchSummaryRpt] @FLAG='A',@FROM_DATE=" + filterstring(from) + ",@TO_DATE=" + filterstring(to) + ","
        ...
        private void loadReport()
        {
            ...
            DataTable dt = _clsDao.getTable("EXEC [ProcBranchSummaryRpt] @FLAG='A',@FROM_DATE=" + filterstring(from) + ",@TO_DATE=" + filterstring(to) + ","
            + " @SUPERVISOR_ID=" + filterstring(ReadSession().Emp_Id.ToString()) + ",@EMPLOYEE_ID=" + filterstring(emp_id) + "");
