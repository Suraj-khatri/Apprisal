...
namespace SwiftHrManagement.web.Report.BranchReport
{
    ...
    {
        ClsDAOInv _clsDaoInv= new ClsDAOInv();
        ...
        public string LoadPayrollRptExcel(string year, string month, string emp_id)
        {
            ...
            {
                return "exec [ProcMonthlyPayrollReport_Dyanamic] @FLAG='a',@year=" + filterstring(year) + ",@month=" + filterstring(month) + ","
           + "@branch="+ ReadSession().Branch_Id.ToString() +","+"@emp=" + filterstring(emp_id);
            }
            else
            {
                return "exec [ProcMonthlyPayrollReport_Dyanamic] @FLAG='a',@year=" + filterstring(year) + ",@month=" + filterstring(month) + ","
           + "@emp=" + filterstring(emp_id);
            }
        ...
        private void init()
        {
            ...
            year.Value = filterstring(Request.QueryString["FY"] == null ? "" : Request.QueryString["FY"].ToString());
            ...
            string monthstartdate = _clsDaoInv.GetSingleresult("select dbo.GetMonthStartDateEng(" + year.Value + ", " + month.Value + ")");
            string monthEnddate = _clsDaoInv.GetSingleresult("select dbo.GetMonthEndDateEng(" + year.Value + ", " + month.Value + ")");

            String sql = "SELECT "
                        +" TC.RUN_MONTH,TC.EMP_ID,Emp_code,TC.FY_ID,M.Name,"
                        +"  FIRST_NAME+' '+MIDDLE_NAME+' '+ LAST_NAME AS EMPNAME, "
                        ...
                        + "  (CONTRI_THIS_MONTH_EMPLOYER+ CONTRI_CURRENT_EMPLOYEE+TAX_THIS_MONTH+ADHOC_DEDUCTION_CURRENT+LOAN_DEDUCTION+ ADVANCE_DEDUCTION+isnull(Deduction_Amt,0))) AS NET_SALARY "
                        +" FROM TAXCALCULATION TC "
                        +" INNER JOIN EMPLOYEE E ON E.EMPLOYEE_ID =TC.EMP_ID "
                        +" INNER JOIN MonthList M ON M.Month_Number=TC.Run_Month  "
                        +" Left Join  " 
                        +" (  "
	                    +" select PD.EMP_ID, SUM(Deduction_Amt) Deduction_Amt, Deduction_Date "
	                    +" from PremiumDeduction PD "  
	                    +" where PD.EMP_ID=isnull("+ emp_id.Value +",PD.EMP_ID)  "
                        + " group by PD.EMP_ID ,Deduction_Date"
                        +" ) pd on pd.EMP_ID=Tc.Emp_Id  "
                        + " AND PD.Deduction_Date between " + filterstring(monthstartdate) + " and " + filterstring(monthEnddate) +""
                        +" WHERE E.BRANCH_ID=(Select BRANCH_ID from Employee where EMPLOYEE_ID=" + filterstring(ReadSession().Emp_Id.ToString()) + ")"
                        +" AND TC.FY_ID ="+year.Value+""
                        +" AND TC.Run_Month = ISNULL(" + month.Value + ",TC.Run_Month) "
                        +" AND E.EMPLOYEE_ID = ISNULL(" + emp_id.Value + ",E.EMPLOYEE_ID) ";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"../PayRollReport/SalaryDetails.aspx?empid=" + dr["EMP_ID"] + "&month=" + dr["RUN_MONTH"] + "&year=" + Request.QueryString["FY"].ToString() + "&Empname=" + dr["EmpName"] + "\" rel='gb_page_center[650,400]'\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
