...
namespace SwiftHrManagement.web.Report.BranchReport
{
    public partial class TrainingRpt : BasePage
    {
        ...
        #region
        //made by bibhut
        public string loadTrainingRptExcel(string from,string to,string emp_id)
        {
            return "EXEC [ProcBranchSummaryRpt] @FLAG='T',@FROM_DATE=" + filterstring(from) + ",@TO_DATE=" +
                   filterstring(to) + "," + " @SUPERVISOR_ID=" + filterstring(ReadSession().Emp_Id.ToString()) + ",@EMPLOYEE_ID=" +
                   filterstring(emp_id);
        }
        #endregion
        ...
        private void loadReport()
        {
            ...
            DataTable dt = _clsDao.getTable("EXEC [ProcBranchSummaryRpt] @FLAG='T',@FROM_DATE=" + filterstring(from) + ",@TO_DATE=" + filterstring(to) + ","
