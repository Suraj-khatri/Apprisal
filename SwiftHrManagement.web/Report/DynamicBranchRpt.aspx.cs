...
{
    public partial class DynamicBranchRpt : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            DataTable dt1 = _rpt.FindsummaryReport("b", Branch_Id, Dept_Id, Emp_Id).Tables[0];
            ...
            foreach (DataRow dr in dt1.Rows)
            {
                str.Append("<tr>");
                        for (int i = 1; i < cols; i++)
                        {
                                ...
                                if (i == cols-2)
                                {
                                    str.Append("<td align=\"left\"><a href=\"DynamicDeptRpt.aspx?branchid= " + dr[0] + "&BranchName="+ dr[2] +"\"" + ">" + dr[i].ToString() + "</a></td>");
                                }
                                else if (i == cols-1)
                                {
                                    str.Append("<td align=\"left\"><a href=\"DynamicEmpRpt.aspx?branchid= " + dr[0] + "\"" + ">" + dr[i].ToString() + "</a></td>");
                                }
                                else
                                {
