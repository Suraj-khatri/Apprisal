...
{
    public partial class AcknowledgeList : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = new RoleMenuDAOInv();
        ClsDAOInv _clsdao = null;
        public AcknowledgeList()
        {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"rm.dis_msg_id",       "Dispatch ID",        "EN"},
                        {"rm.req_msg_id",       "Requisition ID",        "EN"},
                        {"e.FIRST_NAME",       "Received By(First Name)",        "LT"}
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"id",                       "Received Id",           "200",      "D"},
                        {"dis_msg_id",                       "Dispatch Id",           "200",      "D"},
                        {"req_msg_id",                       "Req. Id",           "200",      "D"},
                        {"received_by",                  "Received By",            "200",      "T"},
                        {"received_date",              "Received Date",       "200",      "D"},
                        {"received_msg",          "Received Message",   "200",      "T"}
                        };

            string sortby = "id";
            string table_name = "IN_RECEIVED_MESSAGE";
            String sql = "";

            sql = "SELECT rm.id,rm.req_msg_id,rm.dis_msg_id,dbo.GetEmployeeFullNameOfId(e.employee_id) AS received_by, "
                        +" CONVERT(VARCHAR,rm.received_date,107) AS received_date,rm.received_msg "
                        +" FROM IN_RECEIVED_MESSAGE rm INNER JOIN Employee e ON e.EMPLOYEE_ID=rm.received_by "
                        + " WHERE 1=1 and rm.dis_msg_id=" + GetDispatchedId() + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "AcknowledgeList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/Report/Inventory/RequisitionHisory/ViewAcknowledged.aspx?id=" + dr["id"] + "\">View</a></td>");                                 
                html.Append("</tr>");
            }
