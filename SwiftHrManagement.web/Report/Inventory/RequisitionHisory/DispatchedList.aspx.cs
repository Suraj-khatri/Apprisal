...
{
    public partial class DispatchedList : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = new RoleMenuDAOInv();
        ClsDAOInv _clsdao = null;
        public DispatchedList()
        {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"dm.req_id",       "Requisition ID",        "LT"},
                        {"e.FIRST_NAME",       "Dispatched By(First Name)",        "LT"},                        
                        {"dm.status",     "Status",     "LT"}
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"id",                       "Dispatch Id",           "200",      "D"},
                        {"req_id",                       "Req. Id",           "200",      "D"},
                        {"dispatched_by",                  "Dispatched By",            "200",      "T"},
                        {"dispatched_date",              "Dispatched Date",       "200",      "D"},
                        {"dispatch_message",          "Dispatched Message",   "200",      "T"},
                        {"status",                 "Status",                "200",      "T"}
                        };
            ...
            String sql = "";
                ...
                sql = "SELECT dm.id,dm.req_id,dbo.GetEmployeeFullNameOfId(e.employee_id) AS dispatched_by,"
                    +" CONVERT(VARCHAR,dm.dispatched_date,107) AS dispatched_date,dm.dispatch_message,dm.status "
                    +" FROM IN_DISPATCH_MESSAGE dm INNER JOIN Employee e ON e.EMPLOYEE_ID=dm.dispatched_by"
                    + " WHERE 1=1 and dm.req_id=" + GetReqId() + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "DispatchedList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Products</th>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (status == "Not Acknowledge")
                {
                    html.Append("<td><a href=\"/Report/Inventory/RequisitionHisory/viewDispatched.aspx?id=" + dr["id"] + "\">View</a></td>");
                    html.Append("<td>N/A</td>");
                }
                if (status == "Full Acknowledged" || status=="Partial Acknowledged")
                {
                    html.Append("<td><a href=\"/Report/Inventory/RequisitionHisory/viewDispatched.aspx?id=" + dr["id"] + "\">View</a></td>");
                    html.Append("<td><a href=\"/Report/Inventory/RequisitionHisory/AcknowledgeList.aspx?id=" + dr["id"] + "\">View</a></td>");
                }                                 
                html.Append("</tr>");
            }
