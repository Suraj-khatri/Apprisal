...
namespace SwiftAssetManagement.Report.Inventory.RequisitionHisory
{
    public partial class List : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 118) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"m.id ",       "Requisition ID",        "EN"},
                        {"e.FIRST_NAME ",       "Requested By(First Name)",        "LT"},
                        {"Requ_date",       "Requested Date",        "ED"},
                        {"b.BRANCH_NAME",     "Requesting Branch",     "LT"},
                        {"d.DEPARTMENT_NAME", "Requesting Department", "LT"},
                        {"b1.branch_name", "Forwarded Branch", "LT"},
                        {"p.porduct_code", "Product Name", "LT"},
                        {"Status", "Status", "LT"}
                    };
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"id",                       "Req. Id",           "200",      "T"},
                        {"Requ_by",                  "Requested By",            "200",      "T"},
                        {"req_date",                 "Requested Date",            "200",      "D"},
                        {"recommeded_by",            "Recommended By",            "200",      "T"},
                        {"BRANCH_NAME",              "Requested Branch",       "200",      "T"},
                        {"DEPARTMENT_NAME",          "Requested Dept",   "200",      "T"},
                        {"priority",                 "Priority",                "200",      "T"},
                        {"forwarded_branch",         "Requested With",                "200",      "T"},
                        {"Status",                   "Status",                  "200",      "T"}
                        };

            string sortby = "id";
            string table_name = "IN_Requisition_Message";
            String sql = "";
            if (ReadSession().UserType == "A")
            {
                sql = @"select distinct m.id,dbo.GetEmployeeFullNameOfId(e.employee_id)  as Requ_by,CONVERT(varchar,m.Requ_date,107) req_date,
                        dbo.GetEmployeeFullNameOfId(recommed_by) recommeded_by,b.BRANCH_NAME ,d.DEPARTMENT_NAME, 
                        case when  priority='N' then 'Normal' when priority = 'H' then 'High' when priority = 'L' then 'Low' end  as priority, 
                        Status,b1.branch_name forwarded_branch from IN_Requisition_Message   m inner join  Departments d on 
                        m.dept_id = d.DEPARTMENT_ID inner join Branches b on  m.branch_id = b.BRANCH_ID  inner join Employee e on 
                        m.Requ_by = e.EMPLOYEE_ID inner join branches b1 on b1.branch_id=m.Forwarded_To
                        left join IN_Requisition re on re.Requistion_message_id=m.id
						inner join IN_PRODUCT p on re.item = p.id
                        where 1=1";
            }
            else
            {
                sql = @"select distinct m.id, dbo.GetEmployeeFullNameOfId(e.employee_id)  as Requ_by,CONVERT(varchar,m.Requ_date,107) req_date,
                        ...
                        left join IN_Requisition re on re.Requistion_message_id=m.id
						inner join IN_PRODUCT p on re.item = p.id
                        where b.branch_id=" + ReadSession().Branch_Id + "";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            String sql1 = "select * from (select ROW_NUMBER() OVER (ORDER BY " + sortby + " desc) as rowId,* from(" + sql + " ) as aa) as tmp where 1 = 1 ";
            ...
            StringBuilder html = new StringBuilder("<div class=\"table-responsive branchlist\"> <table class=\"table table-bordered table-striped table-condensed\">");
            ...
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Request</th>");
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Recommend</th>");
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Approve</th>");
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Dispatch</th>");
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Ack</th>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (status == "Requested")
                {
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Report/Inventory/RequisitionHisory/ListRequisition.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a> </td>");
                ...
                {
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Report/Inventory/RequisitionHisory/ListRequisition.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Report/Inventory/RequisitionHisory/ListRecommendation.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                    html.Append("<td>N/A</td>");
                    html.Append("<td>N/A</td>");
                    html.Append("<td>N/A</td>");
                ...
                if (status == "Approved")  
                {
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Report/Inventory/RequisitionHisory/ListRequisition.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Report/Inventory/RequisitionHisory/ListRecommendation.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Report/Inventory/RequisitionHisory/ListApproval.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                    html.Append("<td>N/A</td>");
                    html.Append("<td>N/A</td>");

                }
                if (status == "Full Dispatched")
                {
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Report/Inventory/RequisitionHisory/ListRequisition.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Report/Inventory/RequisitionHisory/ListRecommendation.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Report/Inventory/RequisitionHisory/ListApproval.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Report/Inventory/RequisitionHisory/ListDispatched.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                    html.Append("<td>N/A</td>");
                }
                if (status == "Full Acknowledged")
                {
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Report/Inventory/RequisitionHisory/ListRequisition.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Report/Inventory/RequisitionHisory/ListRecommendation.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Report/Inventory/RequisitionHisory/ListApproval.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Report/Inventory/RequisitionHisory/ListDispatched.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Report/Inventory/RequisitionHisory/ViewAcknowledged.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                }
                if (status == "Rejected")
                {
                    html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Report/Inventory/RequisitionHisory/ListRequisition.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
            ...
            html.Append("<tr><td align=\"center\" colspan=\"13\"> <strong> Page " + _page.ToString() + " Of " + total_page + "</strong></td></tr>");
            html.Append("</table>");
            html.Append("</div>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, false, "");
        ...
        private string make_filter_form(string table_name, string[,] matfilter)
        {
            ...
            for (int i = 0; i <= arr_cnt; i++)
            {
                ...
                if (ctlType == "ED")
                {
                    html.Append("<td width=\"200\"><input type=\"text\" class=\"form-control\" id=\"datepicker\"  name=\"" + ctlName + "\" value=\"" + defValue + "\"></td>"); 
                }
                else
                    html.Append("<td width=\"200\"><input type=\"text\" class=\"form-control\" name=\"" + ctlName + "\" value=\"" + defValue + "\"></td>");
            ...
            for (int i = 0; i <= arr_cnt; i++)
            {
                ...
                if (defValue.Trim() != "")
                {
                    switch (matfilter.GetValue(i, 2).ToString().ToUpper())
                    {
                        ...
                        case "ED":
                            query = query + " and convert(varchar, cast(" + matfilter.GetValue(i, 0).ToString() + " as datetime),101) = convert(varchar, cast(" + filterstring(defValue) + " as datetime),101) ";
                            break;
