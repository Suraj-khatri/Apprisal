...
{
    public partial class ListAcknowlwdge : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = new RoleMenuDAOInv();
        ClsDAOInv _clsDao = null;
        public ListAcknowlwdge()
        {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"Delivered_Quantity",  "Dispatched Quantity", "200",      "EN"},
                        {"unit",                "Unit",              "200",   "LT"},
                        {"Received_Quantity",   "Acknowledged Quantity", "200",     "EN"}
                        };
            ...
            String sql = "select r.id, i.porduct_code , r.unit, r.Delivered_Quantity, r.Received_Quantity from IN_Requisition r inner join "
            + "IN_PRODUCT i on i.id = r.item where  r.Requistion_message_id  = " + hdnapid.Value + "";

            TxtAppMessage.Text = _clsDao.GetSingleresult("select Acknowledged_Message from IN_Requisition_Message where id = " + hdnapid.Value + "");
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //html.Append("<td><a href=\"/Inventory/Requisition/Receive/Manage.aspx?id=" + dr["id"] + "&recid=" + hdnapid.Value + "&label=" + LblBranchDept.Text + "\">Modify</a></td>");
