...
{
    public partial class ListBranchWise : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = new RoleMenuDAOInv();
        ClsDAOInv _clsdao = null;
        public ListBranchWise()
        {
        ...
        private void init()
        {
            ...
            sql = @"select m.id, dbo.GetEmployeeFullNameOfId(e.employee_id) as Requ_by,CONVERT(varchar,m.Requ_date,107) req_date,
                        ...
                        where m.Forwarded_To=" + ReadSession().Branch_Id + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListBranchWise.aspx?sortd=" + sortd + "&sortby={sortby}";
        ...
        private string make_filter_form(string table_name, string[,] matfilter)
        {
            ...
            for (int i = 0; i <= arr_cnt; i++)
            {
                ...
                if (ctlType == "ED")
                {
                    html.Append("<input type=\"text\" name=\"" + ctlName + "\" value=\"" + defValue + "\">" +
                        "<script language=\"JavaScript\"> new tcal ({ 'formname': 'form1','controlname': '" + ctlName + "'});</script> ");
                }
                else
