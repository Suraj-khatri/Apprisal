...
{
    public partial class ListDispatched : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = new RoleMenuDAOInv();
        ClsDAOInv _clsdao = null;
        public ListDispatched()
        {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"unit",                "Unit",                 "200",      "LT"},
                        {"Received_Quantity",  "Dispatched Quantity",    "200",      "EN"}
            ...
            String sql = "select r.id, i.porduct_code , r.Approved_Quantity, r.unit, r.Delivered_Quantity,r.Received_Quantity from IN_Requisition r inner join "
            + "IN_PRODUCT i on i.id = r.item where r.Requistion_message_id = " + hdnapid.Value + "";

            TxtAppMessage.Text = _clsdao.GetSingleresult("SELECT Delivery_Message FROM IN_Requisition_Message WHERE id=" + hdnapid.Value + "");
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListApproval.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, false, "" + hdnapid.Value + "");
