...
{
    public partial class ListRecommendation : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"quantity",            "Requested QTY",             "200",      "EN"},
                        {"unit",                "Unit",                 "200",      "LT"},
                        {"quantity",   "Recommended QTY",    "200",      "EN"},
                        };
            ...
            String sql = "select r.id, p.porduct_code ,r.quantity, r.unit  from IN_Requisition r  "
            + " inner join IN_PRODUCT p on r.item = p.id  where r.Requistion_message_id = " + hdnapid.Value + "";

            txtRecommendMsg.Text = _clsdao.GetSingleresult("select recommed_message from IN_Requisition_Message where id = " + hdnapid.Value + "");
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListRecommendation.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //html.Append("<td><a href=\"/Inventory/Requisition/Approve/Manage.aspx?id=" + dr["id"] + "&msgid=" + hdnapid.Value + "\">Modify</a></td>");
                //html.Append("<td><input type='checkbox' name='chkTran' ROWID='chkTran' value='" + dr["ROWID"].ToString() + "'>Approve</td>");
