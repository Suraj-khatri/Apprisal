...
{
    public partial class ViewAcknowledged : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = new RoleMenuDAOInv();
        ClsDAOInv _clsdao = null;
        public ViewAcknowledged()
        {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]
                    {                        
                        ...
                        {"Received_Quantity",  "Received QTY",    "200",      "M"}                       
                    };

            string sortby = "porduct_code";
            string table_name = "IN_RECEIVED";       

            String sql = @"SELECT P.porduct_code,P.package_unit,Received_Quantity FROM IN_Requisition D INNER JOIN IN_PRODUCT P ON P.id=D.item
                      WHERE Requistion_message_id=" + GetReqId() + "";

            txtAckMsg.Text = _clsdao.GetSingleresult("SELECT Acknowledged_Message FROM IN_Requisition_Message WHERE id=" + GetReqId() + "");
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ViewAcknowledged.aspx?sortd=" + sortd + "&sortby={sortby}";
