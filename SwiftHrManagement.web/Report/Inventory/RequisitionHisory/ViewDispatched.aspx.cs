...
{
    public partial class ViewDispatched : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = new RoleMenuDAOInv();
        ClsDAOInv _clsdao = null;
        public ViewDispatched()
        {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]
                    {                        
                        {"porduct_code",           "Product Name",              "200",      "T"},
                        {"package_unit",                "   Package Unit",                 "200",      "T"},
                        {"dispatched_qty",   "Dispatched QTY",             "200",      "M"},                        
                        {"received_qty",  "Received QTY",    "200",      "M"},
                        {"remain",  "Remain To Received",    "200",      "M"}                        
                    };

            string sortby = "porduct_code";
            string table_name = "IN_DISPATCH";       

            String sql = "SELECT P.porduct_code,P.package_unit,dispatched_qty,received_qty,remain FROM IN_DISPATCH D INNER JOIN IN_PRODUCT P ON P.id=D.product_id"
                         +" WHERE dispatch_msg_id="+GetDispatchId()+"";

            txtDispatchedMsg.Text = _clsdao.GetSingleresult("SELECT dispatch_message FROM IN_DISPATCH_MESSAGE WHERE id=" + GetDispatchId() + "");
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ViewDispatched.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            StringBuilder html = new StringBuilder("<table width=\"700px\" border=\"0\" cellpadding=\"3\" cellspacing=\"1\" align=\"center\" class=\"GridTemplate\" >");
            ...
            html.Append("<table width=\"700px\" cellspacing=\"2\" cellpadding=\"2\" border=\"0\"> <tr> <td align=\"center\"> <strong> Page " + _page.ToString() + " Of " + total_page + "</strong></td></tr>");
        ...
        {
            StringBuilder html = new StringBuilder("<table width=\"700px\" cellspacing=\"2\" cellpadding=\"2\" border=\"0\">");

            html.Append("<table width=\"700px\" cellspacing=\"2\" cellpadding=\"2\" border=\"0\">");
            html.Append("<tr>");
