...
{
    public partial class LFAHistoryReport : BasePage
    {
        RoleMenuDAOInv _roleMenuDao = null;
        public LFAHistoryReport()
        {
        ...
        private void init(long EmpId)
        {
            ...
            string sortby = "LFA_ID";
            string table_name = "LFAHistory";

            String sql = "";

            sql = "select LFA_ID,dbo.GetEmployeeFullNameOfId(emp_ID) emp_ID,Req_ID,LFADays,dbo.ShowDecimal(Amount) Amount from LFAHistory where 1=1 and status is null and CreatedDate between " + filterstring(GetFromDate() + "  00:00:00.000") + " and " + filterstring(GetToDate() + " 23:59:59.000");
            if (GetEmpID() != "")
                sql = sql + " and emp_ID=" + filterstring(GetEmpID());
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Tax Amount</th>");
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Date(mm/dd/yyyy)</th>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><input type=\"text\" name=\"txtTax_" + dr["LFA_ID"] + "\" id=\"txtTax_" + dr["LFA_ID"] + "\" onblur=\"checknumber(this)\"></td>");
                html.Append("<td><input type=\"text\" name=\"txtDate_" + dr["LFA_ID"] + "\" id=\"txtDate_" + dr["LFA_ID"] + "\">*<script language=\"JavaScript\"> new tcal ({ 'formname': 'aspnetForm','controlname': 'txtDate_" + dr["LFA_ID"] + "'});</script></td>");
                html.Append("<td><a href=\"javascript:post('txtTax_" + dr["LFA_ID"] + "','txtDate_" + dr["LFA_ID"] + "','" + dr["LFA_ID"] + "','" + Request.QueryString.ToString() + "');\">Post</a></td>");
                //html.Append("<td><a href=\"LFAHistoryPost.aspx?Id=" + dr["LFA_ID"] + "&check=" + getElementById("txtTax_" + dr["LFA_ID"]) + "&" + Request.QueryString.ToString() + "\">Post</a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/LeaveManagement/LeaveRequest/IndividualLeaveRequest/Manage.aspx");
