...
{
    public partial class LeaveEncashmentIndividual : BasePage
    {
        clsDAO _clsdao = null;
        RoleMenuDAOInv _roleMenuDao = null;
        public LeaveEncashmentIndividual()
        {
            _clsdao = new clsDAO();
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"bs_year",    "Fiscal Year",         "LT"}
                    };
            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"name",            "Employee Name",             "200",      "T"},
                        {"Year",            "Year",             "200",      "T"},
                        {"Gross",            "Gross Salary",             "200",      "T"},
                        //{"AL_Encashment_Days",            "AL Encashment Days",             "200",      "T"}  , 
                        //{"SL_Encashment_Days",            "SL Encashment Days",             "200",      "T"}  , 
                        {"TotEncashment_Days",            "Leave Encashment Days",             "200",      "T"} , 
                        {"Encashment_Amount",            "Encashment Amount",             "200",      "T"}  
                     
                        };
            string sortby = "Year";
            string table_name = "leaveEncashment";
            ...
            String sql = @"Select rtrim(dbo.GetEmployeeFullNameOfId(emp_id)) name,bs_year Year,Gross,AL_Encashment_Days,SL_Encashment_Days,TotEncashment_Days,Encashment_Amount
				            from leaveEncashment le
				            inner join Employee e on le.Emp_Id=e.EMPLOYEE_ID 
                            where  le.EMP_ID= " + ReadSession().Emp_Id.ToString() + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "LeaveEncashmentIndividual.aspx?sortd=" + sortd + "&sortby={sortby}";
