...
namespace SwiftHrManagement.web.Report.PayRollReport
{
    ...
    public partial class Inv_consolidated_report : BasePage
    {
        ...
        private void loadReport()
        {
            ...
            Lblmonth.Text = "Salary Sheet Details For Fiscal Year : " + year + ", Month : " + _clsdao.GetSingleresult("select Name from MonthList where Month_Number=" + filterstring(month) + "");
            
            DataSet ds = _clsdao.getDataset("Exec [Proc_TaxCalculationReport] @fyid=" + filterstring(year.ToString()) + ",@runmonth=" + filterstring(month.ToString()) + ",@empid=" + filterstring(ReadSession().Emp_Id.ToString()) + "");
