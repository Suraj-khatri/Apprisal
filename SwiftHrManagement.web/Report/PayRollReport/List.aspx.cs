        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                for (int i = 0; i <= arr_cnt; i++)
                {
                    ...
                    {
                        case "M":
                            html.Append("<td class=\"text-right\">" + dr[matgrid.GetValue(i, 0).ToString()].ToString() + "</td>");
                            break;
                        case "D":
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" target=\"_blank\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"SalaryDetails.aspx?empid=" + dr["EMP_ID"] + "&month=" + month.Value + "&year=" + year.Value + "&Empname=" + dr["EmpName"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
