...
{
    public partial class ListTrialrun : BasePage
    {
        clsDAO _clsdao = null;
        public ListTrialrun()
        {
        ...
        private void init()
        {
            ...
            string table_name = "TAXCALCULATION_trial";
            year.Value = Request.QueryString["year"] == null ? "" : Request.QueryString["year"].ToString();
            month.Value = Request.QueryString["month"] == null ? "" : Request.QueryString["month"].ToString();

            String sql = "SELECT "
                    + "       T.Emp_Id,Emp_code,T.FY_ID,M.Name,FIRST_NAME+' '+MIDDLE_NAME+' '+ LAST_NAME AS EMPNAME"
                    ...
                    + "  -Contri_This_Month_Employer-Contri_Current_Employee-Adhoc_Deduction_Current-Loan_Deduction-Advance_Deduction"
                    + "      -isnull(Deduction_Amt,0)-Tax_This_Month) NET_SALARY"
                    + " FROM TaxCalculation_trial T"
                    + " Left Join "
                    + " ("
                    + "      select EMP_ID, SUM(Deduction_Amt) Deduction_Amt from PremiumDeduction_trial"
                    + "      where Deduction_Date between dbo.getmonthstartdateeng(" + filterstring(year.Value) + "," + filterstring(month.Value) + ") and dbo.getmonthenddateeng(" + filterstring(year.Value) + "," + filterstring(month.Value) + ")"
                    + "      group by EMP_ID"
                    + "   ) pd on pd.EMP_ID=T.Emp_Id"
                    + "   inner join Employee E on T.emp_id=E.employee_id"
                    + "   INNER JOIN MonthList M ON M.Month_Number=T.Run_Month"
                    + "   where FY_ID =" + filterstring(year.Value) + " AND RUN_MONTH=" + filterstring(month.Value) + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListTrialrun.aspx?sortd=" + sortd + "&sortby={sortby}&year="+year.Value+"&month="+month.Value+"";
            ...
            html.Append("<th class=\"text-center\" >View</th>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" target=\"_blank\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"TrialsalarysheetDetail.aspx?empid=" + dr["EMP_ID"] + "&month=" + month.Value + "&year=" + year.Value + "&Empname=" + dr["EmpName"] + "\" rel='gb_page_center[650,400]'><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
