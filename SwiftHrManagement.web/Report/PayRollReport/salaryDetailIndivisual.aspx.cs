...
{
    public partial class salaryDetailIndivisual :BasePage
    {
        clsDAO _clsdao = null;
        RoleMenuDAOInv _roleMenuDao = null;
        public salaryDetailIndivisual()
        {
            _clsdao = new clsDAO();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_roleMenuDao.hasAccess(ReadSession().AdminId, 95) == false)
                {
        ...
        private void init()
        {
            ...
            String sql =  "SELECT "
                        +" T.id,T.Emp_Id,Emp_code,T.FY_ID,M.Name,FIRST_NAME+' '+MIDDLE_NAME+' '+ LAST_NAME AS EMPNAME"
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "salaryDetailIndivisual.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //html.Append("<td><a href=\"detailedReport.aspx?empid=" + dr["EMP_ID"] + "&Id=" + dr["id"] + "&Empname=" + dr["EmpName"] + "\" rel='gb_page_center[650,400]'>View</a></td>");
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"detailedReport.aspx?empid=" + dr["EMP_ID"] + "&Id=" + dr["id"] + "&Empname=" + dr["EmpName"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
