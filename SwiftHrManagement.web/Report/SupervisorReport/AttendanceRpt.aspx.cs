...
namespace SwiftHrManagement.web.Report.SupervisorReport
{
    public partial class AttendanceRpt : BasePage
    {
        ...
        #region
        //made by bibhut
        public string LoadAttRptExcel(string from,string to,string emp_id)
        {
            return "EXEC [ProcSupervisorSummaryRpt] @FLAG='A',@FROM_DATE=" + filterstring(from) + ",@TO_DATE=" +
                   filterstring(to) + ","
        ...
        {           
            ...
            DataTable dt = _clsDao.getTable("EXEC [ProcSupervisorSummaryRpt] @FLAG='A',@FROM_DATE=" + filterstring(from) + ",@TO_DATE=" + filterstring(to) + ","
