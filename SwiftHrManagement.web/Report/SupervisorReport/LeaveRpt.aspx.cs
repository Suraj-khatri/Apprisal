...
namespace SwiftHrManagement.web.Report.SupervisorReport
{
    public partial class LeaveRpt : BasePage
    {
        ...
        {
            return "EXEC [ProcSupervisorSummaryRpt] @FLAG='L',@FROM_DATE=" + filterstring(from) + ",@TO_DATE=" +
                   filterstring(to) + ","
                   + " @SUPERVISOR_ID=" + filterstring(ReadSession().Emp_Id.ToString()) + ",@EMPLOYEE_ID=" +
                   filterstring(emp_id);
        }
        public string loadDetailRptExcel(string from,string to,string emp_id)
        {
            return "EXEC [ProcSupervisorSummaryRpt] @FLAG='LD',@FROM_DATE=" + filterstring(from) + ",@TO_DATE=" +
                   filterstring(to) + ","
                   + " @SUPERVISOR_ID=" + filterstring(ReadSession().Emp_Id.ToString()) + ",@EMPLOYEE_ID=" +
                   filterstring(emp_id);
        }
        ...
        private void loadDetailedRpt()
        {
            ...
            DataTable dt = _clsDao.getTable("EXEC [ProcSupervisorSummaryRpt] @FLAG='LD',@FROM_DATE=" + filterstring(from) + ",@TO_DATE=" + filterstring(to) + ","
        ...
        {           
            ...
            DataTable dt = _clsDao.getTable("EXEC [ProcSupervisorSummaryRpt] @FLAG='L',@FROM_DATE=" + filterstring(from) + ",@TO_DATE=" + filterstring(to) + ","
            + " @SUPERVISOR_ID=" + filterstring(ReadSession().Emp_Id.ToString()) + ",@EMPLOYEE_ID=" + filterstring(emp_id) + "");
