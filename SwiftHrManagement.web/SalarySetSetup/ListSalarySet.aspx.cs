...
{
    public partial class ListSalarySet : BasePage
    {
        RoleMenuDAOInv _roleMenuDao = null;
        public ListSalarySet()
        {
        ...
        private void init()
        {
            ...
            string sortby = "setId";
            string table_name = "SalarySet";

            String sql = "";

            sql = @"SELECT ss.setId
		                    ,position		=	s.DETAIL_TITLE
		                    ,setTitle		=	ss.setTitle
		                    ,createdDate	=	CONVERT(VARCHAR,createdDate,107)
                    FROM SalarySet ss 
                    INNER JOIN StaticDataDetail s 
                    ON ss.positionId = s.ROWID  
                    WHERE setStatus	=	'yes'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListSalarySet.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/SalarySetSetup/ManageSalarySet.aspx?setId=" + dr["setId"] + "\">View</a></td>");
                html.Append("<td><a href=\"/SalarySetSetup/ManageSalarySetDetails.aspx?setId=" + dr["setId"] +"&positionId="+dr["positionId"]+ "\">Add</a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/SalarySetSetup/ManageSalarySet.aspx");
