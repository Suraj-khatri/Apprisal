    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        { "FISCAL_YEAR_ENGLISH", "Fiscal Year English" ,       "LT"},
                        {"FISCAL_YEAR_NEPALI",  "Fiscal Year Nepali"  ,        "LT"},
                        { "EN_YEAR_START_DATE",  "English Fiscal Year Start Date" ,        "LT"},
                        {  "EN_YEAR_END_DATE",  "English Fiscal Year End Date"  ,          "LT"}
                    };
            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"FISCAL_YEAR_ENGLISH",      "Fiscal Year English",  "200",      "T"},
                        {"FISCAL_YEAR_NEPALI",       "Fiscal Year Nepali",   "200",      "T"},
            ...
            + " FLAG ='False' then 'No' end 'FLAG' FROM FiscalYear";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/StaticView/FiscalYearSetup/Manage.aspx?ID=" + dr["FISCAL_YEAR_ID"] + "\">View</a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/StaticView/FiscalYearSetup/Manage.aspx");
