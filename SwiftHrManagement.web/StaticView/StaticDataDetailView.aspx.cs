...
namespace SwiftHrManagement.web.StaticView
{
    public partial class StaticDataDetailView : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                this._dataTypeCore = this._dataTypeDAO.FindByTypeId(this.GetTypeId());
                //this.LblCountry.Text = _dataTypeCore.Type_title;
            }
            init();
        }
        #region SwiftGrid
        ...
        private void init()
        {
            ...
            rpt.InnerHtml = "<center> <br clear='all'/>" + load_report(table_name, sql, matgrid, matfilter, sortd, sortby, _page, page_size) + "</center>";
        }
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/StaticView/ManageDataDetail.aspx?ID=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a>");
                //html.Append("<td><a href=\"/StaticView/ManageDataDetail.aspx?ID=" + dr["ID"] + "\">View</a></td>");
                html.Append("</tr>");
            }
        ...
        private string make_filter_form(string table_name, string[,] matfilter)
        {
            ...
            StringBuilder html = new StringBuilder("<br clear='all'/><div class=\"col-md-12 filter-bg\">");
