...
{
    public partial class StaticDataTypeView : BasePage
    {
        RoleMenuDAOInv _roleMenuDao = null;
        public StaticDataTypeView()
        {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"TYPE_TITLE",    "Category Title",         "LT"},
                        {"TYPE_DESC",       "Category Description",  "LT"}
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"ROWID",            "Category ID",         "200",      "T"},
                        {"TYPE_TITLE",            "Category Title",         "200",      "T"},
                        {"TYPE_DESC",       "Category Description",        "200",      "T"}
                        };



            string sortby = "ID";
            string table_name = "StaticDataType";

            String sql = "";

            sql = "SELECT ROWID AS ID, TYPE_TITLE, TYPE_DESC FROM StaticDataType WHERE 1=1";
            ...
            rpt.InnerHtml = "<center class='row'>" + load_report(table_name, sql, matgrid, matfilter, sortd, sortby, _page, page_size) + "</center>";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            StringBuilder html = new StringBuilder(" <br clear='all'/><div class=\"table-responsive\"> <table class=\"table table-bordered table-striped table-condensed\">");
            ...
            for (int i = 0; i <= arr_cnt; i++)
            {
                ...
                html.Append("<th>" + sortLink + matgrid.GetValue(i, 1).ToString() + "</a></th>");
            }
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (cnt % 2 == 0)
                {
                    html.Append("<tr onMouseOver=\"this.className='GridOddRowOver'\" onMouseOut=\"this.className='GridOddRow'\" >");
                }
                else
                {
                    html.Append("<tr  onMouseOver=\"this.className='GridEvenRowOver'\" onMouseOut=\"this.className='GridEvenRow'\">");
                }
                ...
                //html.Append("<td><a href=\"/StaticView/StaticDataDetailView.aspx?ID=" + dr["ID"] + "\">View</a></td>");
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/StaticView/StaticDataDetailView.aspx?ID=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
        ...
        private string make_filter_form(string table_name, string[,] matfilter)
        {
            ...
            html.Append("<div class=\"clear-fix\"></div> <br clear='all'/>");
            for (int i = 0; i <= arr_cnt; i++)
            {
        ...
        private string get_paging_block(int _total_record, int _page, int _page_size, Boolean _show_add_button, string _add_page)
        {
            ...
            html.Append("<div class=\"form-inline text-left\"><b> Result &nbsp;" + _total_record.ToString());           
            ...
            html.Append("&nbsp;&nbsp; Per Page </b>");
            html.Append("</div>");
            html.Append("</div>");
            html.Append("<div class=\"col-md-4 text-right\">");
            ...
            html.Append("</div>");
            html.Append("</div></div><br clear='all'/>");
