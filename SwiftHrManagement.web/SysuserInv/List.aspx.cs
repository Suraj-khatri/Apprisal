...
namespace SwiftHrManagement.web.SysUser
{
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"UserName",    "User Name",         "LT"},   
                        {"E.FIRST_NAME",    "User First Name",         "LT"} , 
                        {"ssd.DETAIL_TITLE",    "Assigned Role",         "LT"}
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"AdminID",            "User ID",             "200",      "T"},
                        {"UserName",            "User Name",             "200",      "T"},
                        {"EmployeeName",       "User Full Name",        "200",      "T"},
                        {"DETAIL_TITLE",       "Assigned Role",        "200",      "T"},
                        {"status",    "Status",     "200",      "T"}//,
                        //{"user_status",    "User Status",     "200",      "T"}
                        };



            string sortby = "AdminID";
            string table_name = "ADMINS";

            String sql = "";

            sql = "SELECT DISTINCT AdminID, UserName,dbo.GetEmployeeFullNameOfId(E.Employee_Id) AS EmployeeName,ssd.DETAIL_TITLE,"
                +" CASE WHEN status = 'N' THEN 'In Active' WHEN status = 'Y' THEN 'Active' END AS status"
                //+" case when isnull(session,'') ='' then 'Logout' else 'Login' end as user_status"
                +" FROM ADMINS A WITH (NOLOCK) LEFT JOIN Employee E WITH (NOLOCK) ON CAST(E.EMPLOYEE_ID AS VARCHAR)=A.Name"
                +" LEFT JOIN user_role ur ON ur.user_id=A.AdminID LEFT JOIN StaticDataDetail ssd ON ssd.ROWID=ur.role_id"
                + " WHERE status<>'d'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
           ...
           // html.Append("<th Class=\"HeaderStyle\" align=\"left\"></th>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/SysUserInv/Manage.aspx?Admin_Id=" + Uri.EscapeUriString(StringEncryption.Encrypt(dr["AdminID"].ToString())) + "\"><i class=\"fa fa-eye\"></i></a> <a  class=\"tool btn btn-info btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Assign Role\" href=\"/SysUserInv/RoleAssign.aspx?Admin_Id=" + Uri.EscapeUriString(StringEncryption.Encrypt(dr["AdminID"].ToString())) + "\"><i class=\"fa fa-pencil\"></i></a></td>");
                //html.Append("<td><a href=\"/SysUserInv/RoleAssign.aspx?Admin_Id=" + dr["AdminID"] + "\">Assign Role</a></td>");
               // html.Append("<td><a href=\"/SysUserInv/forceLogoutForm.aspx?Admin_Id=" + dr["AdminID"] + "\">Force Logout</a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/SysUserInv/Manage.aspx");
        ...
        private string make_filter_form(string table_name, string[,] matfilter)
        {
            ...
            for (int i = 0; i <= arr_cnt; i++)
            {
                ...
                html.Append("<input type=\"text\" name=\"" + ctlName + "\" value=\"" + defValue + "\" class=\"form-control\" onclick=\"retrun false\">");
                html.Append("</div>");
                html.Append("</div>");
            }
