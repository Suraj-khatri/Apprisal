    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        {"FY_ID",            "Fiscal ID",             "200",      "T"}
                        };



            string sortby = "ID";
            string table_name = "TAXRULESETUP";

            String sql = "";

            //sql = "SELECT BRANCH_ID,BRANCH_NAME,BRANCH_SHORT_NAME,BRANCH_ADDRESS, BRANCH_PHONE,BRANCH_MOBILE,BRANCH_EMAIL,"
            //    + " BRANCH_FAX,BRANCH_COUNTRY,BRANCH_ZONE,BRANCH_DISTRICT,CONTACT_PERSON FROM BRANCHES where 1=1";
            ...
            sql = " SELECT ID,FY_ID  FROM TAXRULESETUP WHERE 1=1";
            
            //sql = "SELECT ID,FY_ID,CREATEDBY,CONVERT(VARCHAR,CREATEDDATE,107) AS CREATEDDATE FROM TAXRULESETUP WHERE 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            StringBuilder html = new StringBuilder("<table width=\"100%\" class=\"table table-bordered table-striped table-condensed\" class=\"text-center\">");
            ...
            html.Append("<th Class=\"HeaderStyle text-center\">View</th>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/TaxRuleSetup/Manage.aspx?Id=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }

            html.Append("<tr> <td class=\"text-center\" colspan=\"7\"> <strong> Page " + _page.ToString() + " Of " + total_page + "</strong></td></tr>");
            html.Append("</table>");
            html.Append("</div>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/TaxRuleSetup/Manage.aspx");
        ...
        private string make_filter_form(string table_name, string[,] matfilter)
        {
            ...
            html.Append("<div class=\"pull-right\" ><a class=\"btn btn-primary  btn-md\" class=\"text-center\" href=\"javascript:newTableToggle('td_Search', 'img_Search');\"><i class=\"fa fa-filter\" id=\"img_Search\"></i> </a></div>");
