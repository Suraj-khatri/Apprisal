...
namespace SwiftHrManagement.web.TrainingManagement.BranchTrainingParticipant
{
    public partial class ActiveProgramList : BasePage
    {
        RoleMenuDAOInv _roleMenuDao = null;
        public ActiveProgramList()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_roleMenuDao.hasAccess(ReadSession().AdminId, 37) == false)
                {
    ...
    #region SwiftGrid
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        {"TRAINING_ID",            "Category",             "200",      "T"},
                        {"TRAINING_PROGRAM_TITLE",       "Program Title",        "200",      "T"},
                        {"ACTUAL_START_DATE",    "A.Start Date",     "200",      "T"},
                        {"ACTUAL_END_DATE",    "A.End Date",     "200",      "T"},
                         {"VENUE",    "VENUE",     "200",      "T"}
                        };
            ...
            string sortby = "ID";
            string table_name = "TrainingProgram";

            String sql = "";

            sql = "SELECT P.ID, L.TRAINING_NAME AS TRAINING_ID,TRAINING_PROGRAM_TITLE, CAST(PLANNED_START_DATE AS DATE)AS"
 +" PLANNED_START_DATE, CONVERT(VARCHAR,ACTUAL_START_DATE,107) AS ACTUAL_START_DATE, CONVERT(VARCHAR,PLANNED_END_DATE,107) AS PLANNED_END_DATE,"
...
+" P.MODIFIED_DATE FROM TrainingProgram P INNER JOIN TrainingList L ON L.ID=P.TRAINING_ID WHERE P.ISACTIVE=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ActiveProgramList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Participant</th>");
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Evaluation</th>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/TrainingManagement/BranchTrainingParticipant/ViewActivePrograms.aspx?Id=" + dr["ID"] + "\">View</a></td>");
                html.Append("<td><a href=\"/TrainingManagement/BranchTrainingParticipant/List.aspx?ID=" + dr["ID"] + "\">Participant</a></td>");
                html.Append("<td><a href=\"/TrainingManagement/TrainingEvaluation/List.aspx?ID=" + dr["ID"] + "\">Evaluation</a></td>");
                html.Append("</tr>");
            }
