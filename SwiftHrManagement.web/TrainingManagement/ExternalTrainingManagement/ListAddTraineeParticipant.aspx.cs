...
{
    public partial class ListAddTraineeParticipant : BasePage
    {
        RoleMenuDAOInv _roleMenuDao = null;
        public ListAddTraineeParticipant()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_roleMenuDao.hasAccess(ReadSession().AdminId, 44) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{ 
                        ...
                        {"Nominate_within", "Nominate Date",         "ED"},
                    };
            ...
            String[,] matgrid = new String[,]{
                    ...
                    {"Nominate_within",             "Nominate Date",             "200",      "T"},
                    {"Conducted_By",                "Coducted By",               "200",      "T"},
                    {"Venue",                       "Venue",                     "200",      "T"},
                    {"City",                        "City",                      "200",      "T"},
                    {"Country",                     "Country",                   "200",      "T"},
                        };

            string sortby = "external_traingin_id";
            string table_name = "externalTrainingInfo";

            String sql = "";

            //if (ReadSession().UserId.ToString() == "admin")
            //{
            ...
            //        + " ,[Country],[No_of_days],[No_of_Hours],[Cost_Estimate],[Program_Description],[created_by],[created_date],isnull(cast(Approved_dateBy_Coo as varchar),0) as Approved_dateBy_Coo"
            //        + " ,GETDATE() as nominateWithin,convert(varchar,Nominate_within,101) as Nominate_within from [externalTrainingInfo] EI "
            //        //+ " join extTraineeRecommendedBy ER on EI.external_traingin_id=ER.external_training_id "
            //        + " where 1=1 ";
            //}
            //else
            {
                ...
                sql = "	select external_traingin_id,[Program_Name],convert(varchar,[Start_date],101) as Start_date,convert(varchar,[End_Date],101) as End_Date,[Conducted_By],[Venue],[Total_Capacity],[City]"
                    ...
                    + " ,GETDATE() as nominateWithin,convert(varchar,Nominate_within,101) as Nominate_within from [externalTrainingInfo] EI "
                    + " join extTraineeRecommendedBy ER on EI.external_traingin_id=ER.external_training_id "
                    + " where ER.employee in (" + filterstring(ReadSession().Emp_Id.ToString()) + ") ";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Assign Trainee</th>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (dr["Approved_dateBy_Coo"].ToString() != "0")
                {
                    html.Append("<td><a href=\"/TrainingManagement/ExternalTrainingManagement/ManageApproveTrainee.aspx?external_traingin_id=" + dr["external_traingin_id"] + "&flag=z\">Approved</a></td>");
                    html.Append("<td><a href=\"/TrainingManagement/ExternalTrainingManagement/ManageAssignTrainee.aspx?external_traingin_id=" + dr["external_traingin_id"] + "&flag=u\">Assign Trainee</a></td>");
                }
                else if (Convert.ToDateTime(dr["nominateWithin"]) >= Convert.ToDateTime(dr["Nominate_within"]))
                {
                    html.Append("<td><a href=\"/TrainingManagement/ExternalTrainingManagement/ManageExtTraining.aspx?external_traingin_id=" + dr["external_traingin_id"] + "&flag=s\">Cancel</a></td>");
                    html.Append("<td><a href>N/A</a></td>");

                }
                else
                {
                    html.Append("<td><a href=\"/TrainingManagement/ExternalTrainingManagement/ManageExtTraining.aspx?external_traingin_id=" + dr["external_traingin_id"] + "&flag=s\">View</a></td>");
                    html.Append("<td><a href=\"/TrainingManagement/ExternalTrainingManagement/ManageAssignTrainee.aspx?external_traingin_id=" + dr["external_traingin_id"] + "&flag=s\">Assign Trainee</a></td>");
                }
               html.Append("</tr>");
            }
