...
{
    public partial class ListApproveTrainee : BasePage
    {
         RoleMenuDAOInv _roleMenuDao = null;
         public ListApproveTrainee()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_roleMenuDao.hasAccess(ReadSession().AdminId, 45) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                    ...
                    {"Nominate_within",             "Nominate Date",             "200",      "D"},
            ...
            sql = "select external_traingin_id,[Program_Name],convert(varchar,[Start_date],101) as Start_date,convert(varchar,[End_Date],101) as End_Date,"
                    +" [Conducted_By],[Venue],[Total_Capacity],[City],[Country],[No_of_days],[No_of_Hours],"
                    + " [Cost_Estimate],[Program_Description],GETDATE() as nominateWithin ,convert(varchar,Nominate_within,101) as Nominate_within from externalTrainingInfo WITH (NOLOCK)"
                    +" WHERE external_traingin_id  IN "
                    +" (select distinct external_training_id  from externalTraineeAssign WITH (NOLOCK)"
                    + " WHERE request_sendDate_Coo IS NOT NULL AND Approved_dateBy_Coo IS NULL)";

            //if (ReadSession().UserId.ToString() != "admin")
                sql = sql + "and Approved_by=" + filterstring(ReadSession().Emp_Id.ToString());
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/TrainingManagement/ExternalTrainingManagement/ManageApproveTrainee.aspx?external_traingin_id=" + dr["external_traingin_id"] + "\">Approve</a></td>");
                html.Append("</tr>");
            }
