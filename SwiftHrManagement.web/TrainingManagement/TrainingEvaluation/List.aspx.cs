...
namespace SwiftHrManagement.web.TrainingManagement.TrainingEvaluation
{
    public partial class List :BasePage
    {
        ...
        {
            this._trainProgramCore = this._trainProgramDao.FindByTrainProgramId(this.GetProgramID());
            this.LblProgramTitle.Text = _trainProgramCore.TrainingProgramTitle;
            this.LblTrainList.Text = _trainProgramCore.TrainingId;
        }
        #region SwiftGrid
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"S.DETAIL_TITLE",    "Evaluation Rate",         "LT"},
                        {"TE.CREATED_BY",    "Evaluated By",         "LT"}
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"EVALUATION_RATE",       "Evaluation Rate",        "200",      "T"},
                        {"EVALUATION",       "Evaluation Description",        "200",      "T"},
                        {"CREATED_BY",       "Evaluated By",        "200",      "T"},
                        {"CREATED_DATE",       "Evaluated Date",        "200",      "D"}
            ...
            string sortby = "ID";
            string table_name = "TrainingEvaluation";

            String sql = "";

            sql = "SELECT TE.ID,S.DETAIL_TITLE AS EVALUATION_RATE, TE.EVALUATION, TE.CREATED_BY,CONVERT(VARCHAR,TE.CREATED_DATE,107) AS CREATED_DATE "
            +" from TrainingEvaluation TE INNER JOIN StaticDataDetail S ON S.ROWID=TE.EVALUATION_RATE "
            +" where TE.TRAINING_PROGRAM_ID=" + filterstring(GetProgramID().ToString()) + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/TrainingManagement/TrainingEvaluation/Manage.aspx?ID=" + dr["ID"] + "&ProgramId=" + GetProgramID() + "\">View</a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/TrainingManagement/TrainingEvaluation/Manage.aspx?ProgramId=" + GetProgramID() + "");
