    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"TP.TRAINING_PROGRAM_TITLE",    "Program Name",         "LT"}                 
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"TRAINING_PROGRAM_ID",            "Program Name",             "200",      "T"},
                        {"FEEDBACK",       "Feedback Details",        "200",      "T"},
                        {"CREATED_BY",    "Feedback By",     "200",      "T"},
                        {"CREATED_DATE",    "Feedback Date",     "200",      "D"}
                        };



            string sortby = "ID";
            string table_name = "TrainingFeedback";

            String sql = "";

            sql = "SELECT TF.ID, TP.TRAINING_PROGRAM_TITLE AS TRAINING_PROGRAM_ID, FEEDBACK, TF.CREATED_BY,CONVERT(VARCHAR,TF.CREATED_DATE,107) AS CREATED_DATE from "
            + " TrainingFeedback TF inner join TrainingProgram TP ON TP.ID=TF.TRAINING_PROGRAM_ID where 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/TrainingManagement/TrainingFeedback/Manage.aspx");
