    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                            ...
                            {"DEPARTMENT_NAME",       "Department Name",        "200",      "T"},
                            {"POST",       "Position",        "200",      "T"},
                            {"SUPERVISOR",       "Immediate Supervisor",        "200",      "T"},
                            {"SUBORDINATE",       "Immediate Subordinate",        "200",      "T"},
                            {"CREATED_DATE",       "Created Date",        "200",      "D"}
                        };

            string sortby = "ID";
            string table_name = "TRAINING_NEED_ASSESSMENT";

            String sql = "";

            if (FindHrByDeptId(ReadSession().Department.ToString()).ToUpper() == "HRM")
            {
                sql = "SELECT TNA.ID,E.EMP_CODE,E.FIRST_NAME+' '+E.MIDDLE_NAME+' '+LAST_NAME AS EMP_NAME,B.BRANCH_NAME,D.DEPARTMENT_NAME,"
                  + " dbo.GetDetailTitle(TNA.POSITION_ID) AS POST,dbo.GetEmployeeFullNameOfId(IMM_SUP_ID) as SUPERVISOR,"
                  + " dbo.GetEmployeeFullNameOfId(IMM_SUB_ID) as SUBORDINATE,CONVERT(VARCHAR,TNA.CREATED_DATE,107) AS CREATED_DATE"
                  + " FROM TRAINING_NEED_ASSESSMENT TNA INNER JOIN Employee E ON E.EMPLOYEE_ID=TNA.EMPLOYEE_ID"
                  + " INNER JOIN Branches B ON B.BRANCH_ID=TNA.BRANCH_ID INNER JOIN Departments D ON D.DEPARTMENT_ID=TNA.DEPT_ID"
                  + " WHERE 1=1";
            }
            else
            {
                sql = "SELECT TNA.ID,E.EMP_CODE,E.FIRST_NAME+' '+E.MIDDLE_NAME+' '+LAST_NAME AS EMP_NAME,B.BRANCH_NAME,D.DEPARTMENT_NAME,"
                        ...
                        + " FROM TRAINING_NEED_ASSESSMENT TNA INNER JOIN Employee E ON E.EMPLOYEE_ID=TNA.EMPLOYEE_ID"
                        + " INNER JOIN Branches B ON B.BRANCH_ID=TNA.BRANCH_ID INNER JOIN Departments D ON D.DEPARTMENT_ID=TNA.DEPT_ID"
                        + " WHERE TNA.EMPLOYEE_ID="+ReadSession().Emp_Id;
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"Modify.aspx?Id=" + dr["ID"] + "\">View</a></td>");
                html.Append("</tr>");
            }
