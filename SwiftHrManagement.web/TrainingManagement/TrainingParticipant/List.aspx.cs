...
namespace SwiftHrManagement.web.TrainingManagement.TrainingParticipant
{
    public partial class List : BasePage
    {
        RoleMenuDAOInv _roleMenuDao = null;
        TrainingProgramCore _programCore = null;
        TrainingProgramDAO _programDao = null;
        public List()
        {
            _roleMenuDao = new RoleMenuDAOInv();
            _programCore = new TrainingProgramCore();
            _programDao = new TrainingProgramDAO();
        }
        ...
        {
            this._programCore = this._programDao.FindByTrainProgramId(this.GetProgramID());
            this.LblProgramTitle.Text = _programCore.TrainingProgramTitle;
            this.LblTrainList.Text = _programCore.TrainingId; 
        }
        #region SwiftGrid
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"IS_APPROVED",       "Is Approved?",        "200",      "T"}
                        };



            string sortby = "ID";
            string table_name = "TrainingParticipants";

            String sql = "";

            sql = " SELECT TP.ID,E.FIRST_NAME + ' ' + E.MIDDLE_NAME + ' ' +  E.LAST_NAME AS EmpName,"
                    +" case when TP.IS_APPROVED='false' then 'No' else 'Yes' end as IS_APPROVED FROM TrainingParticipants TP"
	                +" INNER join TrainingProgram T on T.ID=TP.TRAINING_PROGRAM_ID LEFT JOIN EMPLOYEE E ON E.EMPLOYEE_ID =  TP.STAFF_ID INNER JOIN"
                    + " TRAININGLIST TL ON TL.ID = T.TRAINING_ID WHERE TP.TRAINING_PROGRAM_ID="+filterstring(GetProgramID().ToString())+"";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "List.aspx?sortd=" + sortd + "&sortby={sortby}&ID="+GetProgramID()+"";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/TrainingManagement/TrainingParticipant/Manage.aspx?ID=" + dr["ID"] + "&ProgramId=" + GetProgramID() + "\">View</a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/TrainingManagement/TrainingParticipant/Manage.aspx?ProgramId=" + GetProgramID() + "");
