    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"L.TRAINING_NAME",    "Category Name",         "LT"},   
                        {"TRAINING_PROGRAM_TITLE",    "Program Name",         "LT"}                   
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"TRAINING_ID",            "Category",             "200",      "T"},
                        {"TRAINING_PROGRAM_TITLE",       "Program Name",        "200",      "T"},
                        {"ACTUAL_START_DATE",    "A.Start Date",     "200",      "D"},
                        {"ACTUAL_END_DATE",    "A.End Date",     "200",      "D"},
                        {"VENUE",       "Venue",        "200",      "T"}
                        };
            ...
            sql = "SELECT P.ID, L.TRAINING_NAME AS TRAINING_ID,TRAINING_PROGRAM_TITLE,"
            + " CONVERT(VARCHAR,ACTUAL_START_DATE,107) AS ACTUAL_START_DATE,"
            + " CONVERT(VARCHAR,ACTUAL_END_DATE,107) AS ACTUAL_END_DATE,VENUE, CITY,ISNULL(P.ISACTIVE,0) AS ISACTIVE,"
            + " P.MODIFIED_DATE FROM TrainingProgram P INNER JOIN TrainingList L ON L.ID=P.TRAINING_ID where 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/TrainingManagement/TrainingProgram/Manage.aspx?ID=" + dr["ID"] + "\">View</a></td>");
                html.Append("<td><a href=\"/TrainingManagement/TrainingParticipant/List.aspx?ID=" + dr["ID"] + "\">Participant</a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/TrainingManagement/TrainingProgram/Manage.aspx");
