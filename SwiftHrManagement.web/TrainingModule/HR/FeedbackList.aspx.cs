    ...
    public partial class FeedbackList : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"dbo.GetEmployeeFullNameOfId(empId)",    "Nominee Name",         "LT"},
                        {"B.programName",    "Program Name",         "LT"}
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"emp",            "Nominee Name",             "200",      "T"},
                        {"program",            "Program Name",             "200",      "T"},
                        {"branch",            "Branch",             "200",      "T"},
                        {"dept",            "Department",             "200",      "T"},
            ...
            string sortby = "trainingFbId";
            string table_name = "training";

            String sql = "";

            sql = @"SELECT trainingFbId,trainingProgramId,dbo.GetEmployeeFullNameOfId(empId) emp,
dbo.GetBranchName(branch) branch,dbo.GetDeptName(dept) dept,
dbo.GetEmployeeFullNameOfId(A.createdBy) createdBy,
convert(varchar,A.createdDate,107) createddate,
B.programName program from TrainingFeedback A 
inner join training B on A.trainingProgramId = B.ID WHERE B.id=" + filterstring(GetId()) + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "FeedbackList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\"href=\"/TrainingModule/NORMAL/ViewFeedback.aspx?flag=h&feedback_id=" + dr["trainingFbId"] + "\"><i class=\"fa fa-plus\"></i></a></td>");
                html.Append("</tr>");
            }
