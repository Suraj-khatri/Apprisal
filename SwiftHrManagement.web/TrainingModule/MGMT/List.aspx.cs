...
namespace SwiftHrManagement.web.TrainingModule.MGMT
{
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            {
                sql = @"select id,programType,programName,convert(varchar,startDate,107) as startDate,convert(varchar,endDate,107) as endDate
                    ,conductedBy,venue,noOfDays,convert(varchar,nomineeWithin,107) nomineeWithin,status
                     from training where 1=1";
            }
            else
            {
                sql = @"select id,programType,programName,convert(varchar,startDate,107) as startDate,convert(varchar,endDate,107) as endDate
                    ,conductedBy,venue,noOfDays,convert(varchar,nomineeWithin,107) nomineeWithin,status
                     from training where status='Forwarded' and forwardWith=" + filterstring(ReadSession().Emp_Id.ToString()) + "";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"  style=\"width:160px;\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/TrainingModule/MGMT/Manage.aspx?ID=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"File\" href=\"/TrainingModule/UPLOAD/Manage.aspx?ID=" + dr["ID"] + "\"><i class=\"fa fa-file-excel-o\"></i></a>");
                html.Append("<a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View Schedule\" href=\"/TrainingModule/Schedule/ViewSchedule.aspx?ID=" + dr["ID"] + "\"><i class=\"fa fa-clock-o\"></i></a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/TrainingModule/MGMT/Manage.aspx");
