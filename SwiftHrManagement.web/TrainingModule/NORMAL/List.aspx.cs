    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"conductedBy",    "Conducted By",         "LT"},
                        {"programName",    "Program Name",         "LT"},
                        {"venue",    "Venue",         "LT"},
                        {"status",    "Status",         "LT"}
                    };
            ...
            String[,] matgrid = new String[,]{
                        {"conductedBy",            "Conducted By",             "200",      "T"},
                        {"programName",            "Program Name",             "200",      "T"},
                        {"startDate",            "Start Date",             "200",      "D"},
                        {"endDate",            "End Date",             "200",      "D"},
                        {"venue",            "Venue Name",             "200",      "T"},
                        {"noOfDays",            "No Of Days",             "200",      "T"},
                        {"nomineeWithin",            "Nominee Within",             "200",      "T"},
                        {"status",            "Status",             "200",      "T"}
                        };
            ...
            if (getEmpId() > 0)
            {
                emp_id = getEmpId().ToString();
            }
            else
            {
                emp_id=ReadSession().Emp_Id.ToString();
            }
            String sql = "";

//            sql = @"select a.id,programType,programName,convert(varchar,startDate,107) as startDate,convert(varchar,endDate,107) as endDate
//                            ,conductedBy,venue,noOfDays,convert(varchar,nomineeWithin,107) nomineeWithin,status,
//                            isnull(cast(c.trainingFbId as varchar),'no') is_feedback
//                            from training a inner join trainingParticipation b on a.id=b.trainingId 
//                            left join TrainingFeedback c on c.trainingProgramId=a.id
//                            where b.employeeId=" +emp_id+"";

            sql = @"select id,programType,programName,startDate,endDate,conductedBy,venue,noOfDays,nomineeWithin,status,
isnull(cast(trainingId as varchar),'no')is_feedback from 
(
	select a.id,programType,programName,convert(varchar,startDate,107) as startDate,
	convert(varchar,endDate,107) as endDate
	,conductedBy,venue,noOfDays,convert(varchar,nomineeWithin,107) nomineeWithin,status
	from training a inner join trainingParticipation b on a.id=b.trainingId 
	where b.employeeId=" + emp_id + ")a left join (select trainingId from trainingParticipation c inner join TrainingFeedback d on d.trainingProgramId=c.trainingId and c.employeeId=d.empId  where d.empId=" + emp_id + ")b on a.id=b.trainingId";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "List.aspx?emp_id="+getEmpId()+"&sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"  style=\"width:160px;\">");
                html.Append("<a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/TrainingModule/NORMAL/Manage.aspx?emp_id=" + getEmpId() + "&ID=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a>");
                if (dr["is_feedback"].ToString() == "no" && dr["status"].ToString() == "Approved" || dr["status"].ToString() == "Closed")
                {
                    html.Append("<a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/TrainingModule/NORMAL/ManageFeedback.aspx?ID=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a>");
                }
                else if (dr["is_feedback"].ToString() != "no")
                {
                    html.Append("<a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/TrainingModule/NORMAL/ViewFeedback.aspx?ID=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a>");
                }
                else
                {
                    html.Append("N/A");
                }
                html.Append("<a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\"  href=\"/TrainingModule/Schedule/ViewSchedule.aspx?ID=" + dr["ID"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }

            html.Append(" <tr> <td colspan=\"9\" align=\"center\"> <strong> Page " + _page.ToString() + " Of " + total_page + "</strong></td></tr>");
            html.Append("</table>");
            html.Append("</div>");
        ...
        private string make_filter_form(string table_name, string[,] matfilter)
        {
            ...
            for (int i = 0; i <= arr_cnt; i++)
            {
                ...
                html.Append("<input type=\"text\" name=\"" + ctlName + "\" class=\"form-control\" value=\"" + defValue + "\">");
                html.Append("</div>");
                html.Append("</div>");
            }
            html.Append("<div class=\" col-md-3\">");
