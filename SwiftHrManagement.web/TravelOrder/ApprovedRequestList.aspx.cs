...
{
    public partial class ApprovedRequestList : BasePage
    {
        RoleMenuDAOInv _roleMenuDao = null;
       

        public ApprovedRequestList()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (_roleMenuDao.hasAccess(ReadSession().AdminId, 173) == false)
            {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{                        
                        {"sd.detail_title",        "Travel Type",           "LT"}, 
                        {"TADA_STATUS",            "Status",                "LT"},
                        {"REQUESTED_BY",           "Requested By",          "LT"}
                        };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        //{"emp_id",    "Employee ID",    "100",     "LT"},
                        {"REQUESTED_BY",            "Requested By",             "200",      "T"},
                        {"REASON_FOR_TRAVEL",           "Travel Type",               "200",      "T"},
                        {"Destination_country",         "Destination Country",  "200",          "T"},
                        {"destination_city",            "Destination City",     "200",           "T"},
                        {"duration_from",               "From Date",                "200",      "D"},                        
                        {"duration_to",                 "To Date",                  "200",      "D"},
            ...
            sql =
                @" select id,emp_id,first_name+' '+MIDDLE_NAME+' '+LAST_NAME AS REQUESTED_BY,
                    sd.detail_title AS REASON_FOR_TRAVEL,dbo.GetDetailtitle(destination_country) Destination_country,destination_city,
                    CONVERT(VARCHAR,duration_from,107)  duration_from,
                    CONVERT(VARCHAR,duration_to,107)  duration_to ,
                    DATEDIFF(DAY,duration_from,duration_to)+1 
                    as REQUESTED_DAYS,tada_status AS TADA_STATUS from TADA ta left 
                    Join Employee e on e.EMPLOYEE_ID=ta.emp_id
                    left join Admins a on e.EMPLOYEE_ID=a.Name
                    left join user_role ur on ur.user_id=a.AdminID
                    left join StaticDataDetail sd on sd.ROWID=ta.reason_for_travel
                    left join StaticDataDetail ss on ss.rowid=ur.role_id
                    where tada_status<>'pending'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ApprovedRequestList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a class=\"tool btn btn-primary btn-xs\"  href=\"/TravelOrder/ApprovedManage.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></a></td>");
                html.Append("</tr>");
            }
        ...
        private string make_filter_form(string table_name, string[,] matfilter)
        {
            ...
            for (int i = 0; i <= arr_cnt; i++)
            {
                ...
                html.Append("<input type=\"text\" class=\"form-control\" name=\"" + ctlName + "\" value=\"" + defValue + "\">");
                html.Append("</div>");
                html.Append("</div>");
            }
            html.Append("<div class=\" col-md-3\">");
