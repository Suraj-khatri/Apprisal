    ...
    public partial class List :BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        ...
                        {"dbo.GetDetailTitle(zoneId)",    "Zone Name",         "LT"},
                        {"dbo.GetDetailTitle(countriesId)",    "Countries Name",         "LT"}
                        };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{                         
                        {"id",        "S.NO",        "100",      "M"},
                        {"zoneName",    "Zone Name",    "100",     "LT"},
                        {"countriesName",        "Countries Name",        "200",      "T"},
            ...
            string sortby = "id";
            string table_name = "TadaClassificationOfAreas";
            ...
            String sql = "";
            sql = @" select id,dbo.GetDetailTitle(zoneId)zoneName,
                    dbo.GetDetailtitle(countriesId)countriesName,dbo.GetEmployeeFullNameOfId(createdBy) createdBy,
                    CONVERT(varchar,createdDate,107) createdDate,isActive
                    from TadaClassificationOfAreas where 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"ChangeZoneCountries.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/TravelOrder/AreasOfClassification/AreaOfClassification.aspx");
