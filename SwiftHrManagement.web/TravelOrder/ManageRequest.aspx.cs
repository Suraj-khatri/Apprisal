...
namespace SwiftHrManagement.web.TravelOrder
{
    public partial class ManageRequest : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_roleMenuDao.hasAccess(ReadSession().AdminId, 171) == false)
                {
                    Response.Redirect("/Error.aspx");
                }
                LblEmpName.Text = CLsDAo.GetSingleresult("SELECT RTRIM(dbo.GetEmployeeFullNameOfId(" + filterstring(ReadSession().Emp_Id.ToString()) + "))");
                hdnID.Value = ReadSession().Emp_Id.ToString();
                ...
                // DdlLeaveApplied.Enabled = false;o
            }
        ...
        {
            CLsDAo.CreateDynamicDDl(Ddlcurrency, "Exec Proc_Travel @flag='fc', @emp_id=" + filterstring(getEmpIdfromInfo(LblEmpName.Text)) + ", @destination_country=" + filterstring(DdlCountry.Text) + "", "Rowid", "DETAIL_TITLE", "", "Select");
        }

        //private void PopluateAuthorisedBy()
        //{
        //    CLsDAo.CreateDynamicDDl(DdlAuthorisedby, "exec Proc_TadaAuthoriseBy @flag='s', @empid=" + filterstring(ReadSession().Emp_Id.ToString()) + "", "SUPERVISOR", "Supervisor_Name", "", "Select");
        //}
        ...
        {
            return CLsDAo.GetSingleresult("EXEC Proc_travel @flag='cl',@emp_id=" + filterstring(getEmpIdfromInfo(LblEmpName.Text)) + ",@extension=" + filterstring(this.Ddlex.SelectedItem.Value) + ", @durex_from=" + filterstring(this.txtdurex_from.Text) + ",@durex_to=" + filterstring(this.txtdurexto.Text) + "");
        }
        ...
        protected void txtEmpName_TextChanged(object sender, EventArgs e)
        {
            ...
            DataTable dt = new DataTable();
            dt = CLsDAo.getDataset("EXEC proc_travel @flag='b', @emp_id=" + emp_id + "").Tables[0];
            DataRow dr = null;
            if (dt.Rows.Count > 0)
            {
                dr = dt.Rows[0];
            ...
            // string sql = "exec proc_travel @emp_id";
            this.txtbranch.Text = dr["branch_name"].ToString();
            this.txtdepartment.Text = dr["department_name"].ToString();
            this.txtposition.Text = dr["detail_title"].ToString();
            //PopluateAuthorisedBy();
        ...
        private void SaveButtonVisible()
        {
            if (txtAuthorisedBy.Text == string.Empty)
            {
