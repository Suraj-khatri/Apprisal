...
{
    public partial class PendingRequestList : BasePage
    {
        ...
        {
            if (_roleMenuDao.hasAccess(ReadSession().AdminId, 172) == false)
            {
        ...
        private void init(long EmpId)
        {
            ...
            sql =
                @"SELECT 
			             ta.id
			            ,emp_id
			            ,FIRST_NAME+' '+MIDDLE_NAME+' '+LAST_NAME AS REQUESTED_BY
			            ,sd.detail_title AS REASON_FOR_TRAVEL
			            ,CONVERT(VARCHAR,duration_from,107)  [duration_from]
			            ,CONVERT(VARCHAR,duration_to,107)  duration_to
			            ,dbo.GetDetailtitle(destination_country) Destination_country
			            ,destination_city
			            ,DATEDIFF(DAY,duration_from,duration_to)+1 AS REQUESTED_DAYS
			            ,tada_status AS TADA_STATUS
		            FROM TADA ta 
                    LEFT JOIN Employee e ON e.EMPLOYEE_ID=ta.emp_id 
                    LEFT JOIN StaticDataDetail sd ON sd.ROWID=ta.reason_for_travel
                    INNER JOIN TADA_Authorization tda ON tda.tada_id = ta.id
		            WHERE tda.Authorised_By = "+ReadSession().Emp_Id.ToString()+" and ta.id = tda.tada_id and tada_status = 'pending'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "PendingRequestList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a class=\"tool btn btn-primary btn-xs\" href=\"/TravelOrder/ApprovedManage.aspx?flagID=1&id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
