...
namespace SwiftHrManagement.web.TravelOrder.Reimbursement
{
    public partial class ApproveList : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (_roleMenuDao.hasAccess(ReadSession().AdminId, 177) == false)
            {
        ...
        private void init(long EmpId)
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"status_tr",               "Status",               "100",              "T"}
                        
                        };

            string sortby = "id";
            string table_name = "tadaReimbersement";
            ...
            sql = @"SELECT Distinct
					...
					FROM TADA ta
					INNER JOIN tadaReimbersement tr ON ta.id=tr.tadaid
					LEFT JOIN Employee e ON e.EMPLOYEE_ID=ta.emp_id
					LEFT JOIN StaticDataDetail sd ON sd.ROWID=ta.reason_for_travel
					LEFT JOIN TADA_Authorization t ON t.tada_id = ta.id
					WHERE isdelete='N' AND t.authorised_by=" + EmpId;
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ApproveList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Edit\" href=\"/TravelOrder/Reimbursement/ApprovalPage.aspx?flagID=1&id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }

            html.Append("<tr> <td align=\"center\" colspan=\"5\"> <strong> Page " + _page.ToString() + " Of " + total_page + "</strong></td></tr>");
            html.Append("</table>");
            html.Append("</div>");
