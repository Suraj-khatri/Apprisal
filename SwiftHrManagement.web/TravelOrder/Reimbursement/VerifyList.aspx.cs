...
namespace SwiftHrManagement.web.TravelOrder.Reimbursement
{
    public partial class VerifyList :BasePage
    {
        ...
        {
            if (_roleMenuDao.hasAccess(ReadSession().AdminId, 178) == false)
            {
        ...
        private void init(long EmpId)
        {
            ...
            sql =
                @"  select Distinct
						ta.id,tada_status,status_tr,first_name+' '+MIDDLE_NAME+' '+LAST_NAME AS REQUESTED_BY,
						sd.detail_title AS REASON_FOR_TRAVEL,CONVERT(VARCHAR,duration_from,107) duration_from,
						dbo.Getdetailtitle(destination_country) Destination_country,destination_city,
	                    ...
	                    where status_tr<>'Pending' and isdelete='N' ";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "VerifyList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/TravelOrder/Reimbursement/ApprovalPage.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
