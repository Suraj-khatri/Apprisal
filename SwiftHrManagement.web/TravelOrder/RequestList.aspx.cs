...
namespace SwiftHrManagement.web.TravelOrder
{
    public partial class RequestList : BasePage
    {
        ...
        private void init()
        {
            ...
            {
                sql =
                    @"select id,emp_id,e.first_name+' '+e.middle_name+' '+e.last_name
                    ...
                    on e.employee_id=emp_id inner join StaticDataDetail sd on sd.ROWID=destination_country where Ismanual is null";
            }

            else
            {
                sql = @"select id,emp_id,e.first_name+' '+e.middle_name+' '+e.last_name
                    ...
                    inner join Fiscal_Month F on cast((TADA.duration_from)as Date)>=f.engDateBaisakh
                       where  employee_id=" + ReadSession().Emp_Id + " and f.DefaultYr='1'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a onclick=alert(); href=\"/TravelOrder/TadaExtention.aspx?id=" + dr["id"] + "&status=" + dr["tada_status"] + "\">Extention</a></td>");
                html.Append("<td><a class=\"tool btn btn-primary btn-xs\" href=\"ManageRequest.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/TravelOrder/ManageRequest.aspx");
