        ...
        
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        ...
                        {"dbo.GetDetailTitle(categoryId)",    "Category Name",         "LT"},
                        {"dbo.GetDetailTitle(positionId)",    "Position Name",         "LT"}
            ...
            String[,] matgrid = new String[,]{                         
                        {"id",              "SNo",             "100",      "M"},
                        {"Category_For",    "Category For",    "100",      "T"},
                        {"categoryName",    "Category Name",    "100",      "T"},
                        {"positionName",    "Position Name",    "200",      "T"},
                        {"createdBy",       "Created By",       "200",      "T"},
                        {"CreatedDate",     "Created Date",     "200",      "D"},
                        {"isActive",        "Is Active",        "100",      "T"}
                        };

            string sortby = "id";
            string table_name = "tadapositioncategory";
            ...
            sql = @"select id,dbo.GetDetailTitle(category_for) Category_For,dbo.GetDetailTitle(categoryId) categoryName,dbo.GetDetailTitle(positionId) positionName
                  ,dbo.GetEmployeeFullNameOfId(createdBy) createdBy,CONVERT(varchar,createdDate,107) createdDate,isActive
                  from tadapositioncategory where 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            //********************************************//
            //string getstatus =
            //    ClsDAO.GetSingleresult("select tada_status from tada where emp_id=" +
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\"  href=\"ChangePositionCatagory.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                //html.Append("<td><a href=\"ManageReimbursement.aspx?id=" + dr["id"] + "\">Reimbursement</a></td>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/TravelOrder/StaffClassification/ClassificationOfStaff.aspx");
