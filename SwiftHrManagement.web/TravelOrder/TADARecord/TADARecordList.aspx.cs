...
{
    public partial class TADARecordList : BasePage
    {
      ...
      + " Departments D ON D.DEPARTMENT_ID=E.DEPARTMENT_ID Inner join StaticDataDetail sdd on E.emp_status=sdd.ROWID WHERE EMPLOYEE_ID<>1000 AND E.EMP_STATUS IN (458,459) ";
            ...
            if (ReadSession().AdminId == 1)
            {
                sql = @"select
					 ta.id,
					 ta.emp_id,
                      Ismanual,
					empname = e.first_name+''+e.middle_name+''+e.last_name
                     ,ta.destination_city,
                     dbo.GetDetailtitle(ta.reason_for_travel) Reason_Travel,
                     sd.DETAIL_TITLE,
                    CONVERT(VARCHAR,ta.duration_from,107)  [duration_from],
                    CONVERT(VARCHAR,ta.duration_to,107) [duration_to],ta.tada_status 
                    from TADA ta
                    left join employee e  on e.employee_id=ta.emp_id 
                    left join StaticDataDetail sd on sd.ROWID=ta.destination_country 
                    where ta.Ismanual='N'";
            }

            else
            {
                sql = @"select ta.id,emp_id,e.first_name+' '+e.middle_name+' '+e.last_name
                    as empname,
                    ta.destination_city,
                    sd.DETAIL_TITLE,
                    CONVERT(VARCHAR,ta.duration_from,107)  [duration_from],
                    CONVERT(VARCHAR,ta.duration_to,107) [duration_to],
                    ta.tada_status,dbo.GetDetailtitle(ta.reason_for_travel) Reason_Travel 
                    from TADA ta 
                    left join employee e  on e.employee_id=emp_id 
                    left join StaticDataDetail sd on sd.ROWID=destination_country 
                    where ta.Ismanual='N' and employee_id=" + ReadSession().Emp_Id;
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "TADARecordList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Extention</th>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //html.Append("<td><a onclick=alert(); href=\"/TravelOrder/Reimbursement/Manage.aspx?id=" + dr["id"] + "&status=" + dr["tada_status"] + "\">Reimbursement</a></td>");
                ////&reimId="+dr["status"]+"
                //html.Append("<td><a href=\"AdvanceApproval.aspx?id=" + dr["id"] + "\">Request</th>");
                html.Append("<td onclick=alert(); class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Extention\"  href=\"/TravelOrder/TadaExtention.aspx?id=" + dr["id"] + "&status=" + dr["tada_status"] + "&Manual=" + dr["Ismanual"] + "\"><i class=\"fa fa-cog\"></i></a></td>");

                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"ManageTADARecord.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");

                //html.Append("<td><a onclick=alert(); href=\"/TravelOrder/TadaExtention.aspx?id=" + dr["id"] + "&status=" + dr["tada_status"] + "&Manual=" + dr["Ismanual"] + "\">Extention</a></td>");
                //html.Append("<td><a href=\"ManageTADARecord.aspx?id=" + dr["id"] + "\">View</a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/TravelOrder/TADARecord/ManageTADARecord.aspx");
