...
namespace SwiftAssetManagement.Voucher
{
    public partial class AddOtherInfo : BasePage
    {
        ...
        private void getProductInfo()
        {
            ...
            dt = _clsdao.getDataset("SELECT P.product_desc as 'Product Code',T.qty as Qty FROM Temp_Purchase T"
            +" INNER JOIN IN_PRODUCT P ON P.id=T.product_code WHERE flag='p' and T.id='"+GetId()+"'").Tables[0];
        ...
        {
            _clsdao.runSQL("delete from Temp_Purchase_Other where is_approved is null");
        }
        ...
        private void manageOtherInfo()
        {
            ...
            {
                string totQty = _clsdao.GetSingleresult("select ISNULL(SUM(qty),0) as totQty from Temp_Purchase_Other where temp_purchase_id ='" + GetId() + "' and session_id="+filterstring(ReadSession().Sessionid)+"");

                if (Double.Parse(lblqty.Text) >= (Double.Parse(totQty)+Double.Parse(qty.Text)))
                {
                    _clsdao.runSQL("INSERT INTO Temp_Purchase_Other(temp_purchase_id,sn_from,sn_to,batch,qty,session_id)VALUES(" + filterstring(GetId().ToString()) + "," + filterstring(sn_from.Text) + "," + filterstring(sn_to.Text) + "," + filterstring(batch.Text) + "," + filterstring(qty.Text) + ","+filterstring(ReadSession().Sessionid)+")");
                    getAddItems();
                    resetAdd();
                }
                else
        ...
        {                   
            double totQty = 0;
            DataTable dt = new DataTable();
            dt = _clsdao.getDataset("select id,sn_from as 'Serial From',sn_to as 'Serial To',batch as Batch,qty as Qty from Temp_Purchase_Other where temp_purchase_id='"+GetId()+"' and session_id="+filterstring(ReadSession().Sessionid)+"").Tables[0];
        ...
        protected void BtnDelProduct_Click(object sender, EventArgs e)
        {
            if (DelProduct != "")
            {
                string sql = "DELETE FROM Temp_Purchase_Other WHERE id='" + DelProduct + "'";
                _clsdao.runSQL(sql);
                getAddItems();
            }
        }
        ...
        {            
           string totQty= _clsdao.GetSingleresult("select ISNULL(SUM(qty),0) as totQty from Temp_Purchase_Other where temp_purchase_id ='" + GetId() + "'");
           ...
           {
               _clsdao.runSQL("update Temp_Purchase_Other set is_approved='y' where temp_purchase_id='" + GetId() + "'");
