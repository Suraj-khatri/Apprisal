...
{
    public partial class AddOtherSalesInfo : BasePage
    {
        String DelProduct = "";
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        ClsDAOInv _clsdao = null;
        public AddOtherSalesInfo()
        {
        ...
        private void getProductInfo()
        {
            ...
            dt = _clsdao.getDataset("SELECT P.product_desc+' ('+P.porduct_code+')' as 'Product Code',T.qty as Qty FROM Temp_Sales T"
        ...
        {
            string flag1 = _clsdao.GetSingleresult("Exec procCheckProduct 'c','" + GetId() + "'");
            string flag2 = _clsdao.GetSingleresult("Exec procCheckProduct 'b','" + GetId() + "'");
            if (flag1 == "N")
            {
        ...
        private void manageOtherInfo()
        {
            ...
            {
                string totQty = _clsdao.GetSingleresult("select ISNULL(SUM(qty),0) as totQty from Temp_Sales_Other where temp_sales_id ='" + GetId() + "' and session_id=" + filterstring(ReadSession().Sessionid) + "");

                if (Double.Parse(lblqty.Text) >= (Double.Parse(totQty) + Double.Parse(qty.Text)))
                {
                    _clsdao.runSQL("INSERT INTO Temp_Sales_Other(temp_sales_id,sn_from,sn_to,batch,qty,session_id)VALUES(" + filterstring(GetId().ToString()) + "," + filterstring(sn_from.Text) + "," + filterstring(sn_to.Text) + "," + filterstring(batch.Text) + "," + filterstring(qty.Text) + "," + filterstring(ReadSession().Sessionid) + ")");
                    getAddItems();
                    resetAdd();
                }
                else
        ...
        private void getAddItems()
        {
            double totQty = 0;
            DataTable dt = new DataTable();
            dt = _clsdao.getDataset("select id,sn_from as 'Serial From',sn_to as 'Serial To',batch as Batch,qty as Qty from Temp_sales_Other where temp_sales_id='" + GetId() + "' and session_id=" + filterstring(ReadSession().Sessionid) + "").Tables[0];
        ...
        protected void BtnDelProduct_Click(object sender, EventArgs e)
        {
            if (DelProduct != "")
            {
                string sql = "DELETE FROM Temp_Sales_Other WHERE id='" + DelProduct + "'";
                _clsdao.runSQL(sql);
                getAddItems();
            }
        }
        ...
        {
            string totQty = _clsdao.GetSingleresult("select ISNULL(SUM(qty),0) as totQty from Temp_Sales_Other where temp_Sales_id ='" + GetId() + "'");
            ...
            {
                _clsdao.runSQL("update Temp_Sales_Other set is_approved='y' where temp_Sales_id='" + GetId() + "'");
        ...
        {
            _clsdao.runSQL("delete from Temp_Sales_Other where is_approved is null");
        }
