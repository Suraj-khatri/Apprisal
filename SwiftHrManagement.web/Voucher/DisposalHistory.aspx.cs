...
{
    public partial class DisposalHistory : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (!IsPostBack)
                {
                    if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 120) == false)
                    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"approval_number",   "Approval Number",         "EN"},
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"approval_number",   "Approval No",            "200",      "T"},
                        {"approval_date",     "Approval Date",          "200",      "D"},
                        {"disposed_amount",   "Disposed Amount",        "200",      "M"},
                        {"Enteredby",         "Disposed By",            "200",      "T"},
                        };

            string sortby = "disposal_id";
            string table_name = "IN_DISPOSAL_HISTORY";

            String sql = "select disposal_id,approval_number, CONVERT(varchar, approval_date,107) as 'approval_date', disposed_amount as "
            + " 'disposed_amount', e.FIRST_NAME + ' ' + e.MIDDLE_NAME + ' ' + e.LAST_NAME as Enteredby "
            + " from IN_DISPOSAL_HISTORY dh inner join Employee e on dh.entered_by = e.EMPLOYEE_ID";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "DisposalHistory.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Voucher/ViewDisposedProduct.aspx?Id=" + dr["disposal_id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Voucher/DisposalVoucher.aspx");
