...
{
    public partial class ListBillPurchaseDetail : BasePage
    {
        ClsDAOInv _clsdao = null;
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public ListBillPurchaseDetail()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 115) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{
                        {"billno",         "Bill Number",         "LT"},
                        {"c.CustomerName",    "Vendor Name",     "LT"}
            ...
            String[,] matgrid = new String[,]{  
                        ...
                        {"billno",           "Bill No",            "200",      "T"},
                        {"bill_date",         "Bill Date",          "200",      "D"},
                        {"bill_amt",          "Bill Amount",        "200",      "M"},
                        {"party_code",       "Vendor Name",             "200",      "T"},
                        {"entered_by",       "Entered By",             "200",      "T"},
                        {"entered_date",       "Entered Date",             "200",      "D"},
                        {"bill_notes",      "Bill Notes",             "200",      "T"}
                        };

            string sortby = "bill_id";
            string table_name = "Bill_info";

            String sql = "SELECT bill_id,billno,c.CustomerName AS party_code,CONVERT(VARCHAR,bill_date,107) AS bill_date,dbo.ShowDecimal(bill_amount) AS"
            + " bill_amt,bill_notes,entered_by,CONVERT(VARCHAR,entered_date,107) as entered_date"
            +" FROM Bill_info b WITH(NOLOCK) inner join Customer c WITH(NOLOCK) ON c.ID=b.party_code WHERE paid_date is null";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListBillPurchaseDetail.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            //html.Append("<th Class=\"HeaderStyle\" align=\"left\"></th>");
            //html.Append("<th Class=\"HeaderStyle\" align=\"left\"></th>");
            //html.Append("<th Class=\"HeaderStyle\" align=\"left\"></th>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Invoice\" href=\"/Voucher/purchase_invoice.aspx?bill_id=" + dr["bill_id"]
                    + "\"><i class=\"fa fa-table\"></i></a>");

                //html.Append("<td><a href=\"/Voucher/purchase_invoice.aspx?bill_id=" + dr["bill_id"] + "\">Invoice</a></td>");
                html.Append("<a class=\"tool btn btn-primary btn-xs\" data-placement=\"top\" title=\"Delete\"><span onclick = \"DeleteRecord('" + dr["bill_id"].ToString() + "')\" style=\"cursor:pointer;\"><i class=\"fa fa-trash\"></i> </span></a>");
                html.Append("<a class=\"tool btn btn-primary btn-xs\" data-placement=\"top\" title=\"Is Paid\"><span onclick = \"IsPaid('" + dr["bill_id"].ToString() + "')\" style=\"cursor:pointer;\"><i class=\"fa fa-check\"></i></span></a></td>"); 
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, false, "ManageBill.aspx");
        ...
        {
            string msg = _clsdao.GetSingleresult("Exec [DeletePurchaseVoucher] @flag='s',@bill_id=" + filterstring(hdnId.Value));

            Response.Redirect("ListBillPurchaseDetail.aspx?msg="+msg+"");
        }
        ...
        private void IsPay()
        {
            string msg = _clsdao.GetSingleresult("update Bill_info set paid_by="+filterstring(ReadSession().UserId.ToString())+","
            +" paid_date="+filterstring(System.DateTime.Now.ToString())+" where bill_id='"+hdnId1.Value+"'");
