...
{
    public partial class ListPurchaseOrder : BasePage
    {
        ...
        private void init()
        {
            ...
            else
            {
                sql = "select p.id,p.status,order_no,convert(varchar,order_date,107) as order_date,c.CustomerName as vendor_code,"
                        ...
                        +" forwarded_to="+filterstring(ReadSession().Emp_Id.ToString())+"";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListPurchaseOrder.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
               ...
               // html.Append("<td><a href=\"/Voucher/ApprovePurchaseOrder.aspx?Id=" + dr["id"] + "\">Approve</a></td>");                
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Approve\" href=\"/Voucher/ApprovePurchaseOrder.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Voucher/ApprovePurchaseOrder.aspx");
