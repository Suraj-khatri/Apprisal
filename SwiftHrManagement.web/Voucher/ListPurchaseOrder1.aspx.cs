...
using SwiftHrManagement.DAL.Assets;
...
{
    public partial class ListPurchaseOrder1 : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        private readonly InventoryDao _obj = new InventoryDao();
        public ListPurchaseOrder1()
        {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"order_no",    "Order No",         "LT"},  
                        {"p.status",    "Order Status",         "LT"},
                         {"order_date",       "Order Date",   "ED"},
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"order_no",            "Order No",             "200",      "T"},
                        {"order_date",       "Order Date",        "200",      "D"},
                        {"vendor_code",    "Vendor Code",     "200",      "T"},
                        {"order_by",    "Order By",     "200",      "T"},
                        {"forwarded_to",    "Forwarded To",     "200",      "T"},
                        {"status",    "Order Status",     "200",      "T"}
                        };



            string sortby = "id";
            string table_name = "Purchase_Order_Message";

            String sql = "";

            if (ReadSession().UserId == "admin")
            {
                sql = "select p.id,p.status,order_no,convert(varchar,order_date,107) as order_date,c.CustomerName as vendor_code,"
                        + " DBO.GetEmployeeFullNameOfId(forwarded_to) AS forwarded_to,DBO.GetEmployeeFullNameOfId(created_by) as order_by"
                        + " from Purchase_Order_Message p inner join customer c on c.id=p.vendor_code where status='Requested'";
            }
            else
            {
                sql = "select p.id,p.status,order_no,convert(varchar,order_date,107) as order_date,c.CustomerName as vendor_code,"
                        + " DBO.GetEmployeeFullNameOfId(forwarded_to) AS forwarded_to,DBO.GetEmployeeFullNameOfId(created_by) as order_by"
                        + " from Purchase_Order_Message p inner join customer c on c.id=p.vendor_code where status='Requested' and"
                        + " created_by=" + filterstring(ReadSession().Emp_Id.ToString()) + "";
            }
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListPurchaseOrder1.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Modify\" href=\"/Voucher/ModifyPurchaseOrder.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-pencil-square-o\" aria-hidden=\"true\"></i></a>");
                html.Append("<a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Remove\" onclick='IsDelete(" + dr["id"] + ")' href=\"#\"><i class=\"fa fa-times\" aria-hidden=\"true\"></a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Voucher/PurchaseOrder.aspx");
        ...
        private string make_filter_form(string table_name, string[,] matfilter)
        {
            ...
            for (int i = 0; i <= arr_cnt; i++)
            {
                ...
                if (ctlType == "ED")
                {
                   // html.Append("<td width=\"200\"><input type=\"text\" name=\"" + ctlName + "\" value=\"" + defValue + "\">" +
        ...
        {
            DbResult dbResult = _obj.DeletePurchaseOrder(ReadSession().Emp_Id.ToString(), hdnId.Value);
