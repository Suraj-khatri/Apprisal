...
{
    public partial class ListPurchaseOrderReceived : BasePage
    {
         RoleMenuDAOInv _RoleMenuDAOInv = null;
         public ListPurchaseOrderReceived()
        {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        {"product_name",            "Product Name",             "200",      "T"},
                        {"ordered_qty",       "Ordered Qty",        "200",      "EN"},
                        {"foc_qty",    "FOC Qty",     "200",      "EN"},
                        {"received_qty",    "Received Qty",     "200",      "EN"},
                        {"received_rate",    "Rate",     "200",      "M"},
                        {"received_amount",    "Amount",     "200",      "M"},
                        {"remaining_qty",    "Remaining Qty",     "200",      "EN"}
                        };
            ...
            String sql = "";
            sql = @"SELECT P.porduct_code AS product_name,po.qty as ordered_qty,
SUM(ISNULL(pr.foc_qty,0)) as foc_qty,
SUM(ISNULL(pr.qty,0)) as received_qty,
dbo.ShowDecimal(AVG(isnull(pr.rate,0))) as received_rate,
dbo.ShowDecimal(SUM(ISNULL(pr.qty,0))*AVG(isnull(pr.rate,0))) as received_amount,
po.qty+SUM(ISNULL(pr.foc_qty,0))-SUM(ISNULL(pr.qty,0)) as remaining_qty
from Purchase_Order po INNER JOIN IN_PRODUCT P ON P.id=PO.product_code 
left join Purchase_Order_Received pr on
 pr.Product_Id =po.product_code and po.order_message_id=pr.Order_Message_Id
 where po.order_message_id="+GetOrderId()+" group by P.porduct_code,po.qty";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ListPurchaseOrderReceived.aspx?sortd=" + sortd + "&sortby={sortby}&Id=" + GetOrderId() + "";
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/Voucher/ReceivePurchaseVoucher.aspx?OrderMsgId=" + GetOrderId() + "");
