...
{
    public partial class OrderReceivedHistory : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public OrderReceivedHistory()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 117) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"received_by",    "Received by",     "200",      "T"},
                        {"received_date",    "Received Date",     "200",      "D"}
                        };
            ...
            sql = "select p.id,p.order_no,convert(varchar,p.order_date,107) as order_date,c.CustomerName as vendor_code,"
            +" dbo.GetEmployeeFullNameOfId(p.created_by) as requested_by,CONVERT(varchar,p.created_date,107) as requested_date,"
            +" dbo.GetEmployeeFullNameOfId(p.approved_by) as approved_by,CONVERT(varchar,p.approved_date,107) as approved_date,"
            +" dbo.GetEmployeeFullNameOfId(p.received_by) as received_by,CONVERT(varchar,p.received_date,107) as received_date"
            + " from Purchase_Order_Message p inner join customer c on c.id=p.vendor_code where p.approved_date is not null and status='Received'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "OrderReceivedHistory.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            html.Append("<th class=\"text-center\" align=\"center\" width=\"95px\">Action</th>");
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View\" href=\"/Voucher/ReceivedProductHistory.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-eye\"></i></a> <a  class=\"tool btn btn-info btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Print\" href=\"/Report/received_order.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-print\"></i></a></td>");

                //html.Append("<td><a href=\"/Voucher/ReceivedProductHistory.aspx?Id=" + dr["id"] + "\">View Received</a></td>");
                //html.Append("<td><a href=\"/Report/received_order.aspx?Id=" + dr["id"] + "\">Print Received</a></td>");
                html.Append("</tr>");
            }

            html.Append("<tr> <td align=\"center\" colspan=\"10\"> <strong> Page " + _page.ToString() + " Of " + total_page + "</strong></td></tr>");
            html.Append("</table>");
            html.Append("</div>");
