...
namespace SwiftAssetManagement.Voucher
{
    public partial class ReceivePurchaseOrder : BasePage
    {
        ...
        private void init()
        {
            ...
            sql = "  SELECT p.id,order_no,CONVERT(VARCHAR,order_date,107) AS order_date,c.CustomerName AS vendor_code,"
                    +" dbo.GetEmployeeFullNameOfId(approved_by) AS approved_by,CONVERT(VARCHAR,approved_date,107) AS approved_date FROM "
                    + " Purchase_Order_Message p inner join customer c ON c.id=p.vendor_code WHERE approved_date is not null AND isnull(STATUS,0)<>'Received'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ReceivePurchaseOrder.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //html.Append("<td><a href=\"/Voucher/ListPurchaseOrderReceived.aspx?Id=" + dr["id"] + "\">Receive Order</a></td>");
                //html.Append("<td><a href=\"/Report/PurchaseOrder.aspx?id=" + dr["id"] + "\">Generate PO</a></td>");
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Receive Order\" href=\"/Voucher/ListPurchaseOrderReceived.aspx?Id=" + dr["id"] + "\"><i class=\"fa fa-download\"></i></a> <a  class=\"tool btn btn-info btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Generate PO\" href=\"/Report/PurchaseOrder.aspx?id=" + dr["id"] + "\"><i class=\"fa fa-pencil\"></i></a>");


                html.Append("<a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Close PO\"><span onclick = \"IsClose('" + dr["id"].ToString() + "')\" style=\"cursor:pointer;\"><i class=\"fa fa-times\" aria-hidden=\"true\"></i></span></a></td>"); 
                html.Append("</tr>");
            }
        ...
        {
            var clsdao= new  ClsDAOInv();
            clsdao.runSQL("update Purchase_Order_Message set approved_date="+filterstring(System.DateTime.Now.ToString())+",status='Received' where id="+hdnPOId.Value+"");

            Response.Redirect("ReceivePurchaseOrder.aspx");
        }
    }
}
