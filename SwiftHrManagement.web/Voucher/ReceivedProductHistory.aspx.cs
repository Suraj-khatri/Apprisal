...
{
    public partial class ReceivedProductHistory : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public ReceivedProductHistory()
        {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"product_code",        "Product Name",              "200",      "LT"},
                        {"qty",            "Qty",             "200",      "EN"},
                        {"rate",                "Rate (Unit Price)",                 "200",      "M"},
                        {"amount",   "Amount",    "200",      "M"}
                        };

            string sortby = "product_code";
            string table_name = "Purchase_Order_Received";


            String sql = "select P.porduct_code AS product_code,SUM(qty) as qty,dbo.ShowDecimal(AVG(Rate)) as rate,dbo.ShowDecimal(SUM(amount)) AS amount from Purchase_Order_Received PR "
            + " INNER JOIN IN_PRODUCT P ON P.id=PR.Product_Id where PR.Order_Message_Id='" + GetId() + "' group by P.porduct_code";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "List.aspx?sortd=" + sortd + "&sortby={sortby}&Id="+GetId()+"";
