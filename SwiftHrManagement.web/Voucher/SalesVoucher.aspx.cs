...
{
    public partial class SalesVoucher : BasePage
    {
        ...
        private void ManageTempPurchase1()
        {
            ...
            _clsdao.runSQL("Exec [procManageSalesVoucher] 'i'," + filterstring(product_id) + "," + filterstring(qty.Text) + "," + filterstring(unitprice.Text) + "," + filterstring(amount.Text) + ","
        ...
        private void getAddItems(string session_id)
        {
                ...
                dt = _clsdao.getDataset("SELECT T.id,P.product_desc+' ('+P.porduct_code+')' as 'Product Code',"
                    + " T.qty as Qty,dbo.ShowDecimal(T.rate) as Rate,dbo.ShowDecimal(T.amount) as Amount FROM Temp_Sales T INNER JOIN IN_PRODUCT P ON "
                    + " P.id=T.product_code WHERE flag='s' and session_id=" + filterstring(session_id) + " and order_message_id is null").Tables[0];
        ...
        {
            _clsdao.runSQL("INSERT INTO Temp_Sales(session_id,product_code,ac_type,amount,flag)VALUES(" + filterstring(ReadSession().Sessionid) + "," + filterstring(TxtAc_Name.Text) + "," + filterstring(DdlType.Text) + "," + filterstring(ac_amt.Text) + ",'a')");
        }
        ...
        private void getAccountInfo(string session_id)
        {
            ...
            double tot_amount = 0.00;
            DataTable dt = new DataTable();
            dt = _clsdao.getDataset("SELECT id,product_code AS 'Account Name',ac_type as 'Type',amount as 'Amount (CR)' FROM Temp_Sales WHERE flag='a' and session_id=" + filterstring(session_id) + "").Tables[0];
        ...
        protected void BtnDelProduct_Click(object sender, EventArgs e)
        {
            if (DelProduct != "")
            {
                string sql = "DELETE FROM Temp_Sales WHERE id='" + DelProduct + "'";
                _clsdao.runSQL(sql);
                getAddItems(ReadSession().Sessionid);
            }
        }
        protected void BtnDeleteAcc_Click(object sender, EventArgs e)
        {
            if (DelAccount != "")
            {
                string sql = "DELETE FROM Temp_Sales WHERE id='" + DelAccount + "'";
                _clsdao.runSQL(sql);
                getAccountInfo(ReadSession().Sessionid);
            }
        }
        ...
        private void managePurchaseVoucher()
        {
            ...
            string SQL = "Exec procSaveSalesVoucher 'i'," + filterstring(remarks.Text) + "," + filterstring(billDate.Text) + "," + filterstring(ReadSession().Sessionid) + "," + filterstring(billno.Text) + "," + filterstring(vender_id) + "," + filterstring(ReadSession().UserId) + "," + filterstring(chkVAT.Checked.ToString());
