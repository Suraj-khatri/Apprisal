...
{
    public partial class ViewDisposedProduct : BasePage
    {
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public ViewDisposedProduct()
        {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"prod_code",        "Product Name",              "200",      "LT"},
                        {"qty",            "Qty",             "200",      "LT"},
                        {"rate",                "Rate (Unit Price)",                 "200",      "M"},
                        {"amount",   "Amount",    "200",      "M"}
                        };

            string sortby = "rowl_id";
            string table_name = "IN_DISPOSAL";


            String sql = "select d.rowl_id,d.disposal_id,p.porduct_code as prod_code,d.p_qty as qty,d.p_rate as rate,"
                    +" d.p_qty*p_rate as amount from IN_DISPOSAL d inner join IN_PRODUCT p on p.id=d.prod_code"
                    +" where disposal_id='"+GetId()+"'";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ViewDisposedProduct.aspx?sortd=" + sortd + "&sortby={sortby}&Id="+GetId()+"";
