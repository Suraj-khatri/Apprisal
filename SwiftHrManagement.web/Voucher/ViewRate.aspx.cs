...
{
    public partial class ViewRate : BasePage
    {
        clsDAO _clsdao = null;
        string Rate = "";
        public ViewRate()
        ...
        {
            if (Request.QueryString["prod_code"].ToString() != null)
                hdnprod_code.Value = Request.QueryString["prod_code"].ToString();

            init();

            if (Request.Form["ckID1"] != null)
                Rate = Request.Form["ckID1"].ToString();
        }
        #region SwiftGrid
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        ...
                        {"bill_id",           "Bill No",                "200",      "T"},
                        {"product_desc",      "Product Descrition",     "200",      "T"},
                        {"p_qty",             "Quantity",               "200",      "M"},
                        {"p_stk_remain",      "Remain Qty",               "200",      "M"},
                        {"p_rate",            "Rate",                   "200",      "T"}
                        };

            string sortby = "bill_id";
            string table_name = "ASSET_BILL";
         
            
            if(Request.QueryString["qty"] != null)
                hdnprod_Quantity.Value = (Request.QueryString["qty"].ToString());


            String sql = "select cast(ip.pur_id as varchar)+'|'+cast(cast(ip.p_rate AS DECIMAL(30,4)) AS VARCHAR(30)) as pur_id,ip.bill_id, ib.product_desc ,ip.p_qty, ip.p_rate, ip.p_stk_remain, ip.p_sn_from, ip.p_sn_to "
            + " from IN_PURCHASE ip inner join IN_PRODUCT ib on ip.prod_code = ib.id where prod_code = "+ filterstring(hdnprod_code.Value) +" and "
            + " branch_id = " + ReadSession().Branch_Id + " and p_stk_remain >= " + filterstring(hdnprod_Quantity.Value.ToString()) + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ViewRate.aspx?sortd=" + sortd + "&sortby={sortby}&prod_code=" + hdnprod_code.Value + "";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td align=\"left\"><input type=\"radio\" id=\"ckID\" name=\"ckID\" value=\"" + dr["pur_id"].ToString() + "\"></td>");
                html.Append("</tr>");
            }
        ...
        //{
        //    Response.Redirect("DisposalVoucher.aspx?rate="+ Rate +"");
        //}
}
