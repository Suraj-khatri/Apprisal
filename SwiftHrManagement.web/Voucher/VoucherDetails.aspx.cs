...
namespace SwiftHrManagement.web.Voucher
{
    public partial class VoucherDetails : BasePage
    {
        ...
        {
            return (Request.QueryString["bill_id"] != null ? long.Parse(Request.QueryString["bill_id"].ToString()) : 0);
        }
        #region SwiftGrid
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{                        
                        {"prod_code",           "Product Code",            "200",      "T"},
                        {"porduct_code",         "Product Name",          "200",      "T"},
                        {"p_qty",         "Qty",          "200",      "D"},
                        {"Rate",          "Rate",        "200",      "M"},
                        {"Amount",       "Amount",             "200",      "M"},
                        {"p_stk_remain",         "Remain Stock",          "200",      "D"}
                        };

            string sortby = "porduct_code";
            string table_name = "IN_PURCHASE";

            String sql = "SELECT a.prod_code,b.porduct_code,p_qty,a.p_stk_remain,DBO.SHOWDECIMAL(p_rate) AS Rate,"
            + " dbo.Showdecimal(p_qty*p_rate) AS Amount FROM IN_PURCHASE a inner join IN_PRODUCT b ON a.prod_code=b.id"
            + " WHERE bill_id=" + filterstring(GetBillId().ToString()) + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "VoucherDetails.aspx?sortd=" + sortd + "&sortby={sortby}&bill_id=" + GetBillId() + "";
