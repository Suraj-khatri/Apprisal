...
{
    public partial class paid_bill_list : BasePage
    {
        ClsDAOInv _clsdao = null;
        RoleMenuDAOInv _RoleMenuDAOInv = null;
        public paid_bill_list()
        {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (_RoleMenuDAOInv.hasAccess(ReadSession().AdminId, 214) == false)
                {
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{  
                        ...
                        {"paid_by",      "Paid By",             "200",      "T"},
                        {"paid_date",      "Paid Date",             "200",      "D"}
            ...
            + " bill_amt,bill_notes,entered_by,CONVERT(VARCHAR,entered_date,107) as entered_date,paid_by,convert(varchar,paid_date,107) as paid_date "
            +" FROM Bill_info b WITH(NOLOCK) inner join Customer c WITH(NOLOCK) ON c.ID=b.party_code WHERE paid_date is not null";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "paid_bill_list.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
               ...
               // html.Append("<td><a href=\"/Voucher/purchase_invoice.aspx?bill_id=" + dr["bill_id"] + "\">Invoice</a></td>");
                html.Append("<td class=\"text-center\"><a class=\"tool btn btn-primary btn-xs\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Invoice\" href=\"/Voucher/purchase_invoice.aspx?bill_id=" + dr["bill_id"] + "\"><i class=\"fa fa-table\"></i></a></td>");
