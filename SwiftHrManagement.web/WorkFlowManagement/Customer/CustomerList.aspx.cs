    ...
    public partial class CustomerList : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"CustomerCode",    "Customer Code",         "LT"},   
                        {"CustomerName",    "Customer Name",         "LT"},
                        {"CustomerAddress",    "Customer Address",         "LT"}
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"CustomerCode",            "Customer Code",             "200",      "T"},
                        {"CustomerName",       "Customer Name",        "200",      "T"},
                        {"CustomerAddress",       "Customer Address",        "200",      "T"},
                        {"CustomerTelNo",       "Contact Number",        "200",      "T"}
                        };



            string sortby = "ID";
            string table_name = "WF_CUSTOMER";

            String sql = "";

            sql = "select id,CustomerCode,CustomerName,CustomerAddress,CustomerTelNo from WF_Customer where 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "CustomerList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/WorkFlowManagement/Customer/CustomerManage.aspx?Id=" + dr["ID"] + "\">View</a></td>");                
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/WorkFlowManagement/Customer/CustomerManage.aspx");
