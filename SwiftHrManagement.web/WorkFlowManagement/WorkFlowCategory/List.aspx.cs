    ...
    public partial class List : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"WF_DeptName",    "Department Name",         "LT"},
                        {"WF_CatName",    "Category Name",         "LT"}
            };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"WF_DeptName",            "Department Name",             "200",      "T"},
                        {"WF_CatName",            "Category Name",             "200",      "T"},
                        {"WF_CatDetails",       "Category Details",        "200",      "T"}                       
                        };



            string sortby = "WF_CategoryID";
            string table_name = "WF_CATEGORY";

            String sql = "";

            sql = "SELECT WF_CATEGORYID, WF_CATNAME,WF_CATDETAILS,WF_DeptName FROM WF_CATEGORY where 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            html.Append("<th Class=\"HeaderStyle\" align=\"left\">Doc. Setup</th>");  
            html.Append("</tr>");
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/WorkFlowManagement/WorkFlowCategory/Manage.aspx?WFCat_Id=" + dr["WF_CATEGORYID"] + "\">View</a></td>");
                html.Append("<td><a href=\"/WorkFlowManagement/DocumentSetupTracking/Manage.aspx?WFCat_Id=" + dr["WF_CATEGORYID"] + "\">Doc. Setup</a></td>");
                html.Append("</tr>");
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/WorkFlowManagement/WorkFlowCategory/Manage.aspx");
