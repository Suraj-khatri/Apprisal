    ...
    public partial class JobList : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{ 
                        {"W.WF_DeptName",    "Dept Name",         "LT"},
                        {"W.WF_CatName",    "Job Category",         "LT"},
                        {"C.CustomerName",    "Customer Name",         "LT"},
                        {"J.JOB_CODE",    "Job Code",         "LT"}   
                                          
                    };

            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"WF_DeptName",            "Department Name",             "100",      "T"}, 
                        {"WF_CatName",            "Job Category",             "125",      "T"},                         
                        {"JOB_CODE",            "Job Code",             "125",      "T"},   
                        {"CUSTOMERNAME",            "Customer Name",             "125",      "T"}, 
                        {"CREATEDBY",       "Created By",        "200",      "T"},
            ...
            string table_name = "MyGrid1";

            String sql = "";


            sql = "SELECT J.WF_JOBID,J.WF_CATID,W.WF_DeptName,W.WF_CatName,C.CustomerName as CUSTOMERNAME,J.JOB_CODE,E.FIRST_NAME + ' ' + E.MIDDLE_NAME + ' ' + E.LAST_NAME as CREATEDBY,"
                + " convert(varchar,J.CREATEDDATE,107) as CREATEDDATE FROM WF_JOB J  INNER JOIN WF_Category W ON W.WF_CategoryID=J.WF_CatID inner join employee e on "
                + " e.employee_id = j.CREATEDBY INNER JOIN Customer C ON C.ID=J.CustomerCode WHERE 1=1";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "JobList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/WorkFlowManagement/WorkFlowJob/ManageJob.aspx?Id=" + dr["WF_JOBID"] + "\">View</a></td>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/WorkFlowManagement/WorkFlowJob/ManageJob.aspx");
