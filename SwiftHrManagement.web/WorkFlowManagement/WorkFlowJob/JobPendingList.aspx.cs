...
{
    public partial class JobPendingList : BasePage
    {
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        {"D.CustomerName",    "Customer Name",         "LT"}, 
                        {"C.WF_DeptName",    "Department Name",         "LT"},  
                        {"WF_CATNAME",    "Job Catetegory",         "LT"},
                        {"JOB_CODE",    "Job Code",         "LT"}  
                        
                        //{"EMPNAME",    "Created By",         "LT"}
                    };
            //WF_CATNAME
            //column type : T = text , M = Money, D = date , I = image
            String[,] matgrid = new String[,]{
                        {"WF_DeptName",            "Dept Name",             "125",      "T"},  
                        {"WF_CATNAME",            "Job Category",             "125",      "T"},  
                        {"JOB_CODE",            "Job Code",             "125",      "T"},
                        {"CustomerName",            "Customer Name",             "200",      "T"}, 
                        {"EMPNAME",       "Created By",        "200",      "T"},
                        {"CREATEDDATE",       "Created Date",        "125",      "T"}
                        };
            string sortby = "WF_JOBID";
            string table_name = "WF_JOB";

            String sql = "";


            sql = "SELECT JOB_CODE,WF_JOBID,WF_CATID,C.WF_DeptName,C.WF_CATNAME,CONVERT(VARCHAR,J.CREATEDDATE,107) AS CREATEDDATE,J.CREATEDBY,"
	                +" E.FIRST_NAME + ' ' + E.MIDDLE_NAME + ' ' + E.LAST_NAME AS EMPNAME,D.CustomerName FROM WF_JOB j"
	                +" INNER JOIN EMPLOYEE E ON E.EMPLOYEE_ID = J.CREATEDBY"
	                +" INNER JOIN WF_CATEGORY C ON C.WF_CATEGORYID = J.WF_CATID"
	                +" INNER JOIN Customer D ON D.ID=j.CustomerCode"
                    +" WHERE 1=1 ";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "JobPendingList.aspx?sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/WorkFlowManagement/WorkFlowJob/JobFileUploader.aspx?Id=" + dr["WF_JOBID"] + "\">Files</a></td>");
                html.Append("<td><a href=\"/WorkFlowManagement/WorkFlowJob/ProcessingJobList.aspx?JobId=" + dr["WF_JOBID"] + "\">Processing</a></td>");
                html.Append("<td><a href=\"/WorkFlowManagement/WorkFlowJob/ManageJob.aspx?Id=" + dr["WF_JOBID"] + "&Flag= " + 'P' + "\">View</a></td>");
                html.Append("<td><a href=\"/WorkFlowManagement/WorkFlowJob/ManageDocSetup.aspx?JOB_ID=" + dr["WF_JOBID"] + "\">Doc. List</a></td>");

                html.Append("</tr>");
                jobCatID = long.Parse(dr["WF_CATID"].ToString());
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/WorkFlowManagement/WorkFlowJob/ManageJob.aspx?Flag="+ 'P');
