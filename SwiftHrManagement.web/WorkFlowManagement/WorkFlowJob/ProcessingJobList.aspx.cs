...
{
    public partial class ProcessingJobList : BasePage
    {
        ...
        private void getJobDetails()
        {
            try
            {
                ...
                lblWorkCategoryname.Text = "Department Name: " + jobCore.DeptName + ", Category Name: " + jobCore.JobCategory + ", Job Code: " + jobCore.JobCode + "";
            }
            catch
            {
            }
        ...
        private void init()
        {
            ...
            String[,] matgrid = new String[,]{
                        {"ACTION_BY_EMP",       "Action By",        "200",      "T"},
                        {"MODE",       "Action",        "200",      "T"},
                        {"ACTION_DATE",       "Action Date",        "200",      "D"},
                        {"ACTION_TO_EMP",       "Action To",        "200",      "T"},
                        {"JOB_STATUS",       "Job Status",        "200",      "T"},
                        };



            string sortby = "ID";
            string table_name = "WF_DETAILS";

            String sql = "";

            sql = "SELECT D.ID,D.WF_JOBID,J.WF_CATID,E.FIRST_NAME + ' ' + E.MIDDLE_NAME + ' ' + E.LAST_NAME as ACTION_BY_EMP,"
            + " EE.FIRST_NAME + ' ' + EE.MIDDLE_NAME + ' ' + EE.LAST_NAME as ACTION_TO_EMP,"
            +" ACTION_DATE,MODE,JOB_STATUS FROM WF_DETAILS D INNER JOIN WF_JOB J ON J.WF_JOBID = D.WF_JOBID"
            + " INNER JOIN EMPLOYEE E ON E.EMPLOYEE_ID = D.ACTION_BY_EMPID LEFT JOIN EMPLOYEE EE ON EE.EMPLOYEE_ID = D.FORWARD_TO WHERE D.WF_JOBID = " + GetJobId() + "";
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "ProcessingJobList.aspx?Id=" + GetJobId() + "&sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/WorkFlowManagement/WorkFlowJob/ManageProcessingJob.aspx?Id="+ dr["ID"] +"&JobId=" + dr["WF_JOBID"] + "&CatID=" + jobCatID + "\">View</a></td>");
                html.Append("<td><a href=\"/WorkFlowManagement/WorkFlowJob/ManageComment.aspx?Id=" + dr["ID"] + "\">Comment</a></td>");
                html.Append("</tr>");
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/WorkFlowManagement/WorkFlowJob/ManageProcessingJob.aspx?CatId=" + jobCatID + "");
