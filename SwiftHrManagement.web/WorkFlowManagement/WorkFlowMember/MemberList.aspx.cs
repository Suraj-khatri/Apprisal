...
namespace SwiftHrManagement.web.WorkFlowManagement.WorkFlowMember
{
    public partial class MemberList : BasePage
    {
        ...
        private void GetWFCategoryName()
        {
            DataTable dt = new DataTable();
            dt = ClsDao.getDataset("SELECT WF_CATNAME,WF_DeptName FROM WF_CATEGORY WHERE WF_CATEGORYID = " + GetCategoryID() + "").Tables[0];

            foreach (DataRow dr in dt.Rows)
            {
                lblDeptName.Text = dr["WF_DeptName"].ToString();
                lblCategoryName.Text = dr["WF_CATNAME"].ToString();
            }
        }
        ...
        private void init()
        {
            ...
            String[,] matfilter = new String[,]{  
                        ...
                        {"EMPLOYEE_NAME",    "Employee Name",         "LT"}                   
                    };
            ...
            string sortby = "WF_MEMBERID";
            string table_name = "WF_MEMBER";

            String sql = "";

            //sql = "SELECT WF_CATEGORYID, WF_CATNAME,WF_CATDETAILS FROM WF_CATEGORY where 1=1";
           

             sql = " SELECT M.WF_MEMBERID,M.EMPLOYEE_ID,E.EMP_CODE,"
                   + " E.FIRST_NAME + ' ' + E.MIDDLE_NAME +' '+ E.LAST_NAME AS EMPLOYEE_NAME,"
                   + " C.WF_CATNAME,C.WF_CATDETAILS FROM WF_MEMBER M INNER JOIN WF_CATEGORY C ON C.WF_CATEGORYID = M.WF_CATEGORYID"
                   + " INNER JOIN EMPLOYEE E ON E.EMPLOYEE_ID = M.EMPLOYEE_ID"
                   + " WHERE C.WF_CategoryID = " + GetCategoryID() + " AND 1=1" ;
        ...
        private string load_report(string table_name, string sql, String[,] matgrid, string[,] matfilter, String sortd, string sortby, int _page, int page_size)
        {
            ...
            string thisPage = "MemberList.aspx?Id="+ GetCategoryID()  +"&sortd=" + sortd + "&sortby={sortby}";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                html.Append("<td><a href=\"/WorkFlowManagement/WorkFlowMember/MemberManage.aspx?Id=" + dr["WF_MEMBERID"] +"&CatId="+ GetCategoryID() + "\">View</a></td>");
                html.Append("</tr>");
                //
                memID = long.Parse(dr["WF_MEMBERID"].ToString());
                //
            }
            ...
            string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/WorkFlowManagement/WorkFlowMember/MemberManage.aspx?CatId=" + GetCategoryID());
            //WF_MEMBERID

            //string navigation_bar = get_paging_block(total_record, _page, page_size, true, "/WorkFlowManagement/WorkFlowMember/MemberManage.aspx?Id=" + dr["WF_MEMBERID"]);
